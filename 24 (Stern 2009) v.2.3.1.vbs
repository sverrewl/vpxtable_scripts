...
'⠀⠀⠀⠀⠓⠵⣣⠳⣜⣊⠷⣌⠳⣌⠳⣍⠶⣩⠞⣡⢳⠹⡴⣩⠞⡱⢎⡣⢞⡱⢎⢧⢫⡔⣫⠖⣭⢒⠧⣚⢥⣚⣥⡫⢊⠁⠀⠀
'
' Based on Stern's 24 / IPD No. 5419 / 2009 / 4 Players
'
' VP9 - Original version by freneticamnesic - 2014
' VPX - Original version by 32assassin - 2019
' FSS version by Morttis - 2022
' Future Pinball version by NitroNimbus - 2022
' VR version by DGrimmReaper - 2023
' VR-Hybrid version by RobbyKingPin, DaRdog, DGRimmReaper, Themer, LoadedWeapon & Gravy based on 32assassin's version - 2024/2025
'
' This mod contains the following improvements:
'
' RobbyKingPin:
' - nFozzy/Rothbauerw physics,Fleep sound with a lot of custom sounds from VPW's Rollercoaster Tycoon and VPW Ambient Ball Shadows (Updated v.2.3)
...
' - Added VR assets from DGrimmReaper's VR version
' - Added more VR assets from Transformers
' - Inlane speed limiters, code created by Wylte
' - New ball image and scratches based on VPW's tables
' - Updated PWM on flashers GI and Inserts, added 3D inserts and converted 1 pair of inserts to work with Flupper Domes
' - Updated apron primitive from VPW's The Simpsons Party
' - Small color and light corrections on the playfield texture
' - Added insert decals from the Emperor Mod
' - Added Instruction Cards provided by Carny Priest
' - New GI
' - Updated Rubber Dampeners (v.2.2)
' - Removed a lot of timers to improve performance (v.2.2)
...
' - Added a second flasher dome inside the safehouse and edited the safehouse primitive (v.2.2)
' - Added more options for reflections (v.2.3)
' - Added a third VR room for Mixed Reality (v.2.3)
' - Created a new Jet plastic primitive, used and edited texture from Future Pinball version made by NitroNimbus (v.2.3)
' - Added shadows texture from FSS version made by Morttis (v.2.3)
' - Added new flipperbats from VPW's AC/DC, changed flippersizes and triggers to match and edited the texture (v.2.3)
'
' DaRdog:
' - Added VR Mega Room
' - New cab art and apron art
' - Big uprez in the playfield using AI (Updated v.2.3.1 by RKP)
'
' DGrimmReaper:
' - Added key, new flipperbuttons, startbutton, screws on the siderails and plunger and animated them.
' - Re-aligned cabinet to prevent the apron being cut off (v.2.3)
'
' Themer (v.2.2):
' - Inverted VPM switch assignments 43 and 45 (both at the visibleLock and the 'Suitcase lock buttons'). Those switches were inverted
' - New code for the SolSniper sub and the entire sniper section
' - New code for the SolSafehouse sub and the entire safehouse section
' - New VPM solenoid assignment 21 for the suitcase motor
' - Removed the SuitRot property from the solSuitcase sub as this property is removed entirly
' - New MotorSuitcase sub and new code for the entire suitcase section
'
' LoadedWeapon (v.2.2):
' - Created a custom sideblades primitive to fit on Legends Cabinets and provided a secondary ini file for the POV to match
'
' Gravy (v.2.3):
' - Edited the old custom jet made by 32assassin
'
' Special thanks to everybody involved during development and testing :H3RBSKIx, geradg, BountyBob, Smaug, MikeDASpike, DarthVito, passion4pins, Apophis, m.carter78, somatik, guus_8005, Studlygoorite, Sinizin
'
' Shoutout to:
'
' - Carny Priest for the instruction cards he gave us!
' - passion4pins for the backdrop he created for desktop players
' - freneticamnesic, Rascal, luvthatapex, Javier15, unclewilly, oooPLAYER1ooo, rom, zedonius, dark, ASOT, gtxjoe, ICPjuggla, 85vett, arngrim, 32assassin, DGrimmReaper, Emperor, NitroNimbus & Morttis for their work on
' previous versions
'
...
Else
  ...
  If Table1.ShowFSS = False And LegendsCabMod = 1 Then
    ...
    If SideBladeMod = 0 Then
      SideBladesLegends.image = "Sidewalls_24_Original"
    Else
      SideBladesLegends.image = "Sidewalls_24"
    End If
  End If
End if
...
Sub FrameTimer_Timer()
  ...
  End if
  f119a.state = f119.state
  If l60.state = 1 Then
    P60.image = "lampbulbON"
  Else
    P60.image = "lampbulb"
  End If
  If l61.state = 1 Then
    P61.image = "lampbulbON"
  Else
    P61.image = "lampbulb"
  End If
  If l62.state = 1 Then
    P62.image = "lampbulbON"
  Else
    P62.image = "lampbulb"
  End If
End Sub
...
Sub UpdatePlunger()
  If plungerpress = 1 then
    If PinCab_Shooter.Y < 90 then
      PinCab_Shooter.Y = PinCab_Shooter.Y + 5
    End If
  Else
    PinCab_Shooter.Y = -10 + (5* Plunger.Position) -20
  End If
End Sub
...
Dim xx
Dim TwentyFourBall1, TwentyFourBall2, TwentyFourBall3, TwentyFourBall4, gBOT
Dim visibleLock, bsTEject, plungerIM
...
Sub Table1_Init
  ...
  vpmNudge.TiltObj = Array(Bumper1, Bumper2, Bumper3, LeftSlingshot, RightSlingshot)
   ...
   'Trough
  Set TwentyFourBall1 = sw18.CreateSizedballWithMass(Ballsize/2,Ballmass)
  Set TwentyFourBall2 = sw19.CreateSizedballWithMass(Ballsize/2,Ballmass)
  Set TwentyFourBall3 = sw20.CreateSizedballWithMass(Ballsize/2,Ballmass)
  Set TwentyFourBall4 = sw21.CreateSizedballWithMass(Ballsize/2,Ballmass)
  gBOT = Array(TwentyFourBall1,TwentyFourBall2,TwentyFourBall3,TwentyFourBall4)
 ...
 bsTeject.KickAngleVar = 3
  ...
  Const IMPowerSetting = 85 'Plunger Power
  ...
  With visibleLock
    .InitVLock Array(sw43,sw44,sw45),Array(k43, k44, k45), Array(45,44,43)
...
SolCallback(4) = "ResetDropsL" 'Drop Target Left
SolCallback(5) = "ResetDropsR" 'Drop Target Right
...
SolCallback(13) =   "ResetDropsU" 'Drop Targets Up
SolCallback(14) =   "SolSniper"
SolCallback(17) =   "solLSling"
SolCallback(18) =   "solRSling"
SolCallback(21) = "MotorSuitcase" 'Solenoid positions 21, 23, 25 and 30
...
If Not UseVPMModSol Then
  SolCallback(19) =   "SolFlash19" 'flash safehouse x3
  SolCallback(20) =   "SolFlash20" 'flash sniper x2
  SolCallback(26) =   "vpmFlasher f126," 'flash pop x3
  SolCallback(27) =   "SolFlash27" 'flash right side under jet?
  SolCallback(31) =   "SolFlash31" 'Orange Dome Slingshots x2
  SolCallback(32) =   "SolFlash32" 'flash left spinner x3
Else
  If PWMflashers Then
    SolModCallback(19) =  "SolModFlashPWM19" 'flash safehouse x3
    SolModCallback(20) =  "SolModFlashPWM20" 'flash sniper x2
    SolModCallback(26) =  "vpmFlasher f126," 'flash pop x3
    SolModCallback(27) =  "SolModFlashPWM27" 'flash right side under jet?
    SolModCallback(31) =  "SolModFlashPWM31" 'Orange Dome Slingshots x2
    SolModCallback(32) =  "SolModFlashPWM32" 'flash left spinner x3
  Else
    SolModCallback(19) =  "SolModFlash19" 'flash safehouse x3
    SolModCallback(20) =  "SolModFlash20" 'flash sniper x2
    SolModCallback(26) =  "vpmFlasher f126," 'flash pop x3
    SolModCallback(27) =  "SolModFlash27" 'flash right side under jet?
    SolModCallback(31) =  "SolModFlash31" 'Orange Dome Slingshots x2
    SolModCallback(32) =  "SolModFlash32" 'flash left spinner x3
  End If
End If
...
Sub solSniper(Enabled)
  If Enabled Then
    sniperState = True
    SniperOpen.Enabled = True
  Else
    sniperState = False
    SniperClose.Enabled = True
    End If
End Sub
...
Sub solSafehouse(Enabled)
  If Enabled Then
    safehouseState = True
    SafehouseOpen.Enabled = True
  Else
    safehouseState = False
    SafehouseClose.Enabled = True
  End If
End Sub

'Suitcase motor
Sub MotorSuitcase(Enabled)
  If Enabled Then
    SuitcaseT.Enabled = True
  Else
    SuitcaseT.Enabled = False
  End If
End Sub
...
Sub SolPostUp(Enabled)
  ...
  Else
    PostUp.Isdropped = 1
  End If
  playsound SoundFX("Stern_UpPost",DOFContactors)
End Sub
...
Sub SolSuitCase(Enabled)
  If Enabled Then
    ...
    playsound SoundFX("Stern_UpPost",DOFContactors)
  Else
    SuitcaseTimer.Enabled = 1
  End If
End Sub

Sub SuitcaseTimer_Timer()
...
Sub GI_Normal(no, Enabled)
  ...
  If Enabled Then
    gilvl = 1
    TwentyFourShadow.Visible = True
  Else
    gilvl = 0
    TwentyFourShadow.Visible = False
  End If
End Sub
...
Sub sw3_Hit : bsTEject.addball 0 : SoundSaucerLock : End Sub
...
Sub ResetDropsL(Enabled)
  If Enabled Then
    PlaySoundAt SoundFX(DTResetSound,DOFContactors),sw60p
    DTRaise 60
  End If
End Sub
...
Sub ResetDropsR(Enabled)
  If Enabled Then
    PlaySoundAt SoundFX(DTResetSound,DOFContactors),sw61p
    DTRaise 61
  End If
End Sub
...
'Spinners
Sub sw10_Spin : vpmTimer.PulseSw 10 : SoundSpinner sw10 : End Sub

 'Stand Up Targets
Sub sw1_hit : STHit 1 : End Sub
Sub sw2_hit : STHit 2 : End Sub
Sub sw4_hit : STHit 4 : End Sub
Sub sw5_hit : STHit 5 : End Sub
Sub sw6_hit : STHit 6 : End Sub
Sub sw7_hit : STHit 7 : End Sub
Sub sw8_hit : STHit 8 : End Sub
...
Sub sw43_Hit : Controller.Switch(45) = 1 : End Sub
Sub sw43_unHit : Controller.Switch(45) = 0 : End Sub
...
Sub sw45_Hit : Controller.Switch(43) = 1 : Post44.IsDropped = False : End Sub   'suitcase lock bottom
Sub sw45_unHit : Controller.Switch(43) = 0 : Post44.IsDropped = True : End Sub
...
Sub SniperOpen_Timer()
  If sniperState = True And sniperOpenStep < 5 Then
    If sniperOpenStep Mod 2 = 0 Then
      Sniper.rotandtra8 = 95
    Else
      Sniper.rotandtra8 = 90
    End If
    sniperOpenStep = sniperOpenStep + 1
  Else
    SniperOpen.Enabled = False
    sniperOpenStep = 0
  End If
End Sub

Sub SniperClose_Timer()
  If sniperState = False And Sniper.rotandtra8 > 25 Then
      Sniper.rotandtra8 = Sniper.rotandtra8 - 5
    Else
      SniperClose.Enabled = False
    End If
End Sub
...
Sub SafehouseOpen_Timer()
  If safehouseState = True And Safehouse.RotY > 180 Then
    DOF 201, DOFOn
    Safehouse.RotY = Safehouse.RotY - 3
    Safehouse_on.RotY = Safehouse_on.RotY - 3
    DOF 201, DOFOff
  Else
    SafehouseOpen.Enabled = False
  End If
End Sub

Sub SafehouseClose_Timer()
  If safehouseState = False And Safehouse.RotY < 270 Then
    DOF 201, DOFOn
    Safehouse.RotY = Safehouse.RotY + 3
    Safehouse_on.RotY = Safehouse_on.RotY + 3
    DOF 201, DOFOff
  Else
    SafehouseClose.Enabled = False
  End If
End Sub
...
Dim suitcaseDirection
suitcaseDirection = 1

Sub SuitcaseT_Timer()
  If suitcaseDirection = 1 Then
    Controller.Switch(48) = 0
    DOF 201, DOFOn
    Suitcase.RotAndTra7 = Suitcase.RotAndTra7 - 1
    DOF 201, DOFOff
    If Suitcase.RotAndTra7 = 320 Then
      suitcaseDirection = 0
    End If
  Else
    DOF 201, DOFOn
    Suitcase.RotAndTra7 = Suitcase.RotAndTra7 + 1
    DOF 201, DOFOff
    If Suitcase.RotAndTra7 = 355 Then
      Controller.Switch(48) = 1
    ElseIf Suitcase.RotAndTra7 = 358 Then
      suitcaseDirection = 1
    End If
  End If
End Sub
...
Set DT60 = (New DropTarget)(sw60, sw60a, sw60p, 60, 0, False)
Set DT61 = (New DropTarget)(sw61, sw61a, sw61p, 61, 0, False)

Dim DTArray
DTArray = Array(DT11, DT13, DT60, DT61)
...
Dim ST1, ST2, ST4, ST5, ST6, ST7, ST8, ST9, ST33, ST34, ST39, ST40, ST41, ST62
...
Set ST6 = (new StandupTarget)(sw6, sw6p, 6, 0)
Set ST7 = (new StandupTarget)(sw7, sw7p, 7, 0)
Set ST8 = (new StandupTarget)(sw8, sw8p, 8, 0)
Set ST9 = (new StandupTarget)(sw9, sw9p, 9, 0)
...
Dim STArray
STArray = Array(ST1, ST2, ST4, ST5, ST6, ST7, ST8, ST9, ST33, ST34, ST39, ST41, ST41, ST62)
...
InitFlasher 5, "white"
InitFlasher 6, "orange"
InitFlasher 7, "orange"
...
Sub SolFlash20(Enabled)
  If Enabled Then
    ObjTargetLevel(5) = 1
    Sniper.image = "SniperTexture_ON"
  Else
    ObjTargetLevel(5) = 0
    Sniper.image = "SniperTexture"
  End If
  FlasherFlash5_Timer
...
Sub SolModFlash19(level)
  Objlevel(6) = level : Objlevel(7) = level : f119 = level : FlasherFlash6_Timer : FlasherFlash7_Timer
  If level > 0 Then
...
Sub SolModFlashPWM19(level)
  ModFlashFlasher 6,level
  ModFlashFlasher 7,level
  f119 = level
End Sub

Sub SolModFlashPWM20(level)
...
Dim ColorLUT : ColorLUT = 1               ' Color desaturation LUTs: 1 to 27
...
Dim SideBladeMod, LegendsCabMod, OutlaneDifficulty, InstMod, FlipperMod, JetMod, GlassMod, DMDDecalMod, GrillDecalmod, TopperMod, LogoPosterMod
...
Sub Table1_OptionEvent(ByVal eventId)
  ...
  SetupOutlanes
  'Instruction Cards
  InstMod = Table1.Option("Instruction Cards", 0, 4, 1, 0, 0, Array("Stern Instruction Cards", "Spanish Instruction Cards", "Custom Instruction Cards 1", "Custom Instruction Cards 2", "Custom Instruction Cards 3"))
  SetupInstructionCards
  'Custom Flippers
  FlipperMod = Table1.Option("Flipperbats", 0, 1, 1, 0, 0, Array("Original", "White Bats Blue Rubbers"))
  SetupFlippers
  'Sideblades
  SideBladeMod = Table1.Option("Side Blades", 0, 1, 1, 0, 0, Array("Original", "Retrorefurbs"))
  SetupBlades
  'Jets
  JetMod = Table1.Option("Jet Toy", 0, 1, 1, 0, 0, Array("Original Plastic", "32assassin's F-35 Jet"))
  SetupJet
  'LegendsCab
  ...
  DMDDecalmod = Table1.Option("Speaker/DMD Panel (VR-FSS)", 0, 2, 1, 0, 0, Array("Original", "Retrorefurbs", "Pinballparts"))
  SetupDMDDecal
  'Speakergrill (VR-FSS)
...
Sub SetRenderProbes(Opt)
  On Error Resume Next
    Select Case Opt
      Case 0:
        Primitive78.RefractionProbe = ""
        Primitive66.RefractionProbe = ""
      Case 1:
        Primitive78.RefractionProbe = "Ramps"
        Primitive66.RefractionProbe = "Ramps"
    End Select
  On Error Goto 0
End Sub
...
Sub SetupOutlanes
  If OutlaneDifficulty = 0 Then
    zCol_Rubber_Peg031.Collidable = True
    zCol_Rubber_Peg003.Collidable = False
    zCol_Rubber_Peg030.Collidable = False
    zCol_Rubber_Peg029.Collidable = True
    zCol_Rubber_Peg004.Collidable = False
    zCol_Rubber_Peg028.Collidable = False
    OutlanepostLEasy.Visible = True
    OutlanepostLMedium.Visible = False
    OutlanepostLHard.Visible = False
    OutlaneRubberpostREasy.Visible = True
    OutlaneRubberpostRMedium.Visible = False
    OutlaneRubberpostRHard.Visible = False
    OutlanepostREasy.Visible = True
    OutlanepostRMedium.Visible = False
    OutlanepostRHard.Visible = False
    OutlaneRubberpostLEasy.Visible = True
    OutlaneRubberpostLMedium.Visible = False
    OutlaneRubberpostLHard.Visible = False
    OutlaneRubberpostREasy.Visible = True
    OutlaneRubberpostRMedium.Visible = False
    OutlaneRubberpostRHard.Visible = False
  End If
  If OutlaneDifficulty = 1 Then
    zCol_Rubber_Peg031.Collidable = False
    zCol_Rubber_Peg003.Collidable = True
    zCol_Rubber_Peg030.Collidable = False
    zCol_Rubber_Peg029.Collidable = False
    zCol_Rubber_Peg004.Collidable = True
    zCol_Rubber_Peg028.Collidable = False
    OutlanepostLEasy.Visible = False
    OutlanepostLMedium.Visible = True
    OutlanepostLHard.Visible = False
    OutlanepostREasy.Visible = False
    OutlanepostRMedium.Visible = True
    OutlanepostRHard.Visible = False
    OutlaneRubberpostLEasy.Visible = False
    OutlaneRubberpostLMedium.Visible = True
    OutlaneRubberpostLHard.Visible = False
    OutlaneRubberpostREasy.Visible = False
    OutlaneRubberpostRMedium.Visible = True
    OutlaneRubberpostRHard.Visible = False
  End If
  If OutlaneDifficulty = 2 Then
    zCol_Rubber_Peg031.Collidable = False
    zCol_Rubber_Peg003.Collidable = False
    zCol_Rubber_Peg030.Collidable = True
    zCol_Rubber_Peg029.Collidable = False
    zCol_Rubber_Peg004.Collidable = False
    zCol_Rubber_Peg028.Collidable = True
    OutlanepostLEasy.Visible = False
    OutlanepostLMedium.Visible = False
    OutlanepostLHard.Visible = True
    OutlanepostREasy.Visible = False
    OutlanepostRMedium.Visible = False
    OutlanepostRHard.Visible = True
    OutlaneRubberpostLEasy.Visible = False
    OutlaneRubberpostLMedium.Visible = False
    OutlaneRubberpostLHard.Visible = True
    OutlaneRubberpostREasy.Visible = False
    OutlaneRubberpostRMedium.Visible = False
    OutlaneRubberpostRHard.Visible = True
  End If
End Sub
...
Sub SetupInstructionCards
  ...
  End If
  If InstMod = 1 Then
    Cards.image = "InstructionCardsSpanish"
  End If
  If InstMod = 2 Then
    Cards.image = "InstructionCards_Custom1"
  End If
  If InstMod = 3 Then
    Cards.image = "InstructionCards_Custom2"
  End If
  If InstMod = 4 Then
    Cards.image = "InstructionCards_Custom3"
  End If
End Sub

Sub SetupFlippers
  If FlipperMod = 1 Then
    LFLogo.image = "BatWhiteBlue"
    RFLogo.image = "BatWhiteBlue"
  Else
...
Sub SetupBlades
  ...
  If SideBladeMod = 1 Then
    PinCab_Blades.image = "PinCab_Blades_Custom"
    SideBladesLegends.image = "Sidewalls_24"
  End if
End Sub
...
Sub SetupJet
  If JetMod = 0 Then
    Jet.Visible = True
    Jet_Custom.Visible = False
  Else
    Jet.Visible = False
    Jet_Custom.Visible = True
  End If
End Sub
