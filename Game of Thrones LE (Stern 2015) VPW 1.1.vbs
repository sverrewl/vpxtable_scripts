...
Sub trigRamp6_hit
  ...
  vpmTimer.AddTimer 1000,"EnableITKicker '"
End Sub
...
'  Create a Battle Score scene specific to this House Battle
    ...
    Public Sub DoMyBattleCompleteScene
        ...
        ElseIf HouseCompleteWithDireWolf = True then
      'no house mode points when using direwolf (other than 5,000,000) but display direwolf?
      debug.print "DoMyBattleCompleteScene: House completed with direwolf"

      Set scene = NewSceneWithVideo("dwolf","got-direwolf")
      scene.AddActor FlexDMD.NewLabel("txt",FlexDMD.NewFont("udmd-f6by8.fnt",vbWhite,vbWhite,0),HouseToUCString(HouseBattle1)&vbLf&"COMPLETED"&vbLf&"5,000,000")
      scene.GetLabel("txt").SetAlignedPosition 88,16,FlexDMD_Align_Center
      DMDEnqueueScene scene,0,1500,2000,3000,"gotfx-direwolf"
    Else
            ' Fail!!
...
Sub BatteringRam_Hit
    If Tilted Then Exit Sub
    Dim scene,line1,i,hitstrength
  hitstrength = Int(ABS(ActiveBall.VelY))
  'Debug.Print hitstrength & " hitstrength"
  ...
  Debug.Print 125*hitstrength & " BatteringRamDOF_Off off timer"
...
Sub tmrMultiballSequencer_Timer
    ...
    if tmrMultiballSequencer.UserValue > 50 Then    ' We've run for 2.5 seconds
        ...
        LightSeqPlayfield_PlayDone
    End If
  if tmrMultiballSequencer.UserValue > 100 Then    ' We've run for 5? seconds
        Exit Sub
    End If
    For each a in aPlayfieldLights
        c = green
