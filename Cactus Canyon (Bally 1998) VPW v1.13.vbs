...
' 1.1 Release
' 1.11  - Niwak     - Warning: VPX 10.8 & PinMame 3.6 only - PWM GI, PWM flashers, move options to tweak UI, use raytraced shadow, add some refraction to plastic ramps
' 1.12 - flux - add standalone patch
' 1.13 - Retro27 - Added Auto VR rendering mode, Rules, VR Logo Option
...
'///////////////////////-----VR Room-----///////////////////////
Dim VRRoom '0 - Minimal Room, 1 - Ultra Minimal

'///////////////////////-----VR Logo-----///////////////////////
Dim VRLogo '0 - VRLogo off, 1 - VR Logo on
...
'///////////////////////-------- Other Options --------///////////////////////
Dim BartHatSoundEffects   '1=include sound effects, 0=turn off sound effects
...
Sub Table1_OptionEvent(ByVal eventId)
  ...
  BartHatSoundEffects = Table1.Option("Bart Hat Sound", 0, 1, 1, 0, 0, Array("Off", "On"))
  VRRoom = Table1.Option("VR Room", 0, 1, 1, 0, 0, Array("Minimal", "Ultra Minimal"))
  VRLogo = Table1.Option("VR Room Logo", 0, 1, 1, 0, 0, Array("Off", "On"))
  ...
  'VR Logo
  '*********************
  If VRLogo = 1 and Renderingmode = 2 then
      VR_Logo.visible = 1
  Else
      VR_Logo.visible = 0
  end If
...
Dim UseVPMDMD
If DesktopMode <> 0 Then UseVPMDMD = True Else UseVPMDMD = DesktopMode
Dim UseSolenoids
...
Sub Table1_Init
  ...
  With Controller
    ...
    ' Flashers
    .SolMask(1018) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Mine Flasher
    .SolMask(1019) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Front Left Flasher
    .SolMask(1020) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Front Right Flasher
    .SolMask(1024) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Beacon Flasher
    .SolMask(1025) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Middle Right Flasher
    .SolMask(1026) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Saloon Flasher
    .SolMask(1027) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Back Right Flasher
    .SolMask(1028) = VPM_MODOUT_BULB_89_32V_DC_S11 ' Back Left Flasher
    ' GI
    .SolMask(1201) = VPM_MODOUT_BULB_44_6_3V_AC
    .SolMask(1202) = VPM_MODOUT_BULB_44_6_3V_AC
    .SolMask(1203) = VPM_MODOUT_BULB_44_6_3V_AC
    .SolMask(1204) = VPM_MODOUT_BULB_44_6_3V_AC
    .SolMask(1205) = VPM_MODOUT_BULB_44_6_3V_AC
  End With
...
Sub FlashCol(col, value)
  Dim x: For Each x in col
    x.Visible = True
    If TypeName(x) = "Light" Then x.State = value / 255.0
    If TypeName(x) = "Flasher" Then x.Opacity = value * 100.0 / 255.0
  Next
End Sub
...
Sub Sol26(value)
  If PROC = 0 Then
    FlashCol F26, value
    F26R.Opacity = value * 100.0 / 255.0
...
Sub UpdateGI(nr, step)
  Dim xxx, v : v = step / 255.0 ' PWM Modulated GI String (0-255)
  If PROC=0 Then
    Debug.Print nr & " = " & step
    Select Case nr
      Case 0
        For each xxx in LeftGI:xxx.state=v:Next
      Case 1
        For each xxx in RightGI:xxx.state=v:Next
      Case 2
        For each xxx in TopGI:xxx.state=v:Next
        If DesktopMode = 0 then
          For each xxx in TopGI2:xxx.state=v:Next
        Else
          For each xxx in TopGI2:xxx.state=0:Next
        End If
        Bart.blenddisablelighting = v
        Bart_Hat.blenddisablelighting = v
        For each xxx in BWGI:xxx.Opacity = v * 100:next
        If v>=0.5 Then Table1.ColorGradeImage = "ColorGrade_8":Else Table1.ColorGradeImage = "ColorGrade_" & (step+1):End If
    End Select
  Else
    Select Case nr
      Case 0
        For each xxx in LeftGI:xxx.state=v:Next ' lights
        If v>0.5 Then Table1.ColorGradeImage = "ColorGrade_8":Else Table1.ColorGradeImage = "ColorGrade_1":End If
      Case 1
        For each xxx in RightGI:xxx.state=step:Next
      Case 2
        For each xxx in TopGI:xxx.state=step:Next ' lights
        For each xxx in BWGI:xxx.Opacity = step * 100:Next ' flashers
        For each xxx in TopGI2:xxx.state = step:Next ' lights
    End Select
  End If
End Sub
