'
' _______                   __      __
' \      \   ______  _  __ /  \    /  \_____ ___  __ ____
' /   |   \_/ __ \ \/ \/ / \   \/\/   /\__  \\  \/ // __ \
'/    |    \  ___/\     /   \        /  / __ \\   /\  ___/
'\____|__  /\___  >\/\_/     \__/\  /  (____  /\_/  \___  >
'        \/     \/                \/        \/          \/
'
'                Bell Games (1985)
'
'                https://www.ipdb.org/machine.cgi?id=3482

'********************************************************************************************************************************
'******** Work done by UnclePaulie on Hybrid version 0.01 - 2.0 *********
'   Complete reuse from UnclePaulie's Black Pyramid table, as New Wave is a conversion kit
'   Things implemented from Black Pyramid:
...
Const cGameName = "newwave"   'PinMAME ROM name
...
dim bumperscale:    bumperscale =     0.2 ' adjusts the level of blenddisablelighting and intensity on bumpers when lights are not on
dim bumperflashlevel:   bumperflashlevel =  0.5   ' adjusts the level of bumper lights when on and / or flashing


Dim FlipperColor, ScrewCapColor, cabsideblades, TargetColor, PegColor, DTImage, PlasticColor, BumperTop, BumperBot, PlasticImage, Rolloverchoice, Rolloverlight
...
Sub Table1_OptionEvent(ByVal eventId)
  ...
  ROMVolume = Table1.Option("ROM Volume (-32 off; 0 max) - Requires Restart; altsound set to 0", -32, 0, 1, 0, 0)
    ...
    ' Ball Choice
    ChooseBall = Table1.Option("Ball Choice", 0, 18, 1, 8, 0, Array("0: Hauntfreaks dark", "1: Hauntfreaks not as dark", _
      ...
      "11: DarthVito ballHDRdark", "12: Borg Ball", "13: SteelBall2", "14: Green GlowBall", _
      "15: Blue Glowball", "16: Red Glowball", "17: Redbone Plasma Glowball", "18: Chartreuse"))
  ChangeBall(ChooseBall)

  ' Bumper Options
  bumperscale = Table1.Option("Bumper brightness when not lit (default 20%)", 0, 1, 0.01, 0.20, 1)
  bumperflashlevel = Table1.Option("Bumper brightness when lit or flashing (default 50%)", 0, 1, 0.01, 0.50, 1)
  l2_animate
  l18_animate

  ' General Options
  FlipperColor = Table1.Option("Flipper Color", 1, 14, 1, 4, 0, Array("Red", "Blue", "Yellow", "Red Worn (default)", "Blue Worn", _
      "White Worn", "Yellow/Orange Worn", "Aqua Worn", "Light Green Worn", "Black Worn", "Light Blue Worn", "Lime Worn", _
      "Yellow Worn", "Custom New Wave"))
  TargetColor = Table1.Option("Target Color for Side Standups", 0, 2, 1, 0, 0, Array("White (default)", "Red", "Yellow"))
  DTImage = Table1.Option("Drop Target Image", 0, 7, 1, 0, 0, Array("Bullseye (default)", "Boy George", "Billy Idol", "Depeche Mode", "Tears for Fears", "Duran Duran", "Wham 1", "Wham 2"))
  PegColor = Table1.Option("Peg Color", 0, 2, 1, 0, 0, Array("Red (default)", "Yellow", "Blue"))
  PlasticImage = Table1.Option("Plastic Images", 0, 1, 1, 0, 0, Array("Original (default)", "Alternate"))
  Rolloverchoice = Table1.Option("Rollovers", 0, 1, 1, 0, 0, Array("Original Red Plastic (default)", "Alternate - like normal tables"))
  Rolloverlight = Table1.Option("Rollover Lights", 0, 1, 1, 0, 0, Array("Off (default)", "On"))
  BumperTop = Table1.Option("Bumper Cap", 0, 1, 1, 0, 0, Array("Original (default)", "Alternate"))
  BumperBot = Table1.Option("Bumper Disk", 0, 3, 1, 2, 0, Array("Yellow/Yellow", "Pink/Pink", "Pink/Yellow (default)", "Yellow/Pink"))
...
Sub CheckBallLocations
  ...
  For b = 0 to UBound(gBOT)
    ...
    If InRect(gBOT(b).X, gBOT(b).Y, 857,1618,383,1897,441,1966,857,1738) Then
...
debug.print  stshptargetc.opacity
...
' Rollover Light
  If Rolloverlight  = 0  Then
    For each x in GI_Rollovers: x.state = 0: Next
  Else
    For each x in GI_Rollovers: x.state = gilvl: Next
  End If

' Rollovers
    For each x in RolloverRed: x.blenddisablelighting = ((0.15 * gilvl)  + 0.005) * (level/0.15): Next
    For each x in RolloverAlt: x.blenddisablelighting = ((0.005 * gilvl)  + 0.005) * (level/0.15): Next
...
Sub Table1_KeyUp(ByVal keycode)   '***What to do when a button is released***
  If keycode = PlungerKey Then
    Plunger.firespeed = RndInt(93,97)   ' Generate random value variance of +/- 2 from 95
    Plunger.Fire
    If BIPL = 1 Then
...
Sub sw7_Hit():  Controller.Switch(7) =  1: psw7a.transy = -18: End Sub
Sub sw7_UnHit:  Controller.Switch(7) =  0: psw7a.transy = 0: End Sub
Sub sw17_Hit(): Controller.Switch(17) = 1: psw17a.transy = -18: End Sub
Sub sw17_UnHit: Controller.Switch(17) = 0: psw17a.transy = 0: End Sub
Sub sw22_Hit(): Controller.Switch(22) = 1: psw22a.transy = -18: End Sub
Sub sw22_UnHit: Controller.Switch(22) = 0: psw22a.transy = 0: End Sub
Sub sw23_Hit(): Controller.Switch(23) = 1: psw23a.transy = -18: End Sub
Sub sw23_UnHit: Controller.Switch(23) = 0: psw23a.transy = 0: End Sub
Sub sw24_Hit(): Controller.Switch(24) = 1: psw24a.transy = -18: End Sub
Sub sw24_UnHit: Controller.Switch(24) = 0: psw24a.transy = 0: End Sub
Sub sw28_Hit(): Controller.Switch(28) = 1: psw28a.transy = -18: End Sub
Sub sw28_UnHit: Controller.Switch(28) = 0: psw28a.transy = 0: End Sub
Sub sw29_Hit(): Controller.Switch(29) = 1: psw29a.transy = -18: End Sub
Sub sw29_UnHit: Controller.Switch(29) = 0: psw29a.transy = 0:  End Sub
...
'alternate
Sub sw7_Animate:  psw7.transz  = sw7.CurrentAnimOffset:  End Sub
...
dim insert_red_on: insert_red_on = insertfactor * 7.5
...
Sub l58_animate:
  p58.BlendDisableLighting = insert_white_off + insert_white_on * (l58.GetInPlayIntensity / l58.Intensity)
  p58off.BlendDisableLighting = a_insert_white_off - a_insert_white_on * (l58.GetInPlayIntensity / l58.Intensity)
...
Sub l3_animate
  p3.BlendDisableLighting = insert_red_off + insert_red_on * (l3.GetInPlayIntensity / l3.Intensity)
  p3off.BlendDisableLighting = a_insert_red_off - a_insert_red_on * (l3.GetInPlayIntensity / l3.Intensity)
  bulb3.BlendDisableLighting = bulb_off + bulb_on * (l3.GetInPlayIntensity / l3.Intensity)
End Sub

Sub l6_animate:
  p6.BlendDisableLighting = insert_red_off + insert_red_on * (l6.GetInPlayIntensity / l6.Intensity)
  p6off.BlendDisableLighting = a_insert_red_off - a_insert_red_on * (l6.GetInPlayIntensity / l6.Intensity)
...
Sub l4_animate
  p4.BlendDisableLighting = insert_orange_off + insert_orange_on * (l4.GetInPlayIntensity / l4.Intensity)
  p4off.BlendDisableLighting = a_insert_orange_off - a_insert_orange_on * (l4.GetInPlayIntensity / l4.Intensity)
...
Sub l17_animate
  p17.BlendDisableLighting = insert_orange_off + insert_orange_on * (l17.GetInPlayIntensity / l17.Intensity)
  p17off.BlendDisableLighting = a_insert_orange_off - a_insert_orange_on * (l17.GetInPlayIntensity / l17.Intensity)
...
Sub l66_animate
  p66.BlendDisableLighting = insert_orange_off + insert_orange_on * (l66.GetInPlayIntensity / l66.Intensity)
  p66off.BlendDisableLighting = a_insert_orange_off - a_insert_orange_on * (l66.GetInPlayIntensity / l66.Intensity)
...
Sub l98_animate
  p98.BlendDisableLighting = insert_orange_off + insert_orange_on * (l98.GetInPlayIntensity / l98.Intensity)
  p98off.BlendDisableLighting = a_insert_orange_off - a_insert_orange_on * (l98.GetInPlayIntensity / l98.Intensity)
...
Sub l81_animate
  p81.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l81.GetInPlayIntensity / l81.Intensity)
  p81off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l81.GetInPlayIntensity / l81.Intensity)
...
Sub l14_animate
  p14.BlendDisableLighting = insert_green_off + insert_green_on * (l14.GetInPlayIntensity / l14.Intensity)
  p14off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l14.GetInPlayIntensity / l14.Intensity)
  bulb14.BlendDisableLighting = bulb_off + bulb_on * (l14.GetInPlayIntensity / l14.Intensity)
End Sub

Sub l15_animate
  p15.BlendDisableLighting = insert_green_off + insert_green_on * (l15.GetInPlayIntensity / l15.Intensity)
  p15off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l15.GetInPlayIntensity / l15.Intensity)
...
Sub l31_animate
  p31.BlendDisableLighting = insert_green_off + insert_green_on * (l31.GetInPlayIntensity / l31.Intensity)
  p31off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l31.GetInPlayIntensity / l31.Intensity)
...
Sub l46_animate
  p46.BlendDisableLighting = insert_green_off + insert_green_on * (l46.GetInPlayIntensity / l46.Intensity)
  p46off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l46.GetInPlayIntensity / l46.Intensity)
...
Sub l54_animate
  p54.BlendDisableLighting = insert_green_off + insert_green_on * (l54.GetInPlayIntensity / l54.Intensity)
  p54off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l54.GetInPlayIntensity / l54.Intensity)
...
Sub l62_animate
  p62.BlendDisableLighting = insert_green_off + insert_green_on * (l62.GetInPlayIntensity / l62.Intensity)
  p62off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l62.GetInPlayIntensity / l62.Intensity)
...
Sub l113_animate
  p113.BlendDisableLighting = insert_green_off + insert_green_on * (l113.GetInPlayIntensity / l113.Intensity)
  p113off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l113.GetInPlayIntensity / l113.Intensity)
...
Sub l2_animate:
  flbumpvalue1 = bumperflashlevel * 2.25 * (l2.GetInPlayIntensity / l2.Intensity)
  FlFadeBumper 1, flbumpvalue1
End Sub

Sub l18_animate:
  flbumpvalue2 = bumperflashlevel * 2.25 * (l18.GetInPlayIntensity / l18.Intensity)
...
' debug.print "SetRelayGI value: " & Enabled
  ...
  if Enabled = 1 Then
    ...
    If DesktopMode = True then
      L_DT_1.intensity = 8
      L_DT_2.intensity = 3.5
      L_DT_2a.intensity = 3.5
    Else
      L_DT_1.intensity = 0
      L_DT_2.intensity = 0
      L_DT_2a.intensity = 0
...
FlInitBumper 1, "newwave"
FlInitBumper 2, "newwave"
...
' FlBumperBase(nr).BlendDisableLighting = 0.5 * DayNightAdjust
  ...
  Select Case FlBumperColor(nr)
    ...
    Case "newwave"
      ...
      FlBumperBulb(nr).BlendDisableLighting = bumperscale * (12 + 2000 * (0.03 * Z + 0.97 * Z ^ 10))
      Flbumperbiglight(nr).intensity = bumperscale * (2.5 * Z )
      FlBumperHighlight(nr).opacity = bumperscale * (1000 * (Z^3) )
      FlBumperTop(nr).BlendDisableLighting = bumperscale * (0.15 + 0.5 * Z)
      FlBumperCapBase(nr).BlendDisableLighting = bumperscale * (0.15 + 0.5 * Z)
      MaterialColor "bumpertopmat" & nr, RGB(255,250 - z*6,220 - Z*15)
      MaterialColor "bumpertopmata" & nr, RGB(255,250 - z*6,220 - Z*15)
      FLBumperLight(nr).intensity =  bumperscale * (30 * Z - 2)
...
'******************************************************
' Ball brightness and Glowball code
'******************************************************
...
Dim GlowBall
Dim CustomBallImage(19), CustomBallLogoMode(19), CustomBallDecal(19), CustomBallRS(19), CustomBulbIntensity(19), CustomBallGlow(19), GBred(19), GBgreen(19), GBblue(19)
...
CustomBallRS(6) =       0.4
GBred(6) = 0 : GBgreen(0) = 0 : GBblue(0) = 0
...
CustomBallRS(7) =       0.4
GBred(7) = 0 : GBgreen(0) = 0 : GBblue(0) = 0
...
CustomBallRS(8) =       0.4
GBred(8) = 0 : GBgreen(0) = 0 : GBblue(0) = 0
...
CustomBallRS(9) =       0.4
GBred(9) = 0 : GBgreen(0) = 0 : GBblue(0) = 0
...
CustomBallRS(10) =      0.4
GBred(10) = 0 : GBgreen(0)  = 0 : GBblue(0) = 0

' DarthVito ballHDRdark
CustomBallGlow(11) =    False
CustomBallImage(11) =     "ball_HDR3dark"
CustomBallLogoMode(11) =  False
CustomBallDecal(11) =     "ball_scratches"
CustomBulbIntensity(11) =   1.0
CustomBallRS(11) =      0.4
GBred(11) = 0 : GBgreen(0)  = 0 : GBblue(0) = 0

' Borg Ball
CustomBallGlow(12) =    False
CustomBallImage(12) =     "BorgBall"
CustomBallLogoMode(12) =  False
CustomBallDecal(12) =     "ball_scratches"
CustomBulbIntensity(12) =   1.0
CustomBallRS(12) =      0.4
GBred(12) = 0 : GBgreen(0)  = 0 : GBblue(0) = 0

' SteelBall2
CustomBallGlow(13) =    False
CustomBallImage(13) =     "SteelBall2"
CustomBallLogoMode(13) =  False
CustomBallDecal(13) =     "ball_scratches"
CustomBulbIntensity(13) =   1.0
CustomBallRS(13) =      0.4
GBred(13) = 0 : GBgreen(0)  = 0 : GBblue(0) = 0

' Green GlowBall
CustomBallGlow(14) =    True
CustomBallImage(14) =     "ball_glowgreen"
CustomBallLogoMode(14) =  True
CustomBallDecal(14) =     ""
CustomBulbIntensity(14) =   0
CustomBallRS(14) =      0
GBred(14) = 20 : GBgreen(14)  = 200 : GBblue(14) = 0

' Blue GlowBall
CustomBallGlow(15) =    True
CustomBallImage(15) =     "glowball blue"
CustomBallLogoMode(15) =  True
CustomBallDecal(15) =     ""
CustomBulbIntensity(15) =   0
CustomBallRS(15) =      0
GBred(15) = 50 : GBgreen(15)  = 50 : GBblue(15) = 255

' Red GlowBall
CustomBallGlow(16) =    True
CustomBallImage(16) =     "glowball orange"
CustomBallLogoMode(16) =  True
CustomBallDecal(16) =     ""
CustomBulbIntensity(16) =   0
CustomBallRS(16) =      0
GBred(16) = 255 : GBgreen(16) = 0 : GBblue(16) = 000

' Redbone Plasma GlowBall
CustomBallGlow(17) =         True
CustomBallImage(17) =        "ball_plasma"
CustomBallLogoMode(17) =     True
CustomBallDecal(17) =        "ball_plasma"
CustomBulbIntensity(17) =    0.7
CustomBallRS(17) =      0
GBred(17) = 8 : GBgreen(17) = 20 : GBblue(17) = 160

'Chartreuse GlowBall
CustomBallGlow(18) =         True
CustomBallImage(18) =        "glowball_chartreuse"
CustomBallLogoMode(18) =     True
CustomBallDecal(18) =        "ball_scratches"
CustomBulbIntensity(18) =    0.7
CustomBallRS(18) =             0
GBred(18) = 223 : GBgreen(18) = 225 : GBblue(18) = 0
...
Sub UpdateGlowball
  Dim b
    ...
    If NOT bBallInTrough(b) Then
      ...
      Glowing(b).y = gBOT(b).y + anglecompensate
...
' Flipper Color
  ...
  Elseif FlipperColor = 13 Then
    Lflipmesh1.image = "flipperAOyellowworn-left"
    Rflipmesh1.image = "flipperAOyellowworn-right"
  Else
    Lflipmesh1.image = "flipperAOredWorn-left-newwave"
    Rflipmesh1.image = "flipperAOredWorn-right-newwave"
  End If
...
' Standup Target Color
  ...
  ElseIf TargetColor  = 2  Then
    For each C in GITargets
      C.image = "target-yellow"
    Next
  Else
    For each C in GITargets
      C.image = "target-white"
    Next
  End If
...
' Drop Target Image
  If DTImage  = 1  Then
    For each C in GIdtTargets
      C.image = "dt_white-boygeorge"
    Next
  ElseIf DTImage  = 2  Then
    For each C in GIdtTargets
      C.image = "dt_white-bullseye-billyidol"
    Next
  ElseIf DTImage  = 3  Then
    For each C in GIdtTargets
      C.image = "dt_white-bullseye-depechemode"
    Next
  ElseIf DTImage  = 4  Then
    For each C in GIdtTargets
      C.image = "dt_white-bullseye-tearsforfears"
    Next
  ElseIf DTImage  = 5  Then
    For each C in GIdtTargets
      C.image = "dt_white-bullseye-duranduran"
    Next
  ElseIf DTImage  = 6  Then
    For each C in GIdtTargets
      C.image = "dt_white-wham-1"
    Next
  ElseIf DTImage  = 7  Then
    For each C in GIdtTargets
      C.image = "dt_white-wham-2"
    Next
  Else
    For each C in GIdtTargets
      C.image = "dt_white-bullseye"
    Next
  End If
...
' Bumper Top
  If BumperTop  = 1  Then
    bumpertop1.image = "BumperCap-2"
    bumpertop2.image = "BumperCap-2"
  Else
    bumpertop1.image = "BumperCap"
    bumpertop2.image = "BumperCap"
  End If

' Bumper Disk
  If BumperBot = 1  Then
    bumperdisk1.image = "Flbumperdisk-Pink-Left"
    bumperdisk2.image = "Flbumperdisk-Pink-Right"
  ElseIf BumperBot = 2  Then
    bumperdisk1.image = "Flbumperdisk-Pink-Left"
    bumperdisk2.image = "Flbumperdisk-Yellow-Right"

  ElseIf BumperBot = 3  Then
...
' Plastic Image
  If PlasticImage  = 1  Then
    For each C in Plastics
      C.image = "Plastics-2"
    Next
  Else
    For each C in Plastics
...
' Rollover Prim
  If Rolloverchoice  = 0  Then
    For each C in RolloverRed
      C.visible = 1
    Next
    For each C in RolloverAlt
      C.visible = 0
    Next
  Else
    For each C in RolloverRed
...
'Bell Games New Wave
'Inkochnito code - for Black Pyramid (New Wave is a conversion kit
Sub editDips
...
'******** Work done by UnclePaulie on New Wave Hybrid version v.01 - 2.0 *********

'0.01 Started with playfield pictures off of web.  Redbone upscaled, straightened, cut holes, cleaned up, redrew, etc.
'   Removed saucer, playfield mesh
'   Made the top gates a little more bouncy
'   Adjusted the plunger speed and randomness to match the one online video of New Wave.
'   Added posts, rubbers, physics to new top lane middle guide areas
'   Created new desktop backglass
'   Adjusted the GI lights, and insert lights to correct location.
'     VR Cab sides, front, and backbox were done by devios626
'   Added all inserts and AllLamps
'   Modded the top rollover lanes, no lane guide plastics.  Just rubbers.
'   Added the GI bake
'   Redbone created plastics image, and bumper image.
'   Updated the VR backglass
'   Updated gamehelp image for F12 menu
'   Convert images to webp
'0.02 Added options for target colors and images
'     Added option for red, yellow, or blue pegs
'     Added options for led lighting around plastic edges
'   Added lots of options for plastics images, flippers, targets, bumpers, rollovers
'   Added GI to rollovers.  Also option for rollover pink lights.
'   Added glowballs, and adjusted the falloff ranges, etc.
'   Added middle standup target shadow
'2.0.0  Fixed script for chartreuse glowball.
'   Added top white pegs to screwcap GI.
'   Hauntfreaks provided new VR backglass images.  Refit them into the VR environment.
'   Updated the VR digits forward value.
'   Modified the plastics to see whole sticker


' ALL BASED OFF OF BLACK PYRAMID:

'******** Work done by UnclePaulie on Black Pyramid Hybrid version v.01 - 2.0 *********
