...
'
'
'  _____  _____            _____  ____  _   _ ______ _____  _____ _______
' |  __ \|  __ \     /\   / ____|/ __ \| \ | |  ____|_   _|/ ____|__   __|
' | |  | | |__) |   /  \ | |  __| |  | |  \| | |__    | | | (___    | |
' | |  | |  _  /   / /\ \| | |_ | |  | | . ` |  __|   | |  \___ \   | |
' | |__| | | \ \  / ____ \ |__| | |__| | |\  | |     _| |_ ____) |  | |
' |_____/|_|  \_\/_/    \_\_____|\____/|_| \_|_|    |_____|_____/   |_|
'
'
'
'               Stern Electronics, Incorporated, 1981; Stern M-200 MPU
'       Dragonfist / IPD No. 731 / November, 1981 / 4 Players
'               https://www.ipdb.org/machine.cgi?id=731
'
...
const UseAltROM = 0     '0 = original factory ROM; 1 = Alternate ROM with fixes and extra features


' NOTE:  IF YOU USE THE ALTERNATE ROM:  It will default to max credits of 0 (essentially freeplay).  Also, the high spins to date
'                   will be shown in player four's score during high score display.  There are also different
'                   settins for various bonus options via the F6 menu (You will need SetDIPSwitches set to 1 to ChangeBall)
...
'      To increase max credits... press 7 to open the ROM menu, keep pressing 7 until you see 18 in the display.
'                 then LONG press the "1" key to increase the max credits.


' Bumper and insert brightness scale

const bumperflashlevel  = 0.6 ' adjusts the level of bumper lights when on and / or flashing
const insertfactor    = 1.5 ' adjusts the level of the brightness of the inserts when on
...
' User Options for In-Game Menu F12 then use Flipper and Magna Keys
  ...
  Dim FlipperColor, ScrewCapColor, SideWallColor, PlasticColor, BumperColor, TargetColor, TargetMarks, SpinnerColor, SUTargetColor
...
Dim cGameName
Dim DT13up,DT14up,DT15up,DT19up,DT20up,DT21up,DT22up,DT23up,DT24up
Dim PlasProt
Dim DTMass: DTMass = 0.2 'initial value
Const DTMassWeak = 0.2 'Mass of the Drop Target (between 0 and 1), higher values provide more resistance
Const DTMassStrong = 0.5 'Mass of the Drop Target (between 0 and 1), higher values provide more resistance

'  Standard definitions

if UseAltROM = 1 Then
  cGameName = "dragfis16"   'PinMAME ROM name
Else
  cGameName = "dragfist"    'PinMAME ROM name
End If
...
LoadVPM "01200000","stern.vbs",3.10
...
Sub Table1_Init
  ...
  With Controller
    ...
    .SplashInfoLine = "Dragonfist (Stern 1981)"&chr(13)&"by UnclePaulie"
    .HandleKeyboard = 0
    .ShowTitle = 0
    .ShowDMDOnly = 1
    .ShowFrame = 0
...
' If user wants to set dip switches themselves it will force them to set it via F6.
  ...
  Set DBall1 = BallRelease.CreateSizedballWithMass(Ballsize/2,Ballmass)
...
Sub Table1_OptionEvent(ByVal eventId)
  ...
  ' Table Options
  FlipperColor = Table1.Option("Flipper Color", 0, 5, 1, 0, 0, Array("Red", "Blue", "Green", "Orange/Black", "Orange", "Yellow"))
  SUTargetColor = Table1.Option("Stand Up Target Color", 0, 1, 1, 0, 0, Array("Yellow", "Sticker"))
  TargetColor = Table1.Option("Drop Target Color", 0, 6, 1, 0, 0, Array("Red", "White", "S Sticker", "Dragonfist Sticker", "Bruce Lee Sticker", "Dragon Sticker", "Star Sticker"))
  TargetMarks = Table1.Option("Ball Marks across Targets", 0, 1, 1, 0, 0, Array("No", "Yes"))
  SpinnerColor = Table1.Option("Spinner Color", 0, 7, 1, 1, 0, Array("Yellow", "Yellow with scratches", "White", "White with scratches", "Glitter", "Glitter with scratches", "Dragon", "Dragon with scratches"))
  ScrewCapColor = Table1.Option("Screw Cap Color", 0, 5, 1, 0, 0, Array("White", "Black", "Bronze", "Silver", "Blue/Green", "Gold"))
  BumperColor = Table1.Option("Bumper Color", 0, 4, 1, 0, 0, Array("Red", "Blue", "Green", "Orange", "Yellow"))
  SideWallColor = Table1.Option("Side Wall Color", 0, 2, 1, 0, 0, Array("Blue", "Black", "Green"))
...
Const PLTop = 525       'Y position of punger lane top
...
Sub Table1_KeyUp(ByVal keycode)
  If keycode = PlungerKey Then
    Plunger.firespeed = RndInt(109,115)     ' Generate random value variance of +/- 3 from 112
    Plunger.Fire
    If BIPL = 1 Then
...
SolCallback(8)    = "SolBallRelease"    ' Trough to Plunger Lane
SolCallback(9)    = "DTBankLeftReset"   ' Drop Target Left Bank Reset
SolCallback(10)   = "DTBankCenterReset" ' Drop Target Center Bank Reset
SolCallback(11)   = "DTBankRightReset"  ' Drop Target Right Bank Reset
...
Sub spindampingtimer_timer()
  If spindamping > .996 Then
    spindamping = spindamping - 0.0002
  Else
    spindamping = .96
...
Sub SpinnerTimer
  SpinnerPrim.Rotx = -sw5.CurrentAngle
  SpinnerRod.TransY = sin((sw5.CurrentAngle) * (PI/180)-0.25) * -SpinnerRadius
  SpinnerRod.TransZ = (cos((sw5.CurrentAngle + 180) * (PI/180))+1) * SpinnerRadius
End Sub


'************************* Bumpers **************************
Sub Bumper1_Hit(): RandomSoundBumperMiddle Bumper1: vpmTimer.PulseSw 9: End Sub
Sub Bumper2_Hit(): RandomSoundBumperTop Bumper2: vpmTimer.PulseSw 10: End Sub


'********************* Leaf Sensors **********************
Sub Phys_RB_Sensor_sw25_hit: vpmTimer.PulseSw 25: End Sub
...
Sub sw36_Animate: psw36.transz = sw36.CurrentAnimOffset: End Sub
Sub sw37_Animate: psw37.transz = sw37.CurrentAnimOffset: End Sub
...
Sub sw16_Hit:vpmTimer.PulseSw 16:me.timerenabled = 0:AnimateStar star16, sw16, 1:End Sub
Sub sw16_UnHit:me.timerinterval = 7:me.timerenabled = 1:End Sub
Sub sw16_timer:AnimateStar star16, sw16, 0:End Sub
...
'********************** Drop Targets ************************
Sub Sw13_Hit: DTMass = DTMassStrong: DTHit 13: TargetBouncer Activeball, 1.5: End Sub
Sub Sw14_Hit: DTMass = DTMassStrong: DTHit 14: TargetBouncer Activeball, 1.5: End Sub
Sub Sw15_Hit: DTMass = DTMassStrong: DTHit 15: TargetBouncer Activeball, 1.5: End Sub
Sub Sw19_Hit: DTMass = DTMassWeak: DTHit 19: TargetBouncer Activeball, 1.5: End Sub
Sub Sw20_Hit: DTMass = DTMassWeak: DTHit 20: TargetBouncer Activeball, 1.5: End Sub
Sub Sw21_Hit: DTMass = DTMassWeak: DTHit 21: TargetBouncer Activeball, 1.5: End Sub
Sub Sw22_Hit: DTMass = DTMassWeak: DTHit 22: TargetBouncer Activeball, 1.5: End Sub
Sub Sw23_Hit: DTMass = DTMassWeak: DTHit 23: TargetBouncer Activeball, 1.5: End Sub
Sub Sw24_Hit: DTMass = DTMassWeak: DTHit 24: TargetBouncer Activeball, 1.5: End Sub
...
Sub DTBankLeftReset (enabled)
  If enabled then
  vpmTimer.AddTimer dtdelaytime, "RandomSoundDropTargetReset sw14p'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 13'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 14'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 15'"
    vpmTimer.AddTimer dtdelaytime, "DT13up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT14up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT15up=1'"
    vpmTimer.AddTimer dtdelaytime, "dtsh13.visible=True'"
...
Sub DTBankRightReset (enabled)
  If enabled then
  vpmTimer.AddTimer dtdelaytime, "RandomSoundDropTargetReset sw20p'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 19'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 20'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 21'"
    vpmTimer.AddTimer dtdelaytime, "DT19up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT20up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT21up=1'"
    vpmTimer.AddTimer dtdelaytime, "dtsh19.visible=True'"
    vpmTimer.AddTimer dtdelaytime, "dtsh20.visible=True'"
    vpmTimer.AddTimer dtdelaytime, "dtsh21.visible=True'"
  End if
End Sub
...
Dim DT13, DT14, DT15, DT19, DT20, DT21, DT22, DT23, DT24
...
Dim DTArray
DTArray = Array(DT13, DT14, DT15, DT19, DT20, DT21, DT22, DT23, DT24)
...
Sub DTShadowInit(dtnbr)
  ...
  if dtnbr = 1 Then
    Set DTShadow(dtnbr) = Eval("dtsh" & 13)
  elseif dtnbr = 2 Then
    Set DTShadow(dtnbr) = Eval("dtsh" & 14)
  elseif dtnbr = 3 Then
    Set DTShadow(dtnbr) = Eval("dtsh" & 15)
  elseif dtnbr = 4 Then
    Set DTShadow(dtnbr) = Eval("dtsh" & 19)
  elseif dtnbr = 5 Then
...
Function DTCheckBrick(aBall, dtprim)
  ...
  ElseIf perpvel > 0 And ((paravel > 0 And paravelafter > 0) Or (paravel < 0 And paravelafter < 0)) Then
    DTCheckBrick = 4
  Else
    If DTMass = DTMassWeak Then
      DTCheckBrick = 0
    Else
      DTCheckBrick = 1 ' this allows side hits
    End If
  End If
End Function
...
Dim ST17,ST18,ST26,ST27,ST28,ST29,ST30,ST31,ST32
...
Dim STArray
STArray = Array(ST17,ST18,ST26,ST27,ST28,ST29,ST30,ST31,ST32)
...
dim insert_blue_on: insert_blue_on = insertfactor * 8.5
const insert_blue_off = 1.0
dim insert_green_on: insert_green_on = insertfactor * 2.5
...
Sub l8_animate
  p8.BlendDisableLighting = insert_red_off + insert_red_on * (l8.GetInPlayIntensity / l8.Intensity)
  p8off.BlendDisableLighting = a_insert_red_off - a_insert_red_on * (l8.GetInPlayIntensity / l8.Intensity)
  bulb8.BlendDisableLighting = bulb_off + bulb_on * (l8.GetInPlayIntensity / l8.Intensity)
End Sub

Sub l11_animate
  p11.BlendDisableLighting = insert_red_off + insert_red_on * (l11.GetInPlayIntensity / l11.Intensity)
...
Sub l24_animate
  p24.BlendDisableLighting = insert_red_off + insert_red_on * (l24.GetInPlayIntensity / l24.Intensity)
  p24off.BlendDisableLighting = a_insert_red_off - a_insert_red_on * (l24.GetInPlayIntensity / l24.Intensity)
...
Sub l1_animate
  p1.BlendDisableLighting = insert_blue_off + insert_blue_on * (l1.GetInPlayIntensity / l1.Intensity)
  p1off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l1.GetInPlayIntensity / l1.Intensity)
  bulb1.BlendDisableLighting = bulb_off + bulb_on * (l1.GetInPlayIntensity / l1.Intensity)
End Sub

Sub l2_animate
  p2.BlendDisableLighting = insert_blue_off + insert_blue_on * (l2.GetInPlayIntensity / l2.Intensity)
  p2off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l2.GetInPlayIntensity / l2.Intensity)
  bulb2.BlendDisableLighting = bulb_off + bulb_on * (l2.GetInPlayIntensity / l2.Intensity)
End Sub

Sub l3_animate
  p3.BlendDisableLighting = insert_blue_off + insert_blue_on * (l3.GetInPlayIntensity / l3.Intensity)
  p3off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l3.GetInPlayIntensity / l3.Intensity)
  bulb3.BlendDisableLighting = bulb_off + bulb_on * (l3.GetInPlayIntensity / l3.Intensity)
End Sub

Sub l17_animate
  p17.BlendDisableLighting = insert_blue_off + insert_blue_on * (l17.GetInPlayIntensity / l17.Intensity)
  p17off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l17.GetInPlayIntensity / l17.Intensity)
  bulb17.BlendDisableLighting = bulb_off + bulb_on * (l17.GetInPlayIntensity / l17.Intensity)
End Sub

Sub l18_animate
  p18.BlendDisableLighting = insert_blue_off + insert_blue_on * (l18.GetInPlayIntensity / l18.Intensity)
  p18off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l18.GetInPlayIntensity / l18.Intensity)
  bulb18.BlendDisableLighting = bulb_off + bulb_on * (l18.GetInPlayIntensity / l18.Intensity)
End Sub

Sub l28_animate
  p28.BlendDisableLighting = insert_blue_off + insert_blue_on * (l28.GetInPlayIntensity / l28.Intensity)
  p28off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l28.GetInPlayIntensity / l28.Intensity)
  bulb28.BlendDisableLighting = bulb_off + bulb_on * (l28.GetInPlayIntensity / l28.Intensity)
End Sub

Sub l33_animate
  p33.BlendDisableLighting = insert_blue_off + insert_blue_on * (l33.GetInPlayIntensity / l33.Intensity)
  p33off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l33.GetInPlayIntensity / l33.Intensity)
  bulb33.BlendDisableLighting = bulb_off + bulb_on * (l33.GetInPlayIntensity / l33.Intensity)
End Sub

Sub l34_animate
  p34.BlendDisableLighting = insert_blue_off + insert_blue_on * (l34.GetInPlayIntensity / l34.Intensity)
  p34off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l34.GetInPlayIntensity / l34.Intensity)
  bulb34.BlendDisableLighting = bulb_off + bulb_on * (l34.GetInPlayIntensity / l34.Intensity)
End Sub

Sub l49_animate
  p49.BlendDisableLighting = insert_blue_off + insert_blue_on * (l49.GetInPlayIntensity / l49.Intensity)
  p49off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l49.GetInPlayIntensity / l49.Intensity)
  bulb49.BlendDisableLighting = bulb_off + bulb_on * (l49.GetInPlayIntensity / l49.Intensity)
End Sub

Sub l50_animate
  p50.BlendDisableLighting = insert_blue_off + insert_blue_on * (l50.GetInPlayIntensity / l50.Intensity)
  p50off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l50.GetInPlayIntensity / l50.Intensity)
...
Sub l6_animate
  p6.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l6.GetInPlayIntensity / l6.Intensity)
  p6off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l6.GetInPlayIntensity / l6.Intensity)
...
Sub l22_animate
  p22.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l22.GetInPlayIntensity / l22.Intensity)
  p22off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l22.GetInPlayIntensity / l22.Intensity)
...
Sub l30_animate
  p30.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l30.GetInPlayIntensity / l30.Intensity)
  p30off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l30.GetInPlayIntensity / l30.Intensity)
  bulb30.BlendDisableLighting = bulb_off + bulb_on * (l30.GetInPlayIntensity / l30.Intensity)
End Sub

Sub l38_animate
  p38.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l38.GetInPlayIntensity / l38.Intensity)
  p38off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l38.GetInPlayIntensity / l38.Intensity)
...
Sub l53_animate
  p53.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l53.GetInPlayIntensity / l53.Intensity)
  p53off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l53.GetInPlayIntensity / l53.Intensity)
  bulb53.BlendDisableLighting = bulb_off + bulb_on * (l53.GetInPlayIntensity / l53.Intensity)
End Sub

Sub l54_animate
  p54.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l54.GetInPlayIntensity / l54.Intensity)
  p54off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l54.GetInPlayIntensity / l54.Intensity)
...
Sub l62_animate
  p62.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l62.GetInPlayIntensity / l62.Intensity)
  p62off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l62.GetInPlayIntensity / l62.Intensity)
...
Sub l19_animate
  p19.BlendDisableLighting = insert_green_off + insert_green_on * (l19.GetInPlayIntensity / l19.Intensity)
  p19off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l19.GetInPlayIntensity / l19.Intensity)
...
Sub l21_animate
  p21.BlendDisableLighting = insert_green_off + insert_green_on * (l21.GetInPlayIntensity / l21.Intensity)
  p21off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l21.GetInPlayIntensity / l21.Intensity)
  bulb21.BlendDisableLighting = bulb_off + bulb_on * (l21.GetInPlayIntensity / l21.Intensity)
End Sub

Sub l35_animate
  p35.BlendDisableLighting = insert_green_off + insert_green_on * (l35.GetInPlayIntensity / l35.Intensity)
  p35off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l35.GetInPlayIntensity / l35.Intensity)
...
Sub l51_animate
  p51.BlendDisableLighting = insert_green_off + insert_green_on * (l51.GetInPlayIntensity / l51.Intensity)
  p51off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l51.GetInPlayIntensity / l51.Intensity)
...
Sub l37_animate
  p37.BlendDisableLighting = insert_pink_off + insert_pink_on * (l37.GetInPlayIntensity / l37.Intensity)
  p37off.BlendDisableLighting = a_insert_pink_off - a_insert_pink_on * (l37.GetInPlayIntensity / l37.Intensity)
  bulb37.BlendDisableLighting = bulb_off + bulb_on * (l37.GetInPlayIntensity / l37.Intensity)
End Sub

Sub l44_animate
  p44.BlendDisableLighting = insert_pink_off + insert_pink_on * (l44.GetInPlayIntensity / l44.Intensity)
  p44off.BlendDisableLighting = a_insert_pink_off - a_insert_pink_on * (l44.GetInPlayIntensity / l44.Intensity)
...
' debug.print "SetRelayGI value: " & Enabled
  ...
  ' Set Drop Target Shadows to state when GI is back on.
    if DT13up=1 Then dtsh13.visible = 1
    if DT14up=1 Then dtsh14.visible = 1
    if DT15up=1 Then dtsh15.visible = 1
    if DT19up=1 Then dtsh19.visible = 1
...
' targets
  For each x in GITargets: x.blenddisablelighting = 0.25 * gilvl + .15: Next
...
' prims (White Screws Main)
  For each x in GIScrewCaps: x.blenddisablelighting = 0.05 * gilvl + .005: Next
...
' Note these are called now in SetupOptions, as bumper colors are an option switchable in the F12 menu
'FlInitBumper 1, "red"
'FlInitBumper 2, "red"
...
Sub FlFadeBumper(nr, Z)
  ...
  Select Case FlBumperColor(nr)
    ...
    Case "blue"
      ...
      FlBumperSmallLight(nr).intensity = 20 * Z
      FlBumperTop(nr).BlendDisableLighting = 0.15 * Z
      FlBumperBulb(nr).BlendDisableLighting = 3000 * (0.03 * Z +0.97 * Z^10)
      Flbumperbiglight(nr).intensity = 25 * Z
      FlBumperHighlight(nr).opacity = 10000 * (Z ^ 3)
    ...
    Case "green"
      ...
      FlBumperSmallLight(nr).intensity = 10 * Z
      FlBumperTop(nr).BlendDisableLighting = 0.15 * Z
      FlBumperBulb(nr).BlendDisableLighting = 6000 * (0.03 * Z + 0.97 * Z ^ 10)
      Flbumperbiglight(nr).intensity = 10 * Z
      FlBumperHighlight(nr).opacity = 6000 * (Z ^ 3)
    ...
    Case "red"
      ...
      FlBumperSmallLight(nr).intensity = 17 * Z
      FlBumperTop(nr).BlendDisableLighting = 0.15 * Z
      FlBumperBulb(nr).BlendDisableLighting = 9000 * (0.03 * Z + 0.97 * Z ^ 10)
      Flbumperbiglight(nr).intensity = 10 * Z
      FlBumperHighlight(nr).opacity = 2000 * (Z ^ 3)
    ...
    Case "orange"
      ...
      FlBumperSmallLight(nr).intensity = 17 * Z
      FlBumperTop(nr).BlendDisableLighting = 0.15 * Z
      FlBumperBulb(nr).BlendDisableLighting = 2500 * (0.03 * Z + 0.97 * Z ^ 10)
      Flbumperbiglight(nr).intensity = 10 * Z
      FlBumperHighlight(nr).opacity = 4000 * (Z ^ 3)
    ...
    Case "yellow"
      ...
      FlBumperSmallLight(nr).intensity = 17 * Z
      FlBumperTop(nr).BlendDisableLighting = 0.15 * Z
      FlBumperBulb(nr).BlendDisableLighting = 2000 * (0.03 * Z + 0.97 * Z ^ 10)
      Flbumperbiglight(nr).intensity = 10 * Z
      FlBumperHighlight(nr).opacity = 1000 * (Z ^ 3)
...
' Number of Spins
Digits(32) = Array(LED004,LED002,LED006,LED007,LED005,LED001,LED003)
Digits(33) = Array(LED011,LED009,LED013,LED014,LED012,LED008,LED010)
...
' Number of Spins
VRDigits(32) = Array(LEDax001,LEDax002,LEDax003,LEDax004,LEDax005,LEDax006,LEDax007)
VRDigits(33) = Array(LEDbx001,LEDbx002,LEDbx003,LEDbx004,LEDbx005,LEDbx006,LEDbx007)
...
' Target Color and Marks
  ...
  If TargetMarks = 1 Then
    If SUTargetColor = 1 Then
      For Each C in GITargets
        C.image = "SternT35StandUp-Sticker-1-Scuffed"
      Next
    Else
      sw18p.image  = "SternT35StandUp-A"
      sw31p.image  = "SternT35StandUp-A"
      sw28p.image  = "SternT35StandUp-A"
      sw17p.image  = "SternT35StandUp-B"
      sw30p.image  = "SternT35StandUp-B"
      sw27p.image  = "SternT35StandUp-B"
      sw32p.image  = "SternT35StandUp-C"
      sw29p.image  = "SternT35StandUp-C"
      sw26p.image  = "SternT35StandUp-C"
    End If

    If TargetColor = 1 Then
      sw13p.image = "DT_Stern_White-A"
      sw14p.image = "DT_Stern_White-B"
      sw15p.image = "DT_Stern_White-C"
      sw19p.image = "DT_Stern_White-A"
      sw20p.image = "DT_Stern_White-B"
      sw21p.image = "DT_Stern_White-C"
      sw22p.image = "DT_Stern_White-A"
      sw23p.image = "DT_Stern_White-B"
      sw24p.image = "DT_Stern_White-C"

    Elseif TargetColor = 2 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-1-Scuffed"
      Next

    Elseif TargetColor = 3 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-2-Scuffed"
      Next

    Elseif TargetColor = 4 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-3-Scuffed"
      Next

    Elseif TargetColor = 5 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-4-Scuffed"
      Next

    Elseif TargetColor = 6 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-5-Scuffed"
      Next

    Else
      sw13p.image = "DT_Stern_Red-A"
      sw14p.image = "DT_Stern_Red-B"
      sw15p.image = "DT_Stern_Red-C"
      sw19p.image = "DT_Stern_Red-A"
      sw20p.image = "DT_Stern_Red-B"
      sw21p.image = "DT_Stern_Red-C"
      sw22p.image = "DT_Stern_Red-A"
      sw23p.image = "DT_Stern_Red-B"
      sw24p.image = "DT_Stern_Red-C"
    End If
  ...
  Else
    ...
    If SUTargetColor = 1 Then
      For Each C in GITargets
        C.image = "SternT35StandUp-Sticker-1"
      Next
    ...
    Elseif TargetColor = 3 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-2"
      Next

    Elseif TargetColor = 4 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-3"
      Next

    Elseif TargetColor = 5 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-4"
      Next

    Elseif TargetColor = 6 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-5"
      Next
    Else
      For Each C in GIdtTargets
        C.image = "DT_Stern_Red"
      Next
    End If
...
' Bumper Color
  ...
  If BumperColor = 1 Then
      Bumpertop1.image = "Bumper_Blue"
      Bumpertop2.image = "Bumper_Blue"
      FlInitBumper 1, "blue"
      FlInitBumper 2, "blue"
      Bumperdisk1.image = "FLbumperdisk-Blue-Left"
      Bumperdisk2.image = "FLbumperdisk-Blue-Right"
      FlFadeBumper 1, bumperflashlevel * (b1.GetInPlayIntensity / b1.Intensity)
      FlFadeBumper 2, bumperflashlevel * (b2.GetInPlayIntensity / b2.Intensity)
  Elseif BumperColor = 2 Then
      Bumpertop1.image = "Bumper_Green"
      Bumpertop2.image = "Bumper_Green"
      FlInitBumper 1, "green"
      FlInitBumper 2, "green"
      Bumperdisk1.image = "FLbumperdisk-Green-Left"
      Bumperdisk2.image = "FLbumperdisk-Green-Right"
      FlFadeBumper 1, bumperflashlevel * (b1.GetInPlayIntensity / b1.Intensity)
      FlFadeBumper 2, bumperflashlevel * (b2.GetInPlayIntensity / b2.Intensity)
  Elseif BumperColor = 3 Then
      Bumpertop1.image = "Bumper_Orange"
      Bumpertop2.image = "Bumper_Orange"
      FlInitBumper 1, "orange"
      FlInitBumper 2, "orange"
      Bumperdisk1.image = "FLbumperdisk-Orange-Left"
      Bumperdisk2.image = "FLbumperdisk-Orange-Right"
      FlFadeBumper 1, bumperflashlevel * (b1.GetInPlayIntensity / b1.Intensity)
      FlFadeBumper 2, bumperflashlevel * (b2.GetInPlayIntensity / b2.Intensity)
  Elseif BumperColor = 4 Then
      Bumpertop1.image = "Bumper_Yellow"
      Bumpertop2.image = "Bumper_Yellow"
      FlInitBumper 1, "yellow"
      FlInitBumper 2, "yellow"
      Bumperdisk1.image = "FLbumperdisk-Yellow-Left"
      Bumperdisk2.image = "FLbumperdisk-Yellow-Right"
      FlFadeBumper 1, bumperflashlevel * (b1.GetInPlayIntensity / b1.Intensity)
      FlFadeBumper 2, bumperflashlevel * (b2.GetInPlayIntensity / b2.Intensity)
  Else
      Bumpertop1.image = "Bumper_Red"
      Bumpertop2.image = "Bumper_Red"
      Bumperdisk1.image = "FLbumperdisk-Red-Left"
      Bumperdisk2.image = "FLbumperdisk-Red-Right"
      FlInitBumper 1, "red"
      FlInitBumper 2, "red"
      FlFadeBumper 1, bumperflashlevel * (b1.GetInPlayIntensity / b1.Intensity)
      FlFadeBumper 2, bumperflashlevel * (b2.GetInPlayIntensity / b2.Intensity)
...
' Outer Wall Color
  ...
  If SideWallColor = 1 Then
    ...
    OuterPrimBlack_cab.image = "OuterPrim_DT_UV-Black"
  Elseif SideWallColor = 2 Then
    OuterPrimBlack_DT.image = "OuterPrim_DT_UV-Green"
    OuterPrimBlack_cab.image = "OuterPrim_DT_UV-Green"
  Else
    OuterPrimBlack_DT.image = "OuterPrim_DT_UV-Blue"
    OuterPrimBlack_cab.image = "OuterPrim_DT_UV-Blue"
  End If
  ...
  Elseif SpinnerColor = 6 Then
    SpinnerPrim.image = "Spinner-Dragon"
  Elseif SpinnerColor = 7 Then
    SpinnerPrim.image = "Spinner-Dragon-Scratches"
  Else
...
Sub editDips
  if SetDIPSwitches = 1 Then
    ...
    if UseAltROM = 1 Then
      With vpmDips
      .AddForm 700,400,"Dragonfist - DIP switches"
      .AddChk 2,10,180,Array("Match feature",&H00100000)'dip 21
      .AddChk 2,25,115,Array("Credits display",&H00080000)'dip 20
      .AddChk 2,40,190,Array("7X Multiplier awards special when lit",&H00800000)'dip 24
      .AddChk 2,55,300,Array("Middle Drop Target Awards Special or EB when lit",&H00002000)'dip 14
      .AddChk 2,75,190,Array ("Standup Bonus Carryover On",&H00000010)'dip 5
      .AddChk 2,90,190,Array ("Outhole Bonus Carryover On",&H00020000)'dip 18
      .AddChk 2,115,190,Array ("Drop Target Sweep Award On",&H00008000)'dip 16
      .AddChk 2,130,190,Array ("Drop Bonus on all drop hits On",&H00010000)'dip 17
      .AddChk 2,145,190,Array ("Freeplay On",&H00040000)'dip 19
      .AddChk 2,160,190,Array ("High Spin to date Award On",&H00200000)'dip 22
      .AddChk 2,175,190,Array ("Award for Spin Muliples of 20 On",&H00400000)'dip 23
      .AddChk 205,10,190,Array("Background sound",&H00000080)'dip 8
      .AddChk 205,25,190,Array("Extra Ball Feature Off",&H00001000)'dip 13
      .AddChk 205,40,190,Array("Multiplier Reset Off",&H20000000)'dip 30
      .AddFrame 2,235,190,"High score feature",&H00000020,Array("extra ball",0,"replay",&H00000020)'dip 6
      .AddFrame 205,75,190,"Special award",&HC0000000,Array("no award",0,"100,000 points",&H40000000,"free ball",&H80000000,"free game",&HC0000000)'dip 31&32
      .AddFrame 205,155,190,"Special Replay: 1 Per ",&H10000000,Array("ball",0,"game",&H10000000)'dip 29
      .AddFrame 205,205,190,"Balls per game",&H00000040,Array("3 balls",0,"5 balls",&H00000040)'dip 7
    ...
    Else
      ...
      .AddChk 205,25,190,Array("Extra Ball Feature Off",&H00001000)'dip 13
      .AddChk 205,40,190,Array("Multiplier Reset Off",&H20000000)'dip 30
      .AddFrame 2,75,190,"Maximum award credits",&H00060000,Array("10 credits",0,"15 credits",&H00020000,"25 credits",&H00040000,"40 credits",&H00060000)'dip 18&19
      .AddFrame 2,155,190,"High game to date",49152,Array("points",0,"1 free game",&H00004000,"2 free games",32768,"3 free games",49152)'dip 15&16
      .AddFrame 2,235,190,"High score feature",&H00000020,Array("extra ball",0,"replay",&H00000020)'dip 6
      .AddFrame 205,75,190,"Special award",&HC0000000,Array("no award",0,"100,000 points",&H40000000,"free ball",&H80000000,"free game",&HC0000000)'dip 31&32
      .AddFrame 205,155,190,"Special Replay: 1 Per ",&H10000000,Array("ball",0,"game",&H10000000)'dip 29
      .AddFrame 205,205,190,"Balls per game",&H00000040,Array("3 balls",0,"5 balls",&H00000040)'dip 7
...
' 5     0   n/a - set to zero for original ROM (Alt ROM 1 will carry standup bonus to next ball)
...
' 13    0   Extra Ball Feature On
' 14    0   Middle Drop Target awards Special or EB if lit
' 15-16   10    High Game to Date earns 1 free game for original ROM (Alt ROM 16 set to 1 will add 25k for sweep of drop targets)
' 17    0   n/a - set to zero for original (Alt ROM 0 enables 7x bonus on drop targets, 1 will do for every drop Hit)
' 18-19   00    10 max awarded credits for orig (Alt ROM 18 set to 1 carries over outhole bonus.  19 set to 1 will enable freeplay)
' 20    1   Credit is displayed for oringal ROM (Alt ROM it's freeplay - can adjust in adjustments menu)
' 21    1   Match Feature is On
' 22-23   01    Add A Ball feature is maxed at 1 for original ROM (Alt ROM this enables spin awards of multiples of 20)
' 24    1   7X Multiplier lit awards special
' 25-28   0000  1 credit per coin in coin chute 3
' 29    0   Special Replay; allows 1 special replay per game
' 30    0   Multiplier reset is NOT reset
...
Sub SetDefaultDips
  If SetDIPSwitches = 0 Then
    ...
    Controller.Dip(2) = 216   '24-17 = 11011000
...
Sub UpdateDTLamps()
  ...
  If GOn = 0 Then
    If Controller.Lamp(45) = 0 Then
      ...
      SpinsReel.setValue(0)
      ToDateReel.setValue(0)

    Else
      GameOverReel.setValue(1)
      SpinsReel.setValue(1)
      ToDateReel.setValue(1)
    End If
...
Sub UpdateVRLamps()
  ...
  If Controller.Lamp(45) = 0 Then: fhighspins.visible=0: else: fhighspins.visible=1 ' high spins
  If Controller.Lamp(61) = 0 Then: ftilt.visible=0: else: ftilt.visible=1 ' tilt
  If Controller.Lamp(63) = 0 Then: fMatch.visible=0: else: fMatch.visible=1 ' tilt
...
'0.01 Playfield used from Redbone.
'   Started with UP's Catacomb as baseline for script, physics, etc. to utilize latest VPW standards
'   Added flippers, flipper physics, accurate flipper prims and images
'   All walls and guide rails placed
'   Plunger added and adjusted to the walls... matching videos.
...
'   Added spinner and annimated it.
'     Added flupper bumper stype bumpers.
'   Redbone added several images for the spinner, bumpers, flippers, playfield, plastics, apron, targets.
'   Added logic to control spinner damping, and fine tuned it for low speed and high speed hits to match videos.
'   Added cutouts, bulb prims, triggers and trigger prims
'   Placed all white screw caps.  Also put longer peg inside posts where they go through plastics.
'   Added three leaf sensors (10pt bumpers) and physics
'   Added all physics walls
'   Added guide rails, pegs, posts, rubbers, and all associated physics
'   Adjusted top sling posts and outlanes to hard difficulty
...
'   Added all plastics and underlying plastics acrylics.
...
'   Added GI tops
'     Enabled the backglass reels, SA, Tilt, GameOver, Match, Spins, HSTD for Desktop Mode.
'   Put code in to control match. Also added a Credits, High Spins, and BIP Reels.
...
'   Updated cabinet pov
'0.02 Moved the plunger gate slightly.
'   Updated the target and drop target elasticity to .725 and the elasticity falloff to 0
'   Did a UV map of dtouterprim
'   Redbone updated the graphics for the playfield, insert text, plastics, outer walls, bumpers, targets, apron, plunger cover
...
'   Small adjustment to the flipper prim location to actual flipper.
'   Added a physics glass top
'   Added options for colors on flippers, bumpers, screw caps, drop targets, spinner, side walls, and marks on targets from ball wear with images from redbone
'   Redbone added new material and peg images.
'   Added option for LED plastics edges
'   Adjusted plunger groove lane
'   Adjusted ball brightness in plunger lane.
'   Updated the 3D inserts colors for blue and green on and off.
'   Redbone added updated reflective metal walls.  Also, added additional target options with stickers.
'   Changed to a graphic for F12 instructions / rules
'   Adjusted the size of the spinner, as there are corner conditions it doesn't hit.
'   Turned off desktop backglass lamp reflectivity.
'   Changed the Rothbauer drop target code to allow side hit by the checkbrick function to not always going to zero.
'   Increased the dtmass of the left drop targets to not drop as easily and set dtmass individually.
'   Did a new GI bake to accomodate for the increased playfield size.
'   Added an inside shadow prim to darken holes from cutout prims
'   Updated the desktop backglass image, adjusted lamp color and reflectivity.
'   Added additional spinner code to stop count at certain point.
'   Updated spinner primitive bracket, and ball hit metal plate.
'   Added code / logic to ensure no false spin count when spinner slows down by measureing time between spins.
'   Updated the material on the red pegs to look better in GI with more definition.
'   Converted images to webp
'2.0.0  Changed material on VR cab to remove shininess and light reflections
'   Small update to vr cab to avoid side image rollovers by added lower VR Backglass base wall
'   Changed the drop target drop speed a little faster
'   Adjusted the day/night light environment down
'   Added new VR Backglass images done by Hauntfreaks and Redbone
'   Tweaked red peg material slightly
'   Added metal image to inner lane right wall
'   Updated the desktop backglass with new backglass images
'   Added alternative for an alternative ROM that corrects some issues, and opens up additional bonus'.
'Released
'   Thanks to the VPW team for testing!  And especially Redbone for all the graphics!
'2.0.01 Added support for alternative ROM.
'2.0.02 Corrected the spinner issue where it could be not hit when rolling on the right side.  Has to do with a knife edge on the metal wall next to it, VPX physics
'     engine can't deal with that correctly.
