...
' / /  / _ \/ __| __| \ \/  \/ / _ \| '__| |/ _` |
'/ /__| (_) \__ \ |_   \  /\  / (_) | |  | | (_| |
'\____/\___/|___/\__|   \/  \/ \___/|_|  |_|\__,_|


'002 - iaakki - physics codes updated, frametimer added
'003 - iaakki - most of the ssf sounds included; lot still missing
'004 - bord - tuned insert brightness for 10.8 beta 6
'005 - TastyWasps - Fleep sounds, VR integration, Dynamic Ball Shadows, script organization
'006 - TastyWasps - Desktop Digits, VR Deluxe Room
'007 - TastyWasps - VR Backglass updated from Hauntfreaks
'008 - bord - incorporate Wylte physics feedback, minor visual tweaks
'009 - iaakki - physics change for top lane nails, bumper ring speed increased slightly, minor change to sling rubber, ball release sound added, lowered zcol_posts falloff to 0.05 as Wylte proposed
'       Upped flipper elasticity and zcol_rubbers friction
'010 - bord - bumper base visuals tweak, refraction reduction, siderails added to DT
'011 - iaakki - bumpers chopped
'012 - TastyWasps - VR cab art upscaled by DGrimmReaper/DarthVito
'013 - TastyWasps - VR cab art upscaled by Hauntfreaks / DGrimmReaper
'014 - TastyWasps - visible tag fixed on VR Deluxe room book object.
'015 - bord - cleanup for release
...
SolCallBack(8) = "SolLSaucer"
SolCallback(13) = "SolRSaucer"
...
Sub Table1_Init
  ...
  With Controller
    ...
    .SplashInfoLine = "Lost World"&chr(13)&"(Bally 1978)"
    .HandleMechanics=0
    .HandleKeyboard=0
    .ShowDMDOnly=1
    .ShowFrame=0
...
'   .InitNoTrough BallRelease,8,125,5
    .InitKick BallRelease,80,14
    .InitExitSnd Soundfx("BallRelease7",DOFContactors), Soundfx("fx2_solenoid",DOFContactors)
    .Balls=1
  End With

  Set bsLSaucer=New cvpmBallStack
  With bsLSaucer
'   .InitSaucer sw23,23,108,14
'   .InitExitSnd Soundfx("fx_kicker",DOFContactors), Soundfx("fx2_ballrel",DOFContactors)
'   .CreateEvents "bsLSaucer", sw23
  End With

  Set bsRSaucer=New cvpmBallStack
  with bsRSaucer
'   .InitSaucer sw24,24,108,14
'   .InitExitSnd Soundfx("fx_kicker",DOFContactors), Soundfx("fx2_solenoid",DOFContactors)
'   .CreateEvents "bsRSaucer", sw24
  end with

  vpmNudge.TiltSwitch = 7
  vpmNudge.Sensitivity = 5
  vpmNudge.TiltObj=Array(sw38, sw39, sw40, LeftSlingshot, RightSlingshot)

  If VRMode = True Then
    setup_backglass
    VRRoom = LoadValue("LostWorld", "V1.0.0")
    VRChangeRoom
  End If
...
Sub SolLSaucer(Enable)
    ...
    kickstep1 = 0
    pkickarm.rotz=15
    If sw23b.ballcntover > 0 then
      KickBall KickerBall, 140, 14, 5, 30
      'SoundSaucerKick 1, sw23b
      PlaySoundAt Soundfx("BallRelease1",DOFContactors), sw23b
    End If
    sw23b.timerenabled = 1
    End If
End Sub
...
Sub SolRSaucer(Enable)
    ...
    kickstep2 = 0
    pkickarm001.rotz=15
    If sw24b.ballcntover > 0 then
      KickBall KickerBall, -140, 14, 5, 30
      PlaySoundAt Soundfx("BallRelease1",DOFContactors), sw24b
      'SoundSaucerKick 1, sw24b
    End If
    sw24b.timerenabled = 1
    End If
End Sub
...
Sub sw24b_timer
  Select Case kickstep2
    ...
    Case 9:pkickarm001.rotz=0:sw24b.TimerEnabled = 0: if sw24b.ballcntover > 0 then SolRSaucer -1
    End Select
...
Sub sw40_timer
  Select Case Bump1Step
    ...
    Case 7: bumpskirt1.RotY=0:sw40.timerenabled = 0
  End Select
...
Sub sw39_timer
  Select Case bump2step
    ...
    Case 7: bumpskirt2.RotY=0:sw39.timerenabled = 0
  End Select
...
Sub sw38_timer
  Select Case bump3step
    ...
    Case 7: bumpskirt3.RotY=0:sw38.timerenabled = 0
  End Select
  bump3step=bump3step + 1
End Sub

' Rollovers
Sub sw5_Hit:Controller.Switch(5)=1:star.transz=-6:End Sub
Sub sw5_unHit:Controller.Switch(5)=0:star.transz=0:End Sub
...
' Wire Triggers
Sub sw3_Hit:Controller.Switch(3)=1:tr3.transz=-6: End Sub
Sub sw3_unHit:Controller.Switch(3)=0:tr3.transz=0:End Sub
Sub sw4_Hit:Controller.Switch(4)=1:tr4.transz=-6:End Sub
Sub sw4_unHit:Controller.Switch(4)=0:tr4.transz=0:End Sub
Sub sw25_Hit:Controller.Switch(25)=1:tr25.transz=-6:End Sub
Sub sw25_unHit:Controller.Switch(25)=0:tr25.transz=0:End Sub
Sub sw26_Hit:Controller.Switch(26)=1:tr26.transz=-6:End Sub
Sub sw26_unHit:Controller.Switch(26)=0:tr26.transz=0:End Sub
Sub sw31_Hit:Controller.Switch(31)=1:tr31.transz=-6:End Sub
Sub sw31_unHit:Controller.Switch(31)=0:tr31.transz=0:End Sub
Sub sw32_Hit:Controller.Switch(32)=1:tr32.transz=-6:End Sub
Sub sw32_unHit:Controller.Switch(32)=0:tr32.transz=0:End Sub
...
'    SpinnerRod.TransX = (sin( (sw17.CurrentAngle- 90) * (2*3.14/360)) * -5)
  ...
  SpinnerRod.TransZ = (cos((sw17.CurrentAngle + 180) * (PI/180))+1) * SpinnerRadius
  SpinnerRod.TransY = sin((sw17.CurrentAngle) * (PI/180)) * -SpinnerRadius
End Sub
...
Sub sw28_timer
  Select Case Step28
    Case 3: sw28target.TransX=3
    Case 4: sw28target.TransX=-4
    Case 5: sw28target.TransX=2
    Case 6: sw28target.TransX=-3
    Case 7: sw28target.TransX=1
    Case 8: sw28target.TransX=-2
    Case 9: sw28target.TransX=0
    Case 10: sw28target.TransX=0: sw28.timerenabled = 0
  End Select
  Step28=Step28 + 1
End Sub
...
Sub sw29_timer
  Select Case Step29
    Case 3: sw29target.TransX=-3
    Case 4: sw29target.TransX=4
    Case 5: sw29target.TransX=-2
    Case 6: sw29target.TransX=3
    Case 7: sw29target.TransX=-1
    Case 8: sw29target.TransX=2
    Case 9: sw29target.TransX=0
    Case 10: sw29target.TransX=0: sw29.timerenabled = 0
  End Select
  Step29=Step29 + 1
End Sub
...
Sub sw30_timer
  Select Case Step30
    Case 3: sw30target.TransX=3
    Case 4: sw30target.TransX=-4
    Case 5: sw30target.TransX=2
    Case 6: sw30target.TransX=-3
    Case 7: sw30target.TransX=1
    Case 8: sw30target.TransX=-2
    Case 9: sw30target.TransX=0
    Case 10: sw30target.TransX=0:sw30.timerenabled = 0
  End Select
  Step30=Step30 + 1
End Sub
...
'********************************************************
' Map lights into array / Set unmapped lights to nothing.
'********************************************************
...
VRRoom = LoadValue("LostWorld", "V1.0.0")  ' loading the last value saved.  These are saved below each time you make a change with magnasave
...
vDigits(6) = Array(LED8x0,LED8x1,LED8x2,LED8x3,LED8x4,LED8x5,LED8x6)
vDigits(7) = Array(LED9x0,LED9x1,LED9x2,LED9x3,LED9x4,LED9x5,LED9x6)
vDigits(8) = Array(LED10x0,LED10x1,LED10x2,LED10x3,LED10x4,LED10x5,LED10x6)
vDigits(9) = Array(LED11x0,LED11x1,LED11x2,LED11x3,LED11x4,LED11x5,LED11x6)
vDigits(10) = Array(LED12x0,LED12x1,LED12x2,LED12x3,LED12x4,LED12x5,LED12x6)
vDigits(11) = Array(LED13x0,LED13x1,LED13x2,LED13x3,LED13x4,LED13x5,LED13x6)

vDigits(12) = Array(LED1x000,LED1x001,LED1x002,LED1x003,LED1x004,LED1x005,LED1x006)
vDigits(13) = Array(LED1x100,LED1x101,LED1x102,LED1x103,LED1x104,LED1x105,LED1x106)
vDigits(14) = Array(LED1x200,LED1x201,LED1x202,LED1x203,LED1x204,LED1x205,LED1x206)
vDigits(15) = Array(LED1x300,LED1x301,LED1x302,LED1x303,LED1x304,LED1x305,LED1x306)
vDigits(16) = Array(LED1x400,LED1x401,LED1x402,LED1x403,LED1x404,LED1x405,LED1x406)
vDigits(17) = Array(LED1x500,LED1x501,LED1x502,LED1x503,LED1x504,LED1x505,LED1x506)

vDigits(18) = Array(LED2x000,LED2x001,LED2x002,LED2x003,LED2x004,LED2x005,LED2x006)
vDigits(19) = Array(LED2x100,LED2x101,LED2x102,LED2x103,LED2x104,LED2x105,LED2x106)
vDigits(20) = Array(LED2x200,LED2x201,LED2x202,LED2x203,LED2x204,LED2x205,LED2x206)
vDigits(21) = Array(LED2x300,LED2x301,LED2x302,LED2x303,LED2x304,LED2x305,LED2x306)
vDigits(22) = Array(LED2x400,LED2x401,LED2x402,LED2x403,LED2x404,LED2x405,LED2x406)
vDigits(23) = Array(LED2x500,LED2x501,LED2x502,LED2x503,LED2x504,LED2x505,LED2x506)

vDigits(24) = Array(LEDax300,LEDax301,LEDax302,LEDax303,LEDax304,LEDax305,LEDax306)
vDigits(25) = Array(LEDbx400,LEDbx401,LEDbx402,LEDbx403,LEDbx404,LEDbx405,LEDbx406)
vDigits(26) = Array(LEDcx500,LEDcx501,LEDcx502,LEDcx503,LEDcx504,LEDcx505,LEDcx506)
vDigits(27) = Array(LEDdx600,LEDdx601,LEDdx602,LEDdx603,LEDdx604,LEDdx605,LEDdx606)
...
Sub UpdateMultipleLamps()
  ...
  If Controller.Lamp(29) = 0 Then: f_HS.visible=0: else: f_HS.visible=1 'High Score
  If Controller.Lamp(45) = 0 Then: f_Match.visible=0: else: f_Match.visible=1 'Match
  If Controller.Lamp(11) = 0 Then: f_SA.visible=0: else: f_SA.visible=1 'Shoot Again
  ...
  If Controller.Lamp(14) = 0 Then: f_p1.visible=0: else: f_p1.visible=1 'Player 1
  If Controller.Lamp(15) = 0 Then: f_p1a.visible=0: else: f_p1a.visible=1 'Player 1
  If Controller.Lamp(30) = 0 Then: f_p2.visible=0: else: f_p2.visible=1 'Player 2
  If Controller.Lamp(31) = 0 Then: f_p2a.visible=0: else: f_p2a.visible=1 'Player 2
  If Controller.Lamp(46) = 0 Then: f_p3.visible=0: else: f_p3.visible=1 'Player 3
  If Controller.Lamp(47) = 0 Then: f_p3a.visible=0: else: f_p3a.visible=1 'Player 3
  If Controller.Lamp(62) = 0 Then: f_p4.visible=0: else: f_p4.visible=1 'Player 4
  If Controller.Lamp(63) = 0 Then: f_p4a.visible=0: else: f_p4a.visible=1 'Player 4
