...
Sub aMaderas_Hit(idx):PlaySound "fx_Woodhit", 0, Vol(ActiveBall), pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0, AudioFade(ActiveBall):End Sub
...
End Sub
 ...
 Sub RampRFX_Hit()
     PlaySoundAtVol "ramp_enter", ActiveBall, 1
End Sub
...
Sub CreateNewBall()
    ...
    ' kick it out..
    PlaySoundAtVol SoundFXDOF("fx_Ballrel",122,DOFPulse,DOFContactors), Ballrelease, 1
...
Sub Sw1_hit
    If Tilted Then Exit Sub
    PlaySoundAtVol SoundFXDOF("fx_rubber",117,DOFPulse,DOFTargets), ActiveBall, 1
    AddScore 1000
...
Sub Sw4_hit
    If Tilted Then Exit Sub
    PlaySoundAtVol SoundFXDOF("fx_rubber",113,DOFPulse,DOFTargets), ActiveBall, 1
    AddScore 1000
  If bMultiBallMode Then Exit Sub
    l4.state = 1
...
Sub Sw7_hit
    If Tilted Then Exit Sub
    PlaySoundAtVol SoundFXDOF("fx_rubber",114,DOFPulse,DOFTargets), ActiveBall, 1
    AddScore 1000
  If bMultiBallMode Then Exit Sub
    l7.state = 1
...
Sub Sw12_Hit()
    If Tilted Then Exit Sub
    PlaySoundAtVol "fx_hole-enter", ActiveBall, 1
    AddScore 2000
    Me.DestroyBall
    SetFlash 8, 2
  DOF 209, DOFPulse
...
Sub Sw20_Hit()
  ...
  DOF 207, DOFPulse
    ...
    If LightSkillShot.state = 0 Then
    PlaySoundAtVol "Kahn_2", ActiveBall, 1
    End If
    If LightSkillShot.state = 1 Then
    PlaySoundAtVol "scorp_spear", ActiveBall, 1
    End If
...
Sub Sw27_hit
    PlaySoundAtVol SoundFXDOF("fx_rubber",116,DOFPulse,DOFTargets), ActiveBall, 1
    If Tilted Then Exit Sub
    PlaySoundEffect
    Addscore 20000
  If bMultiBallMode Then Exit Sub
...
Sub Sw29_hit
    PlaySoundAtVol SoundFXDOF("fx_rubber",115,DOFPulse,DOFTargets), ActiveBall, 1
    If Tilted Then Exit Sub
    PlaySoundEffect
    Addscore 20000
  If bMultiBallMode Then Exit Sub
...
Sub CheckLock()
    ...
    If LockLight1.State + LockLight2.State + LockLight3.State = 3 Then
        PlaySoundAtVol "fx_diverter",ActiveBall, 1
        Post.Isdropped = 1
        LockLight.State = 2
        bLockEnabled = True
        MultiLight1.state = 2
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "MortalKombat" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / MortalKombat.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "MortalKombat" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / MortalKombat.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "MortalKombat" is the name of the table
    Dim tmp
...
Function AudioFade(ball) ' Can this be together with the above function ?
  Dim tmp
  tmp = ball.y * 2 / MortalKombat.height-1
  If tmp > 0 Then
    AudioFade = Csng(tmp ^10)
  Else
    AudioFade = Csng(-((- tmp) ^10) )
