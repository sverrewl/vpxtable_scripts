...
'********Right Hole***********
	...
	With bsRHole
		...
		.KickZ = 0.4
		.InitExitSnd SoundFX("scoop_exit",DOFContactors), SoundFX("Solenoid",DOFContactors)
		.KickForceVar = 2
	End With
...
'********DropTargets
   	...
   	Set DTBank3 = New cvpmDropTarget
        ...
        .Initsnd "", SoundFX("fx_resetdrop2",DOFContactors)
       End With

   	Set DTBank1 = New cvpmDropTarget
   	  With DTBank1
   		.InitDrop sw58, 58
        .Initsnd "", SoundFX("fx_resetdrop3",DOFContactors)
       End With
...
dim flipperHoldCodeLeft, flipperHoldCodeRight
 ...
 Sub Table_KeyDown(ByVal keycode)
    ...
    If keycode = CenterTiltKey Then Playsound SoundFX("nudge",0)
...
SolCallback(2) = "bsTrough.SolOut" 'Ball Release
	SolCallback(3) = "vpmSolSound SoundFX(""knocker"",DOFKnocker),"			'Knocker
	SolCallback(4) = "vpmSolSound SoundFX(""SlingshotLeft"",DOFContactors),"		'LeftSling
	SolCallback(5) = "vpmSolSound SoundFX(""SlingshotRight"",DOFContactors),"		'RightSling
...
SolCallback(17) = "sol3Bank" '3-Bank drop target
	SolCallback(18) = "vpmSolSound SoundFX(""fx_bumper1"",DOFContactors)," 		'Left Bumper
	SolCallback(19) = "vpmSolSound SoundFX(""fx_bumper2"",DOFContactors),"		'Middle Bumper
	SolCallback(20) = "vpmSolSound SoundFX(""fx_bumper3"",DOFContactors),"		'Right Bumper
...
'   Left Side Captive Ball Hits (Up Motion)
	Sub CapLHit_Hit()
		...
		If BP3L Then : BP3L = False : ForceTransL = 0.2 :PlaySoundAtVol "fx_collide", CapMove1, 1: Captive3L.DestroyBall : CapMove3L.CreateBall : CapMove3L.Kick mBallDir, force : Else :_
		If BP4L Then : BP4L = False : ForceTransL = 0.3 : Controller.Switch(78) = False :PlaySoundAtVol "fx_collide", CapMove1, 1: Captive4L.DestroyBall : CapMove4L.CreateBall : CapMove3L.enabled = False : CapMove4L.Kick mBallDir, force : Else :_
		If BP2  Then : BP2  = False : ForceTransL = 0.4:PlaySoundAtVol "fx_collide", CapMove1, 1: Captive2.DestroyBall : CapMove2.CreateBall : CapMove4L.enabled = False : CapMove2.Kick mBallDir, force : Else :_
...
'   Right Side Captive Ball Hits (Up Motion)
	Sub CapRHit_Hit()
		...
		If BP1R Then : BP1R = False : ForceTransR = 0.2 :PlaySoundAtVol "fx_collide", CapMove3, 1: Captive1R.DestroyBall : CapMove1R.CreateBall : CapMove1R.Kick mBallDir, force2 : Else :_
		If BP2R Then : BP2R = False : ForceTransR = 0.3 : Controller.Switch(80) = False :PlaySoundAtVol "fx_collide", CapMove3, 1: Captive2R.DestroyBall : CapMove2R.CreateBall : CapMove1R.enabled = False : CapMove2R.Kick mBallDir, force2 : Else :_
		If BP4  Then : BP4  = False : ForceTransR = 0.4 :PlaySoundAtVol "fx_collide", CapMove3, 1: Captive4.DestroyBall : CapMove4.CreateBall : CapMove2R.enabled = False : CapMove4.Kick mBallDir, force2 : Else :_
...
dim postdwn : postdwn = 0
  Sub SolLowerLockPin(Enabled)
	if enabled then
		If lockPin.IsDropped=0 Then:PlaysoundAtVol SoundFX("diverter",DOFContactors), sw48, 1:End If
...
'******************************************
  ...
  Sub SolKickBack(enabled)
	If enabled then
		PlaySoundAtVol SoundFX("Solenoid",DOFContactors),KickBack,VolKick
		kickback.Fire
	else
		kickback.PullBack
	End If
...
'******************************************
  ...
  Sub SolLRDIvert(Enabled)
	If Enabled then
		...
		DivLR.IsDropped=0
	PlaySoundAtVol SoundFX("Diverter",DOFContactors),Primitive18,1
	Else
  ...
  Sub SolRDivert1(Enabled)
	If Enabled then
		...
		DivTube.isDropped=1
		PlaySoundAtVol SoundFX("Diverter",DOFContactors),DivTubeP,1
...
Sub SolRDivert2(Enabled)
	...
	PlaySoundAtVol SoundFX("Diverter",DOFContactors),DivTube2P,1
	Else
  ...
  Sub SolRDivert3(Enabled)
	If Enabled then
		...
		DivLock.IsDropped=1
	PlaySoundAtVol SoundFX("Diverter",DOFContactors),GateR,1
	Else
		debug.print "L Alien div on"
		DivLock.IsDropped=0
	End If
...
'	Sub BallRelease_UnHit():BIPFF = BIPFF + 1:End Sub
	Sub sw61_Hit():bsALL.AddBall Me:End Sub
	Sub sw62_Hit():bsALR.AddBall Me:End Sub
	Sub sw67_Hit:PlaysoundAtVol "kicker_enter", sw67, VolKick:bsRHole.AddBall Me:End Sub
...
Sub Targets_Hit (idx)
	PlaySound SoundFX("target",DOFTargets), 0, Vol(ActiveBall)*VolTarg, AudioPan(ActiveBall), 0, Pitch(ActiveBall), 0, 0, AudioFade(ActiveBall)
End Sub

Sub TargetBankWalls_Hit (idx)
	PlaySound SoundFX("droptarget",DOFDropTargets), 0, Vol(ActiveBall), AudioPan(ActiveBall), 0, Pitch(ActiveBall), 0, 0, AudioFade(ActiveBall)
End Sub
