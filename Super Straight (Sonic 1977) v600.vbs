' Super Straight / IPD No. 2449 / May, 1977 / 4 Players
' Segasa d.b.a. Sonic, of Spain [Trade Name: Sonic]
' VPX8 table by jpsalas & updon719 v6.0.0
...
' Valores Constants
Const TableName = "SuperStraight" ' file name to save highscores and other variables
Const cGameName = "SuperStraight" ' B2S name
Const MaxPlayers = 4              ' 1 to 4 can play
Const MaxMultiplier = 2           ' limit bonus multiplier
Const FreePlay = False            ' Free play or coins
Const Special1 = 450000           ' extra ball or credit
Const Special2 = 630000
Const Special3 = 810000           'not used
...
Sub Table1_Init()
    ...
    Else
        ...
        Next
    End If
    If credits > 0 then:DOF 133, DOFOn
...
Sub CreateNewBall()
    ...
    UpdateBallInPlay
    PlaySoundAt SoundFXDOF("fx_Ballrel", 135, DOFPulse, DOFContactors), BallRelease
...
Sub BonusCountTimer_Timer
    ...
    Else
        ...
        If bBallinKicker Then
            vpmTimer.AddTimer 500, "PlaySoundAt SoundFXDOF(""fx_kicker"",139,DOFPulse,DOFContactors), kicker1:kicker1.Kick 132, 15 + RndNbr(3):kicker1f.RotatetoEnd '"
            vpmTimer.AddTimer 700, "kicker1f.RotatetoStart '"
            bBallinKicker = False
        Else
...
Sub Verification_Match()
    PlaySound "fx_match"
    Match = INT(RND(1) * 10) * 100 ' random between 0 and 900
    Display_Match
    For x = 1 to PlayersPlayingGame
        If(Score(x)MOD 1000) = Match Then
...
Sub Update_Match
    Select Case Match \ 100
        Case 10:m0.State = 0:m000.state = 0:m1.State = 0:m001.state = 0:m2.State = 0:m002.state = 0:m3.State = 0:m003.state = 0:m4.State = 0:m004.state = 0:m5.State = 0:m005.state = 0:m6.State = 0:m006.state = 0:m7.State = 0:m007.state = 0:m8.State = 0:m008.state = 0:m9.State = 0:m009.state = 0
    ...
    Select Case Points
        Case 10, 100, 1000, 10000
            ...
            If Points = 1000 AND(Score(CurrentPlayer)MOD 10000) \ 1000 = 0 Then  'new reel 10000
                PlaySound SoundFXDOF("bell10000", 143, DOFPulse, DOFChimes)
            ElseIf Points = 100 AND(Score(CurrentPlayer)MOD 1000) \ 100 = 0 Then 'new reel 1000
                PlaySound SoundFXDOF("bell1000", 142, DOFPulse, DOFChimes)
            ElseIf Points = 10 AND(Score(CurrentPlayer)MOD 100) \ 10 = 0 Then    'new reel 100
                PlaySound SoundFXDOF("bell100", 141, DOFPulse, DOFChimes)
            Else
                PlaySound SoundFXDOF("bell" &Points, 141, DOFPulse, DOFChimes)
            End If
        Case 20, 30, 40, 50
...
Sub AddBonus(bonuspoints)
    If(Tilted = False)Then
        ...
        If Bonus = 10 Then l016.State = 1 '1000 points to the Spinner
        If Bonus > 10 Then
            Bonus = 10
        End If
        UpdateBonusLights
...
Sub kicker2f_Animate: Remk001.RotX = kicker2f.CurrentAngle: End Sub
...
Sub Bumper003_Hit 'right bumper
    ...
    DOF 110,DOFPulse
    AddScore 100 + 900 * Lbumper3a.State
End Sub
...
Sub sw1_Hit
    ...
    DOF 124,DOFPulse
        AddScore 10000
        AddBonus 1
        If l011.State Then AwardSpecial
    End If
End Sub
...
Sub sw2_Hit
    ...
    DOF 125,DOFPulse
        AddScore 5000
        If l038.State Then l100.State = 1:BonusMultiplier = 2
        l015.State = 1
    End If
End Sub
...
Sub sw3_Hit
    ...
    DOF 126,DOFPulse
        AddScore 5000
        If l039.State Then l100.State = 1:BonusMultiplier = 2
        l015.State = 1
    End If
End Sub
...
Sub sw4_Hit
    ...
    DOF 127,DOFPulse
        AddScore 10000
        AddBonus 1
        If l012.State Then AwardSpecial
    End If
End Sub
...
Sub kicker1_Hit
    ...
    If NOT Tilted Then
        ...
        AddBonus 1
        l015.State = 1
        If l014.State Then 'collect bonus
            BonusCountTimer.Interval = 200 + 50 * BonusMultiplier
            BonusCountTimer.Enabled = 1
            bBallinKicker = True 'this is for the bonus count so it kicks the ball when finished
            Exit Sub
        End If
    End If
    vpmTimer.AddTimer 1500, "PlaySoundAt SoundFXDOF(""fx_kicker"",139,DOFPulse,DOFContactors), kicker1:kicker1.Kick 132, 15 + RndNbr(3):kicker1f.RotatetoEnd '"
...
Sub kicker2_Hit
    ...
    If NOT Tilted Then
        ...
        AddBonus 1
        l015.State = 1
        If l041.State Then 'award extra ball
            l041.State = 0
            ShootAgain.State = 1
            AwardExtraBall
        End If
    End If
    vpmTimer.AddTimer 1500, "PlaySoundAt SoundFXDOF(""fx_kicker"",132,DOFPulse,DOFContactors), kicker2:kicker2.Kick 30, 24 + RndNbr(5):kicker2f.RotatetoEnd '"
    vpmTimer.AddTimer 1700, "kicker2f.RotatetoStart '"
End Sub

' 100 point rebounds

Sub rsband008_Hit:AddScore 100:RotateBLights:End Sub
Sub rsband005_Hit:AddScore 100:RotateBLights:End Sub

Sub rsband011_Hit 'top lane
    l040.State = 1
    AddScore 100
    If l033.State Then
        l033.State = 0
...
Sub t1_Hit '10
    PlaySoundAtBall SoundFXDOF("fx_target",120,DOFPulse,DOFContactors)
    If NOT Tilted Then
        AddScore 1000
        If l026.State Then
            l026.State = 0
...
Sub t2_Hit 'J
    ...
    If NOT Tilted Then
        ...
        If l027.State Then
            l027.State = 0
            l023.State = 1
            If l022.State Then l014.State = 1 'hole
            CheckSpecials
        End If
    End If
End Sub
...
Sub t4_Hit 'J
    ...
    If NOT Tilted Then
        ...
        If l029.State Then
            l029.State = 0
            l022.State = 1
            If l023.State Then l014.State = 1 'hole
            CheckSpecials
        End If
    End If
End Sub
...
Sub t6_Hit 'K
    ...
    If NOT Tilted Then
        ...
        If l031.State Then
            l031.State = 0
            l020.State = 1
            If l019.State Then Gate002.Open = True:l036.State = 1
            CheckSpecials
        End If
    End If
End Sub
...
Sub t7_Hit 'K
    ...
    If NOT Tilted Then
        ...
        If l032.State Then
            l032.State = 0
            l019.State = 1
            If l020.State Then Gate002.Open = True:l036.State = 1
            CheckSpecials
        End If
    End If
End Sub
...
Sub dt1_Hit 'A
    PlaySoundAt SoundFXDOF("fx_droptarget",113,DOFPulse,DOFContactors), dt1
    If NOT Tilted Then
        AddScore 1000
        l017.State = 1
        If l018.State Then
            if l038.State + l039.State = 0 Then
                l038.State = 1
            End If
        End If
        CheckSpecials
...
Sub ResetDropTargets
    PlaySoundAt SoundFXDOF("fx_dropreset",114,DOFPulse,DOFContactors), Spinner
    dt1.IsDropped = 0
    dt2.IsDropped = 0
End Sub

Sub RotateBLights 'double bonus and special lights
    Dim tmp
    tmp = l038.State
    l038.State = l039.State
    l039.State = tmp
    tmp = l011.State
...
Sub CheckSpecials 'strait & strait flush
    Dim tmp
    tmp = l017.State + l020.State + l021.State + l023.State + l025.State
    If tmp = 5 Then
        l041.State = 1 'extra ball
    End If

    tmp = l024.State + l022.State + l021.State + l019.State + l018.State
    If tmp = 5 Then
        If l011.State + l012.State = 0 Then
            l011.State = 1 'special
        End If
    End If
End Sub
