...
Sub sw41_Spin()
  ...
  PlaysoundAtVol SoundFX("Gate2_low",DOFContactors), sw41, 1
End Sub
...
dim RampLiftDir, RampLiftHeight
Sub SolRampLift(Enabled)
  if enabled Then
    RampLiftr.Collidable = 1
    RampLiftDir=1 'Go Down
    RampLiftHeight= 70
    RampLift.HeightBottom=RampLiftHeight
...
Sub SolRocketEject(Enabled)
  If Enabled Then
    ...
    PlaySound SoundFX("SolenoidOn",DOFContactors)
    RocketTrough.Enabled = 0
    RocketTroughHole.Enabled = 0 ' Close rocket hole as soon as ball ejects
  End If
End Sub
...
Sub RocketTimer_Timer()
  ...
  Else
    If Rocket.RotX >= 0 Then
      ...
      Controller.Switch(50)   = True
      RocketTrough.Enabled = 0
      RocketTroughHole.Enabled = 0 ' close rocket hole as soon as rocket starts to descend
      Exit Sub
    End If
    RocketTrough.Enabled = 0
    RocketTroughHole.Enabled = 0 ' close rocket hole as soon as rocket starts to descend
...
Sub SubwayBallReleaseSuperVUK_Hit()
  PlaysoundAtVol SoundFX("ball_bounce_low",DOFContactors), RocketTrough, 1
  me.destroyball
...
Sub MoonLock_Hit()
  If Controller.Switch(36) Then
    ...
    actvb=1
    PlaySoundAtVol SoundFX("collide1",DOFContactors), MoonLock, 1
  ...
  if controller.Solenoid(19) Then
    If Moon.RotX <= -180 Then
      ...
      If Controller.Solenoid(34)=0 Then
        set MoonBall = Nothing
        If actvb = 1 Then
          PlaySoundAtVol SoundFX("ball_bounce_low",DOFContactors), MoonLock, 1
          actvb=0
        End If
        Exit Sub
      End If
...
Sub sw54k_Hit()
  ...
  PlaySoundAtVol SoundFX("kicker_enter",DOFContactors), sw54k, 1
End Sub
Sub sw54k_Unhit()
...
Sub sw47k_Hit()
  ...
  PlaySoundAtVol SoundFX("kicker_enter",DOFContactors), sw47k, 1
End Sub
Sub sw47k_Unhit()
...
Sub FallingBall_Hit(idx)
  PlaySoundAtVol SoundFX("ballhit4",DOFContactors), FallingBall, 1
End Sub
...
' plastics
Sub Plastics_Hit()
  PlaySoundAtVol SoundFX("PlasticHit",DOFContactors), ActiveBall, 1
End Sub
...
Sub MetalRails_Hit(idx)
  PlaySound "wire ramp", 0, VolMulti(ActiveBall,Volmetal), Pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0, AudioFade(ActiveBall)
' PlaySound "metalrolling"
End Sub
Sub MetalRails_unHit(idx)
  stopSound "wire ramp", 0, VolMulti(ActiveBall,VolMetal), Pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0, AudioFade(ActiveBall)
' StopSound "metalrolling"
End Sub

' plastic rails
Sub PlasticRail_Hit(idx)
  PlaySound "fx_ballrolling0", 0, VolMulti(ActiveBall,VolPlast), Pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0, AudioFade(ActiveBall)
' PlaySound "metalrolling"
End Sub
Sub PlasticRail_unHit(idx)
  stopSound "fx_ballrolling0", 0, VolMulti(ActiveBall,VolPlast), Pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0, AudioFade(ActiveBall)
' StopSound "metalrolling"
End Sub
...
Sub Metals_Thin_Hit (idx)
  PlaySound "metalhit_thin", 0, VolMulti(ActiveBall,VolMetal), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
End Sub
...
Sub Gates_Hit (idx)
  PlaySound "gate4", 0, VolMulti(ActiveBall,VolGates), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
End Sub
...
Sub Rubbers_Hit(idx)
  ...
  If finalspeed > 20 then
    PlaySound "fx_rubber2", 0, VolMulti(ActiveBall,VolRH), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
  End if
...
Sub RandomSoundFlipper()
  Select Case Int(Rnd*3)+1
    Case 1 : PlaySound "flip_hit_1", 0, VolMulti(ActiveBall,VolFlip), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
    Case 2 : PlaySound "flip_hit_2", 0, VolMulti(ActiveBall,VolFlip), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
    Case 3 : PlaySound "flip_hit_3", 0, VolMulti(ActiveBall,VolFlip), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
  End Select
End Sub
