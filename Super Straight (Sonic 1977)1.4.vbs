...
sub newgame
    ...
    playsoundAtVol SoundFXDOF("ballrelease",135,DOFPulse,DOFContactors), BallRelease, 1
end sub
...
Sub resetDT
    playsound SoundFXDOF("BankReset",114,DOFPulse,DOFContactors) ' TODO
...
Sub Bumper2_Hit
   if tilt=false then
    playsoundAtVol SoundFXDOF("fx_bumper4",111,DOFPulse,DOFContactors), Bumper2, VolBump
    DOF 112,DOFPulse
...
Sub Bumper3_Hit
   if tilt=false then
    playsoundAtVol SoundFXDOF("fx_bumper4",109,DOFPulse,DOFContactors), Bumper3, VolBump
    DOF 110,DOFPulse
...
Sub TopHole_timer
    ...
    playsoundAtVol SoundFXDOF("HoleKick",132,DOFPulse,DOFContactors),TopHole,VolKick
    DOF 131, DOFPulse
    Pkickarm.rotz=0
    me.timerenabled=0
End Sub
...
sub Kicker1_timer
    ...
    playsoundAtVol SoundFXDOF("HoleKick",139,DOFPulse,DOFContactors),Kicker1, VolKick
    DOF 131, DOFPulse
    me.timerenabled=0
end sub
'****** Wire Triggers
...
' Targets
Sub Tg1_Hit
    playsoundAtVol SoundFXDOF("target",120,DOFPulse,DOFContactors), Tg1, VolTarg
...
Sub Tg2_Hit
    playsoundAtVol SoundFXDOF("drop1",120,DOFPulse,DOFContactors), Tg2, VolTarg
...
Sub Tg3_Hit
    playsoundAtVol SoundFXDOF("drop1",120,DOFPulse,DOFContactors), Tg3, VolTarg
...
Sub Tg4_Hit
    playsoundAtVol SoundFXDOF("drop1",120,DOFPulse,DOFContactors), Tg4, VolTarg
...
Sub Tg5_Hit
    playsoundAtVol SoundFXDOF("drop1",120,DOFPulse,DOFContactors), Tg5, VolTarg
...
Sub Tg6_Hit
    playsoundAtVol SoundFXDOF("drop1",121,DOFPulse,DOFContactors), Tg6, VolTarg
...
Sub Tg7_Hit
    PlaysoundAtVol SoundFXDOF("drop1",121,DOFPulse,DOFContactors), Tg7, VolTarg
...
Sub Drop1_Hit
    ...
    playsoundAtVol SoundFXDOF("drop1",113,DOFPulse,DOFContactors), Tg2, VolTarg
    addscore 10000
    Ldrop1.state=1
...
Sub Drop2_Hit
    ...
    playsoundAtVol SoundFXDOF("drop1",113,DOFPulse,DOFContactors), Tg4, VolTarg
    addscore 10000
    Ldrop2.state=1
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "SuperStraight" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / SuperStraight.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "SuperStraight" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / SuperStraight.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "SuperStraight" is the name of the table
    Dim tmp
    tmp = ball.x * 2 / SuperStraight.width-1
    If tmp > 0 Then
        Pan = Csng(tmp ^10)
    Else
        Pan = Csng(-((- tmp) ^10) )
...
Function AudioFade(ball) ' Can this be together with the above function ?
  Dim tmp
  tmp = ball.y * 2 / SuperStraight.height-1
  If tmp > 0 Then
    AudioFade = Csng(tmp ^10)
  Else
    AudioFade = Csng(-((- tmp) ^10) )
