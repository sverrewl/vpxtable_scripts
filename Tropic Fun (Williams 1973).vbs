...
'   115 - 116 OutLane Rollovers
'   117 & 118 InLane Rollovers
'   119 Center  Target
'   120 - 121 Top Targets
...
Const cGameName = "tropicFun_1973"
Const cOptions = "tropicFun_1973_v1.06.txt"
Const HSFileName="Tropic Fun (Williams 1973)"
...
' RuleSet = 1 ' uncomment to collect S with I and P with C with any of the rollovers (Liberal/Conservative jacks in backbox)
  CornerSetting = 2 'Change this to any number from 1 to 3 for pay out if getting all four corners without a tictactoe, this is very hard to do
  starsOption = 1 'Change this to 1 to enable add-a-ball stars for balls > 5
'***************************************************************************************
...
' vrOption = 0 ' 0 - VR Room off, 1 - VR Cab On, 2- VR Cab and Room On
  ...
  If highScore(0) = "" Then highScore(0) = 300000
  If highScore(1) = "" Then highScore(1) = 250000
  If highScore(2) = "" Then highScore(2) = 200000
  If highScore(3) = "" Then highScore(3) = 150000
  If HighScore(4) = "" Then highScore(4) = 100000
  ...
  If vrOption > 0 Then
    vrOverflow.image = "o0k"
    vrOverflow1.image = "o0k"
  End If
...
Sub bootTable_Timer
  ...
  If bootCount = 1 Then
    If vrOption > 0 Then vrBackGlass.image = "tfOn"
...
Sub bipDisplay
  ...
  Else
    If ballInPlay > 5 Then
      ...
      If B2SOn Then
        controller.B2SSetBallinPlay 32, 0
        controller.B2SsetData (90 + ballInPlay - 5), 1
        controller.B2SsetData (90 + ballInPlay - 4), 0 'this is for decreasing bip
        If ballInPlay > 6 Then controller.B2SsetData (90 + ballInPlay - 6), 0 'this is for increasing bip
      End If
      starReel.setValue(ballInPlay - 5)
    Else
      bipReel.setValue(ballInPlay)
      starReel.setValue(0)
      If B2SOn Then
...
Sub Triggers_hit(Index)
  If Tilt = False Then
    ...
    End If
    Select Case Index
      Case 7: AddScore 10000: If B2SOn Then DOF 115, DOFPulse
      Case 8: InLanes(Index): If B2SOn Then DOF 118, DOFPulse
      Case 9: InLanes(Index): If B2SOn Then DOF 117, DOFPulse
      Case 10: AddScore 10000: If B2SOn Then DOF 116, DOFPulse
    End Select
  End If
  SpecialLanes = 0
  wireNumber = index
...
Sub targets_hit(index)
  ...
  If tilt = False Then
    Select Case Index
      Case 0: For each Light in BumperLights:light.state = 1: Next
          bumpersOn
          If B2SOn Then DOF 121, DOFPulse
          addScore 100
      Case 1: For each Light in BumperLights:light.state = 1: Next
          bumpersOn
          If B2SOn Then DOF 120, DOFPulse
          addScore 100
      Case 2: If TicTacToe = False Then
...
'  These Flags are passed by scores with multiple of 10, 100 or 1000
  If flag = 1 or flag10 = 1 or flag100 = 1 or flag1000 = 1 or flag10000 = 1 or saucerFlag = 1 Then
...
'***************Scoring Routine
Dim flag1, flag10, flag100, flag1000, flag10000, point, point10
...
Sub pts10000
  If pts10000Timer.enabled = False Then
    pts10000Timer.enabled = True
    playFieldSound "Chime1000", 0, soundPoint13, 1
  End If
End Sub

Dim pts10Block, pts100Block, pts1000Block, pts10000Block, intvl
intvl = 40
...
Sub pts10000Timer_Timer
  pts10000Block = pts10000Block + 1
  If pts10000Block = 1 Then
    pts10000Block = 0
    pts10000Timer.enabled = False
  End If
End Sub
...
Sub totalUp(points)
  ...
  If pts10Timer.enabled = False and points = 10 Or pts100Timer.enabled = False and points = 100 Or pts1000Timer.enabled = False and points = 1000 Or pts10000Timer.enabled = False and points = 10000 Then block = 0 Else block = 1
  ...
  If points = 100 Or flag100 = 1 Then
  ...
  If points = 1000 Or flag1000 = 1 Then
    If chime = 0 Then
      pts1000
    Else
  ...
  If points = 10000 Or flag10000 = 1 Then
    If chime = 0 Then
      pts10000
    Else
      If B2SOn Then DOF 155,DOFPulse
  ...
  If score(player) > 99999 Then
    If B2SOn Then
      Select Case (score(1) \ 100000)
        Case 1: controller.B2SSetData 70, 1
        Case 2: controller.B2SSetData 71, 1: controller.B2SSetData 70, 0
        Case 3: controller.B2SSetData 72, 1: controller.B2SSetData 71, 0
        Case 4: controller.B2SSetData 73, 1: controller.B2SSetData 72, 0
        Case 5: controller.B2SSetData 74, 1: controller.B2SSetData 73, 0
        Case 6: controller.B2SSetData 75, 1: controller.B2SSetData 74, 0
        Case 7: controller.B2SSetData 76, 1: controller.B2SSetData 75, 0
        Case 8: controller.B2SSetData 77, 1: controller.B2SSetData 76, 0
        Case 9: controller.B2SSetData 78, 1: controller.B2SSetData 77, 0
        Case 10: controller.B2SSetData 79, 1: controller.B2SSetData 78, 0
        Case 11: controller.B2SSetData 70, 1
        Case 12: controller.B2SSetData 71, 1: controller.B2SSetData 70, 0
        Case 13: controller.B2SSetData 72, 1: controller.B2SSetData 71, 0
        Case 14: controller.B2SSetData 73, 1: controller.B2SSetData 72, 0
        Case 15: controller.B2SSetData 74, 1: controller.B2SSetData 73, 0
        Case 16: controller.B2SSetData 75, 1: controller.B2SSetData 74, 0
        Case 17: controller.B2SSetData 76, 1: controller.B2SSetData 75, 0
        Case 18: controller.B2SSetData 77, 1: controller.B2SSetData 76, 0
        Case 19: controller.B2SSetData 78, 1: controller.B2SSetData 77, 0
      End Select
    End If
    If score(1) < 1900000 Then
      overflowReel.setValue(score(1) \ 100000)
    Else
      overflowReel.setValue(19)
    End If
    If vrOption > 0 Then vrOverFlowSet(score(1) \ 100000)
  End If
...
Sub Special
  ...
  If B2SOn Then
    controller.B2SSetData (69 + (score(1)\100000)), 1
    controller.B2SSetScorePlayer player, score(player)
    If (score(1)\100000) > 1 Then controller.B2SSetData (68 + (score(1)\100000)), 0
  End If
...
Sub vrBip
  Select Case ballInPlay
    ...
    Case 6: If starsOption = 0 Then
          vrBipBg.image = "bip6"
        Else
          vrBipBg.image = "s6"
        End If
    Case 7: If starsOption = 0 Then
          vrBipBg.image = "bip7"
        Else
          vrBipBg.image = "s7"
        End If
    Case 8: If starsOption = 0 Then
          vrBipBg.image = "bip8"
        Else
          vrBipBg.image = "s8"
        End If
    Case 9: If starsOption = 0 Then
          vrBipBg.image = "bip9"
        Else
          vrBipBg.image = "s9"
        End If
    Case 10: If starsOption = 0 Then
          vrBipBg.image = "bip10"
        Else
          vrBipBg.image = "s10"
        End If
...
Dim overFlow
Sub vrOverFlowSet(overflow)
  Select Case overflow
    Case 1: vrOverflow.image = "o100k"
    Case 2: vrOverflow.image = "o200k"
    Case 3: vrOverflow.image = "o300k"
    Case 4: vrOverflow.image = "o400k"
    Case 5: vrOverflow.image = "o500k"
    Case 6: vrOverflow.image = "o600k"
    Case 7: vrOverflow.image = "o700k"
    Case 8: vrOverflow.image = "o800k"
    Case 9: vrOverflow.image = "o900k"
    Case 10: vrOverflow.image = "o1000k"
    Case 11: vrOverflow1.image = "o100k"
    Case 12: vrOverflow1.image = "o200k"
    Case 13: vrOverflow1.image = "o300k"
    Case 14: vrOverflow1.image = "o400k"
    Case 15: vrOverflow1.image = "o500k"
    Case 16: vrOverflow1.image = "o600k"
    Case 17: vrOverflow1.image = "o700k"
    Case 18: vrOverflow1.image = "o800k"
    Case 19: vrOverflow1.image = "o900k"
