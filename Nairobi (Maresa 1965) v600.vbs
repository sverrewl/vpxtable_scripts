' ********************************************************
'               VISUAL PINBALL X 10.8
'       Nairobi / IPD No. 6229 / 1966 / 1 Player
'                   Version 6.0.0
' ********************************************************
...
'
' LED backboard
' Flasher Outside Left - 203/210/214/216/221/240/250
' Flasher left - 205/212/217/225/240/250
' Flasher center - 209/218/224/226/240/229/250
' Flasher right - 206/213/219/227/240/250
' Flasher Outside Right - 204/215/220/222/223/240/228/250
'
' Start Button - 200
' Undercab - 201/202
' Strobe - 230
...
' Valores Constants
Const TableName = "nairobi" ' se usa para cargar y grabar los highscore y creditos
Const cGameName = "nairobi" ' para el B2S
Const MaxPlayers = 1        ' de 1 a 4
Const MaxMultiplier = 1     ' limita el bonus multiplicador a 5
Const Special1 = 1000       ' puntuación a obtener para partida extra
Const Special2 = 1200       ' puntuación a obtener para partida extra
Const Special3 = 1400       ' puntuación a obtener para partida extra
Const Special4 = 1600       ' puntuación a obtener para partida extra
...
Sub Bumper000_Hit
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF ("fx_Bumper",105,DOFPulse,DOFContactors), bumper000
    DOF 209, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    ' añade algunos puntos
    AddScore 1 + 9 * LightBumper000.State
    RotateLights
End Sub
...
Sub Bumper004_Hit
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF ("fx_Bumper",109,DOFPulse,DOFContactors), bumper004
    DOF 213, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    ' añade algunos puntos
    AddScore 1 + 9 * LightBumper004.State
    RotateLights
End Sub

Sub TopLBumper_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_Target", LightTopLBumper
    DOF 214, DOFPulse
    ' añade algunos puntos
    AddScore 1 + 9 * LightTopLBumper.State
    RotateLights
End Sub

Sub TopRBumper_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_Target", LightTopRBumper
    DOF 215, DOFPulse
    ' añade algunos puntos
    AddScore 1 + 9 * LightTopRBumper.State
    RotateLights
End Sub
...
Sub Trigger009_Hit
    ...
    'check?
    If li002.State Then AwardSpecial
    CheckSpecial
End Sub
...
Sub CheckSpecial 'todas las luces de los pasillos tienen que estar apagadas para que se encienda el special
    'arriba
    If li016.State + li017.State + li018.State + li019.State + li020.State = 0 Then
        li028.State = 1
    li013.State = 1
    End If

    'izquierda
    If li014.State + li009.State + li010.State + li014.State = 0 Then
        li022.State = 1
        li023.State = 1
    End If

    'derecha
    If li015.State + li011.State + li012.State + li005.State = 0 Then
        li024.State = 1
        li021.State = 1
    End If

    ' todas
    If li016.State + li017.State + li018.State + li019.State + li020.State + li014.State + li009.State + li010.State + li014.State + li015.State + li011.State + li012.State + li005.State = 0 Then
        li002.State = 1
    End If
End Sub

Sub RotateLights 'alterna las luces centrales
    Dim tmp
    tmp = li001.State
    li001.State = li003.State
    li003.State = li027.State
    li027.State = li026.State
    li008.State = li026.State
    li026.State = li025.State
    li007.State = li025.State
    li025.State = tmp
    li006.State = tmp
End Sub
...
Sub Trigger017_Hit
    ...
    AddScore 1
    If li028.State Then AwardSpecial
End Sub

Sub Trigger014_Hit
    PlaySoundAt "fx_sensor", Trigger014
    If Tilted Then Exit Sub
    If Epileptikdof = True Then DOF 225, DOFPulse End If
    AddScore 1 + 9 * li025.State
End Sub
...
Sub Kicker001_Hit
    ...
    If NOT Tilted Then
    AddScore 100 + 100 * li021.State
    If li022.State Then AwardSpecial
  End If
    vpmtimer.addtimer 1000, "DOF 203, DOFPulse: DOF 230, DOFPulse: PlaySoundAt SoundFXDOF(""fx_kicker"",110, DOFPulse, DOFContactors),kicker001: kicker001.kick 155,12 + RND(1)*5 '"
End Sub

Sub Kicker002_Hit
    PlaySoundAt "fx_kicker_enter", Kicker002
    If NOT Tilted Then
    AddScore 100 + 100 * li023.State
    If li024.State Then AwardSpecial
  End If
    vpmtimer.addtimer 1000, "DOF 204, DOFPulse: DOF 230, DOFPulse: PlaySoundAt SoundFXDOF(""fx_kicker"",111, DOFPulse, DOFContactors),kicker002: kicker002.kick 205,12 + RND(1)*5 '"
End Sub
