...
Sub LeftSlingShot_Slingshot()
	if tilt=false then PlaySound SoundFXDOF("slingshot",103,DOFPulse,DOFContactors):DOF 104, DOFPulse
    addscore 1
End Sub

Sub RightSlingShot_Slingshot()
	if tilt=false then PlaySound SoundFXDOF("slingshot",105,DOFPulse,DOFContactors):DOF 106, DOFPulse
	addscore 1
End Sub

sub bumper1_hit
    if tilt=false then playsound SoundFXDOF("jet1",107,DOFPulse,DOFContactors):DOF 111, DOFPulse
    if bumperlight1.state=1 then addscore 10 else addscore 1
end sub

sub bumper2_hit
    if tilt=false then playsound SoundFXDOF("jet1",108,DOFPulse,DOFContactors):DOF 112, DOFPulse
    if bumperlight2.state=1 then addscore 10 else addscore 1
end sub

sub bumper3_hit
    if tilt=false then playsound SoundFXDOF("jet1",109,DOFPulse,DOFContactors):DOF 113, DOFPulse
    if bumperlight3.state=1 then addscore 10 else addscore 1
end sub

sub bumper4_hit
    if tilt=false then playsound SoundFXDOF("jet1",110,DOFPulse,DOFContactors):DOF 114, DOFPulse
...
Sub OnBallBallCollision(ball1, ball2, velocity)
  If JollyRoger.VersionMinor > 3 OR JollyRoger.VersionMajor > 10 Then
