...
'   141 Chime1-1s, 142 Chime2-10s, 143 Chime3-100s
'   150 - 153 Rollover Buttons
'   199 credit light
'   128 Knocker & Strobe
...
'                        EndGame
'                         ^
'   Start Game -> New Game -> Reset Reel -> Check Continue -> Release Ball -> Drain -> Score Bouns -> Advance Player -> Next Ball
'                         ^                                    |
'                       EndGame = True <--------------------------------------------------------------
'
...
Const cGameName = "PirateGold_1969"
Const cOptions = "PirateGold_1969.txt"
Const hsFileName = "Pirate Gold (Chicago Coin 1969)"
...
Sub Table1_init
  ...
  If highScore(0) = "" Then highScore(0) = 2500
  If highScore(1) = "" Then highScore(1) = 2430
  If highScore(2) = "" Then highScore(2) = 2140
  If highScore(3) = "" Then highScore(3) = 1530
  If HighScore(4) = "" Then highScore(4) = 1020
  ...
  For x = 1 to maxPlayers
    ...
    sReel.setvalue(score(x))
  Next

  CoinCard.image = "CoinCard" & balls
  coveLight = 1
  coveBonus = 1
  tenPoint = 0
  firstBallOut = 0
...
Sub Table1_KeyDown(ByVal keycode)
  ...
  If keycode = addCreditKey Then
    ...
    If enableInitialEntry = False and operatormenu = 0 and backGlassOn = 1 Then
      If freePlay = 0 and credit > 0 and players < 2 and state = False Then
        StartUp
        If showDT = False Then PlayReelSound "Reel5", bgcrReel Else PlayReelSound "Reel5", dtcrReel
      End If
      If freePlay = 1 and credit = 0 and players < 2 and state = False Then
        AddCoin
        StartUp
      End If
      If freePlay = 1 and credit > 0 and players < 2 and state = False Then
...
Sub bootTable_Timer()
  ...
  If bootCount = 1 Then
    ...
    layeri3.image="layer4"
    Light1.state = 1
    point500Light.state = 1
    CollectBonus1.state = 1
    CollectBonus4.state = 1
...
'***********Ball In Play Lights
' This sub only needed for tables with GAME OVER and Ball In Play accounting on the apron
Sub ballInPlayLights
  If ballinPlay > balls Then
    l_gameover1.state = 1
    l_gameover2.state = 1
    l_ballinplay1.state = 0
    l_ballinplay2.state = 0
    EVAL("l" & balls).state = 0
    Exit Sub
  Else
    l_gameover1.state = 0
    l_gameover2.state = 0
    l_ballinplay1.state = 1
    l_ballinplay2.state = 1
    If ballInPlay > 1 Then EVAL("l" & ballinPlay - 1).state = 0
    EVAL("l" & ballInPlay).state = 1
  End If
End Sub
...
Sub ScoreBonus
  bonusScore = 0
  If BonusScore = 0 And ShootAgain.state = 1 Then ReleaseBall: Exit Sub
  If BonusScore = 0 Then AdvancePlayers
End Sub
...
Sub nextBall
  ...
  If onePointCount = 0 and ballInPlay = Balls Then
    coveLight = 1
  Else
    coveLight = 0
  End If
...
Sub Trigger_Hit (Index)
  ...
  Select Case (Index)
    Case 0: If CollectBonus1.state = 1 Then
          BonusPayOut
        Else
          addScore 100
        End If
    Case 1: addScore 100: If Special1.state = 1 Then ShootAgain.state = 1:  If B2SOn Then controller.B2SSetShootAgain 36,1
    Case 2: If CollectBonus2.state = 1 Then
          BonusPayOut
        Else
          addScore 100
        End If
    Case 3: If Special2.state = 1 Then Special: Else addScore 100
    Case 4: If CollectBonus3.state = 1 Then
          BonusPayOut
        Else
          addScore 100
        End If
    Case 5: If CollectBonus4.state = 1 Then
          BonusPayOut
        Else
          addScore 100
        End If
    Case 6: If Special3.state = 1 Then: Special: Else: addScore 100
  End Select
End Sub
...
'************** RollOvers
Dim buttonNumber, coveBonus, coveLight
Sub rollOver_Hit (Index)
  ...
  If buttonNumber > 1 Then
    ...
    End If
  End If
  If coveBonus < 5 and Index < 3 Then coveBonus = coveBonus + 1
  If coveBonus = 2 or coveBonus = 4 and ballInPlay = 1 or ballInPlay = Balls Then
    Bumper1On
  Else
    Bumper1Off
  End If
  ...
  If coveBonus = 5 Then
    layeri0.image="layer4"
    layeri1.image="layer4"
    rollOverLight0.state = 1
    rollOverLight1.state = 1
  End If
  EVAL ("light" & coveBonus).state = 1
  If coveBonus > 1 Then EVAL ("light" & coveBonus - 1).state = 0
  rollOverAnimation.enabled = 1
  If B2SOn Then DOF (150 + Index), DOFPulse
  Select Case (Index)
    Case 0: If rollOverLight0.state = 1 Then addScore 100
    Case 1: If rollOverLight1.state = 1 Then addScore 100
    Case 3: If coveLight = 1 Then
          addScore 1000
        Else
          addScore 500
        End If
    Case 2: If coveBonus = 5 Then addScore 10
  End Select
End Sub
...
Sub rollOverAnimation_Timer
  button = button + 1
  Select Case Button
    Case 1: EVAL ("Button" & ButtonNumber).transz = -2
    Case 2: EVAL ("Button" & ButtonNumber).transz = 0
    Case 3: EVAL ("Button" & ButtonNumber).transz = 1
    Case 4: EVAL ("Button" & ButtonNumber).transz = 1
        button = 0
...
Sub kickBall(kball, kangle, kvel, kvelz, kzlift)
  dim scat
  scat=0.2  'Increase for more saucer kicker randomness.  Random velx & vely velocity adder ranging from +-scat
  dim rangle
  rangle = 3.14 * (kangle - 90) / 180
  kball.z = kball.z + kzlift
  kball.velz = kvelz
  kball.velx = (cos(rangle)*kvel)+(((int(rnd*3))-1)*scat)
  kball.vely = (sin(rangle)*kvel)+(((int(rnd*3))-1)*scat)
End Sub


Dim kickerBall, inSaucer
Sub saucers_Hit (Index)
  set kickerBall = activeBall
  If PFOption = 1 Or PFOption = 2 Then
    PlaySound "SaucerIn", 0, 1 * xGain(saucers(index)), AudioPan(saucers(index)), 0, 0, 1, 0, 0 'Left & Right stereo or Top & Bottom stereo PF Speakers.
  End If
  If PFOption = 3 Then
    PlaySound "SaucerIn", 0, 1 *    XVol(saucers(index))  *    YVol(saucers(index)),  -1, 0, 0, 1, 0, -1  'Top Left PF Speaker
    PlaySound "SaucerIn", 0, 1 * (1-XVol(saucers(index))) *    YVol(saucers(index)),   1, 0, 0, 1, 0, -1  'Top Right PF Speaker
    PlaySound "SaucerIn", 0, 1 *    XVol(saucers(index))  * (1-YVol(saucers(index))), -1, 0, 0, 1, 0,  1  'Bottom Left PF Speaker
    PlaySound "SaucerIn", 0, 1 * (1-XVol(saucers(index))) * (1-YVol(saucers(index))),  1, 0, 0, 1, 0,  1  'Bottom Right PF Speaker
  End If
  kickStep = 0
  pKickArm.rotz = 15: pKickArm1.rotz = 15
  inSaucer = Index
  saucerKick.enabled = 1
End Sub
...
Sub saucerKick_timer
    Select Case kickStep
        Case 3:pKickArm.rotz=15: pKickArm1.rotz=15
        If inSaucer = 0 Then
          kickBall kickerBall, 60, 8, 5, 30
        Else
          kickBall kickerBall, -60, 8, 5, 30
        End If
        playFieldSound "SaucerKick",0, saucer1, 1: playFieldSound "SaucerKick",0, saucer0, 1
        If B2SOn Then DOF 120, DOFPulse
        BonusPayOut
        Case 4:pKickArm.rotz=15: pKickArm1.rotz=15
        Case 5:pKickArm.rotz=15: pKickArm1.rotz=15
        Case 6:pKickArm.rotz=15: pKickArm1.rotz=15
        Case 7:pKickArm.rotz=8: pKickArm1.rotz=8
        Case 8:pKickArm.rotz=3: pKickArm1.rotz=3
    Case 9:pKickArm.rotz=0:: pKickArm1.rotz=15: saucerKick.Enabled = 0: KickStep = 0
    End Select
   kickstep = kickstep + 1
End Sub
...
dim onePointCount
Sub onePointStepper
  onePointCount = onePointCount + 1
  If onePointCount > 9 Then
    onePointCount = 0
    tenPointFlipFlop
  End If
  If onePointCount = 0 and ballInPlay = 1 or onePointCount = 0 and ballInPlay = Balls Then
    coveLight = 1
  Else
    coveLight = 0
  End If
...
dim tenPoint
Sub tenPointFlipFlop
  tenPoint = tenPoint + 1
  If tenPoint > 1 Then tenPoint = 0
  If tenPoint = 1 Then
    CollectBonus1.state = 0
    CollectBonus2.state = 1
    CollectBonus3.state = 1
    CollectBonus4.state = 0
  Else
    CollectBonus1.state = 1
    CollectBonus2.state = 0
    CollectBonus3.state = 0
    CollectBonus4.state = 1
  End If
End Sub

'************1000PointLight
Sub thousandPointLight
  If coveLight = 0 Then
    layer1_1000.image="layer1_off"
    layer21000plastic.image="layer2_off"
    layer3plastic.image="layer3_off"
    flasher001.visible=0
    For each Light in GI1000plastic:light.state = 0:Next
    For each Light in SpecialLights:light.state = 0:Next
    Point500Light.state = 1
  Else
    layer1_1000.image="layer1"
    layer21000plastic.image="layer2"
    layer3plastic.image="layer3"
    flasher001.visible=1
    For each Light in GI1000plastic:light.state = 1:Next
    For each Light in SpecialLights:light.state = 1:Next
    Point500Light.state = 0
  End If
End Sub
...
Sub scoremotor5_Timer
  ...
  playFieldSound "ScoreMotorSingleFire", 0, SoundPointScoreMotor, 1 'need to set location of score motor under the PF
  If scoremotorCount = 5 Then
    If relBall = 1 Then
      ...
      If coveBonus = 5 Then
        coveBonus = 1
        layeri0.image="layer4_off"
        layeri1.image="layer4_off"
        rollOverLight0.state = 0
...
'***************Match
Sub match
  matchNumber = ((int(rnd(1)*10)) * 1)
  MatchReel.setValue(matchNumber/10) 'need to change if 1 point table
...
Sub updateReels
  ...
  If reelStop = 0 Then If showDT = False Then PlayReelSound "Reel1", bgs1Reel Else PlayReelSound "Reel1", dts1Reel
...
Sub resetReel_Timer
  ...
  Select Case resetLoop
    ...
    Case 6: If reelStop = 1 Then
          ...
          reelFlag = 0
          reelStop = 0
          If testFlag = 0 Then releaseBall: ballInPlayLights
          testFlag = 0
          resetReel.enabled = 0
          Exit Sub
        End If
...
Sub checkHighScores
  ...
  For hs = 1 to 4                   'look at 4 player scores
    For chY = 0 to 4                  'look at 4 player scores'look at all 5 saved high scores
...
Sub loadHighScore
  ...
  Set textStr = scoreFile.OpenAsTextStream(1,0)
    ...
    balls = cdbl (temp(28))
    replayEB = cdbl (temp(29))
    matchNumber = cdbl (temp(30))
    chime = cdbl (temp(31))
    score(1) = cdbl (temp(32))
    pfOption = cdbl (temp(33))
    onBumper = cdbl (temp(34))
