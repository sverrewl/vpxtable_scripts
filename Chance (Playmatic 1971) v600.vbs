...
'                       VPX8 Version 6.0.0
' Chance    IPDB: https://www.ipdb.org/machine.cgi?id=4878
' ****************************************************************
...
'
' Flippers L/R - 401/402
' Slingshot L/R - 403/404
' Bumpers - 405/406/407
' Kickers - 409/410
' Targets - 411/412/413/414/415/416/417/418/419
'
' LED backboard
...
' Valores Constants
Const TableName = "chance" ' se usa para cargar y grabar los highscore y creditos
Const cGameName = "chance_1971" ' para el B2S
Const MaxPlayers = 1       ' de 1 a 4
Const MaxMultiplier = 2    ' limita el bonus multiplicador a 2
Const Special1 = 3200      ' puntuación a obtener para partida extra
Const Special2 = 3700      ' puntuación a obtener para partida extra
Const Special3 = 4500      ' puntuación a obtener para partida extra
Const Special4 = 5200      ' puntuación a obtener para partida extra
...
Sub EndOfBall()
    ...
    '=========================================================
    ' EN ESTA MESA NO SE CUENTAN LOS BONOS AL FINAL DE LA BOLA
...
' en esta mesa se cuentan los bonos al meter la bola en uno de los agujeros y la aguja te marca que cuentes los bonos
Sub BonusCountTimer_Timer 'añade los bonos y actualiza las luces
    ...
    If Bonus > 0 Then
        ...
        UpdateBonusLights
    Else
        ' termina la cuenta de los bonos y continúa el reloj
        BonusCountTimer.Enabled = 0
        BonusMultiplier = 1 'reset the multiplier
...
' lights as flashers
match001.ImageA = "match"
match002.ImageA = "match"
match003.ImageA = "match"
match004.ImageA = "match"
match005.ImageA = "match"
match006.ImageA = "match"
match007.ImageA = "match"
match008.ImageA = "match"
match009.ImageA = "match"
match010.ImageA = "match"
...
Select case match
Case 0: match010.ImageA = "match0"
Case 1: match001.ImageA = "match1"
Case 2: match002.ImageA = "match2"
Case 3: match003.ImageA = "match3"
Case 4: match004.ImageA = "match4"
Case 5: match005.ImageA = "match5"
Case 6: match006.ImageA = "match6"
Case 7: match007.ImageA = "match7"
Case 8: match008.ImageA = "match8"
Case 9: match009.ImageA = "match9"
End Select
...
Sub AddScore(Points)
    ...
    Select Case Points
        ...
        PlaySound SoundFXDOF("fx_bell" &Points, 301, DOFPulse, DOFChimes)
            End If
        Case 200
            Add100 = Add100 + 2
...
Sub AddBonus(bonuspoints)
    If(Tilted = False)Then
        ...
        PlaySound SoundFXDOF ("fx_bell",301,DOFPulse,DOFChimes)
        If Bonus > 10 Then
            Bonus = 10
        End If
        ' actualiza las luces
...
Sub Bumper1_Hit
    ...
    PlaySoundAt SoundFXDOF ("fx_Bumper",405,DOFPulse,DOFContactors), bumper1
    DOF 207, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    ' añade algunos puntos
    AddScore 1 + 9 * LightBumper1.State
End Sub

Sub Bumper2_Hit
    If Tilted Then Exit Sub
    'PlaySoundAt "fx_Bumper", bumper2
    PlaySoundAt SoundFXDOF ("fx_Bumper",406,DOFPulse,DOFContactors), bumper2
    DOF 208, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    ' añade algunos puntos
    AddScore 1 + 9 * LightBumper2.State
End Sub

Sub Bumper3_Hit
    If Tilted Then Exit Sub
    'PlaySoundAt "fx_Bumper", bumper3
    PlaySoundAt SoundFXDOF ("fx_Bumper",407,DOFPulse,DOFContactors), bumper3
    DOF 209, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    ' añade algunos puntos
...
Sub Trigger001_Hit
  ...
  DOF 217, DOFPulse
    AddScore 100
    'check?
    If li011.State Then AwardSpecial
End Sub
...
Sub PostDown
    PlaySoundAt "fx_solenoidoff", postlight
    Post.IsDropped = 1
    Postlight.State = 0
End Sub

Sub PostUp
    PlaySoundAt "fx_solenoidon", postlight
    Post.IsDropped = 0
    Postlight.State = 1
End Sub
...
Sub kicker001_hit
  ...
  PlaySoundAt SoundFXDOF ("fx_kicker_enter", 409, DOFPulse, DOFContactors), kicker001
    If Not Tilted Then
        AwardClock
        'vpmtimer.addtimer 1500, "kicker001.kick 275, 18:PlaySoundAt""fx_kicker"",kicker001 '"
        vpmtimer.addtimer 1500, "kicker001.kick 275, 18:PlaySoundAt SoundFXDOF (""fx_kicker"", 409, DOFPulse, DOFContactors),kicker001 '"
    DOF 210, DOFPulse
    DOF 230, DOFPulse
    Else 'if tilted
        'vpmtimer.addtimer 100, "kicker001.kick 275, 18:PlaySoundAt""fx_kicker"",kicker001 '"
        vpmtimer.addtimer 100, "kicker001.kick 275, 18:PlaySoundAt SoundFXDOF (""fx_kicker"", 409, DOFPulse, DOFContactors),kicker001 '"
    DOF 210, DOFPulse
    DOF 230, DOFPulse
    End If
End Sub
...
Sub kicker002_hit
  ...
  PlaySoundAt SoundFXDOF ("fx_kicker_enter", 410, DOFPulse, DOFContactors), kicker002
    If Not Tilted Then
        AwardClock
        'vpmtimer.addtimer 1500, "kicker002.kick 175, 16:PlaySoundAt""fx_kicker"",kicker002 '"
    vpmtimer.addtimer 1500, "kicker002.kick 175, 16:PlaySoundAt SoundFXDOF (""fx_kicker"", 410, DOFPulse, DOFContactors),kicker002 '"
    DOF 210, DOFPulse
    DOF 230, DOFPulse
    Else 'if tilted
        'vpmtimer.addtimer 100, "kicker002.kick 175, 16:PlaySoundAt""fx_kicker"",kicker002 '"
        vpmtimer.addtimer 100, "kicker002.kick 175, 16:PlaySoundAt SoundFXDOF (""fx_kicker"", 410, DOFPulse, DOFContactors),kicker002 '"
    DOF 210, DOFPulse
    DOF 230, DOFPulse
    End If
End Sub
...
Sub clocktimer_timer
  Dim temppos
  temppos = int(NeedlePos/6)+100
  If B2SOn then
    Controller.B2SSetData temppos,0
  end if
    NeedlePos = (needlePos + 1)MOD 360
    needle.RotZ = NeedlePos
  temppos = int(NeedlePos/6)+100
  If B2SOn then
    Controller.B2SSetData temppos,1
  end if
End Sub
...
Sub AwardClock
    ...
    If NeedlePos > 13 AND NeedlePos < 30 Then
        li011.State = 1 'enciende Special
ElseIf NeedlePos > 36 AND NeedlePos < 73 Then
        AddScore 200
ElseIf NeedlePos > 78 AND NeedlePos < 95 Then
        BonusMultiplier = 2:BonusCountTimer.Enabled = 1 'Double Bonus y suma bonos
ElseIf NeedlePos > 101 AND NeedlePos < 138 Then
        AddScore 200
ElseIf NeedlePos > 144 AND NeedlePos < 160 Then
        li011.State = 1 'enciende Special
ElseIf NeedlePos > 165 AND NeedlePos < 194 Then
        BonusCountTimer.Enabled = 1 'suma bonos
ElseIf NeedlePos > 200 AND NeedlePos < 216 Then
        AddScore 500
ElseIf NeedlePos > 221 AND NeedlePos < 245 Then
        AwardMystery
ElseIf NeedlePos > 251 AND NeedlePos < 289 Then
        AddScore 200
ElseIf NeedlePos > 295 AND NeedlePos < 325 Then
        BonusCountTimer.Enabled = 1 'suma bonos
ElseIf NeedlePos > 331 AND NeedlePos < 360 Then
        AddScore 200
ElseIf NeedlePos > -1 AND NeedlePos < 8 Then
        AddScore 200
    Else
        AddScore 200
    End If
    vpmtimer.addtimer 1500, "clocktimer.Enabled = 1 '"
End Sub
