' Kingdom / IPD No. 5168 / 4 Players
' J. Esteban, of São Paulo, Brazil
' VPX8 table by jpsalas v6.0.0, 2025, with graphics by Halen.
...
' Valores Constants
Const TableName = "kingdom" ' file name to save highscores and other variables
Const cGameName = "kingdom" ' B2S name
Const MaxPlayers = 4        ' 1 to 4 can play
Const MaxMultiplier = 3     ' limit bonus multiplier
Const FreePlay = False      ' Free play or coins
Const Special1 = 82000      ' extra ball or credit
Const Special2 = 99000
...
Sub RightFlipper2_Animate
    RightFlipper2Top.Rotz = RightFlipper2.CurrentAngle
End Sub
...
Sub BonusCountTimer_Timer 'when hitting the top hole
    ...
    Else
        ...
        ' Exit kicker
        vpmTimer.AddTimer 1500, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker001:kicker001.kick 190,10 '"
    End If
End Sub
...
Sub UpdateBonusLights
    Select Case Bonus
        ...
        Case 10:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 0:bl7.State = 0:bl8.State = 0:bl9.State = 0:bl10.State = 1:Light012.State = 1
    End Select
End Sub
...
Sub AddScore(Points)
    ...
    Select Case Points
        Case 10, 100, 1000
            ...
            ElseIf Points = 100 AND(Score(CurrentPlayer) MOD 1000) = 0 Then 'new reel 1000
...
Sub Bumper003_Hit 'right
    ...
    DOF 209, DOFPulse
    AddScore 100 + 900 * BumperL003.State
    ToggleLights
End Sub
...
Sub ToggleLights
    Dim tmp
    tmp = Light015.state
    Light015.State = Light016.state
    Light016.State = tmp

    ' special
    tmp = Light002.state
    Light002.State = Light003.State
    Light003.State = tmp
End Sub
...
Sub rsband006_Hit:AddScore 10:ToggleLights:End Sub
Sub rsband005_Hit:AddScore 10:ToggleLights:End Sub
Sub rlband007_Hit:AddScore 10:ToggleLights:End Sub
Sub rlband005_Hit:AddScore 10:ToggleLights:End Sub
...
Sub Trigger003_Hit 'left outlane
    ...
    If Tilted Then Exit Sub
    AddScore 500 + 2500 * Light015.State
End Sub

Sub Trigger005_Hit 'left inlane
...
Sub Trigger006_Hit 'right inlane
    PlaySoundAt "fx_sensor", Trigger006
    If Tilted Then Exit Sub
    AddScore 500
    if Light003.State Then AwardSpecial:Light003.State = 0
    ResetRedBank
End Sub
...
Sub Trigger004_Hit 'right outlane
    ...
    If Tilted Then Exit Sub
    AddScore 500 + 2500 * Light016.State
End Sub

' Top Lanes: KING lanes
...
Sub CheckKING 'KING lights
    If Light023.State + Light022.State + Light024.State + Light025.State = 0 Then
        KINGhits = KINGhits + 1
        Select case KINGhits
            Case 1:Light011.State = 1 'extra ball top light
            Case 2:AwardExtraBall:Light011.State = 0
            Case Else:Addscore 5000
        End Select
        ResetKINGLights
    End If
End Sub
...
Sub dt1_Dropped
    If Tilted Then Exit Sub
    DOF 203, DOFPulse
    AddScore 500 + 2500 * Light026.state
    Light043.State = 0
    AddBonus 1
    CheckYellowBank
End Sub
...
Sub dt3_Dropped 'center red drop target
    ...
    AddScore 500 + 2500 * Light026.state
    Light045.State = 0
    If Light010.State Then AwardSpecial
    AddBonus 1
    CheckYellowBank
End Sub
...
Sub CheckYellowBank
    If Light043.State + Light044.State + Light045.State + Light046.State + Light047.State = 0 Then
        YellowBankHits = YellowBankHits + 1
        Select Case YellowBankHits
            Case 1:Light026.State = 1:vpmTimer.AddTimer 500, "ResetYellowBank '"
            Case 2:Light026.State = 0:Light010.State = 1:vpmTimer.AddTimer 500, "ResetCenterYellowBank '"
            Case 3:Light010.State = 0:AwardSpecial:YellowBankHits = 0:vpmTimer.AddTimer 500, "ResetYellowBank '"
        End Select
    End If
End Sub
...
Sub dt7_Hit:PlaySoundAt SoundFXDOF("fx_droptarget", 112, DOFPulse, DOFContactors), dt7:End Sub
Sub dt8_Hit:PlaySoundAt SoundFXDOF("fx_droptarget", 112, DOFPulse, DOFContactors), dt8:End Sub

Sub dt6_Dropped
    If Tilted Then Exit Sub
    DOF 205, DOFPulse
    AddScore 500 + 2500 * Light013.State
    If Light014.State then AwardExtraBall
    Light054.State = 0
    AddBonus 1
    CheckRedBank
End Sub
...
Sub CheckRedBank
    If Light052.State + Light053.State + Light054.State = 0 Then
        RedBankHits = RedBankHits + 1
        Select Case RedBankHits
            Case 1:Light013.State = 1
            Case 2:Light013.State = 0:Light014.State = 1
            Case 3:Light014.State = 0:AwardExtraBall:RedBankHits = 0
        End Select
        vpmTimer.AddTimer 500, "ResetRedBank '"
    End If
End Sub

Sub ResetRedBank
    PlaySoundAt SoundFXDOF("fx_resetdrop", 112, DOFPulse, DOFContactors), dt7
    dt6.Isdropped = 0
    dt7.Isdropped = 0
    dt8.Isdropped = 0
    Light052.State = 1
...
Sub Kicker001_Hit 'Top Right
    ...
    If Tilted Then vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker001:kicker001.kick 190,10 '"
    DOF 211, DOFPulse
    DOF 231, DOFPulse
    If Light012.State Then AwardExtraBall:Light012.State = 0
    'Count the Bonus
...
' Top Left Loop rollovers

Sub TT1_Hit
    PlaySoundAt "fx_sensor", TT1
    If Tilted Then Exit Sub
    AddScore 100 + 900 * Light007.State
End Sub

Sub TT2_Hit
    PlaySoundAt "fx_sensor", TT2
    If Tilted Then Exit Sub
    AddScore 100 + 900 * Light008.State
End Sub

Sub TT3_Hit
    PlaySoundAt "fx_sensor", TT3
    If Tilted Then Exit Sub
    AddScore 100 + 900 * Light009.State
End Sub

' Center Loop rollovers

Sub CLR_Hit
    PlaySoundAt "fx_sensor", CLR
    If Tilted Then Exit Sub
    LastSwicthHit = "CLR"
End Sub

Sub CRR_Hit
    PlaySoundAt "fx_sensor", CRR
    If Tilted Then Exit Sub
    LastSwicthHit = "CRR"
End Sub

Sub CCR_Hit 'center rollover
    PlaySoundAt "fx_sensor", CCR
    If Tilted Then Exit Sub
    Addscore 5000
    If LastSwicthHit = "CLR" Then
        CLRhits = CLRhits + 1
        Select Case CLRhits
            Case 1:Light049.State = 1
            Case 2:Light049.State = 0:Light020.State = 1
            Case 3:Light020.State = 0:Light018.State = 1
            Case 4:Light018.State = 0:Light017.State = 1
            Case 5:Light017.State = 0:Light004.State = 1
        End Select
    Else 'LastSwicthHit = "CRR"
        CRRhits = CRRhits + 1
        Select Case CRRhits
            Case 1:Light050.State = 1
            Case 2:Light050.State = 0:Light021.State = 1
            Case 3:Light021.State = 0:Light019.State = 1
            Case 4:Light019.State = 0:Light017.State = 1
            Case 5:Light017.State = 0:Light004.State = 1
        End Select
    End If
    'check for specials
    Light048.State = 0
    If Light049.State + Light050.State = 2 Then Light048.State = 1
    If Light020.State + Light021.State = 2 Then Light048.State = 1
    If Light018.State + Light019.State = 2 Then Light048.State = 1
    If Light017.State + Light004.State = 1 Then Light048.State = 1 'any of the specials is lit
End Sub
...
Sub spinner_Spin
    ...
    If Tilted Then Exit Sub
    Addscore 100 + 900 * Light051.state
End Sub

' Center target
' Collect Loop awards, both sides must be on to collect.
...
Sub Target009_Hit
    ...
    DOF 203, DOFPulse
    ResetYellowBank
    If Light048.State Then
        If Light049.State + Light050.State = 2 Then
            Addscore 5000
            ResetLoopLights
        End If
        If Light020.State + Light021.State = 2 Then
            BonusMultiplier = 2:UpdateMultiplierLights
            ResetLoopLights
        End If

        If Light018.State + Light019.State = 2 Then
            BonusMultiplier = 3:UpdateMultiplierLights
            ResetLoopLights
        End If

        If Light017.State Then
            Light012.State = 1 'lits extra ball at the hole
            ResetLoopLights
        End If

        If Light004.State Then
            Light002.State = 0
            Light003.State = 1 'lits special at the inlanes
            ResetLoopLights
        End If
    Else 'if light048 is off
        Addscore 500
    End If
End Sub

Sub ResetLoopLights
    For each x in aLoopLights
        x.State = 0
    Next
    CLRhits = 0
    CRRHits = 0
...
Sub StopT1_Hit: Stop1.Collidable = 1: stop2.Collidable = 1: Stop3.Collidable = 1: End Sub
Sub StopT2_Hit: Stop1.Collidable = 1: stop2.Collidable = 1: Stop3.Collidable = 1: End Sub
Sub StopT3_Hit: Stop1.Collidable = 0: stop2.Collidable = 0: Stop3.Collidable = 0: End Sub


'================== PLAYER OPYIONS =======================

' If you prefer the apron rules in English then uncomment the next line
'Apron.Image = "Apron_US"
...
Sub Table1_OptionEvent(ByVal eventId)
    ...
    ' Apron card
    x = Table1.Option("Apron card language", 0, 1, 1, 0, 0, Array("Portuguese", "English") )
    If x = 1 Then Apron.Image = "Apron_US" Else Apron.Image = "apron_BR"
End Sub
