...
' MachWon:  Playfield/inserts graphics cleanup,lighting corrected,fasteners added, sounds added/corrected, script corrections and nFozzy/Flipper Tricks added
' pdrum:    Playfield, Bumpers, Plastics and Apron Enhanced to 4K Graphics
...
SVol =  0.09         ' Callouts Volume (Max 1.0)
TVol =  0.8          ' Background Track Volume (Not Used)
CVol =  0.9          ' Bell Volume (Max 1.0)
...
Const VolBump   =  1    ' Bumpers volume: Higher Number Higher Volume
Const VolGates  =  5    ' Gates volume.
Const VolFlip   =  5    ' Flipper volume.
Const VolTrig   =  6    ' Trigger volume.
Const VolRol    =  7    ' Rollover volume.
Const VolRH     =  5    ' Rubber volume
...
Sub Table1_OptionEvent(ByVal eventId)
    ...
    ' --- Table LUT ---
    LUTimage = Table1.Option("LUT Table Color Setting", 0, 15, 1, 0, 0, Array("Normal","New ColorLUT","Warm Dark","Warm Bright","Warm Vivid Soft","Warm Vivid Hard","Natural Balanced","Natural High Contrast","THX Standard","Punchy","Desaturated","Washed Out","Natural Dark","Bassgeige","Blacklight","B&W Comic Book"))
...
Sub StopAllTunes()   ' Stop sounds for high score
    StopSound "Sound-0x0da"
    StopSound "Sound-0x0ad"
    StopSound "Sound-0x0aa"
    StopSound "Sound-0x0f7"
    StopSound "Sound-0x0ae"
    StopSound "Sound-0x02b"
    StopSound "Sound-0x02c"
    StopSound "Sound-0x05c"
    StopSound "Sound-0x05d"
    StopSound "Sound-0x06a"
    StopSound "Sound-0x06b"
    StopSound "Sound-0x087"
    StopSound "Sound-0x088"
    StopSound "Sound-0x089"
    StopSound "Sound-0x0fb"
    StopSound "Sound-0x0e1"
    StopSound "Sound-0x0e2"
    StopSound "Sound-0x0e3"
    StopSound "Sound-0x0e4"
    StopSound "Sound-0x0e5"
    StopSound "Sound-0x0e6"
...
Sub TiltCheck
   If Tilted = FALSE then
      ...
      Else
         Tilts = Tilts + int(rnd*100)
      End If
      If Tilts >= 500 and Tilted = FALSE then
...
Sub EndOfGame()
    ...
    Light26.state = LightStateOn:Light27.state = LightStateOn:Light6.state = LightStateOn:Light7.state = LightStateOn
    If B2SOn Then
        Controller.B2SSetGameOver 1
...
Sub Trigger3_Hit()
  ...
  If L3.State = LightStateOn and  L1.State = LightStateOn then
    ...
    skillshotoff()
  End if
  If L3.State = LightStateOn and L1.State = LightStateOn and L2.State = LightStateOn and L4.State = LightStateOn then
    AddScore (9)
    skillshotoff()
  End if
  If L3.State = LightStateOn and L1.State <> LightStateOn then
...
Sub Bumper1_Hit()
  ...
  PlaySoundAt "primbump14",Bumper1
    DOF 105, 2
  if b1=0 then
    AddScore 1
    skillshotoff()
...
Sub Bumper2_Hit()
  ...
  PlaySoundAt "primbump14",Bumper2
  DOF 107, 2
  if b2=0 then
    AddScore 1
    skillshotoff()
...
Sub Bumper3_Hit()
  ...
  PlaySoundAt "primbump14",Bumper3
  DOF 106, 2
  if b3=0 then
    AddScore 1
    skillshotoff()
...
Sub Bumper4_Hit()
  ...
  PlaySoundAt "primbump14",Bumper4
  DOF 108, 2
  if b4=0 then
    AddScore 1
    skillshotoff()
...
Sub RightSlingShot_Slingshot
  ...
  PlaySoundAt SoundFX("Sling_R1", DOFContactors),SLING9
  PlaySoundAt "Sound-0x0042",SLING9
    DOF 104, 2
    Addscore(1)
    skillshotoff()
...
Sub LeftSlingShot_Slingshot
  ...
  PlaySoundAt SoundFX("Sling_L1", DOFContactors),SLING8
  PlaySoundAt "Sound-0x0042",SLING8
    DOF 103, 2
    Addscore(1)
    skillshotoff()
 ...
 If Tilted = FALSE then
    ...
    Else
        If L14.State = LightStateOn then ' Upper Left Light
            ...
            L8.State = LightStateOn   ' Right Red Rollover
        Else
           AddScore(10) ' Get 10pts when light is off
           skillshotoff()
        End if
    End if
 ...
 If Tilted = FALSE then
    ...
    Else
        If L11.State = LightStateOn then ' Upper Left Light
            ...
            L10.State = LightStateOn  ' 2nd rollover
        Else
           AddScore(10) ' Get 1pts when light is off
           skillshotoff()
         End if
    End if
...
Sub SkillshotOff   'turn off when points are scored
  if LampSplash2.enabled = True then
    ...
    Light27.state = LightStateOff:Light26.state = LightStateOff:Light6.state = LightStateOff:Light7.state = LightStateOff
  End if
End Sub
...
Sub LampSplash2_Timer() ' flash the lights while ball is in the trough
  ...
  Select Case lno
         Case 1
            Light27.state = LightStateOff:Light26.state = LightStateOff:Light6.state = LightStateOff:Light7.state = LightStateOff
        Case 2
            Light27.state = LightStateOn:Light26.state = LightStateOn:Light6.state = LightStateOn:Light7.state = LightStateOn
        Case 3
            Light27.state = LightStateOff:Light26.state = LightStateOff:Light6.state = LightStateOff:Light7.state = LightStateOff
        Case 4
            Light27.state = LightStateOn:Light26.state = LightStateOn:Light6.state = LightStateOn:Light7.state = LightStateOn
...
Sub Posts_Hit(idx)
    If Table1.VersionMinor > 3 OR Table1.VersionMajor > 10 Then
        RandomSoundRubber()
    Else
        PlaySound "Rubber_Strong_", 0, VolMulti(ActiveBall,VolRH), Pan(ActiveBall), 0, Pitch(ActiveBall), 0, 1, AudioFade(ActiveBall)
    End If
End Sub
...
Sub RandomSoundRubber()
    Select Case Int(Rnd*3)+1
        Case 1 : PlaySoundAtBallVol "Rubber_Strong_1",VolRH
        Case 2 : PlaySoundAtBallVol "Rubber_Strong_2",VolRH
        Case 3 : PlaySoundAtBallVol "Rubber_Strong_3",VolRH
    End Select
End Sub

Sub RandomSoundFlipper()
    Select Case Int(Rnd*3)+1
        Case 1 : PlaySoundAtBall "Flipper_Rubber_1"
        Case 2 : PlaySoundAtBall "Flipper_Rubber_2"
        Case 3 : PlaySoundAtBall "Flipper_Rubber_3"
    End Select
End Sub
...
Sub AttractMode_Timer()
    ...
    MyLightSeq.Play SeqUpOn, 25, 3
    MyLightSeq.Play SeqDownOn, 25, 3
