' Tiro's / IPD No. 5818 / 1 Player
' Maquinas Recreativas Sociedad Anonima, of Madrid,
...
' Valores Constants
Const TableName = "tiros" ' file name to save highscores and other variables
Const cGameName = "tiros" ' B2S name & DOF config
Const MaxPlayers = 1      ' 1 to 4 can play
Const MaxMultiplier = 1   ' limit bonus multiplier
Const MaxBonus = 0        ' highest bonus count
...
Sub ResetForNewPlayerBall()
    ...
    If Balls = BallsPerGame Then TurnON aLastBall
...
Sub EndOfBall()
    ...
    ' no bonus so go to the next step
    If NOT Tilted Then
...
Sub AddScore2(Points) 'No sound, when scored from the spinner
    If Tilted Then Exit Sub
    Score(CurrentPlayer) = Score(CurrentPlayer) + points
    UpdateScore points
End Sub
...
Dim SpinPos
Dim SpinnerValue(2)
Dim NumberPos ' when the special is lit

Sub Game_Init 'called at the start of a new game
    Dim x
    'Start music?
    'Init variables?
    bYellowBumpers = False
    ...
    UpdateSpinnerLights
    UpdateWheel
    For each x in aArrowNumbers:x.State = 1:Next
    For each x in aTableNumbers:x.State = 1:Next
    TurnOn aBumper1L
    Light006.State = 1
    Light007.State = 1
End Sub
...
Sub Bumper001_Hit 'bottom bumper
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_bumper", 111, DOFPulse, DOFContactors), Bumper001
    If BumperL1.State Then
        AddScore 10
    Else
        Addscore 1
        ToggleLights
        If bSpecialActive Then AdvanceNumbers
    End If
End Sub
...
Sub CheckSpecial
    Dim i, tmp
    tmp = 0
    If NOT bSpecialActive Then
        For each i in aArrowNumbers
            tmp = tmp + i.state
        Next
        if tmp = 0 Then
            bSpecialActive = True
            Light012.State = 1
            'and light a random Light
            NumberPos = RndNbr(9)
            aTableNumbers(NumberPos).State = 1
            aArrowNumbers(NumberPos).State = 1
        End If
    End If
End Sub
...
Sub Trigger006_Hit
    ...
    If Tilted Then Exit Sub
    If bSpecialActive AND Light001.State Then
        If l1.State Then
            AwardSpecial
        End If
    Else
...
' yellow bumper Trigger001

Sub Trigger010_Hit
    PlaySoundAt "fx_sensor", Trigger010
    If Tilted Then Exit Sub
    If NOT bYellowBumpers Then
        bYellowBumpers = True
        TurnON aBumper2L
        TurnON aBumper3L
        ilubuma.State = 0
...
Sub spinner1_Spin
    PlaySoundAt SoundFXDOF("fx_spinner", 107, DOFPulse, DOFContactors), spinner1
    If Tilted Then Exit Sub
    AddScore2 SpinnerValue(SpinPos)
    AdvanceSpinner
    If Light007.State Then AdvanceWheel
End Sub

Sub spinner2_Spin
    PlaySoundAt SoundFXDOF("fx_spinner", 108, DOFPulse, DOFContactors), spinner2
    If Tilted Then Exit Sub
    AddScore2 SpinnerValue(SpinPos)
    AdvanceSpinner
    If Light006.State Then AdvanceWheel
End Sub

Sub AdvanceSpinner
    SpinPos = (SpinPos + 1) MOD 3
    UpdateSpinnerLights
End Sub

Sub UpdateSpinnerLights
    Select Case SpinPos
        Case 0:Light010.State = 1:Light008.State = 0:Light004.State = 0
            Light011.State = 1:Light009.State = 0:Light005.State = 0
        Case 1:Light010.State = 0:Light008.State = 1:Light004.State = 0
            Light011.State = 0:Light009.State = 1:Light005.State = 0
        Case 2:Light010.State = 0:Light008.State = 0:Light004.State = 1
            Light011.State = 0:Light009.State = 0:Light005.State = 1
    End Select
End Sub

' Wheel

Sub AdvanceWheel 'arrow pos
    PlaySoundAt "Advance", Light012
    WheelPos = (WheelPos + 1) MOD 10
    UpdateWheel
    CommitTimer.Enabled = 0
    CommitTimer.Enabled = 1
End Sub

Sub UpdateWheel 'arrow pos
    Dim i
    For each i in aArrows
        i.State = 0
    Next
    aArrows(WheelPos).State = 1
End Sub

Sub CommitTimer_Timer
    Me.Enabled = 0
    If bSpecialActive AND Light012.State Then
        If WheelPos = NumberPos Then awardSpecial
        Else
            aTableNumbers(WheelPos).State = 0
            aArrowNumbers(WheelPos).State = 0
            CheckSpecial
    End If
End Sub

Sub AdvanceNumbers 'after the special is lit
    NumberPos = (NumberPos + 1) MOD 10
    UpdateWheelNumbers
End Sub

Sub UpdateWheelNumbers 'after the special is lit
    Dim i
    For each i in aTableNumbers
        i.State = 0
    Next
    For each i in aArrowNumbers
        i.State = 0
