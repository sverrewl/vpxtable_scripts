'*
'*        Gottlieb's Super Soccer/Soccer (1975)
'*
'*
...
Const cGameName = "SuperSoccer_1975"
...
Const HSFileName="SuperSoccer_75VPX.txt"
Const B2STableName="SuperSoccer_1975"
...
Sub Table1_KeyDown(ByVal keycode)
  ...
  If keycode = LeftFlipperKey and InProgress=true and TableTilted=false Then
    ...
    PlaySoundAtVol SoundFXDOF("FlipperUp",301,DOFOn,DOFFlippers), LeftFlipper, 1
  ...
  If keycode = RightFlipperKey  and InProgress=true and TableTilted=false Then
    ...
    PlaySoundAtVol SoundFXDOF("FlipperUp",302,DOFOn,DOFFlippers), ActiveBall, 1
...
Sub Table1_KeyUp(ByVal keycode)
  ...
  If keycode = LeftFlipperKey and InProgress=true and TableTilted=false Then
    ...
    PlaySoundAtVol SoundFXDOF("FlipperDown",301,DOFOff,DOFFlippers), LeftFlipper, 1
    StopSound "buzzL"
  End If
  ...
  If keycode = RightFlipperKey and InProgress=true and TableTilted=false Then
    RightFlipper.RotateToStart
        StopSound "buzz"
    PlaySoundAtVol SoundFXDOF("FlipperDown",302,DOFOff,DOFFlippers), RightFlipper, 1
  End If
...
Sub RightSlingShot_Slingshot
    PlaySoundAtVol SoundFXDOF("slingshot",305,DOFPulse,DOFContactors), ActiveBall, 1
    RSling0.Visible = 0
    RSling1.Visible = 1
    sling1.TransZ = -20
    RStep = 0
...
Sub LeftSlingShot_Slingshot
    PlaySoundAtVol SoundFXDOF("slingshot",303,DOFPulse,DOFContactors), ActiveBall, 1
    LSling0.Visible = 0
    LSling1.Visible = 1
    sling2.TransZ = -20
    LStep = 0
...
Sub Bumper1_Hit
  If TableTilted=false then
    ...
    PlaySoundAtVol SoundFXDOF("bumper1",307,DOFPulse,DOFContactors), ActiveBall, 1
    bump1 = 1
    AddScore(100)
...
Sub Bumper2_Hit
  If TableTilted=false then
    ...
    PlaySoundAtVol SoundFXDOF("bumper1",308,DOFPulse,DOFContactors), ActiveBall, 1
    bump2 = 1
    AddScore(100)
...
Sub Kicker1_Timer()
  ...
  ' mHole.MagnetOn=0
    ...
    Pkickarm1.rotz=15
    PlaySoundAtVol "saucer", Pkickarm1, 1
    X= INT(RND()*5+1)
...
Sub Kicker2_Timer()
  ...
  ' mHole.MagnetOn=0
    ...
    Pkickarm2.rotz=15
    PlaySoundAtVol "saucer", Pkickarm2, 1
    X= INT(RND()*5+1)
...
Sub AddSpecial()
  PlaySound SoundFXDOF("knocker",309,DOFPulse,DOFKnocker)
  Credits=Credits+1
  If Credits > 0 Then DOF 314, DOFOn
...
'   Controller.B2SSetScorePlayer4 0
    Controller.B2SSetBallInPlay BallInPlay
  End if
  for each obj in LightsOffMotor
    obj.state=0
  next
  PrimHole1.image="gottlieb kicker22_off"
  PrimHole2.image="gottlieb kicker22_off"
...
sub BumpersOn
  ...
  For each obj in LightsOffMotor
    obj.visible=true
  next
  PrimHole1.image="gottlieb kicker22_on"
  PrimHole2.image="gottlieb kicker22_on"
...
Sub SetupReplayTables
  ...
  Replay1Table(1)=48000
  Replay1Table(2)=51000
  Replay1Table(3)=54000
  Replay1Table(4)=57000
  Replay1Table(5)=60000
  Replay1Table(6)=62000
  Replay1Table(7)=66000
  Replay1Table(8)=69000
  Replay1Table(9)=73000
  Replay1Table(10)=77000
  Replay1Table(11)=80000
  Replay1Table(12)=85000
  Replay1Table(13)=66000
  Replay1Table(14)=69000
  Replay1Table(15)=999000

  Replay2Table(1)=62000
  Replay2Table(2)=65000
  Replay2Table(3)=68000
  Replay2Table(4)=71000
  Replay2Table(5)=74000
  Replay2Table(6)=76000
  Replay2Table(7)=80000
  Replay2Table(8)=83000
  Replay2Table(9)=87000
  Replay2Table(10)=91000
  Replay2Table(11)=98000
  Replay2Table(12)=99000
  Replay2Table(13)=80000
  Replay2Table(14)=83000
  Replay2Table(15)=999000

  Replay3Table(1)=70000
  Replay3Table(2)=73000
  Replay3Table(3)=76000
  Replay3Table(4)=79000
  Replay3Table(5)=82000
  Replay3Table(6)=84000
  Replay3Table(7)=88000
  Replay3Table(8)=91000
  Replay3Table(9)=95000
...
'   CurrentFrame=X+100
' end if
  If AddABall=104 then
    BallLoadedFlag=false
  end if
  If AddABall<108 then

    'DisplayBackglassBallAnimation via Lights
    Eval("BallReel"&AddABall).SetValue(0)
    Eval("BallReel"&(AddABall+1)).SetValue(1)

    If B2SOn then
      Controller.B2SSetData AddABall,0
      Controller.B2SSetData AddABall+1,2
    end if
    If (BallCounter<9) and (BallLoadedFlag=false) then
      BallLoadCounter=101
      BallLoadedFlag=true
      BallLoaderTimer.enabled=true
    end if
  end if
  If AddABall>107 then
    'DisplayBackglassBallAnimation via REELS
    Eval("BallReel"&AddABall).SetValue(0)
    Eval("BallReel"&(AddABall+1)).SetValue(1)
    If B2SOn then
      Controller.B2SSetData AddABall,0
      Controller.B2SSetData AddABall+1,1
    end if
  end if

  AddABall=AddABall+1
  If AddABall=AddABallFrames then
...
'   CurrentFrame=X+100
' end if
  if DropABall = 118 then
    'Display 1st frame of drop animation
    BallReel118.SetValue(2)
    Eval("BallReel"&(118-BallCounter)).setvalue(0)

    if B2SOn then
      Controller.B2SSetData 118,1
      Controller.B2SSetData 118-BallCounter,0
    end if
  end if
  if DropABall =119 then
    'display remaining frames
    Eval("BallReel"&(DropABall-1)).SetValue(0)
    Eval("BallReel"&(DropABall)).SetValue(2)
    if B2SOn then
      Controller.B2SSetData DropABall-1,0
      Controller.B2SSetData DropABall,1
    end if
  end if

  if DropABall =120 then
    'display remaining frames
    Eval("BallReel"&(DropABall-1)).SetValue(0)
    Eval("BallReel"&(DropABall)).SetValue(1)
    if B2SOn then
      Controller.B2SSetData DropABall-1,0
      Controller.B2SSetData DropABall,1
    end if
  ...
  If DropABallFrames=DropABall then
    Eval("BallReel"&(DropABall-1)).SetValue(0)
    if B2SOn then
      Controller.B2SSetData DropABall-1,0
    end if
...
sub BallLoaderTimer_timer()
  Select Case BallLoadCounter
    case 101:
      'Display first animation only
      BallReel103.SetValue(1)
      if B2SOn then
        Controller.B2SSetData 101,1
      end if
    case 102:
      'Display 2nd animation
      BallReel103.SetValue(2)
      if B2SOn then
        Controller.B2SSetData 101,0
        Controller.B2SSetData 102,1
      end if
    case 103:
      'display 3nd animation
      BallReel103.SetValue(3)
      if B2SOn then
        Controller.B2SSetData 102,0
        Controller.B2SSetData 103,1
      end if
      BallLoaderTimer.enabled=false
  end select
  BallLoadCounter=BallLoadCounter+1
end sub
...
Sub BonusMotorAdder_Timer
  ...
  if BonusMotorRunning<>1 and InProgress=true and QueuedBonusAdds>0 then
    if BallCounter<10 then
      AddABall=103
      AddABallFrames=117-BallCounter
