...
Const cGameName = "Flicker_1974"
Const cOptions = "Flicker_1974.txt"
Const HSFileName="Flicker (Bally 1974)"
...
Sub bootTable_Timer
  ...
  If bootCount = 1 Then
    ...
    Else
      creditlight.image = "layer4off"
    End If
    If freePlay = 1 Then creditLight.image = "layer4"
    For x = 1 to maxPlayers
    ...
    For each xx in layerbakecol: xx.image = "bakery": Next
    layer4.image="layer4"
    backmetal.image="backmetal"
    flipLmesh.blenddisablelighting=1
    flipRmesh.blenddisablelighting=1
    backGlassOn = 1
    me.enabled = False
  End If
End Sub
...
Sub scoreBonus
  ...
  If bonusScore = 0 And shootAgain.state = 1 And scoreMotorLoop = 0 Then resetTable: releaseBall
...
Sub wireRollOvers_hit(index)
  If tilt = False Then
    Select Case (index)
      Case 0: addScore 500: If extraBall1.state = 1 Then shootAgain.state = 1:If B2SOn Then controller.B2SSetShootAgain 36,1: If B2SOn Then DOF 421, DOFPulse
      ...
      Case 3: addScore 500: If extraBall2.state = 1 Then shootAgain.state = 1:If B2SOn Then controller.B2SSetShootAgain 36,1: If B2SOn Then DOF 422, DOFPulse
    End Select
  End If
  wireNumber = index
...
Sub kickerTimer_timer
    ...
    if kicker.ballcntover > 0 Then
      ...
      kickBall kickerBall1, 135 - INT(RND*6), 13 + INT(RND*2), 5, 30
...
'*************** Spinner
Sub spinner_Spin
  playFieldSound "spinner", 0, Spinner, 1
  If tilt = False Then
    addscore 100
    advanceRing
