...
Sub SpaceWalk_KeyDown(ByVal keycode)
  ...
  If keycode = LeftFlipperKey Then
    ...
    PlaySoundAtVol SoundFXDOF("flipperup",101,DOFOn,DOFFlippers), LeftFlip, VolFlip
    PlaySoundAtVol "flipperup", LeftFlip1, VolFlip
    PlaySoundAtVol "Buzz", LeftFlip, VolFlip
    PlaySoundAtVol "Buzz", LeftFlip1, VolFlip
  End If
  ...
  If keycode = RightFlipperKey Then
    ...
    PlaySoundAtVol SoundFXDOF("flipperup",102, DOFOn, DOFFlippers), RightFlip, VolFlip
    PlaySoundAtVol "flipperup", RightFlip1, VolFlip
    PlaySoundAtVol "Buzz1", RightFlip, VolFlip
...
Sub SpaceWalk_KeyUp(ByVal keycode)
  ...
  If keycode = LeftFlipperKey Then
    ...
    TLLFlip.state=0
    PlaySoundAtVol SoundFXDOF("flipperdown", 101, DOFOff, DOFContactors),LeftFlip, VolFlip
    PlaySoundAtVol "flipperdown",LeftFlip1, VolFlip
    StopSound "Buzz"
  End If
  ...
  If keycode = RightFlipperKey Then
    ...
    TLRFlip.state=0
    PlaySoundAtVol SoundFXDOF("flipperdown", 102, DOFOff, DOFContactors), RightFlip, VolFlip
    PlaySoundAtVol "flipperdown", RightFlip, VolFlip
    StopSound "Buzz1"
  End If
  end if
End Sub
...
Sub DTred1_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",103,DOFPulse,DOFContactors), DTred1, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTred2_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",103,DOFPulse,DOFContactors), DTred2, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTred3_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",103,DOFPulse,DOFContactors), DTred3, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTred4_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",103,DOFPulse,DOFContactors), DTred4, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblue1_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",104,DOFPulse,DOFContactors), DTblue1, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblue2_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",104,DOFPulse,DOFContactors), DTblue2, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblue3_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",104,DOFPulse,DOFContactors), DTblue3, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblue4_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",104,DOFPulse,DOFContactors), DTblue4, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack1_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",105,DOFPulse,DOFContactors), DTblack1, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack2_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",105,DOFPulse,DOFContactors), DTblack2, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack3_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",105,DOFPulse,DOFContactors), DTblack3, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack4_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",105,DOFPulse,DOFContactors), DTblack4, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack5_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",106,DOFPulse,DOFContactors), DTblack5, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack6_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",106,DOFPulse,DOFContactors), DTblack6, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack7_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",106,DOFPulse,DOFContactors), DTblack7, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub DTblack8_dropped
  if tilt=false then
  playsoundAtVol SoundFXDOF("target",106,DOFPulse,DOFContactors), DTblack8, VolTarg
  flashbumper
  addbonus
  if L5K.state=1 then
    addscore 5000
...
Sub resetDT_timer
  playsoundAtVol SoundFXDOF("bankreset",126,DOFPulse,DOFContactors), DTred4, VolTarg
  PlaySoundAtVol "bankreset", DTblue4, VolTarg
  PlaySoundAtVol "bankreset", DTblack4, VolTarg
  PlaySoundAtVol "bankreset", DTblack8, VolTarg
  for j = 0 to 15
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "SpaceWalk" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / SpaceWalk.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "SpaceWalk" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / SpaceWalk.width-1
  If tmp > 0 Then
...
Function AudioFade(ball) ' Can this be together with the above function ?
  Dim tmp
  tmp = ball.y * 2 / SpaceWalk.height-1
  If tmp > 0 Then
    AudioFade = Csng(tmp ^10)
  Else
    AudioFade = Csng(-((- tmp) ^10) )
