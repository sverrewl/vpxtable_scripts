' Aspen / IPD No. 3660 / 1979 / 4 Players
' Brunswick Manufacturing Company, of Chicago, Illinois, USA (1932-1980)
' table by jpsalas(vpx) & updon719(graphics) v 6.0.0
...
' Valores Constants
Const TableName = "Aspenjp" ' file name to save highscores and other variables
Const cGameName = "Aspen"   ' B2S name & DOF
Const MaxPlayers = 4        ' 1 to 4 can play
Const MaxMultiplier = 3     ' limit bonus multiplier
Const FreePlay = True       ' Free play or coins
Const Special1 = 560000     ' win extra game or ball
Const Special2 = 720000
Const Special3 = 999000
...
Sub CreateNewBall()
    ...
    UpdateBallInPlay
    PlaySoundAt SoundFXDOF("fx_Ballrel", 128, DOFPulse, DOFContactors), BallRelease
...
Sub UpdateMultiplierLights 'no lights in this table
...
Dim Trgts(8)    '9 targets to hit to light ShootAgain

Sub Game_Init() 'called at the start of a new game
    'Start music?
    'Init variables?
    For x = 0 to 8:Trgts(x) = 0:Next
...
Sub LeftSlingShot_Slingshot
    ...
    ' add points
    If gi008.State Then
        PlaySound SoundFXDOF("tone500", 143, DOFPulse, DOFChimes)
        AddScore 500
    Else
...
Sub CheckTrgts 'if all 9 targets & rebounds has been hit then turn on the ShootAgain light (extra ball)
    Dim tmp
    tmp = 0
    For x = 0 to 8
        tmp = tmp + Trgts(x)
    Next
    If tmp = 9 Then ShootAgain.State = 1
End Sub
...
Sub CheckBonusMulti
    If Light004.State = 0 AND Light006.State = 0 Then Light002.State = 1:BonusMultiplier = 2
    If Light004.State = 0 AND Light005.State = 0 AND Light006.State = 0 Then Light003.State = 1:BonusMultiplier = 3
End Sub
