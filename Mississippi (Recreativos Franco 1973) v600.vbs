' **************************************
' Mississippi / IPD No. 5955 / 1 Player
' Recreativos Franco, of Madrid, Spain
'    VPX 10.8 table version 6.0.0
' **************************************

Option Explicit
Randomize

' DOF config - leeoneil and Arngrim
'
...
' Bumpers L/R - 105/106/107/108
' Targets L/R - 109/110/111/112/113/114
'
' LED backboard
' Flasher Outside Left - 205/210/211/214/216/217/220/240/260/281/290
' Flasher left - 202/210/211/216/217/240/260/281
' Flasher center - 203/222/240/260
' Flasher right - 204/212/213/218/219/240/260/282
' Flasher Outside Right - 206/212/213/215/218/219/221/240/251/260/282/291
'
' Start Button - 200
' Undercab - 201/280
' Strobe - 230
...
' Valores Constants
Const TableName = "Mississipi" ' se usa para cargar y grabar los highscore y creditos
Const cGameName = "Mississipi" ' para el B2S
Const MaxPlayers = 1           ' de 1 a 4
Const MaxMultiplier = 5        ' limita el bonus multiplicador a 5
Const Special1 = 55000         ' puntuación a obtener para partida extra
Const Special2 = 82000         ' puntuación a obtener para partida extra
...
Sub CreateNewBall()
    ...
    If Epileptikdof = True Then DOF 251, DOFPulse End If
    ...
    Select Case Points
        Case 10, 100, 1000
            ...
            If Points = 100 AND(Score(CurrentPlayer)MOD 1000) \ 100 = 0 Then  'nuevo reel de 1000
                PlaySound SoundFXDOF ("fx_bell1000", 301, DOFPulse, DOFChimes)
            ...
            Else
                PlaySound SoundFXDOF ("fx_bell", 303, DOFPulse, DOFChimes) &Points
            End If
        Case 50
            Add10 = Add10 + 5
...
Dim BumperCAct                                        ' indica si los bumpers centrales están activos
Dim BumperLAct                                        ' indica si los bumpers centrales están activos
...
Sub LeftSlingShot_Slingshot
    ...
    DOF 290, DOFPulse
    If Epileptikdof = True Then DOF 205, DOFPulse End If
    If Epileptikdof = True Then DOF 280, DOFPulse End If
    If Epileptikdof = True Then DOF 281, DOFPulse End If
...
Sub RightSlingShot_Slingshot
    ...
    If Epileptikdof = True Then DOF 280, DOFPulse End If
    If Epileptikdof = True Then DOF 282, DOFPulse End If
...
' Enciande los bumpers alternativamente si están activados
...
Sub Bumper4_Hit
    ...
    If Epileptikdof = True Then DOF 280, DOFPulse End If
    ' añade algunos puntos
    AddScore 100 + 900 * LightBumper4.State
    LightBumper4.State = 0
    LightBumper2.State = 0
    If BumperLAct Then
        LightBumper1.State = 1
...
Sub Trigger2_Hit
    ...
    AddScore 1000
    If li7.State = 1 Then ' la luz del Especial está encendida
        AwardSpecial
    End If
End Sub
...
Sub Trigger3_Hit
    ...
    AddScore 1000
    If li8.State = 1 Then ' la luz del Especial está encendida
        AwardSpecial
    End If
End Sub
...
Sub Check8Lights
    Dim tmp
    tmp = bl6.State + bl7.State + bl8.State + bl9.State + bl10.State + bl11.State + bl12.State + bl13.State
    If tmp = 8 Then 'enciente el Special
        If Lights8Completed = False Then
            Lights8Completed = True
            li8.State = 1
            If Lights5Completed = True Then 'todas las luces ha sido activadas, así que enciende una de las cinco luces del centro
                tmp = INT(RND(1) * 5)
                Select Case tmp
                    Case 0:li1.State = 1
                    Case 1:li2.State = 1
...
Sub AlternateSpecials
    Dim tmp
    tmp = li7.State + li8.State
    If tmp = 1 Then 'solo una luz está encendida
        Li7.State = ABS(li7.State - 1)
        Li8.State = ABS(li8.State - 1)
    End If
End Sub
...
Sub Check5Lights
    Dim tmp
    tmp = bl1.State + bl2.State + bl3.State + bl4.State + bl5.State
    If tmp = 5 Then
        If Lights5Completed = False Then
            Lights5Completed = True
            AwardSpecial
            If Lights8Completed = True Then 'todas las luces ha sido activadas, así que enciende una de las cinco luces del centro
                tmp = INT(RND(1) * 5)
                Select Case tmp
                    Case 0:li1.State = 1
                    Case 1:li2.State = 1
...
Sub Wheeltimer_timer
    wheel.Rotz = WheelStep
End Sub

Sub WheelSpinner_Spin
    WheelStep = (WheelStep + 24)MOD 360
    ' debug.print wheelstep
    Select Case WheelStep
    End Select
End Sub
...
' Las dianas del centro no hacen que la rueda gire, por eso hay la variable IsSpin.
' Esto es para no repetir todas las rutinas de los pasillos sin que gire la rueda.

Sub CentralHit(hittarget)
    Dim tmp
    tmp = ((Wheelstep \ 24)- hittarget + 15)MOD 15
    'debug.print tmp
    Select Case tmp
        Case 0:IsSPin = False:Trigger7_Hit  '9
        Case 1:IsSPin = False:Trigger10_Hit '6
        Case 2
            Target4_hit
            If li3.State Then AwardSpecial 'a
        Case 3:IsSPin = False:Trigger7_Hit '9
        Case 4:IsSPin = False:Trigger4_Hit '2
        Case 5:IsSPin = False:Trigger1_Hit '3
        Case 6
            Target2_hit
            If li5.State Then AwardSpecial '10
        Case 7:IsSPin = False:Trigger5_Hit '5
        Case 8
            Target3_hit
            If li1.State Then AwardSpecial   'k
        Case 9:IsSPin = False:Trigger9_Hit   '7
        Case 10:IsSPin = False:Trigger10_Hit '6
        Case 11:IsSPin = False:Trigger8_Hit  '8
        Case 12
            Target5_hit
            If li4.State Then AwardSpecial  'q
        Case 13:IsSPin = False:Trigger6_Hit '4
        Case 14
            Target1_hit
            If li2.State Then AwardSpecial 'j
    End Select
End Sub
