...
sub SolRFlipper(enabled)
  if enabled Then
    PlaySoundAtVol "FPFlipperUp", RightFlipper, VolFlip
    RightFlipper.RotateToEnd

  Else
    PlaySoundAtVol "FPFlipperDown", RightFlipper, VolFlip
...
sub SolLFlipper(enabled)
  if enabled Then
    PlaySoundAtVol "FPFlipperUp", LeftFlipper, VolFlip
    LeftFlipper.RotateToEnd
  Else
    PlaySoundAtVol "FPFlipperD", LeftFlipper, VolFlip
...
sub SolEjectHole(enabled)
  if enabled then PlaySoundAtVol "FPBallRelease", Kicker1, VolKick
  Kicker1.Kick -90, 15
end Sub
...
Sub Gate2_Hit()
  ...
  PlaysoundAtVol "FPGate", Gate2, VolGates
End Sub
...
Sub Gate1_Hit():PlaysoundAtVol "FPGate", Gate1, VolGates:End Sub
...
Sub Gate3_Hit():PlaysoundAtVol "FPGate", Gate3, VolGates:vpmtimer.PulseSw sRampOutRollUnder:End Sub

Sub Gate6_Hit():PlaysoundAtVol "FPGate", Gate6, VolGates:End Sub

Sub Gate5_Hit():PlaysoundAtVol "FPGate", Gate5, VolGates:End Sub

Sub Gate4_Hit():PlaysoundAtVol "FPGate", Gate4, VolGates:vpmTimer.PulseSw sOrbitInRollUnder:End Sub

Sub Spinner1_Spin():PlaysoundAtVol "FPSpinner", Spinner1, VolSpin:vpmTimer.PulseSw sSpinner:End Sub
...
Sub Bumper1_Hit():vpmTimer.PulseSw sUpperLeftBumper:PlaysoundAtVol "FPBumper", Bumper1, VolBump:End Sub

Sub Bumper2_Hit():vpmTimer.PulseSw sUpperRightBumper:PlaysoundAtVol "FPBumper", Bumper2, VolBump:End Sub

Sub Bumper4_Hit():vpmTimer.PulseSw sLowerLeftBumper:PlaysoundAtVol "FPBumper", Bumper4, VolBump:End Sub

Sub Bumper3_Hit():vpmtimer.pulsesw sLowerRightBumper:PlaysoundAtVol "FPBumper", Bumper3, VolBump:End Sub

Sub Trigger3_Hit()
  PlaysoundAtVol "Wire Ramp", Trigger3, 1
End Sub
...
Sub Trigger4_Hit()
  PlaysoundAtVol "Wire Ramp", Trigger4, 1
End Sub
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "FirePower2" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / FirePower2.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "FirePower2" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / FirePower2.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "FirePower2" is the name of the table
    Dim tmp
    tmp = ball.x * 2 / FirePower2.width-1
    If tmp > 0 Then
        Pan = Csng(tmp ^10)
    Else
        Pan = Csng(-((- tmp) ^10) )
...
Function AudioFade(ball) ' Can this be together with the above function ?
  Dim tmp
  tmp = ball.y * 2 / FirePower2.height-1
  If tmp > 0 Then
    AudioFade = Csng(tmp ^10)
  Else
    AudioFade = Csng(-((- tmp) ^10) )
