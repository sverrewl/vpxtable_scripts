...
' Release notes:
' Version 1.1.0 - Physics adjustments and table tweeks such as sounds, add dampeners, fix nfozzy flippers, etc.
' Version 1.0.3 - Minor changes to some graphical items.
...
Dim AutoPowerSaver, DynamicLampIntensity, DynamicGIIntensity, LetTheBallJump, ShowBallShadow, SubstMagnaSaveButtons, FlipperCoilRampupMode
...
Sub Science_Init
  ...
  Set bsRSaucer=New cvpmBallStack
    bsRSaucer.InitSaucer Kicker1,32,250,30
    bsRSaucer.InitExitSnd "popper","solon"
 ' bsRSaucer.KickAngleVar = 1.5
 ' bsRSaucer.KickAngleVar = 1
    bsRSaucer.KickForceVar = 3
    bsRSaucer.KickAngleVar = 3
...
Sub LeftFlipper_Collide(parm)
  PlaySoundAtBallVol "flip_hit_" & Int(Rnd*3)+1, parm
End Sub
...
Sub Trigger002_Hit              '       switch 4 AntiGrav entrance
  vpmTimer.PulseSw 4
...
'Sub Trigger9_Hit:vpmTimer.PulseSw 3:End Sub '        switch 3 exit AntiGrav bottom
'Sub Trigger9t_Hit:vpmTimer.PulseSw 3:End Sub '       switch 3 exit AntiGrav top

Sub AntiGravGate_Hit:vpmTimer.PulseSw 3:End Sub '       switch 3  AntiGrav gate
...
Sub T17_Hit:vpmTimer.PulseSw 17:StandupTargetHit:End Sub

Sub T18_Hit:vpmTimer.PulseSw 18:StandupTargetHit:End Sub

Sub T19_Hit:vpmTimer.PulseSw 19:StandupTargetHit:End Sub

Sub T20_Hit:vpmTimer.PulseSw 20:StandupTargetHit:End Sub

Sub T21_Hit:vpmTimer.PulseSw 21:StandupTargetHit:End Sub

Sub T22_Hit:vpmTimer.PulseSw 22:StandupTargetHit:End Sub
...
Sub Bumper1_Hit:vpmTimer.PulseSw 26:Bumper1sound:End Sub

Sub Bumper2_Hit:vpmTimer.PulseSw 27:Bumper2sound:End Sub

Sub Bumper3_Hit:vpmTimer.PulseSw 28:Bumper3sound:End Sub


Sub LeftSlingshot_Slingshot:vpmTimer.PulseSw(29):SlingSound:End Sub'    switch 29
Sub RightSlingshot_Slingshot:vpmTimer.PulseSw(30):SlingSound:End Sub'   switch 30

                              ' switch 31 not used

Sub Kicker1_Hit:bsRSaucer.AddBall Me:End Sub        '   switch 32
Sub Kicker1Outer_Hit:bsRSaucer.AddBall Me:End Sub       '   switch 32
...
'Antigravity Hole
Dim MyBall7,MyBallY7,MyBallVel

Sub Trigger010_Hit
  MyBallVel=ABS(BallVel(ActiveBall))
  If MyBallVel>3 Then
    YRampFloor.collidable=True
  Else
    YRampFloor.collidable=False
  End If
End Sub
...
Sub LK1_Hit
  MYBLY=ActiveBall.VelY
  MYBLYA=ABS(ActiveBall.VelY)
  If MYBLY>0 and BallStop2.IsDropped Then
    LK1.Kick 180,MYBLYA
  Else
...
Sub LowBallStop(Enabled)
  If Enabled Then
    ...
    LK4.Kick 180,1
    LK5.Kick 180,1
    PlaySoundAtVol SoundFX("soloff",DOFContactors), BallStop, 2
  End If
End Sub
...
Sub RaiseBallStop(Enabled)
  If Enabled Then
    ...
    PlaySoundAtVol SoundFX("solon",DOFContactors), BallStop, 2
  End If
End Sub
...
Sub SetDynamicLampIntensity
  ...
  If xx.name <> "FBackWallAtom" and xx.name <> "F90" and xx.name <> "F75" and xx.name <> "F59" Then
    xx.intensity = (xx.intensity + 10) * DynamicLampIntensity
  End If
  Next
End Sub
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table.
  Dim tmp
  On Error Resume Next
  tmp = tableobj.y * 2 / Science.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table.
  Dim tmp
  On Error Resume Next
  tmp = tableobj.x * 2 / Science.width-1
  If tmp > 0 Then
...
Sub RubberWalls_Hit(idx)
    PlaySoundAtBallVol "rubber_hit_" & Int(Rnd*3)+1, 15
  RubberRingHit
End Sub
...
Sub RampRubber_Hit()
  PlaySoundAtBallVol "flip_hit_" & Int(Rnd*3)+1, 10
End Sub
...
' *********************************************************************
Sub RollOverSound()
  PlaySoundAtVolPitch SoundFX("rollover",DOFContactors), ActiveBall, 0.02, .25
End Sub
Sub DropTargetSound()
  PlaySoundAtVolPitch SoundFX("droptarget",DOFTargets), ActiveBall, 2, .25
End Sub
Sub StandUpTargetSound()
  PlaySoundAtVolPitch SoundFX("target",DOFTargets), ActiveBall, 2, .25
End Sub
Sub GateSound()
  PlaySoundAtVolPitch SoundFX("fx_gate",DOFContactors), ActiveBall, 0.02, .25
End Sub
Sub SlingSound()
  PlaySoundAtBallVol SoundFX("sling",DOFContactors), 10
End Sub
Sub Bumper1Sound()
  PlaySoundAtBallVol SoundFX("Bumper1",DOFContactors), 5
End Sub
Sub Bumper2Sound()
  PlaySoundAtBallVol SoundFX("Bumper2",DOFContactors), 5
End Sub
Sub Bumper3Sound()
  PlaySoundAtBallVol SoundFX("Bumper3",DOFContactors), 5
End Sub
...
Sub GameTimer_Timer()
    Dim b, maxvel, speedfactor, metalRampArray
    BOT = GetBalls
  maxvel = 999   'set high - wont use maxvel routine
  ...
  ' play the rolling sound for each ball
    For b = 0 to UBound(BOT)
     If BallVel(BOT(b) ) > 1 Then
      ...
      if MRBallArray(b) = True Then
        StopSound("fx_ballrolling" & b)
        PlaySound("fx_metalrolling" & b), -1, Vol(BOT(b)) * 10, AudioPan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) )
      Else
        StopSound("fx_metalrolling" & b)
        if BOT(b).z < 30 Then ' Ball on playfield
          PlaySound("fx_ballrolling" & b), -1, Vol(BOT(b)), AudioPan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0, AudioFade(BOT(b) )
        Else ' Ball on raised ramp
          PlaySound("fx_ballrolling" & b), -1, Vol(BOT(b)) * 3, AudioPan(BOT(b) ), 0, Pitch(BOT(b) )+20000, 1, 0, AudioFade(BOT(b) )
        End If
      End If
    Else
    ...
    '***Ball Drop Sounds***
    If BOT(b).VelZ < 0 and BOT(b).z < 55 and BOT(b).z > 27 Then 'height adjust for ball drop sounds
      PlaySound "ball_drop" & b, 0, ABS(BOT(b).velz)/17, AudioPan(BOT(b)), 0, Pitch(BOT(b)), 1, 0, AudioFade(BOT(b))
    End If

    If BOT(b).X < Science.Width/2 Then
      BallShadow(b).X = ((BOT(b).X) - (Ballsize/6) + ((BOT(b).X - (Science.Width/2))/7)) + 6
    Else
      BallShadow(b).X = ((BOT(b).X) + (Ballsize/6) + ((BOT(b).X - (Science.Width/2))/7)) - 6
    End If
    ...
    If ballspeed(BOT(b)) > maxvel Then
      speedfactor = ABS(maxvel / ballspeed(BOT(b)))
      BOT(b).VelX = BOT(b).VelX * speedfactor
      BOT(b).VelY = BOT(b).VelY * speedfactor
    End If
    Next
  ...
  pAntiGravGate.RotZ = AntiGravGate.currentangle
...
Sub TestKickerIn_Hit
  ...
  TestKickerOut.Kick 330, 40
End Sub
Sub TestKickerLoop_Hit
  TestKickerLoop.DestroyBall
  'TestKickerOut.Kick Angle,velocity
  TestKickerOut.CreateBall
  TestKickerOut.Kick 330, 35
End Sub
Sub TestKickerOut_Hit
  'TestKickerOut.Kick Angle,velocity
  TestKickerOut.Kick 330, 45
End Sub
