...
Sub SW34a_Hit:Controller.Switch(34)=1 : playsoundAtVol"rollover" ,ActiveBall, 1: End Sub
...
Sub SW32a_Hit:Controller.Switch(32)=1 : playsoundAtVol"rollover" ,ActiveBall, 1: End Sub
...
Sub SW53a_Hit:Controller.Switch(53)=1 : playsoundAtVol"rollover" ,ActiveBall, 1: End Sub
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "PinballPool" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / PinballPool.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "PinballPool" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / PinballPool.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "PinballPool" is the name of the table
    Dim tmp
    tmp = ball.x * 2 / PinballPool.width-1
    If tmp > 0 Then
        Pan = Csng(tmp ^10)
    Else
        Pan = Csng(-((- tmp) ^10) )
...
Function AudioFade(ball) ' Can this be together with the above function ?
  Dim tmp
  tmp = ball.y * 2 / PinballPool.height-1
  If tmp > 0 Then
    AudioFade = Csng(tmp ^10)
  Else
    AudioFade = Csng(-((- tmp) ^10) )
