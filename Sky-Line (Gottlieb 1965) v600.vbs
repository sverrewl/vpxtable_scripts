' ****************************************************************
' Sky·Line / IPD No. 3240 / January, 1965 / 1 Player
...
' Valores Constants
Const TableName = "skyline" ' se usa para cargar y grabar los highscore y creditos
Const cGameName = "skyline" ' para el B2S
Const MaxPlayers = 1        ' de 1 a 4
Const MaxMultiplier = 1     ' limita el bonus multiplicador a 5
Const Special1 = 600        ' puntuación a obtener para partida extra
Const Special2 = 800        ' puntuación a obtener para partida extra
Const Special3 = 900        ' puntuación a obtener para partida extra
Const Special4 = 1000       ' puntuación a obtener para partida extra
...
Sub AddScore(Points)
    ...
    UpdateScore points
    Select Case Points
        Case 1:PlaySound("Bell1")
        Case 10, 20:PlaySound("Bell10")
    End Select
...
Sub Bumper006_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_Bumper", bumper006
    ' añade algunos puntos
    AddScore 1 + 9 * BumperL006.State
    Change10PointLights
End Sub
...
Sub Trigger007_Hit '7
    PlaySoundAt "fx_sensor", Trigger007
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl007.State
    'Special?
    If SpecialEnabled AND cl007.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light008.State = 0
...
Sub Trigger008_Hit '10
    PlaySoundAt "fx_sensor", Trigger008
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl010.State
    'Special?
    If SpecialEnabled AND cl010.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light009.State = 0
...
Sub Trigger009_Hit '11
    PlaySoundAt "fx_sensor", Trigger009
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl011.State
    'Special?
    If SpecialEnabled AND cl011.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light010.State = 0
...
Sub Trigger010_Hit '2
    PlaySoundAt "fx_sensor", Trigger010
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl002.State
    'Special?
    If SpecialEnabled AND cl002.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light011.State = 0
...
Sub Trigger012_Hit '1
    PlaySoundAt "fx_sensor", Trigger012
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl001.State
    'Special?
    If SpecialEnabled AND cl001.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light012.State = 0
...
Sub Trigger013_Hit '12
    PlaySoundAt "fx_sensor", Trigger013
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl012.State
    'Special?
    If SpecialEnabled AND cl012.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light013.State = 0
...
Sub Trigger006_Hit '5
    PlaySoundAt "fx_sensor", Trigger006
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl005.State
    'Special?
    If SpecialEnabled AND cl005.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light006.State = 0
...
Sub Trigger005_Hit '6
    PlaySoundAt "fx_sensor", Trigger005
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl006.State
    'Special?
    If SpecialEnabled AND cl006.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light007.State = 0
...
Sub Trigger001_Hit '3
    PlaySoundAt "fx_sensor", Trigger001
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl003.State
    'Special?
    If SpecialEnabled AND cl003.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light001.State = 0
...
Sub Trigger002_Hit '4
    PlaySoundAt "fx_sensor", Trigger002
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl004.State
    'Special?
    If SpecialEnabled AND cl004.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light002.State = 0
...
Sub Trigger003_Hit '8
    PlaySoundAt "fx_sensor", Trigger003
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl008.State
    'Special?
    If SpecialEnabled AND cl008.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light003.State = 0
...
Sub Trigger004_Hit '9
    PlaySoundAt "fx_sensor", Trigger004
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl009.State
    'Special?
    If SpecialEnabled AND cl009.State Then
        AwardSpecial
        ChangeSpecialLight
    End if
    Light004.State = 0
...
' Center bumper triggers

'top & side lanes

Sub ct007_Hit '7
    PlaySoundAt "fx_sensor", ct007
    If Tilted Then Exit Sub
    AddScore 1 + 9 * cl007.State
    'Special?
    If SpecialEnabled AND cl007.State Then
...
Sub ChangeSpecialLight          'turn on 1 of the 12 Lights
    Dim i
    For each i in aLightNumbers 'turn off all
        i.State = 0
    Next
    'turn on one light number (2 Lights)
    i = RndNbr(12) + 1
    Select Case i
        Case 1:Light012.State = 1:cl001.State = 1
        Case 2:Light011.State = 1:cl002.State = 1
        Case 3:Light001.State = 1:cl003.State = 1
        Case 4:Light002.State = 1:cl004.State = 1
        Case 5:Light006.State = 1:cl005.State = 1
        Case 6:Light007.State = 1:cl006.State = 1
        Case 7:Light008.State = 1:cl007.State = 1
        Case 8:Light003.State = 1:cl008.State = 1
        Case 9:Light004.State = 1:cl009.State = 1
        Case 10:Light009.State = 1:cl010.State = 1
        Case 11:Light010.State = 1:cl011.State = 1
        Case 12:Light013.State = 1:cl012.State = 1
    End Select
End Sub
...
'**************************
' Advance Arrow - ascensor
'**************************

Sub ResetAscensor
    ResetAscensorTimer.Enabled = 1
End Sub

Sub Advance
    If AscensorCount <15 Then
        AscensorCount = AscensorCount + 1
        ascensor.SetValue AscensorCount
        If B2SOn then
            Controller.B2SSetData 50 + AscensorCount, 1
        End If
        Select Case AscensorCount
            Case 6, 7, 8, 9, 13, 15:AwardSpecial
        End Select
        AnimateDoor.Enabled = 1
    End If
End Sub

Sub ResetAscensorTimer_Timer
    If AscensorCount> 0 Then
        AscensorCount = AscensorCount - 1
        ascensor.SetValue AscensorCount
        If B2SOn then
            Controller.B2SSetData 50 + AscensorCount, 0
        End If
    End If
    If AscensorCount <= 0 Then
        ResetAscensorTimer.Enabled = 0
        AscensorCount = 0
        ascensor.SetValue 0
        If B2SOn then
...
Sub AnimateDoor_Timer
    DoorCount = DoorCount + 1
    Select Case DoorCount
        Case 1:puerta.SetValue 1
        Case 2:puerta.SetValue 2
        Case 3:puerta.SetValue 3
        Case 4:puerta.SetValue 4
        Case 5:puerta.SetValue 5
        Case 10:puerta.SetValue 4
        Case 11:puerta.SetValue 3
        Case 12:puerta.SetValue 2
        Case 13:puerta.SetValue 1
        Case 14:puerta.SetValue 0
        Case 15:DoorCount = 0:AnimateDoor.Enabled = 0
    End Select
    If B2SOn then
        Select Case DoorCount
            ...
            Case 4:Controller.B2SSetData 4, 0:Controller.B2SSetData 5, 1
            Case 5:Controller.B2SSetData 5, 0:Controller.B2SSetData 6, 1
            Case 10:Controller.B2SSetData 6, 0:Controller.B2SSetData 5, 1
            Case 11:Controller.B2SSetData 5, 0:Controller.B2SSetData 4, 1
            Case 12:Controller.B2SSetData 4, 0:Controller.B2SSetData 3, 1
            Case 13:Controller.B2SSetData 3, 0:Controller.B2SSetData 2, 1
            Case 14:Controller.B2SSetData 2, 0
        End Select
    End If
End Sub
...
Sub Change10PointLights
    If Light014.State Then
        TurnOFF a10PointLights
    Else
        TurnON a10PointLights
    End If
    If SpecialEnabled Then ChangeSpecialLight
End Sub
...
Sub rlband009_Hit:Addscore 1:Change10PointLights:End Sub
Sub rlband010_Hit:Addscore 1:Change10PointLights:End Sub
Sub rlband005_Hit:Addscore 1:Change10PointLights:End Sub
Sub rlband007_Hit:Addscore 1:Change10PointLights:End Sub
Sub rsband001_Hit:Addscore 1:Change10PointLights:End Sub
Sub rsband003_Hit:Addscore 1:Change10PointLights:End Sub
