' Jalisco / IPD No. 4667 / 1976 / 1 Player
' Recreativos Franco, of Madrid, Spain
' VPX8 table by jpsalas with graphics by akiles50000
...
' Valores Constants
Const TableName = "jalisco" ' file name to save highscores and other variables
Const cGameName = "jalisco" ' B2S name
Const MaxPlayers = 1        ' 1 player only for this table
...
Sub EndOfBall()
    ...
    ' no bonus count on this table when draining, just reset the bonus to 0
...
Sub BonusCountTimer_Timer 'when hitting the center hole
    ...
    ElseIf Bonus> 0 Then
        Bonus = Bonus -1
        AddScore 5000 ' no BonusMultiplier in this table
        UpdateBonusLights
    Else
        Bonus = 1
        UpdateBonusLights
        ...
        ' Exit kicker
        vpmTimer.AddTimer 1500, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker003:kicker003.kick 170,20 '"
    End If
End Sub

Sub BonusCountTimer2_Timer 'when draining, no scores
    If Bonus> 0 Then
        Bonus = Bonus -1
        PlaySound "fx_GiOn"
        UpdateBonusLights
...
Sub UpdateBonusLights
    Select Case Bonus
        ...
        Case 1:bl1.State = 1:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 0
        Case 2:bl1.State = 0:bl2.State = 1:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 0
        Case 3:bl1.State = 0:bl2.State = 0:bl3.State = 1:bl4.State = 0:bl5.State = 0:bl6.State = 0
        Case 4:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 1:bl5.State = 0:bl6.State = 0
        Case 5:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 1:bl6.State = 0
...
Sub ToggleLights
    ...
    Light008.State = Light007.state
    Light006.State = tmp
    Light007.State = tmp

    ' special
    tmp = Light025.state
    Light025.State = Light026.State
    Light026.State = tmp
End Sub
...
Sub rlband003_Hit:AddScore 10:ToggleLights:End Sub
...
Sub Trigger001_Hit
    ...
    If Tilted Then Exit Sub
    AddScore 50 + 450 * Light001.State
End Sub

Sub Trigger002_Hit
    PlaySoundAt "fx_sensor", Trigger002
    If Tilted Then Exit Sub
    AddScore 50 + 450 * Light002.State
End Sub

Sub Trigger003_Hit
    PlaySoundAt "fx_sensor", Trigger003
    If Tilted Then Exit Sub
    AddScore 50 + 450 * Light003.State
End Sub

Sub Trigger004_Hit
    PlaySoundAt "fx_sensor", Trigger004
    If Tilted Then Exit Sub
    AddScore 50 + 450 * Light004.State
End Sub
...
Sub CheckSideSpecial
    If SpecialIsLit = False Then
        If Light009.State + Light010.State + Light011.State + Light012.State + Light013.State = 5 Then
            SpecialIsLit = True
            Light026.State = 1
        End If
    End If
...
Sub Kicker002_Hit 'left
    Dim tmp
    'PlaySoundAt "fx_kicker_enter", Kicker002
    PlaySoundAt SoundFXDOF("fx_kicker_enter", 108, DOFPulse, DOFContactors), Kicker002
    If Tilted Then vpmTimer.AddTimer 200, "PLaySoundAt""fx_kicker"", kicker002:kicker002.kick 135,20 '"
    tmp = (Light009.State + Light010.State + Light011.State + Light012.State + Light013.State) * 1000
    'AddScore
    If Light025.State Then
        AwardSpecial
    Else
        AddScore tmp
    End If
    DOF 209, DOFPulse
    DOF 229, DOFPulse
    vpmTimer.AddTimer 1500, "PlaySoundAt SoundFXDOF (""fx_kicker"", 108, DOFPulse, DOFContactors), kicker002:kicker002.kick 135,20 '"
End Sub
...
Sub Kicker001_Hit 'right
    ...
    If Tilted Then vpmTimer.AddTimer 200, "PLaySoundAt""fx_kicker"", kicker001:kicker001.kick 225,20 '"
    tmp = (Light009.State + Light010.State + Light011.State + Light012.State + Light013.State) * 1000
    'AddScore
    If Light026.State Then
        AwardSpecial
    Else
        AddScore tmp
    End If
    DOF 210, DOFPulse
    DOF 230, DOFPulse
    vpmTimer.AddTimer 1500, "PlaySoundAt SoundFXDOF (""fx_kicker"", 109, DOFPulse, DOFContactors), kicker001:kicker001.kick 225,20 '"
End Sub

Sub Kicker003_Hit 'center
    Dim tmp
    'PlaySoundAt "fx_kicker_enter", Kicker003
    PlaySoundAt SoundFXDOF("fx_kicker_enter", 110, DOFPulse, DOFContactors), Kicker003
    If Tilted Then vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker003:kicker003.kick 170,20 '"
    DOF 211, DOFPulse
    DOF 231, DOFPulse
    'AddScore
    BonusCountTimer.Enabled = 1
' the bounus timer kicks out the ball
' vpmTimer.AddTimer 1500, "PLaySoundAt""fx_kicker"", kicker003:kicker003.kick 170,20 '"
End Sub
