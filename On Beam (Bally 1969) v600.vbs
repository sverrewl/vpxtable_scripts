' On Beam / IPD No. 1715 / August, 1969 / 1 Player
...
Const WhiteFlippers = False 'change to true if you want white flippers

If WhiteFlippers Then
    LeftFlipperP.material = "Plastic Flippers"
    RightFlipperP.material = "Plastic Flippers"
    LeftFlipperP.image = "flipper-t2-left"
    RightFlipperP.image = "flipper-t2-right"
End If
...
' Valores Constantes
Const TableName = "onbeam" ' file name to save highscores and other variables
Const cGameName = "onbeam" ' B2S name
Const MaxPlayers = 1         ' 1 to 4 can play
Const FreePlay = False       ' Free play or coins
Const Special1 = 48000       ' extra ball or credit
Const Special2 = 60000
Const Special3 = 66000
Const Special4 = 66000
...
Sub Table1_KeyDown(ByVal Keycode)
    ...
    if keycode = "3" then verification_match
End Sub
...
Sub DisableTable(Enabled)
    If Enabled Then
        ...
        LeftFlipper.RotateToStart
        RightFlipper.RotateToStart
        BumperA.Threshold = 100
        BumperB.Threshold = 100
        BumperC.Threshold = 100
        BumperD.Threshold = 100
...
' When TotalPeriod done, the light will be set to FinalState value where
...
Sub EndOfBall()
    ...
    ' if tilted, the tilt system will take care of the next ball
...
Sub EndOfBall2()
    ...
    If(ExtraBallsAwards(CurrentPlayer)> 0) Then
        ...
        ' kick the ball up again
        Drain.CreateSizedBallWithMass BallSize / 2, BallMass
        BallsOnPlayfield = BallsOnPlayfield + 1
        UpdateExtraBalls
        PlaySoundAt SoundFXDOF("fx_kicker", 104, DOFPulse, DOFContactors), Drain
        Drain.Kick 6, 16
    Else ' no extra ball
...
Dim Stationpos, StationSteps, StationBeam
Dim ShipPos, ShipDir, ShipSteps
Dim Beam, BeamSteps
...
Sub ResetNewBallVariables 'init variables & lights new ball/player
    TurnOffPlayfieldLights
    TurnOFF aBumperAlights
    TurnOFF aBumperBlights
    TurnOFF aBumperClights
    TurnOFF aBumperDlights
    UpdateShip
End Sub
...
Sub BumperD_Hit 'left bumper
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_bumper", 105, DOFPulse, DOFContactors), BumperD
    DOF 207, DOFPulse
    If BumperLD001.State Then
        Addscore 10
        AdvanceShip 1
    Else
        Addscore 1
...
Sub BumperB_Hit 'left bumper
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_bumper", 105, DOFPulse, DOFContactors), BumperB
    DOF 207, DOFPulse
    If BumperLB001.State Then
        Addscore 10
        AdvanceShip 1
    Else
        Addscore 1
...
Sub BumperA_Hit 'right
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_bumper", 107, DOFPulse, DOFContactors), BumperA
    DOF 209, DOFPulse
    If BumperLA001.State Then
        Addscore 10
        AdvanceShip 1
    Else
        Addscore 1
...
Sub BumperC_Hit 'right
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_bumper", 107, DOFPulse, DOFContactors), BumperC
    DOF 209, DOFPulse
    If BumperLC001.State Then
        Addscore 10
        AdvanceShip 1
    Else
        Addscore 1
...
' 1 point rebounds - advance ship

Sub rsband012_Hit:AddScore 1:AdvanceShip 1:End Sub
Sub rsband007_Hit:AddScore 1:AdvanceShip 1:End Sub
Sub rsband008_Hit:AddScore 1:AdvanceShip 1:End Sub
Sub rlband007_Hit:AddScore 1:AdvanceShip 1:End Sub
Sub rlband004_Hit:AddScore 1:AdvanceShip 1:End Sub
Sub rlband005_Hit:AddScore 1:AdvanceShip 1:End Sub

' Top Lanes
Sub Trigger005_Hit 'A
    PlaySoundAt "fx_sensor", Trigger005
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperAlights
    If Light015.State Then MoveBeamtoStation
End Sub

Sub Trigger006_Hit 'B
    PlaySoundAt "fx_sensor", Trigger006
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperBlights
    If Light016.State Then MoveBeamtoStation
End Sub

Sub Trigger007_Hit 'C
    PlaySoundAt "fx_sensor", Trigger007
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperClights
    If Light017.State Then MoveBeamtoStation
End Sub

Sub Trigger008_Hit 'D
    PlaySoundAt "fx_sensor", Trigger008
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperDlights
    If Light018.State Then MoveBeamtoStation
End Sub

' MushRooms
Sub mushroom004_Hit 'A
    PlaySoundAt "fx_mini bumper", mushroom004
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperAlights
    If Light012.State Then MoveBeamtoStation
End Sub

Sub mushroom003_Hit 'B
    PlaySoundAt "fx_mini bumper", mushroom003
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperBlights
    If Light011.State Then MoveBeamtoStation
End Sub

Sub mushroom002_Hit 'C
    PlaySoundAt "fx_mini bumper", mushroom002
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperClights
    If Light013.State Then MoveBeamtoStation
End Sub

Sub mushroom001_Hit 'D
    PlaySoundAt "fx_mini bumper", mushroom001
    If Tilted Then Exit Sub
    Addscore 1000
    TurnON aBumperDlights
    If Light014.State Then MoveBeamtoStation
End Sub


' Outlanes - change space station
...
' Inlanes - space ship to new beam
...
' Center Target - move space ship, scores 1000, special

Sub Target001_Hit
    PlaySoundAtBall SoundFXDOF("fx_target", 111, DOFPulse, DOFTargets)
    If Tilted Then Exit Sub
    Addscore 1000
    If Light010.State Then 'the extra ball is lit
        Light010.State = 0
        AwardExtraBall
        AdvanceStation 1
    End If
...
Sub Kicker001_Hit 'Top Right
    Dim tmp
    PlaySoundAt SoundFXDOF("fx_kicker_enter", 110, DOFPulse, DOFContactors), Kicker001
    If Tilted Then vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker001:kicker001.kick 230,10 '"
    DOF 211, DOFPulse
    DOF 231, DOFPulse
    Addscore 500
    AdvanceBeam 3
    vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker001:kicker001.kick 230,10 '"
End Sub

' Advance SHIP and STATION
...
Sub AdvanceShip(steps)
    ShipSteps = steps
    ShipTimer.Enabled = 1
End Sub

Sub ShipTimer_Timer
    If ShipPos = 9 then ShipDir = -1
    If ShipPos = 0 then ShipDir = 1
    ShipPos = ShipPos + ShipDir
    UpdateShip
    ShipSteps = ShipSteps - 1
    If ShipSteps = 0 Then ShipTimer.Enabled = 0
End Sub

Sub AdvanceStation(steps)
    StationSteps = steps
    StationTimer.Enabled = 1
End Sub

Sub StationTimer_Timer
    Stationpos = (Stationpos + 1) MOD 9
    UpdateShip
    StationSteps = StationSteps - 1
    If StationSteps = 0 Then StationTimer.Enabled = 0
End Sub

Sub AdvanceBeam(steps)
    BeamSteps = steps
    BeamTimer.Enabled = 1
End Sub
...
Sub BeamTimer_Timer
    Beam = (Beam + 1) MOD 4
    UpdateShip
    BeamSteps = BeamSteps - 1
    If BeamSteps = 0 then BeamTimer.Enabled = 0
End Sub

Sub MoveBeamtoStation
    If StationBeam = Beam Then Exit Sub
    If StationBeam> Beam Then BeamSteps = StationBeam - Beam
    If StationBeam <Beam Then BeamSteps = 4 - Beam + StationBeam
    BeamTimer.Enabled = 1
End Sub

Sub UpdateShip 'updates all ship, beam, mushroom and station lights
    PlaySound "fx_relay"
    ...
    If B2SOn then
        ...
        Controller.B2SSetData 121, 0
        Controller.B2SSetData 122, 0
        Controller.B2SSetData 123, 0
        tmp = 120 + Beam
        Controller.B2SSetData tmp, 10 - ShipPos
    End If

    'Station
    TurnOFF aStations
    aStations(Stationpos).State = 1
    If B2SOn then
        Controller.B2SSetData 124, Stationpos + 1
    End If

    'Beam lights on desktop - those are the ship
    Select Case Beam
        Case 0:bea4.State = 1:bea3.State = 0:bea2.State = 0:bea1.State = 0 'D
        Case 1:bea4.State = 0:bea3.State = 1:bea2.State = 0:bea1.State = 0 'C
        Case 2:bea4.State = 0:bea3.State = 0:bea2.State = 1:bea1.State = 0 'B
        Case 3:bea4.State = 0:bea3.State = 0:bea2.State = 0:bea1.State = 1 'A
    End Select
    ' Lights on playfield
    TurnOFF aPFBeams
    Select Case Beam
        Case 0:Light002.State = 1:Light003.State = 0:Light004.State = 0:Light005.State = 0 'D
        Case 1:Light002.State = 0:Light003.State = 1:Light004.State = 0:Light005.State = 0 'C
        Case 2:Light002.State = 0:Light003.State = 0:Light004.State = 1:Light005.State = 0 'B
        Case 3:Light002.State = 0:Light003.State = 0:Light004.State = 0:Light005.State = 1 'A
    End Select
    If B2SOn then
        Controller.B2SSetData 126, 4-Beam
    End If

    ' Musroom Lights
    TurnOFF aMushroomLights
    Select Case Beam
        Case 0:Light015.State = 1:Light012.State = 1 'D
        Case 1:Light016.State = 1:Light011.State = 1 'C
        Case 2:Light017.State = 1:Light013.State = 1 'B
        Case 3:Light018.State = 1:Light014.State = 1 'A
    End Select

    'Station lights on playfield
    If Stat008.State OR stat009.State Then Light009.State = 1:StationBeam = 3                  'A
    If Stat007.State OR stat006.State OR stat005.State Then Light008.State = 1:StationBeam = 2 'B
    If Stat004.State OR stat003.State Then Light007.State = 1:StationBeam = 1                  'C
    If Stat002.State OR stat001.State Then Light006.State = 1:StationBeam = 0                  'D
    CheckExtraBall

    'And play a bell sound
    PlaySound"Bell"
End Sub

Sub CheckExtraBall 'check if ship and station are on the same Beam, to lit the extra ball at the center Target001
    'check the lights
    If s039.State + stat009.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s026.State + stat006.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s028.State + stat007.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s019.State + stat004.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s007.State + stat002.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s003.State + stat001.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s014.State + stat003.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s022.State + stat005.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
    If s034.State + stat008.State = 2 Then Light010.State = 1:AdvanceStation 1: AddScore 3000
End Sub

Sub UpdateExtraBalls
    Select Case ExtraBallsAwards(CurrentPlayer)
        Case 0:star1.State = 0:star2.State = 0:star3.State = 0:star4.State = 0:star5.State = 0
        Case 1:star1.State = 1:star2.State = 0:star3.State = 0:star4.State = 0:star5.State = 0
        Case 2:star1.State = 1:star2.State = 1:star3.State = 0:star4.State = 0:star5.State = 0
        Case 3:star1.State = 1:star2.State = 1:star3.State = 1:star4.State = 0:star5.State = 0
        Case 4:star1.State = 1:star2.State = 1:star3.State = 1:star4.State = 1:star5.State = 0
        Case 5:star1.State = 1:star2.State = 1:star3.State = 1:star4.State = 1:star5.State = 1
    End Select
    If B2SOn then
    Select Case ExtraBallsAwards(CurrentPlayer)
        Case 0:Controller.B2SSetData 130,0:Controller.B2SSetData 131,0:Controller.B2SSetData 132,0:Controller.B2SSetData 133,0:Controller.B2SSetData 134,0
        Case 1:Controller.B2SSetData 130,1:Controller.B2SSetData 131,0:Controller.B2SSetData 132,0:Controller.B2SSetData 133,0:Controller.B2SSetData 134,0
        Case 2:Controller.B2SSetData 130,1:Controller.B2SSetData 131,1:Controller.B2SSetData 132,0:Controller.B2SSetData 133,0:Controller.B2SSetData 134,0
        Case 3:Controller.B2SSetData 130,1:Controller.B2SSetData 131,1:Controller.B2SSetData 132,1:Controller.B2SSetData 133,0:Controller.B2SSetData 134,0
        Case 4:Controller.B2SSetData 130,1:Controller.B2SSetData 131,1:Controller.B2SSetData 132,1:Controller.B2SSetData 133,1:Controller.B2SSetData 134,0
        Case 5:Controller.B2SSetData 130,1:Controller.B2SSetData 131,1:Controller.B2SSetData 132,1:Controller.B2SSetData 133,1:Controller.B2SSetData 134,1
    End Select
    End If
End Sub
