...
Const cGameName = "CrossTown_1966"
Const cOptions = "CrossTown_1966.txt"
Const hsFileName = "CrossTown (Gottlieb 1966)"
...
Sub Table1_init
  ...
  If redCount = "" Then redCount = 0
  If yellowCount = "" Then yellowCount = 0
  replaySettings
  ...
  If LayoutDifficulty = 1 Then
    ...
    rubbersl.visible = 1
    screwsl.visible = 1
'   gionl_flash.visible = 1
    lslingl.visible = 1
    rslingl.visible = 1
    phys_posts_lib.collidable = 1
    phys_posts_cons.collidable = 0
    postsc.visible = 0
    rubbersc.visible = 0
    screwsc.visible = 0
'   gionc_flash.visible = 0
    lsling.visible = 0
    rsling.visible = 0
  ...
  Else
    ...
    rubbersc.visible = 1
    screwsc.visible = 1
'   gionc_flash.visible = 1
    lsling.visible = 1
    rsling.visible = 1
    phys_posts_lib.collidable = 0
    phys_posts_cons.collidable = 1
    postsl.visible = 0
    rubbersl.visible = 0
    screwsl.visible = 0
'   gionl_flash.visible = 0
    lslingl.visible = 0
    rslingl.visible = 0
  End If
...
'************************Start Of Test Keys****************************
  If keycode= 30 Then
    score(1) = 1230
    sReel.setValue(score(1))
  End If

  If keycode = 31 Then CycleDoors.enabled = 1
'
'************************End Of Test Keys****************************
End Sub
  ...
  If bootCount = 1 Then
    ...
    For each Light in GIlights:Light.state = 1: Next
      ...
      Else
        gionc_flash.visible=1
      End if
      gioff_flash.visible=0
      lflip.blenddisablelighting = 0.2
      rflip.blenddisablelighting = 0.2
      lflipr.blenddisablelighting = 0.2
      rflipr.blenddisablelighting = 0.2
      apron_etc.blenddisablelighting = .2
      apron_metal.image="apronrmetalon"
      apron_etc.image="backon"
      apron1.image="apron1on"
      apron2.image="apron2on"
      apron3.image="apron3on"
      apron.image="apronrender"
      leftrail.image="outerlefton"
      outerback.image="backrailon"
      pRubberWheel.image="rubberwheelon"
      LeftTopTarget.image="redtargeton"
      holes.image="holeson"
      plastic1edges.image="plastic1edgeon"
      plastic1.image="plastic1on"
      centerbracket.image="centerbracketon"
      rubberslefttarget.image="rubberlefttargeton"
      postslefttarget.image="postlefttargeton"
      rubbers.image="rubberson"
      posts.image="postson"
      guideswhite.image="guideswhiteon"
      gatebracket.image="gatebracketon"
      topguide.image="topguideon"
      plungelaneguide.image="plungelaneon"
      postsrighttarget.image="postrighttargeton"
      rubbersrighttarget.image="rubberrighttargeton"
      plastic2edges.image="plastic2edgeon"
      plastic2.image="plastic2on"
      railrightouter.image="rightouteron"
      railright.image="rightrailon"
      RightTopTarget.image="yllwtargeton"
      guidesyellow.image="guidesyellowon"
      plastic3edges.image="plastic3edgeon"
      plastic3.image="plastic3on"
      plastic4edges.image="plastic4edgeon"
      plastic4.image="plastic4on"
      plastic5edges.image="plastic5edgeon"
      plastic5.image="plastic5on"
      metalright.image="metalright1on"
      metalleft.image="metalleft1on"
      centertarget.image="shelltargeton"
      leftspecial.image="leftspecialon"
      rightspecial.image="rightspecialon"
      rubbersleftspecial.image="rubberleftspecialon"
      postsrightspecial.image="postrightspecialon"
      rubbersrightspecial.image="rubberrightspecialon"
      rsling.image="rubberson"
      rslingl.image="rubberson"
      lsling.image="rubberson"
      lslingl.image="rubberson"
      deadbumpright.image="deadbr1on"
      deadbumpright1.image="deadbr2on"
      deadbumpright2.image="deadbr3on"
      deadbumpleft.image="deadbl1on"
      deadbumpleft1.image="deadbl2on"
      deadbumpleft2.image="deadbl3on"
      bump2.image="bumpson"
      bump21.image="bumpson1"
      bump22.image="bumpson2"
      bump3.image="bumpson"
      bump31.image="bumpson1"
      bump32.image="bumpson2"
      bump1.image="bumpson"
      bump11.image="bumpson1"
      bump12.image="bumpson2"
      bump4.image="bumpson"
      bump41.image="bumpson1"
      bump42.image="bumpson2"
    ...
    For X = 1 to 4
      EVAL("TopLane" & x).state = 1
    Next

    Alternator

    EVAL("Red" & RedCount).state = 1
    EVAL("Yellow" & YellowCount).state = 1

    CycleDoors.enabled = 1
    backGlassOn = 1
    bootCount = 0
...
Sub startGame
  If state = False Then
    ...
    If redCount = yellowCount Then
      redYellow
    Else
      specialCenter.state = 0
      centerPoint = 10
    End If
    ...
    GI_12.state = 0  '(right special)
    railright.image = "rightrailon"
    holesleftspecial.image = "holesoff"
    rightspecial.image = "rightspecialon"
    rubbersrightspecial.image = "rubberrightspecialon"
    specialflasherright.visible = 0

    GI_11.state = 0  '(left special)
    holesleftspecial.image = "holesoff"
    leftspecial.image = "leftspecialon"
    leftrail.image = "outerlefton"
    rubbersleftspecial.image = "rubberleftspecialon"
    specialflasherleft.visible = 0
    newGame
...
Sub lastBallTimer_Timer
  If balls = 3 Then
    If D1 = 1 and D2 = 1 and D3 = 0 Then
      lastBallOn
    Else
      lastBallOff
    End If
  ...
  If balls = 5 Then
    If D1 = 1 and D2 = 1 and D3 = 1 and D4 = 1 and D5 = 0 Then
      lastBallOn
    Else
      lastBallOff
    End If
...
Sub resetLights
  For x = 1 to 4
    EVAL("TopLane" & x).state = 1
    EVAL("Bumper" & x & "Light1").state = 0
    EVAL("Bump" & x).Image = "BumpsOff"
    EVAL("Bump" & x & "1").Image = "BumpsOff1"
    EVAL("Bump" & x & "2").Image = "BumpsOff2"
    EVAL("BumpFlasher" & x).visible = 0
  Next
End Sub
...
Sub bumper4_timer
  Select Case bump4Step
    Case 3: bump42.RotY = 3
    Case 4: bump42.RotY = -2
    Case 5: bump42.RotY = 1
    Case 6: bump42.RotY = -1
    Case 7: bump42.RotY = 0: Bumper4.timerenabled = 0
  End Select
  Bump4Step=Bump4Step + 1
End Sub

Sub dBumper0_hit
  deadBumpLeft2.RotY=  5
  dlBumpStep = 0
  dBumper0timer.enabled = 1
End Sub

Sub dBumper0timer_timer
  Select Case dlBumpStep
    Case 3: deadBumpLeft2.RotY = 3
    Case 4: deadBumpLeft2.RotY = -2
    Case 5: deadBumpLeft2.RotY = 1
    Case 6: deadBumpLeft2.RotY = -1
    Case 7: deadBumpLeft2.RotY = 0: dBumper0timer.enabled = 0
  End Select
  dlBumpStep = dlBumpStep + 1
End Sub

Sub dBumper1_hit
  deadBumpRight2.RotY = -5
  drBumpStep = 0
  dBumper1timer.enabled = 1
End Sub

Sub dBumper1timer_timer
  Select Case drBumpStep
    Case 3: deadBumpRight2.RotY = -3
    Case 4: deadBumpRight2.RotY = 2
    Case 5: deadBumpRight2.RotY = -1
    Case 6: deadBumpRight2.RotY = 1
    Case 7: deadBumpRight2.RotY = 0: dBumper1timer.enabled = 0
  End Select
  drBumpStep = drBumpStep + 1
End Sub
...
'************** Targets
Sub leftTopTarget_Hit
  If Tilt = False Then
    If B2SOn Then DOF 218, DOFPulse
    If GI_10.state = 0 Then
      addScore 5
    Else
      addScore 50
    End If
...
Sub rightTopTarget_Hit
  If Tilt = False Then
    If B2SOn Then DOF 219, DOFPulse
    If GI_9.state = 0 Then
      addScore 5
    Else
      addScore 50
    End If
...
Sub middleTarget_Hit
  ...
  tStep = 0
  If Tilt = False Then
    addScore centerPoint
    If specialCenter.state = 1 Then Special
  End If
...
Sub centerTimer_timer
  Select Case tStep
    ...
    Case 10: centerTarget.TransY = -1: centerTimer.enabled = 0
  End Select
  tStep = tStep + 1
End Sub
...
Sub topLanes_Hit (Index)
  If Tilt = False Then
    ...
    EVAL("topLane" & Index + 1).state = 0
    addScore 5
    EVAL("bumper" & Index + 1 & "Light1").state = 1
    EVAL("bump" & Index + 1).Image = "bumpsOnLit"
    EVAL("bump" & Index + 1 & "1").Image = "bumpsOnLit1"
    EVAL("bump" & Index + 1 & "2").Image = "bumpsOnLit2"
    EVAL("bumpFlasher" & Index + 1).visible = 1
    If lane(0) = 1 and lane(1) = 1 and lane (2) =1 and lane(3) = 1 Then lanesLit = 1
  End If
End Sub

Sub leftOutLane_hit
  If Tilt = False Then
    If fiftyOutLeft.state = 0 Then
      addScore 10
    Else
      addScore 50
    End If
...
Sub triggers_Hit (Index)
  If Tilt = False Then
    If B2sOn Then DOF 214 + Index, DOFPulse
    Select Case (Index)
      Case 0: addScore 10
      Case 1: addScore 10: If GI_11.state = 1 Then special
      Case 2: addScore 10: If GI_12.state = 1 Then special
      Case 3: addScore 10
    End Select
  End If
End Sub
...
'**************Red and Yellow Units
Dim redCount, yellowCount
Sub redUnit
  redCount = redCount + 1
  If redCount > 4 Then redCount = 0
  If redCount = yellowCount Then
    redYellow
  Else
    specialOn = 0
  ...
  If redCount = 0 Then
    red4.state = 0
  Else
    EVAL("red" & RedCount - 1).state = 0
  End If
End Sub

Sub yellowUnit
  yellowCount = yellowCount + 1
  If yellowCount > 4 Then yellowCount = 0
  If redCount = yellowCount Then
    RedYellow
  Else
    specialOn = 0
  ...
  If yellowCount= 0 Then
    yellow4.state = 0
  Else
    EVAL("yellow" & YellowCount - 1).state = 0
  End If
End Sub

Dim centerPoint, specialOn
Sub redYellow
  Select Case (redCount)
    Case 0: centerPoint = 50: SpecialOn = 0
    Case 1: centerPoint = 100
    Case 2: centerPoint = 200
    Case 3: centerPoint = 300
    Case 4: specialCenter.state = 1: centerPoint = 10: specialOn = 1
  End Select
End Sub
...
Sub eeTimer_Timer
  eeOn = 1
  If ballOut > balls - 1 Then
    stopsound "CrossTownTraffic"
    bg_plate.image = "bg_BackPlate"
    bg_door0.visible = 1
    bg_door1.visible = 1
    DOF 107, 0
    DOF 93, 0
...
Sub alternator
  ...
  If alternate = 0 Then
'   Turn on upper right target lighting
    postsRightTarget.image = "postrighttargetonlit"
    holes2.image = "holeson"
    rubbersRightTarget.image = "rubberrighttargetonlit"
    plastic2Edges.image = "plastic2edgeonlit"
    plastic2.image = "plastic2onlit"
    railRightOuter.image = "rightouteronlit"
    rightTopTarget.image = "yllwtargetonlit"
    targetFlasherRight.visible = 1
    GI_9.state = 1
'   Turn off upper left target lighting
    pRubberWheel.image ="rubberwheelon"
    leftTopTarget.image = "redtargeton"
    holes1.image = "holesoff"
    plastic1Edges.image = "plastic1edgeon"
    plastic1.image = "plastic1on"
    rubbersLeftTarget.image = "rubberlefttargeton"
    postsLeftTarget.image = "postlefttargeton"
    targetFlasherLeft.visible = 0
    GI_10.state = 0
'   Turn on right outlane light
      fiftyOutRight.state = 1
'   Turn on right sling light
      tenSlingRight.state = 1
'   Turn off left outlane light
      fiftyOutLeft.state = 0
'     Turn off left sling light
      tenSlingLeft.state = 0
  Else
'   Turn off right target lighting
    postsRightTarget.image = "postrighttargeton"
    holes2.image = "holesoff"
    rubbersRightTarget.image = "rubberrighttargeton"
    plastic2Edges.image = "plastic2edgeon"
    plastic2.image = "plastic2on"
    railRightOuter.image = "rightouteron"
    rightTopTarget.image = "yllwtargeton"
    targetFlasherRight.visible = 0
    GI_9.state = 0
'   Turn on left target lighting
    pRubberWheel.image = "rubberwheelonlit"
    leftTopTarget.image = "redtargetonlit"
    holes1.image = "holeson"
    plastic1Edges.image = "plastic1edgeonlit"
    plastic1.image = "plastic1onlit"
    rubbersLeftTarget.image = "rubberlefttargetonlit"
    postsLeftTarget.image = "postlefttargetonlit"
    targetFlasherLeft.visible = 1
    GI_10.state = 1
'   Turn on left outlane light
      fiftyOutLeft.state = 1
'   Turn on left sling light
      tenSlingLeft.state = 1
'   Turn off right outlane light
      fiftyOutRight.state = 0
'   Turn off right sling light
  ...
  If lifter = 1 Then
    If ballinPlay < balls Then
      ...
      If ballInPlay = balls and ballOut = 0 Then
        ...
        bg_door0.visible = 0
        bg_door1.visible = 0
        bg_plate.image = "Hendrix"
        playSound "CrossTownTraffic"
        If B2SOn Then
          For x = 80 to 105
            DOF x,0
          Next
...
'***********Door Animation
Dim doorStep, doorPosition, cycleDirection, doorStop, doorOpen
Sub cycleDoors_Timer
  If eeOn = 1 Then Exit Sub
  ...
  If B2SOn and ShowDt = False Then
    ...
    If doorStep < 26 Then
      If doorstep = 1 Then
        ...
        Next
      End If
      doorPosition = doorStep + 80
      DOF doorPosition, 1
      DOF (doorPosition - 1), 0
'     TB.TEXT = doorPosition
    Else
      doorStep = 0
      cycleDoors.enabled = 0
    End If
  Else
    If cycleDirection = 0 Then
      doorStop = doorstop + 1
      BG_Door0.transX = ((90/24) * -DoorStop)
      BG_Door1.transX = ((90/24) * DoorStop)
      If doorStop = 24 Then cycleDirection = 1
    End If
...
'     Number Matching, decrement the match unit for each 1 point score roll over from 10 to 1
      ...
      If bellRing > 1 Then point = 1: flag1 = 1: scoreMotorLoop = 0: scoreMotor.enabled = 1
      Exit Sub
    End If

    If points > 9 and points <100 Then
      bellRing = (points / 10)
      If bellRing > 1 Then point = 10: flag10 = 1: scoreMotorLoop = 0: scoreMotor.enabled = 1
      If bellRing = 1 Then
        totalUp(10)
        If chime = 0 Then
          playSound "Bell10"
    ...
    If points > 99 and points < 1000 Then
      bellRing = (points / 100)
      If bellRing > 1 Then point = 100: flag100 = 1: scoreMotorLoop = 0: scoreMotor.enabled = 1
      If bellRing = 1 Then
        totalUp(100)
        If chime = 0 Then
          playSound "Bell100"
...
Sub totalUp(points)
  ...
  if score(player) > 999 Then reel1000.SetValue(1)

  If B2SOn and score(1) > 999 Then controller.B2SSetData 25, 1
...
Sub onesUnit
  If LayoutDifficulty = 0 Then
    Select Case (Ones)
      Case 0: redSpecial = 2: If specialOn = 0 Then specialCenter.state = 0
      Case 1: redSpecial = 2: If specialOn = 0 Then specialCenter.state = 0
      Case 2: redSpecial = 2: If specialOn = 0 Then specialCenter.state = 0
      Case 3: redSpecial = 0: If balls = 3 Then
                    specialCenter.state = 1
                  Else
                    If specialOn = 0 Then specialCenter.state = 0
                  End If
      Case 4: redSpecial = 1: If specialOn = 0 Then specialCenter.state = 0
      Case 5: redSpecial = 2: If specialOn = 0 Then specialCenter.state = 0
      Case 6: redSpecial = 2: specialCenter.state = 1
      Case 7: redSpecial = 1: If specialOn = 0 Then If specialOn = 0 Then specialCenter.state = 0
      Case 8: redSpecial = 0: If specialOn = 0 Then specialCenter.state = 0
      Case 9: redSpecial = 2: specialCenter.state = 1
    End Select
  Else
    Select Case (ones)
      Case 0: redSpecial = 2: If specialOn = 0 Then specialCenter.state = 0
      Case 1: redSpecial = 1: If specialOn = 0 Then specialCenter.state = 0
      Case 2: redSpecial = 2: If SpecialOn = 0 Then specialCenter.state = 0
      Case 3: redSpecial = 0: If balls = 3 Then
                    specialCenter.state = 1
                  Else
                  ...
                  End If
      Case 4: redSpecial = 1: If specialOn = 0 Then specialCenter.state = 0
      Case 5: redSpecial = 0: If specialOn = 0 Then specialCenter.state = 0
      Case 6: redSpecial = 2: specialCenter.state = 1
      Case 7: redSpecial = 1: If specialOn = 0 Then If SpecialOn = 0 Then specialCenter.state = 0
      Case 8: redSpecial = 0: If specialOn = 0 Then specialCenter.state = 0
      Case 9: redSpecial = 2: specialCenter.state = 1
    End Select
  End If
  If redCount <> yellowCount Then specialCenter.state = 0
...
Sub redSpecials
  If lanesLit = 1 Then
    Select Case (redSpecial)
      Case 0:
        GI_11.state = 1 '(left special)
        holesLeftSpecial.image = "holeson"
        leftSpecial.image = "leftspecialonlit"
        leftRail.image = "outerleftonlit"
        rubbersLeftSpecial.image = "rubberleftspecialonlit"
        specialFlasherLeft.visible = 1
      ...
      Case 1:
        ...
        GI_12.state  = 1 '(right special)
        railRight.image = "rightrailonlit"
        holesRightSpecial.image = "holeson"
        rightSpecial.image = "rightspecialonlit"
        rubbersRightSpecial.image = "rubberrightspecialonlit"
        specialFlasherRight.visible = 1
...
Sub resetReel_Timer
  ...
  If resetLoop = 1 and score(1) = 0 Then
    resetLoop = 0
    If testFlag = 0 Then resetLights
    testFlag = 0
    resetReel.enabled = 0
    Exit Sub
  End If
...
RubbersD.addpoint 0, 0.1, 1.1 '0.96 'point# (keep sequential), ballspeed, CoR (elasticity)
