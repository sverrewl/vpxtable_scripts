'*
'*        Gottlieb's Big Brave/Big Indian/Big Injun (1974)
'*        Original Table build/scripted by Loserman! RIP
'*        Layout/Images/visuals/lighting/shadows by Hauntfreaks (2022 Update)
'*      Code update/nfozzy physics/fleep sounds by Fuffhead35 (2022 Update)
'*        http://www.ipdb.org/machine.cgi?id=1724
'*      VR room and cab added by TastyWasps (2023) v4.0
'*        VR backbox/backglass reel reset code by leojreimroc (2023) v4.0
'*        Various DOF / sound commands uncommented - TastyWasps v4.01
'*
'******** 11-2024 VPX 10.8 update 4.1 by Hauntfreaks and Cliffy  *********
'*      New plastic images (Hauntfreaks)
'*      Lighting adjustments (Hauntfreaks)
'*      Materials adjustments (Hauntfreaks)
'*      Replace arch, apron, shooter gauge primitives (Cliffy) (thanks to BorgDog)
'*      Create new artwork with webbing for apron, shooer gauge, arch (Cliffy)
'*      New wood rails and primalls, sized, shaped and positioned (Cliffy)
'*      Reposition posts, rubbers and plastics and fix height Cliffy)
'*      Fix pop bumpers height and materials (Cliffy)
'*      Add bumper body lighting and add to script (Cliffy)
'*      Add correct rear playfield lock and glass channel (Cliffy)
'*      Replace all insert lighting with halo, fix colors (Cliffy)
'*      Big Brave certificate added (cab users) (Cliffy)
'*      Create new desktop backdrops for B2S or desktop scoring (Cliffy)
'***********************************************************************************
...
Const HSFileName="BigBrave_74VPX.txt"
Const B2STableName="BigBrave_1974"
...
Else
  ...
  VR_cab_head.Visible = 0
  VR_Backbox_Wall.SideVisible = 0
  VR_ApronWall.SideVisible = 0
  VR_LeftWall.SideVisible = 0
  VR_RightWall.SideVisible = 0
  VR_RightWall1.SideVisible = 0
  BGdark.visible = 0
  BGBright.visible = 0
End If
...
Sub Table1_Init()
  ...
  If VRMode = true Then
    BGBrightGameOver.visible = 1
    BGBrightTilt.visible = 0
    FlasherMatch
...
Sub CheckForTargetSpecial
  If (LightB.state=0) AND (LightI.state=0) AND (LightG.state=0) then
    DropTargetSpecial.state=1
  end if
end sub
'******************************************
' Drop Targets
...
Sub ResetDropsTimer2_timer
  ...
  SetMotor(5000)
  if DropTargetSpecial.state=1 then
    AddSpecial
  end if
  ResetDrops
    DT1.visible=True
...
Sub ResetDrops
  ...
  RandomSoundDropTargetReset Target1
  DropTargetCounter=0
...
Sub LeftVT_Hit(idx)
  ...
  If ActiveBall.VelY <0 Then
    ...
    PlaySoundAtBall "fx_solenoidoff"
...
sub nextball
  ...
  If Player>Players Then
    ...
    If BallInPlay>BallsPerGame then
      ...
      If VRMode = True Then
        BGBrightGameOver.visible = 1
        BGBrightP1.visible = 0
        BGBrightP2.visible = 0
        FlasherBalls
        FlasherMatch
      End If
      If B2SOn Then
...
sub BumpersOff
  ...
  BaseLight1.visible=0
  BaseLight3.visible=0
  Bumper2Light.visible=0
  BaseLight2.visible=0
  BaseLight4.visible=0

end sub

sub BumpersOn
  Bumper1Light.visible=1
  BaseLight1.visible=1
  BaseLight3.visible=1
  Bumper2Light.visible=1
  BaseLight2.visible=1
  BaseLight4.visible=1
...
Sub SetupReplayTables
  ...
  Replay1Table(1)=39000
  Replay1Table(2)=42000
  Replay1Table(3)=46000
  Replay1Table(4)=50000
  Replay1Table(5)=52000
  Replay1Table(6)=54000
  Replay1Table(7)=57000
  Replay1Table(8)=60000
  Replay1Table(9)=64000
  Replay1Table(10)=69000
  Replay1Table(11)=75000
  ...
  Replay2Table(2)=56000
  Replay2Table(3)=60000
  Replay2Table(4)=64000
  Replay2Table(5)=66000
  Replay2Table(6)=68000
  Replay2Table(7)=71000
  Replay2Table(8)=74000
  Replay2Table(9)=78000
  Replay2Table(10)=83000
  Replay2Table(11)=89000
  Replay2Table(12)=86000
  Replay2Table(13)=91000
  Replay2Table(14)=95000
  Replay2Table(15)=95000

  Replay3Table(1)=61000
  Replay3Table(2)=64000
  Replay3Table(3)=68000
  Replay3Table(4)=72000
  Replay3Table(5)=74000
  Replay3Table(6)=76000
  Replay3Table(7)=79000
  Replay3Table(8)=82000
  Replay3Table(9)=86000
  Replay3Table(10)=91000
  Replay3Table(11)=97000
...
Sub RollingUpdate()
  ...
  For b = 0 to UBound(BOT)
    ...
    If AmbientBallShadowOn = 0 Then
      If BOT(b).Z > 30 Then
        BallShadowA(b).height=BOT(b).z - 50/4   'This is technically 1/4 of the ball "above" the ramp, but it keeps it from clipping
      Else
        BallShadowA(b).height=BOT(b).z - 50/2 + 5
      End If
      BallShadowA(b).Y = BOT(b).Y + 50/5 + fovY
...
Sub TimerPlunger_Timer
  If VR_Plunger.Y < 2230 then
      VR_Plunger.Y = VR_Plunger.Y + 5
  End If
End Sub

Sub TimerPlunger2_Timer
  VR_Plunger.Y = 2130 + (5* Plunger.Position) -20
End Sub
...
Sub SetBackglass()
  ...
  For Each BGObj In VRBackglass
    ...
    BGObj.y =120 'adjusts the distance from the backglass towards the user
  Next
...
Sub FlasherPlayers
  If Players = 1 Then BGBrightP1.visible = 1 Else BGBrightP1.visible = 0 End If
  If Players = 2 Then BGBrightP2.visible = 1 Else BGBrightP2.visible = 0 End If
End Sub
