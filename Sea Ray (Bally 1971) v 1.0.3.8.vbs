...
Sub Table1_KeyDown(ByVal keycode)
  ...
  If keycode = AddCreditKey Then
    ...
    If EnableInitialEntry = False and OperatorMenu = 0 Then
      ...
      If FreePlay = 0 and Credit > 0 and Players < 2 and FirstBallOut = False Then
        Credit = Credit - 1
        CreditTxt.text = Credit
        If B2SOn Then
          Controller.B2SSetCredits Credit
  ...
  If Tilt = False and State = True Then
    ...
    If keycode = RightFlipperKey Then
      ...
      PlaySoundAtVol SoundFXDOF("flipperup",102,DOFOn,DOFContactors), LeftFlipper,VolFlip
...
Sub Drain_Hit()
  BIP = BIP - 1
  BIPtxt.text = BIP
  PlaySoundAtVol SoundFXDOF("fx_drain",124,DOFPulse,DOFContactors),drain, 1
  GateState = False
...
Sub ReleaseBall_Timer
  If BIP = 0 Then
    ...
    PlaysoundAtVol "HoleKick", drain, 1
    BIP = BIP + 1
    BIPtxt.text = BIP
    Launched = 0
...
Sub Bumpers_Hit(Index)
  If Tilt = False Then
    Select Case (Index)
      Case 0: Playsound SoundFXDOF("fx_Bumper1",105,DOFPulse,DOFContactors) ' TODO
...
Sub LeftSlingShot_Slingshot
  PlaysoundAtVol SoundFXDOF("left_slingshot",103,DOFPulse,DOFContactors), sling1, 1
    LSling.Visible = 0
    LSling1.Visible = 1
    Sling1.Transx = -20
    LStep = 0
...
Sub RightSlingShot_Slingshot
  PlaysoundAtVol SoundFXDOF("right_slingshot",104,DOFPulse,DOFContactors), sling2, 1
    RSling.Visible = 0
    RSling1.Visible = 1
    Sling2.Transx = -20
    RStep = 0
...
Sub BallLaunchedTrigger_Hit
  ...
  End If
  If B2SOn Then
    If Credit > 0 Then DOF 127, DOFOff
  End If
End Sub
...
Sub RightBonus_timer
  ...
  Select Case BonusCount
    ...
    PlaySoundAtVol SoundFXDOF("holekick",112,DOFPulse,DOFContactors), RightBonusPlunger, VolKick
...
Sub Kicker1Timer_Timer
  ...
  PlaysoundAtVol "HoleKick", kicker1, VolKick
  me.enabled = 0
End Sub
...
Sub a_Rubbers_hit (idx)
  ...
  If finalspeed > 20 then
    PlaySound "fx_rubber2", 0, Vol(ActiveBall)*VolRH, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
  End if
...
Sub a_Posts_Hit(idx)
  ...
  If finalspeed > 16 then
    PlaySound "fx_rubber2", 0, Vol(ActiveBall)*VolPo, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
  End if
...
Sub RandomSoundRubber()
  Select Case Int(Rnd*3)+1
    Case 1 : PlaySound "rubber_hit_1", 0, Vol(ActiveBall)*VolRH, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
    Case 2 : PlaySound "rubber_hit_2", 0, Vol(ActiveBall)*VolRH, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
    Case 3 : PlaySound "rubber_hit_3", 0, Vol(ActiveBall)*VolRH, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
  End Select
End Sub
...
Sub RandomSoundFlipper()
  Select Case Int(Rnd*3)+1
    Case 1 : PlaySound "flip_hit_1", 0, Vol(ActiveBall)*VolRH, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
    Case 2 : PlaySound "flip_hit_2", 0, Vol(ActiveBall)*VolRH, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
    Case 3 : PlaySound "flip_hit_3", 0, Vol(ActiveBall)*VolRH, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, Fade(ActiveBall)
  End Select
End Sub
...
'************Save Scores
Sub SaveHS
    savevalue "SeaRay", "HighScore(0)", HighScore(0)
    savevalue "SeaRay", "HighScore(1)", HighScore(1)
    savevalue "SeaRay", "HighScore(2)", HighScore(2)
    savevalue "SeaRay", "HighScore(3)", HighScore(3)
    savevalue "SeaRay", "HighScore(4)", HighScore(4)
    savevalue "SeaRay", "Initial(0,1)", Initial(0,1)
    savevalue "SeaRay", "Initial(0,2)", Initial(0,2)
    savevalue "SeaRay", "Initial(0,3)", Initial(0,3)
    savevalue "SeaRay", "Initial(1,1)", Initial(1,1)
    savevalue "SeaRay", "Initial(1,2)", Initial(1,2)
    savevalue "SeaRay", "Initial(1,3)", Initial(1,3)
    savevalue "SeaRay", "Initial(2,1)", Initial(2,1)
    savevalue "SeaRay", "Initial(2,2)", Initial(2,2)
    savevalue "SeaRay", "Initial(2,3)", Initial(2,3)
    savevalue "SeaRay", "Initial(3,1)", Initial(3,1)
    savevalue "SeaRay", "Initial(3,2)", Initial(3,2)
    savevalue "SeaRay", "Initial(3,3)", Initial(3,3)
    savevalue "SeaRay", "Initial(4,1)", Initial(4,1)
    savevalue "SeaRay", "Initial(4,2)", Initial(4,2)
    savevalue "SeaRay", "Initial(4,3)", Initial(4,3)
    savevalue "SeaRay", "Credit", Credit
    savevalue "SeaRay", "FreePlay", FreePlay
  savevalue "SeaRay", "Balls", Balls
  savevalue "SeaRay", "ReplayEB", ReplayEB
  savevalue "SeaRay", "ShowBallShadow", ShowBallShadow
  savevalue "SeaRay", "Chime", Chime
  savevalue "SeaRay", "MatchNumber", MatchNumber
End Sub

'*************Load Scores
Sub LoadHighScore
    dim temp
    temp = LoadValue("SeaRay", "HighScore(0)")
    If (temp <> "") then HighScore(0) = CDbl(temp)
    temp = LoadValue("SeaRay", "HighScore(1)")
    If (temp <> "") then HighScore(1) = CDbl(temp)
    temp = LoadValue("SeaRay", "HighScore(2)")
    If (temp <> "") then HighScore(2) = CDbl(temp)
    temp = LoadValue("SeaRay", "HighScore(3)")
    If (temp <> "") then HighScore(3) = CDbl(temp)
    temp = LoadValue("SeaRay", "HighScore(4)")
    If (temp <> "") then HighScore(4) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(0,1)")
    If (temp <> "") then Initial(0,1) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(0,2)")
    If (temp <> "") then Initial(0,2) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(0,3)")
    If (temp <> "") then Initial(0,3) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(1,1)")
    If (temp <> "") then Initial(1,1) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(1,2)")
    If (temp <> "") then Initial(1,2) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(1,3)")
    If (temp <> "") then Initial(1,3) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(2,1)")
    If (temp <> "") then Initial(2,1) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(2,2)")
    If (temp <> "") then Initial(2,2) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(2,3)")
    If (temp <> "") then Initial(2,3) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(3,1)")
    If (temp <> "") then Initial(3,1) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(3,2)")
    If (temp <> "") then Initial(3,2) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(3,3)")
    If (temp <> "") then Initial(3,3) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(4,1)")
    If (temp <> "") then Initial(4,1) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(4,2)")
    If (temp <> "") then Initial(4,2) = CDbl(temp)
    temp = LoadValue("SeaRay", "Initial(4,3)")
    If (temp <> "") then Initial(4,3) = CDbl(temp)
  temp = LoadValue("SeaRay", "credit")
    If (temp <> "") then Credit = CDbl(temp)
  temp = LoadValue("SeaRay", "FreePlay")
    If (temp <> "") then FreePlay = CDbl(temp)
    temp = LoadValue("SeaRay", "balls")
    If (temp <> "") then Balls = CDbl(temp)
    temp = LoadValue("SeaRay", "ReplayEB")
    If (temp <> "") then ReplayEB = CDbl(temp)
    temp = LoadValue("SeaRay", "ShowBallShadow")
    If (temp <> "") then ShowBallShadow = CDbl(temp)
    temp = LoadValue("SeaRay", "Chime")
    If (temp <> "") then Chime = CDbl(temp)
    temp = LoadValue("SeaRay", "MatchNumber")
