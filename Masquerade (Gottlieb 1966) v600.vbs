' ****************************************************************
' Masquerade / IPD No. 1553 / January, 1966 / 4 Players
...
' Valores Constants
Const TableName = "masquerade" ' se usa para cargar y grabar los highscore y creditos
Const cGameName = "masquerade" ' para el B2S
Const MaxPlayers = 4           ' de 1 a 4
Const MaxMultiplier = 1        ' limita el bonus multiplicador a 5
Const Special1 = 1200          ' puntuación a obtener para partida extra
Const Special2 = 1500          ' puntuación a obtener para partida extra
Const Special3 = 1800          ' puntuación a obtener para partida extra
...
Sub Table1_KeyDown(ByVal Keycode)
    ...
    If keycode = "3" then SpinTargetValues
End Sub
...
Sub Blink(Col) 'turn off all the lights in a collection
    Dim i
    For each i in Col
        i.State = 2
    Next
...
'***********************************
'   JP's VP10 Rolling Sounds Rev 3
'***********************************
...
Sub AwardExtraBall()
    If NOT bExtraBallWonThisBall Then
        ...
        DOF 230, DOFPulse
        ExtraBallsAwards(CurrentPlayer) = ExtraBallsAwards(CurrentPlayer) + 1
        bExtraBallWonThisBall = True 'only one ball per ball
...
Dim SpinValues, Spin1, Spin2, Spin3, Spin4, Spin5, NbrSpins
Dim MASKED
Dim BEAUTY
Dim PowerValue
Dim BumperPos
...
'these values are never reset on a new game, only when poweron the machine
FaceSteps=Array(1,2,3,4,5,6,6,6,6,5,4,3,2,1,0) '14 steps
SpinValues = Array(2, 3, 2, 4, 3, 2, 4, 3, 2, 2, 1, 2, 4, 3, 5) '15 values
Spin1 = 0
Spin2 = 1
Spin3 = 2
Spin4 = 3
...
Sub rlband009_Hit: Addscore 1: End Sub
...
Sub Bumper001_Hit
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_bumper", 110, DOFPulse, DOFContactors), Bumper001
    ' añade algunos puntos
    AddScore 5
    SpinTargetValues
End Sub
...
Sub Bumper003_Hit
    If Tilted Then Exit Sub
   PlaySoundAt SoundFXDOF("fx_bumper", 112, DOFPulse, DOFContactors), Bumper003
    ' añade algunos puntos
...
Sub Bumper004_Hit
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF("fx_bumper", 113, DOFPulse, DOFContactors), Bumper004
    ' añade algunos puntos
    AddScore 1 + 9 * BumperL004.State
End Sub

Sub RotateBumpers
    PlaySound "scorewheel"
    BumperPos = (BumperPos + 1) MOD 3
    UpdateBumpers
End Sub

Sub UpdateBumpers
    Select Case BumperPos
        Case 0:BumperL002.State = 0:BumperL003.State = 0:BumperL004.State = 0
            BumperL002a.State = 0:BumperL003a.State = 0:BumperL004a.State = 0
        Case 1:BumperL002.State = 0:BumperL003.State = 1:BumperL004.State = 0
            BumperL002a.State = 0:BumperL003a.State = 1:BumperL004a.State = 0
        Case 2:BumperL002.State = 1:BumperL003.State = 0:BumperL004.State = 1
            BumperL002a.State = 1:BumperL003a.State = 0:BumperL004a.State = 1
    End Select
End Sub
...
Sub CheckMASKED
    MASKED = mm.State + ma.State + ms.State + mk.State + mel.State + md.State
    UpdatePowerValue
End Sub

Sub CheckBEAUTY
    BEAUTY = bb.State + be.State + ba.State + bu.State + bt.State + by.State
    UpdatePowerValue
End Sub

Sub UpdatePowerValue
    If MASKED + BEAUTY = 0 Then 'all lights are off the turn 100 light
        PowerValue = 100
        Light008.State = 1:Light014.State = 0:Light015.State = 0
    ElseIf(MASKED = 0) OR(BEAUTY = 0) then 'turn on 10 points
        PowerValue = 10
        Light008.State = 0:Light014.State = 0:Light015.State = 1
    Else
        PowerValue = 1
        Light008.State = 0:Light014.State = 1:Light015.State = 0
    End If
End Sub
...
Sub tgt1_Hit
    PlaySoundAtBall "fx_target"
    If Tilted Then Exit Sub
    Addscore PowerValue * SpinValues(Spin1)
    RotateBumpers
    tgt1.roty = -6
    vpmTimer.AddTimer 100, "tgt1.roty = 0 '"
    If PowerValue = 100 then TurnON aMABElights:MASKED = 7:BEAUTY = 7:UpdatePowerValue
    If SpinValues(Spin1) = 1 AND LightShootAgain.State = 0 Then AwardExtraBall:Light016.State = 0
    AnimateFace
End Sub
...
Sub tgt2_Hit
    PlaySoundAtBall "fx_target"
    If Tilted Then Exit Sub
    Addscore PowerValue * SpinValues(Spin2)
    RotateBumpers
    tgt2.roty = -6
    vpmTimer.AddTimer 100, "tgt2.roty = 0 '"
    If PowerValue = 100 then TurnON aMABElights:MASKED = 7:BEAUTY = 7:UpdatePowerValue
    If SpinValues(Spin2) = 1 AND LightShootAgain.State = 0 Then AwardExtraBall:Light016.State = 0
    AnimateFace
End Sub
...
Sub tgt3_Hit
    PlaySoundAtBall "fx_target"
    If Tilted Then Exit Sub
    Addscore PowerValue * SpinValues(Spin3)
    RotateBumpers
    tgt3.roty = -6
    vpmTimer.AddTimer 100, "tgt3.roty = 0 '"
    If PowerValue = 100 then TurnON aMABElights:MASKED = 7:BEAUTY = 7:UpdatePowerValue
    If SpinValues(Spin3) = 1 AND LightShootAgain.State = 0 Then AwardExtraBall:Light016.State = 0
    AnimateFace
End Sub
...
Sub tgt4_Hit
    PlaySoundAtBall "fx_target"
    If Tilted Then Exit Sub
    Addscore PowerValue * SpinValues(Spin4)
    RotateBumpers
    tgt4.roty = -6
    vpmTimer.AddTimer 100, "tgt4.roty = 0 '"
    If PowerValue = 100 then TurnON aMABElights:MASKED = 7:BEAUTY = 7:UpdatePowerValue
    If SpinValues(Spin4) = 1 AND LightShootAgain.State = 0 Then AwardExtraBall:Light016.State = 0
    AnimateFace
End Sub
...
Sub tgt5_Hit
    PlaySoundAtBall "fx_target"
    If Tilted Then Exit Sub
    Addscore PowerValue * SpinValues(Spin5)
    RotateBumpers
    tgt5.roty = -6
    vpmTimer.AddTimer 100, "tgt5.roty = 0 '"
    If PowerValue = 100 then TurnON aMABElights:MASKED = 7:BEAUTY = 7:UpdatePowerValue
    If SpinValues(Spin5) = 1 AND LightShootAgain.State = 0 Then AwardExtraBall:Light016.State = 0
    AnimateFace
End Sub

Sub SpinTargetValues
    Blink aValueLights
    NbrSpins = 5 + RndNbr(5)
    SpinValueTimer.Enabled = 1
End Sub

Sub UpdateTargetValues
    v1.ImageA = CStr(SpinValues(Spin1) )
    v2.ImageA = CStr(SpinValues(Spin2) )
    v3.ImageA = CStr(SpinValues(Spin3) )
    v4.ImageA = CStr(SpinValues(Spin4) )
    v5.ImageA = CStr(SpinValues(Spin5) )
End Sub

Sub SpinValueTimer_Timer
    If NbrSpins> 0 Then
        NbrSpins = NbrSpins - 1
        Spin1 = (Spin1 + 1) MOD 15
        Spin2 = (Spin2 + 1) MOD 15
        Spin3 = (Spin3 + 1) MOD 15
        Spin4 = (Spin4 + 1) MOD 15
        Spin5 = (Spin5 + 1) MOD 15
        UpdateTargetValues
    Else
        SpinValueTimer.Enabled = 0
        TurnON aValueLights
        If SpinValues(Spin1) = 1 OR SpinValues(Spin2) = 1 OR SpinValues(Spin3) = 1 OR SpinValues(Spin4) = 1 OR SpinValues(Spin5) = 1 Then 'a star
            If LightShootAgain.State = 0 Then Light016.State = 1
            Else
                Light016.State = 0
        End If
    End If
...
Sub FaceTimer_Timer
Dim i
face1.SetValue FaceSteps(FacePos)
face2.SetValue FaceSteps(FacePos)
    If B2SOn then
        For i = 66 to 72: Controller.B2SSetData i, 0: Next
        Controller.B2SSetData 66+FaceSteps(FacePos), 1
    End If
FacePos = FacePos + 1
If FacePos = 15 Then FaceTimer.Enabled = 0
End Sub
