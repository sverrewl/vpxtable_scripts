...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "BigGameRO" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = tableobj.y * 2 / BigGameRO.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "BigGameRO" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = tableobj.x * 2 / BigGameRO.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "BigGameRO" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = ball.x * 2 / BigGameRO.width-1
  If tmp > 0 Then
    Pan = Csng(tmp ^10)
  Else
    Pan = Csng(-((- tmp) ^10) )
