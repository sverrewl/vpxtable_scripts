...
Sub Rubbers_Hit(idx):PlaySound "rubber11", 0, Vol(ActiveBall)*10*GlobalSoundLevel , pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0:End Sub
Sub Posts_Hit(idx):PlaySound "post5", 0, Vol(ActiveBall)*10*GlobalSoundLevel , pan(ActiveBall), 0, Pitch(ActiveBall), 0, 0:End Sub
...
Sub LeftFlipper_Collide(parm) : PlaySound "flip_hit_1", 0, GlobalSoundLevel * parm / 50, -0.1, 0.25 : End Sub
Sub RightFlipper_Collide(parm) : PlaySound "flip_hit_1", 0, GlobalSoundLevel * parm / 50, 0.1, 0.25 : End Sub
...
Sub RollingUpdate()
    ...
    For b = 0 to UBound(BOT)
        If BallVel(BOT(b) ) > 1 and (BOT(b).z < 60 or CupExit.Enabled) Then
			rolling(b) = True : PlaySound("fx_ballrolling" & b), -1, GlobalSoundLevel * Vol(BOT(b) ), Pan(BOT(b) ), 0, Pitch(BOT(b) ), 1, 0
		Else
			If rolling(b) = True Then StopSound("fx_ballrolling" & b) : rolling(b) = False : End If
		End If
	Next
End Sub
