...
' Thanks to randr for the castle pics
...
If keycode = 33 then vers=0.005 :quota=0.2 ' drago vola
If keycode = 34 and increm> 0 then vers=-0.002 :quota=-0.4  ' drago ritorna
End Sub
...
IronGateDir = 1
PlaySound SoundFX("fx_gateUp",DOFContactors)
TwrShake
End If
End Sub
...
Sub SolCastlegateHold(Enabled)
	...
	If NOT Enabled AND IronGateDir = 1 Then
		IronGateDir = -1
		CastleGateTimer.Enabled = 1
		PlaySound SoundFX("fx_gateDown",DOFContactors)
		DOF 102, DOFPulse
    End If
End Sub
...
Sub sw38_Hit:PlaySound "fx_kicker_enter":bsCat.AddBall Me: sw38.createSizedballWithMass Ballsize/2,BallMass : End Sub
...
Sub SolTowerDivHold(Enabled)
	If NOT Enabled AND DiverterDir = 1 Then
		...
		DiverterDir = -1
		Diverter.TimerEnabled = 1
		PlaySound SoundFX("fx_DiverterDown",DOFContactors)
    End If
End Sub
...
Select Case dbpos
	Case 1:			'bridge is going down
		...
		If drawbridgep.RotX <= -90 Then
			...
			Playsound SoundFX("Bridge_Stop", 0), 0, 0.1, 0, 0, 0, 1, 0
		End If
...
Sub SolTowerLock(Enabled)
	StopSound "fx_Postup":PlaySound SoundFX("fx_Postup",DOFContactors)
...
Sub Bumper1_Hit:vpmTimer.PulseSw 53:PlaySound SoundFX("LeftBumper_Hit",DOFContactors):Me.TimerEnabled = 1:me.timerinterval = 10:End Sub

Sub Bumper2_Hit:vpmTimer.PulseSw 54:PlaySound SoundFX("RightBumper_Hit",DOFContactors):Me.TimerEnabled = 1:me.timerinterval = 10:End Sub

Sub Bumper3_Hit:vpmTimer.PulseSw 55:PlaySound SoundFX("TopBumper_Hit",DOFContactors):Me.TimerEnabled = 1:me.timerinterval = 10:End Sub
...
Sub SolLeftTrollPow(Enabled)
    If Enabled then
		PlaySound SoundFX("fx_TrollUp",DOFContactors)
        ltroll.Z = -8
        liftL.Z = ltroll.Z
...
Sub SolLeftTrollHold(Enabled)
	If NOT Enabled AND ltroll.Z = -8 Then
		...
		PlaySound SoundFX("fx_TrollDown",DOFContactors)
		LTP.Enabled=0
    End If
End Sub
...
Sub sw45_Hit():PlaySound "fx_woodhit":vpmTimer.PulseSw 45:Me.TimerEnabled = 1:sbou=ActiveBall.vely/4 :End Sub
Sub sw46_Hit():PlaySound "fx_woodhit":vpmTimer.PulseSw 46:Me.TimerEnabled = 1:sbou2=ActiveBall.vely/4 :End Sub
...
Sub sw36_Hit:PlaySound "fx_Moat_enter":bsMo.AddBall Me:End Sub

' Lanes
Sub sw66_Hit:Controller.Switch(66) = 1:PlaySound "fx_sensor":End Sub
Sub sw66_UnHit:Controller.Switch(66) = 0:End Sub

Sub sw67_Hit:Controller.Switch(67) = 1:PlaySound "fx_sensor":End Sub
Sub sw67_UnHit:Controller.Switch(67) = 0:End Sub

Sub sw18_Hit:Controller.Switch(18) = 1:PlaySound "fx_sensor":End Sub
Sub sw18_UnHit:Controller.Switch(18) = 0: If bsTrough.Balls < 3 Then dragonup= True:vers=0.005 :quota=0.2:End If:End Sub		'drago vola
...
Sub sw65_Hit:Controller.Switch(65) = 1:PlaySound "fx_sensor":End Sub
...
Sub sw68_Hit:Controller.Switch(68) = 1:PlaySound "fx_sensor":End Sub
Sub sw68_UnHit:Controller.Switch(68) = 0:End Sub

' Targets
Sub sw12_Hit:vpmTimer.PulseSw 12:PlaySound SoundFX("fx_target",DOFTargets):End Sub
Sub sw15_Hit:vpmTimer.PulseSw 15:PlaySound SoundFX("fx_target",DOFTargets):End Sub
Sub sw25_Hit:vpmTimer.PulseSw 25:PlaySound SoundFX("fx_target",DOFTargets):End Sub
Sub sw71_Hit:vpmTimer.PulseSw 71:PlaySound SoundFX("fx_target",DOFTargets):End Sub
Sub sw72_Hit:vpmTimer.PulseSw 72:PlaySound SoundFX("fx_target",DOFTargets):End Sub
Sub sw73_Hit:vpmTimer.PulseSw 73:PlaySound SoundFX("fx_target",DOFTargets):End Sub

' Triggers on the ramps
Sub sw61_Hit:Controller.Switch(61) = 1:End Sub
Sub sw61_Unhit:Controller.Switch(61) = 0:End Sub

Sub sw62_Hit:Controller.Switch(62) = 1:sw62flip.rotatetoend:PlaySound "fx_gate4":End Sub
Sub sw62_Unhit:Controller.Switch(62) = 0:sw62flip.rotatetostart:End Sub

Sub sw63_Hit:Controller.Switch(63) = 1:End Sub
Sub sw63_Unhit:Controller.Switch(63) = 0:End Sub

Sub sw64_Hit:Controller.Switch(64) = 1:sw64flip.rotatetoend:PlaySound "fx_gate4":End Sub
...
Sub Gate9_Hit():PlaySound "fx_gate":End Sub
Sub Gate6_Hit():PlaySound "fx_gate":End Sub
...
Sub sw44_Hit():vpmtimer.PulseSw 44:PlaySound "fx_gate": End Sub
...
mml1.colorfull=RGB(223*l32.state +32,223*l31.state+32,223*l33.state+32)
mml2.colorfull=RGB(223*l32.state +32,223*l31.state+32,223*l33.state+32)
mml3.colorfull=RGB(223*l32.state +32,223*l31.state+32,223*l33.state+32)
If NOT Controller.Lamp(32) AND NOT Controller.Lamp(31) AND NOT Controller.Lamp(33) Then
For each ii in MerlinMod_Ambient:ii.IntensityScale=0:next
mml1.state=0
mml2.state=0
mml3.state=0
Else
For each ii in MerlinMod_Ambient:ii.IntensityScale=1:next
mml1.state=1
mml2.state=1
mml3.state=1
End If
For each ii in MerlinMod_Ambient:ii.color = RGB(223*l32.state +32,223*l31.state+32,223*l33.state+32):next
End Sub
...
Sub BallDropSound(dummy):PlaySound "fx_BallDrop":End Sub
...
'troll mod
    ltroll.rotz=-((BOT(b).x)/8)+50
    Rtroll.rotz=-((BOT(b).x)/8)+50



    if BOT(b).y>1700 and BOT(b).x>430  then
'            trollLtiming=trollLtiming+1
            ttrollL=ttrollL+0.15
            thshakeL=cos(ttrollL)
            ltroll.rotz=ltroll.rotz+(thshakeL*7)
    ...
    if BOT(b).y>1700 and BOT(b).x<430  then
 '             trollRtiming=trollRtiming+1
              ttrollR=ttrollR+0.15
              thshakeR=cos(ttrollR)
              Rtroll.rotz=Rtroll.rotz+(thshakeR*7)
...
Sub RandomSoundHole()
	Select Case Int(Rnd*4)+1
		Case 1 : PlaySound "fx_Hole1", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
		Case 2 : PlaySound "fx_Hole2", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
		Case 3 : PlaySound "fx_Hole3", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
		Case 4 : PlaySound "fx_Hole4", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
	End Select
End Sub
...
dim ali,increm,vers,quota,incquota,dfire
dfire=5
Sub drago_timer()
     ali=ali+0.03 ' ciclo ali
     increm=increm+vers ' velocita ali
     if increm>1 or increm<0 then vers=0 ' intervallo di flapping
     ...
     if incquota<0 or incquota>140 then quota=0 ' intervallo di movimento verticale
       ...
       lwing.roty=(cos(ali)*50)*increm : lwing.rotx=(cos((ali+90))*50)*increm : lwing.z=((cos((ali)-110)*50)*increm+(200))+incquota  : lwing.size_x=((cos(ali+30)/2))*increm+1
    rwing.roty=(cos(ali+160)*50)*increm : rwing.rotx=(sin((ali+280.2))*50)*increm : rwing.z=(cos((ali)-110)*50)*increm+(200)+incquota  : rwing.size_x=((cos(ali+30)/2))*increm+1
    dragon.transz=((cos((ali)-110)*45)*increm)
    dragon.rotx=(cos(ali)*10)*increm
...
'********** This routine will check if ballsaver is ON ****************

Dim lcountoff : lcountoff = 0
Dim lcounton : lcounton = 0
Dim ballsaver : ballsaver = False
Dim dragonup:dragonup=False

Sub CheckBallsaver
'**********using the ballsaver insert
If Controller.Lamp(72) Then
lcountoff = 0
If bsTrough.Balls <4 then lcounton = lcounton + 1
Else
lcounton = 0
If bsTrough.Balls <4  Then lcountoff = lcountoff + 1
End If

If bsTrough.Balls <4 and lcounton < 50 Then ballsaver = True
If bsTrough.Balls <4 and lcountoff> 250 Then ballsaver = False

'textbox1.text= "dragonup" & " " & dragonup
'textbox2.text= "ballsaver" & " " & ballsaver
'textbox3.text= "lcountoff" & " " & lcountoff
'textbox4.text= "lcounton" & " " & lcounton
End Sub
...
sub test_timer
'fop=cos(fop+0.1)*500
'flasherF.opacity=fop
flasherF.height=(dragon.z+(cos(ali+91)*20)*increm)+10
flasherF.opacity=(cos(dfire*40)*50)+100
select case fcol
Case 1 'red
flasherF.color=RGB(255,0,0):fcol=2
flasherF1.color=RGB(255,0,0):fcol=2
flasherF2.color=RGB(255,0,0):fcol=2
Case 2 'yellow
flasherF.color=RGB(255,255,0):fcol=3
flasherF1.color=RGB(255,255,0):fcol=3
flasherF2.color=RGB(255,255,0):fcol=3
Case 3 'orange
flasherF.color=RGB(255,0,0):fcol=1
flasherF1.color=RGB(255,0,0):fcol=1
flasherF2.color=RGB(255,0,0):fcol=1
End Select
