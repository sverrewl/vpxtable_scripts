...
'  Shamrock / IPD No. 5717 / INDER 1977 / 4 Players
...
'
' LED backboard
' Flasher Outside Left - 203/209/214/220/240/250
' Flasher left - 207/213/215/219/222/227/240/250
' Flasher center - 210/212/225/227/240/250
' Flasher right - 208/213/217/219/223/226/227/240/250
' Flasher Outside - 205/211/216/218/221/228/240/250
...
Sub Table1_Init()
    ...
    Set cball4 = New cvpmCaptiveBall
    With cball4
        .InitCaptive cbt4, cbw4, cbk4, 0
        .Start
        .ForceTrans = .99
        .MinForce = 3.8
        .CreateEvents "cball4"
...
Sub EndOfGame()
    ...
    bGameInPLay = False
    bJustStarted = False
    'turnon/off backdrop lights
    bip5.State = 0
    bip1.State = 0
    lastball.State = 0
    PlayersPlayingGame = 0:UpdatePlayersPlaying
    CurrentPlayer = 0:UpdateCurrentPlayer
    GameOver.State = 1
    If B2SOn then
        ...
        Controller.B2SSetData 110, 0 'Lastball
    end if
    ' turn off flippers
    SolLFlipper 0
    SolRFlipper 0
...
'PlaySound "fx_match"
'Match = INT(RND(1) * 10) * 10 ' random between 0 and 90 'in this table the match is already displayed on the first ball
'Display_Match
...
Sub FirstBallMatch 'sets the match at the begining of the game
    Clear_Match()
    Match = INT(RND(1) * 10) * 10
    Display_Match
End Sub
...
Sub StartBonusCount
    ...
    Select Case BonusMultiplier
        ...
        Case 3:BonusCountTimer.Interval = 800
    End Select
...
Sub UpdateBallInPlay
    Select Case Balls
        Case 0:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 0
        Case 1:bip1.State = 1:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 0
        Case 2:bip1.State = 0:bip2.State = 1:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 0
        Case 3:bip1.State = 0:bip2.State = 0:bip3.State = 1:bip4.State = 0:bip5.State = 0:bip6.State = 0
        Case 4:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 1:bip5.State = 0:bip6.State = 0
        Case 5:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 1:bip6.State = 0
        Case 6:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 1
    End Select
    If Balls = BallsPerGame Then
        LastBall.State = 1
        If B2SOn then
            Controller.B2SSetData 110, 1 'Lastball
        end if
    Else
        LastBall.State = 0
        If B2SOn then
...
Sub AwardExtraBall()
    '   If NOT bExtraBallWonThisBall Then 'limit to 1 extra ball per ball
    PlaySound SoundFXDOF ("fx_knocker", 300, DOFPulse, DOFKnocker)
    DOF 230, DOFPulse
    ExtraBallsAwards(CurrentPlayer) = ExtraBallsAwards(CurrentPlayer) + 1
...
Sub Trigger005_Hit
    ...
    If Tilted Then Exit Sub
    DOF 224, DOFPulse
    Addscore 5000 + 5000 * li012.State
    If li012.State Then
        li012.State = 0
        li010.State = 1
        li005.State = 0
...
Sub Trigger006_Hit
    ...
    DOF 225, DOFPulse
    Addscore 5000
    If li025.State Then AwardSpecial
End Sub
...
Sub Target003_hit 'center drop target
    PlaySoundAtBall SoundFXDOF ("fx_droptarget", 108, DOFPulse, DOFTargets)
    Target003.IsDropped = 1
    Target003P.TransZ = -70
    If Tilted Then Exit Sub
    DOF 212, DOFPulse
...
If Target003.IsDropped Then
    PlaySoundAt SoundFXDOF ("fx_resetdrop", 108, DOFPulse, DOFTargets), Target003P
    Target003.IsDropped = 0
    Target003P.TransZ = -20
End If
End Sub
...
Sub Target004_hit
    ...
    AddBonus 1
    If li001.State Then AwardSpecial
End Sub
...
Sub BonusKickOut
    PlaySoundAt SoundFXDOF ("fx_kicker", 111, DOFPulse, DOFContactors), BonusKicker
    DOF 218, DOFPulse
  DOF 230, DOFPulse
    BallinBonusKicker = False
...
Sub kicker001_Timer
    Me.TimerEnabled = 0
    PlaySoundAt SoundFXDOF ("fx_kicker", 112, DOFPulse, DOFContactors), kicker001
    DOF 219, DOFPulse
    kicker001.kick 180, 2
    kwall1.IsDropped = 0
    vpmtimer.addtimer 350,"PlaySoundAt ""fx_Solenoidoff"", Kicker001:kwall1.IsDropped = 1 '"
End Sub

Sub kicker002_Hit 'left red
...
Sub kicker002_Timer
    Me.TimerEnabled = 0
    PlaySoundAt SoundFXDOF ("fx_kicker", 112, DOFPulse, DOFContactors), kicker002
    DOF 219, DOFPulse
    kicker002.kick 180, 2
    kwall2.IsDropped = 0
    vpmtimer.addtimer 350,"PlaySoundAt ""fx_Solenoidoff"", Kicker002:kwall2.IsDropped = 1 '"
End Sub

Sub kicker003_Hit 'right red
...
Sub kicker003_Timer
    Me.TimerEnabled = 0
    PlaySoundAt SoundFXDOF ("fx_kicker", 112, DOFPulse, DOFContactors), Kicker003
    DOF 219, DOFPulse
    Kicker003.kick 180, 2
    kwall3.IsDropped = 0
    vpmtimer.addtimer 350,"PlaySoundAt ""fx_Solenoidoff"", Kicker003:kwall3.IsDropped = 1 '"
End Sub

Sub kicker004_Hit 'right black
...
Sub kicker004_Timer
    Me.TimerEnabled = 0
    PlaySoundAt SoundFXDOF ("fx_kicker", 112, DOFPulse, DOFContactors), kicker004
    DOF 219, DOFPulse
    kicker004.kick 180, 2
    kwall4.IsDropped = 0
    vpmtimer.addtimer 350,"PlaySoundAt ""fx_Solenoidoff"", Kicker004:kwall4.IsDropped = 1 '"
End Sub

Sub CheckCards
    If li019.State + li020.State = 2 Then li025.State = 1
    If li018.State + li017.State = 2 Then li025.State = 1
    If li015.State + li016.State = 2 Then li002.State = 1
    If li013.State + li014.State = 2 Then li004.State = 1
    If li007.State + li008.State + li009.State + li011.State = 4 Then li001.State = 1:li006.State = 1
End Sub
