' Wimbledon / IPD No. 6581 / 1978 / 1 Player
' Electromatic Brasil
...
' Valores Constants
Const TableName = "wimbledon" ' file name to save highscores and other variables
Const cGameName = "wimbledon" ' B2S name
Const MaxPlayers = 1          ' only 1 player
Const MaxMultiplier = 2       ' limit bonus multiplier
Const FreePlay = False        ' Free play or coins
Const Special1 = 130000         ' extra ball or credit
Const Special2 = 160000
Const Special3 = 180000
...
Sub TurnON(Col) 'turn on all the lights in a collection, used mostly for letters, like gameover, tilt.
    Dim i
    For each i in Col
        i.State = 1
    Next
...
Sub AddScore(Points)
    ...
    Select Case Points
        Case 10, 100, 1000
            ...
            UpdateScore points
            ' sounds
            If Points = 1000 AND(Score(CurrentPlayer) MOD 1000) \ 10 = 0 Then   'new reel 1000
                PlaySound SoundFXDOF("Chime_1000", 132, DOFPulse, DOFChimes)
            ElseIf Points = 100 AND(Score(CurrentPlayer) MOD 100) \ 10 = 0 Then 'new reel 100
...
Sub Trigger001_Hit
    ...
    If NOT Tilted Then
        AddScore 500 + 4500 * Light001.State
    End If
End Sub
...
Sub Trigger006_Hit
    ...
    If NOT Tilted Then
        AddScore 500 + 4500 * Light003.State
    End If
End Sub
...
Sub Trigger007_Hit
    ...
    If NOT Tilted Then
        ...
        BumperL001.State = 1
        BumperL002.State = 1
        If Light008.State Then AwardSpecial: Light008.State = 0
    End If
End Sub
...
Sub Trigger009_Hit
    ...
    If NOT Tilted Then
        ...
        BumperL005.State = 1
        BumperL006.State = 1
        If Light010.State Then AwardSpecial: Light010.State = 0
    End If
End Sub
...
Sub Target002_Hit
    PlaySoundAt "fx_droptarget", Target002
    If Tilted Then Exit Sub
    AddScore 500 + 4500 * Light006.State
    YellowTargets(1) = 1
    CheckYellowBank
...
Sub Target003_Hit
    PlaySoundAt "fx_droptarget", Target003
    If Tilted Then Exit Sub
    AddScore 500 + 4500 * Light006.State
    YellowTargets(2) = 1
    CheckYellowBank
...
Sub CheckYellowBank
    ...
    tmp = 0
    For i = 0 to 4
        tmp = tmp + YellowTargets(i)
    Next
    If tmp = 5 Then
        Light009.State = 1
        vpmTimer.AddTimer 2000, "ResetYellowBank '"
    End If
End Sub
...
Sub CheckBlueBank
    ...
    tmp = 0
    For i = 0 to 4
        tmp = tmp + BlueTargets(i)
    Next
    If tmp = 5 Then
        Light008.State = 1
        Light010.State = 0
        Light007.State = 1
        vpmTimer.AddTimer 2000, "ResetBlueBank '"
    End If
End Sub

Sub ResetBlueBank
    Dim i
    PlaySoundAt "fx_resetdrop", Target008
    For i = 0 to 4
        aBlueTargets(i).IsDropped = 0
        BlueTargets(i) = 0
    Next
End Sub
...
Sub CheckGreenBank
    ...
    tmp = 0
    For i = 0 to 4
        tmp = tmp + GreenTargets(i)
    Next
    If tmp = 5 Then
        Light008.State = 0
        Light010.State = 1
        Light003.State = 1
        vpmTimer.AddTimer 2000, "ResetGreenBank '"
    End If
End Sub

Sub ResetGreenBank
    Dim i
    PlaySoundAt "fx_resetdrop", Target012
    For i = 0 to 4
        aGreenTargets(i).IsDropped = 0
        GreenTargets(i) = 0
    Next
End Sub
