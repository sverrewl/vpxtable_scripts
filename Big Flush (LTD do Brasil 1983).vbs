...
sub RoyalFlush_init
  ...
  maxplayers=4
  Replay1Table(1)=620000
  Replay2Table(1)=760000
  Replay3Table(1)=840000
  Replay1Table(2)=620000
  Replay2Table(2)=760000
  Replay3Table(2)=840000
  Replay1Table(3)=620000
  Replay2Table(3)=760000
  Replay3Table(3)=840000
  hideoptions
  player=1
  rotatortemp=1
  balls=5
...
sub Lbonus1_timer
  ...
  if bonuscount>DoubleBonus.state then
    ...
    if AddScore1000Timer.enabled = false then
      ...
      Lbonus1A.state=1
      if Lbonus1.state=1 then addscore 10000
    else
      bonuscount=bonuscount-1
  ...
  if bonuscount>DoubleBonus.state then
    ...
    Lbonus2.timerinterval=325
      ...
      if AddScore1000Timer.enabled = false then
        ...
        Lbonus1A.state=0
        Lbonus2A.state=1
        if Lbonus2.state=1 then addscore 20000
      else
      bonuscount=bonuscount-1
  ...
  if bonuscount>DoubleBonus.state then
    ...
    Lbonus3.timerinterval=325
      ...
      if AddScore1000Timer.enabled = false then
        ...
        Lbonus2A.state=0
        Lbonus3A.state=1
        if Lbonus3.state=1 then addscore 30000
      else
      bonuscount=bonuscount-1
  ...
  if bonuscount>DoubleBonus.state then
    ...
    Lbonus4.timerinterval=325
      ...
      if AddScore1000Timer.enabled = false then
        ...
        Lbonus3A.state=0
        Lbonus4A.state=1
        if Lbonus4.state=1 then addscore 40000
      else
      bonuscount=bonuscount-1
  ...
  if bonuscount>DoubleBonus.state then
    ...
    Lbonus5.timerinterval=325
      ...
      if AddScore1000Timer.enabled = false then
        ...
        Lbonus4A.state=0
        Lbonus5A.state=1
        if Lbonus5.state=1 then addscore 50000
      else
      bonuscount=bonuscount-1
...
sub ballreltimer_timer
  ...
  Drain.kick 60,45,0
    ballreltimer.enabled=false
  PlaySoundAtVol SoundFXDOF("drainkick",135,DOFPulse,DOFContactors), Drain, 1
  end if
end sub
...
sub dingwalla_hit
  if state=true and tilt=false then addscore 100
  SlingA.visible=0
  SlingA1.visible=1
...
sub TGoutL_hit
    playsong"Left Outside Lane Rollover"
  DOF 124, DOFPulse
...
sub TGinL_hit
    playsong"Left Inside Lane Rollover"
  DOF 125, DOFPulse
...
sub TGoutR_hit
    playsong"Right Outline Lane Rollover"
  DOF 126, DOFPulse
...
sub TGinR_hit
    playsong"Right Inside Lane Rollover"
  DOF 127, DOFPulse
...
sub TGtopoutR_hit   '****** top R rollover
  ...
  PlaySoundAtVol "fx_spinner", ActiveBall, 1
end sub
...
sub TGballgate_hit
  ...
  PlaySoundAtVol "fx_spinner", TGballgate, 1
end sub
...
sub Tjacks10_hit
  PlaySoundAtVol SoundFXDOF("target",130, DOFPulse, DOFTargets), ActiveBall, 1
  Playsound "ten"
  addscore 5000
end sub

sub Tacekings_hit
  PlaySoundAtVol SoundFXDOF("target",128, DOFPulse, DOFTargets), ActiveBall, 1
  Playsound "kings"
  addscore 5000
end sub

sub Tqueens_hit
  PlaySoundAtVol SoundFXDOF("target",129, DOFPulse, DOFTargets), ActiveBall, 1
  Playsound "queens"
...
sub Tpurple_hit
  PlaySoundAtVol SoundFXDOF("target",116, DOFPulse, DOFTargets), ActiveBall, 1
  addscore 5000
...
sub Twhite_hit
  PlaySoundAtVol SoundFXDOF("target",115, DOFPulse, DOFTargets), ActiveBall, 1
  addscore 5000
...
sub Tgreen_hit
  PlaySoundAtVol SoundFXDOF("target",117, DOFPulse, DOFTargets), ActiveBall, 1
  addscore 5000
...
sub DTA_dropped
     Playsound"Ace"
  PlaySoundAtVol SoundFXDOF("drop1",111,DOFPulse,DOFDropTargets), ActiveBall, 1
...
sub DTQH_dropped
     Playsound"Queens"
  PlaySoundAtVol SoundFXDOF("drop1",113,DOFPulse,DOFDropTargets), ActiveBall, 1
  if tilt=false then addscore holevalue
  checkbonus
end sub
...
sub resetDT
  PlaySoundAtVol SoundFXDOF("DTreset",112,DOFPulse,DOFContactors), DT10H, 1
  PlaySoundAtVol SoundFXDOF("DTreset",114,DOFPulse,DOFContactors), DTJC, 1
  for i = 0 to 8
...
Sub kicker_Hit()
  ...
  PlaySoundAtVol "cluper", kicker, 1
  me.uservalue=1
  me.timerenabled=1
...
Sub kicker_timer
  select case kicker.uservalue
    case 4:
    PlaySoundAtVol SoundFXDOF("holekick",118,DOFPulse,DOFContactors), Pkickarm, 1
    DOF 139, DOFPulse
    Kicker.kick 215,6,0
    Pkickarm.rotz=15
    case 6:
...
sub addscore(points)
    ...
    Select Case Points
        ...
        Case 200, 300, 400, 500
            Add10 = Add10 + Points \ 100
            AddScore10Timer.Enabled = TRUE
        Case 2000, 3000, 4000, 5000
            Add100 = Add100 + Points \ 1000
            AddScore100Timer.Enabled = TRUE
        Case 20000, 30000, 40000, 50000
            Add1000 = Add1000 + Points \10000
            AddScore1000Timer.Enabled = TRUE
    End Select
End Sub

Sub AddScore10Timer_Timer() '100 points
    if Add10 > 0 then
        AddPoints 100
        Add10 = Add10 - 1
    Else
...
Sub AddScore100Timer_Timer() '1000 points
    if Add100 > 0 then
        AddPoints 1000
        Add100 = Add100 - 1
    Else
...
Sub AddScore1000Timer_Timer() '10000 points
    if Add1000 > 0 then
        AddPoints 10000
        Add1000 = Add1000 - 1
    Else
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "RoyalFlush" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = tableobj.y * 2 / RoyalFlush.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "RoyalFlush" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = tableobj.x * 2 / RoyalFlush.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "RoyalFlush" is the name of the table
  Dim tmp
  On Error Resume Next
...
'Stop the current song and play a new song

Dim song: song = ""

Sub PlaySong(newsong)
    StopSound song
    song = newsong
    PlaySound song
End Sub
