' ***********************************************************************
' Tasty Samba / IPD No. 5719 / 1977 / 4 Players
' Industria (Electromecánica) de Recreativos S.A, of Madrid,
' Spain (1970-1993) [Trade Name: INDER]
' VPX8 table by jpsalas, 2026, version 1.0.0
' ***********************************************************************
...
' Valores Constants
Const TableName = "TastySamba4p" ' file name to save highscores and other variables
Const cGameName = "TastySamba"   ' B2S name & DOF config
...
Sub UpdateStartup
    ...
    ScoreReel2.SetValue HSScore(2)
    ScoreReel3.SetValue HSScore(3)
    ScoreReel4.SetValue HSScore(4)
...
Function Balls
    Balls = BallsRemaining(CurrentPlayer)
End Function
    ...
    Select Case Points
        Case 10, 100, 1000, 10000
            ...
            If Points = 1000 AND(Score(CurrentPlayer) MOD 10000) \ 1000 = 0 Then  'new reel 10000
                PlaySound SoundFXDOF("chime10000", 301, DOFPulse, DOFChimes)
            ElseIf Points = 100 AND(Score(CurrentPlayer) MOD 1000) \ 100 = 0 Then 'new reel 1000
                PlaySound SoundFXDOF("chime1000", 302, DOFPulse, DOFChimes)
            ElseIf Points = 10 AND(Score(CurrentPlayer) MOD 100) \ 10 = 0 Then    'new reel 100
                PlaySound SoundFXDOF("chime100", 303, DOFPulse, DOFChimes)
            Else
                PlaySound SoundFXDOF("chime", 304, DOFPulse, DOFChimes) &Points
            End If
...
Sub UpdateBallInPlay 'update backdrop lights
    'Ball in play
    Select Case Balls
        Case 0:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 0:LastBall.State = 0
        Case 1:bip1.State = 1:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 0:LastBall.State = 1
        Case 2:bip1.State = 0:bip2.State = 1:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 0:LastBall.State = 0
        Case 3:bip1.State = 0:bip2.State = 0:bip3.State = 1:bip4.State = 0:bip5.State = 0:bip6.State = 0:LastBall.State = 0
        Case 4:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 1:bip5.State = 0:bip6.State = 0:LastBall.State = 0
        Case 5:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 1:bip6.State = 0:LastBall.State = 0
        Case 6, 7:bip1.State = 0:bip2.State = 0:bip3.State = 0:bip4.State = 0:bip5.State = 0:bip6.State = 1:LastBall.State = 0
    End Select
    If B2SOn then
...
Sub AwardAddaBall() 'only for the 1 player game
    PlaySound SoundFXDOF("fx_knocker", 122, DOFPulse, DOFknocker)
...
Sub rlband001_Slingshot:AddScore 10:End Sub
Sub rlband003_Slingshot:AddScore 10:End Sub
Sub rlband004_Slingshot:AddScore 10:End Sub
Sub rlband007_Slingshot:AddScore 10:End Sub
...
Sub CheckAwards
    If Light017.State AND Light020.State Then Light027.State = 1                                                          'lite xtraball left rollovers
    If Light023.State AND Light026.State Then Light028.State = 1                                                          'lite xtraball right rollovers
    If Light020.State AND Light023.State Then Light008.State = 1                                                          'lite left spinner 10000
    If Light017.State AND Light026.State Then Light007.State = 1                                                          'lite right spinner 10000
    If Light017.State AND Light020.State AND Light023.State AND Light026.State Then Light005.State = 1:Light006.state = 0 'side specials
End Sub
...
Sub Trigger006_Hit 'left outlane
...
Sub target001_Hit
    ...
    If Tilted Then Exit Sub
    Addscore 1000
    If Light005.State Then AwardSpecial
End Sub
...
Sub target002_Hit
    ...
    If Tilted Then Exit Sub
    Addscore 1000
    If Light006.State Then AwardSpecial
End Sub

' Top Targets

Sub target003_Hit
    PlaySoundAt SoundFXDOF("fx_droptarget", 108, DOFPulse, DOFTargets), Target003
    If Tilted Then Exit Sub
    Addscore 500
End Sub

Sub Target003_Dropped
    If Target004.IsDropped Then
...
Sub Target003_Animate():t3p.Z = Target003.CurrentAnimOffset - 15:End Sub

Sub target004_Hit
    PlaySoundAt SoundFXDOF("fx_droptarget", 107, DOFPulse, DOFTargets), Target004
    If Tilted Then Exit Sub
    Addscore 500
End Sub

Sub Target004_Dropped
    If Target003.IsDropped Then
        TurnON aBumper1Light
        TurnON aBumper2Light
        If bSecondDrop Then
            Light010.State = 1
...
Sub Target004_Animate():t4p.Z = Target004.CurrentAnimOffset - 15:End Sub

Sub ResetDrops
    PLaySoundAt "fx_resetdrop", Light010
...
Sub spinner1_Spin
    PlaySoundAt SoundFXDOF("fx_spinner", 112, DOFPulse, DOFTargets), spinner1
    If Tilted Then Exit Sub
    Addscore 100 + 9900 * Light008.State
End Sub

Sub spinner2_Spin
    PlaySoundAt SoundFXDOF("fx_spinner", 113, DOFPulse, DOFTargets), spinner2
    If Tilted Then Exit Sub
    Addscore 100 + 9900 * Light007.State
End Sub

' Hole

Sub Kicker001_Hit 'center Right
    Dim tmp
    PlaySoundAt "fx_kicker_enter", Kicker001
    If Tilted Then vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 114, DOFPulse, DOFContactors), kicker001:kicker001.kick 195,14 '"
    If Light010.State Then AwardSpecial:Light010.State = 0: ResetDrops
    Addscore 5000 + 45000 * Light009.State
    vpmTimer.AddTimer 800, "PlaySoundAt SoundFXDOF (""fx_kicker"", 114, DOFPulse, DOFContactors), kicker001:kicker001.kick 195,14 '"
End Sub
