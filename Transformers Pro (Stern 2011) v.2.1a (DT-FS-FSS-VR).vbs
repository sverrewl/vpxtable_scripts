...
' - Slingshots and bumpers are solenoid driven, based on VPW's Spider-Man Classic (v.2.1.1)
' - Updated trough code based on VPW's AC/DC (v.2.1.1)
' - Converted Flupper Round Domes to work like 3d lights on top of the bumpers (v.2.1.1)
'
' TastyWasps:
...
Dim UseVPMModSol
Dim ModSol      '0 = Non Modulated Lights. 1 = Simple Modulated Lights. 2 = PWM Lights, Requires VPinMame 3.6 or later
Dim PWMflashers
...
'Preload lightsettings for UseVPMModSol before VPM loads the script
Dim Loadx
Loadx = LoadValue(cGameName, "MODSOL") : If Loadx <> "" Then ModSol = CInt(Loadx) Else ModSol = 2
...
Sub solLBump (enabled): If enabled then RandomSoundBumperTop(Bumper1) : End If: End Sub
Sub solRBump (enabled): If enabled then RandomSoundBumperMiddle(Bumper2) : End If: End Sub
Sub solBBump (enabled): If enabled then RandomSoundBumperBottom(Bumper3) : End If: End Sub
...
Const TableVersion = "2.1.1"  'Table version (shown in option UI)
