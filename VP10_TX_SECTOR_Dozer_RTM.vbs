...
Sub TR_Help1_Hit()
'MsgBox ActiveBall.VelY
PlaySound "scoopenter", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
End Sub
...
Sub SW74_Spin:vpmTimer.PulseSw 74:PlaySoundAtVol "fx_spinner",sw74,VolSpin:End Sub
Sub SW73_Spin:vpmTimer.PulseSw 73:PlaySoundAtVol "fx_spinner",sw73,VolSpin:End Sub
...
Sub LminiKick_SlingShot():DOF 123,2:PlaySoundAtVol "left_slingshot",ActiveBall, 1:LminiKick.isdropped = 1:me.timerenabled = 1:me.timerinterval = 20:leftm=1:vpmTimer.PulseSw 76:End Sub
Sub RminiKick_SlingShot():DOF 124,2:PlaySoundAtVol "right_Slingshot",ActiveBall, 1:RminiKick.isdropped = 1:me.timerenabled = 1:me.timerinterval = 20:rightm=1:vpmTimer.PulseSw 76:End Sub
...
'Right Hole
Sub KickerPassageBottom_Hit:bsRTop.AddBall 0:PlaySound "kicker_enter_center", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall):End Sub                '65 Top Right
Sub KickerPassageBottom1_Hit:bsRBot.AddBall 0:PlaySound "kicker_enter_center", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall):End Sub               '75 Bottom Right

'Left Hole
Sub KickerTRE_Hit:bsLTop.AddBall 0:PlaySound "kicker_enter_center", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall):End Sub                '45 Top Left
Sub KickerTRE1_Hit:bsLBot.AddBall 0:PlaySound "kicker_enter_center", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall):End Sub               '55 Bottom Left
Sub KickerTR_Hit:PlaySound "scoopenter", 0, Vol(ActiveBall), Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall):End Sub
    ...
    For B1 = 0 to UBound(BOT)
      ...
      If dz <= radii Then
            ...
            If distance <= radii AND (collision(b1) = -1 OR collision(b2) = -1) Then
                collision(b1) = b2
                collision(b2) = b1
                PlaySound("fx_collide"), 0, Vol2(BOT(b1), BOT(b2)), Pan(BOT(b1)), 0, Pitch(BOT(b1)), 0, 0, AudioFade(Bit(b1) )
            Else
