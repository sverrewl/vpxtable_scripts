...
' Valores Constants
Const TableName = "JakeMateRecel" 'saving the highscores and credits
Const cGameName = "CheckMate"     ' mostly for DOF
Const MaxPlayers = 1              ' solo un jugador
...
Sub BonusLCountTimer_Timer 'añade los bonos y actualiza las luces
    ' debug.print "BonusCount_Timer"
    ' check all bonus lights and if it is on then add the bomus points
    DisableTable True
    If BonusCount < 8 AND NOT Tilted Then
        If aLeftDropLights(BonusCount).State Then 'the light is on so count the bonus
            Addscore 1000 * doublebonus
            BonusLCountTimer.Interval = 200 * doublebonus
        Else
            BonusLCountTimer.Interval = 50
        End If
    Else
        ' termina la cuenta de los bonos
        BonusLCountTimer.Enabled = 0
        'CreditLight.State = 0
        DisableTable False
        DrainWall.IsDropped = 1 'and the ball goes into the drain
    End If
    BonusCount = BonusCount + 1
End Sub

Sub BonusRCountTimer_Timer 'añade los bonos y actualiza las luces
    ' debug.print "BonusCount_Timer"
    ' check all bonus lights and if it is on then add the bomus points
    DisableTable True
    If BonusCount < 8 AND NOT Tilted Then
        If aRightDropLights(BonusCount).State Then 'the light is on so count the bonus
            Addscore 1000 * doublebonus
            BonusRCountTimer.Interval = 200 * doublebonus
        Else
            BonusRCountTimer.Interval = 50
        End If
    Else
        ' termina la cuenta de los bonos
        BonusRCountTimer.Enabled = 0
        'CreditLight.State = 0
        DisableTable False
        DrainWall.IsDropped = 1 'and the ball goes into the drain
    End If
...
Sub Drop10_Hit ' Not a drop target in this 1 player table.
    ...
    If li004.State = 1 Then
        AwardSpecial
        li004.State = 0
        vpmtimer.addtimer 300, "BankLDropup '" 'sube las dianas
        vpmtimer.addtimer 500, "BankRDropup '" 'sube las dianas
    Else
    ' haz algún chequeo
    CheckRBankTargets
    End If
...
Sub Trigger1_Hit
    ...
    Else
        DoubleBonus = 1
    End If
    If li001.State Then AwardSpecial: rese
    ' count left Bonus
    BonusCount = 0
    'CreditLight.State = 2
    DrainWall.IsDropped = 0
    BonusLCountTimer.Enabled = 1
End Sub
...
Sub Table1_OptionEvent(ByVal eventId)
    ...
    If BallsPerGame = 5 Then
        ...
        If ApronEnglish Then
            apron.image = "apron_JakeMate5i"
        Else
            apron.image = "apron_JakeMate5e"
        End If
    Else '3 bolas
        ...
        If ApronEnglish Then
            apron.image = "apron_JakeMate3i"
        Else
            apron.image = "apron_JakeMate3e"
        End If
    End If
End Sub
