...
' Thalamus 2018-12-07 : Improved directional sounds
...
Sub CardWhiz_KeyDown(ByVal keycode)
...
Sub CardWhiz_KeyUp(ByVal keycode)
...
sub TGtopoutR_hit   '****** top R rollover
  ...
  playsoundAtVol "fx_spinner", DiverterFlipper, 1
end sub
...
sub Tjacks10_hit
  PlaySoundAtVol SoundFXDOF("target",130, DOFPulse, DOFTargets), ActiveBall, VolTarg
  addscore 500
end sub

sub Tacekings_hit
  PlaySoundAtVol SoundFXDOF("target",128, DOFPulse, DOFTargets), Activeball, VolTarg
  addscore 500
end sub

sub Tqueens_hit
  PlaySoundAtVol SoundFXDOF("target",129, DOFPulse, DOFTargets), ActiveBall, VolTarg
...
sub Tpurple_hit
  PlaySoundAtVol SoundFXDOF("target",116, DOFPulse, DOFTargets), ActiveBall, VolTarg
  addscore 500
...
sub Twhite_hit
  PlaySoundAtVol SoundFXDOF("target",115, DOFPulse, DOFTargets), ActiveBall, VolTarg
  addscore 500
...
sub Tgreen_hit
  PlaySoundAtVol SoundFXDOF("target",117, DOFPulse, DOFTargets), ActiveBall, VolTarg
  addscore 500
...
sub resetDT
  PlaySoundAtVol SoundFXDOF("DTreset",112,DOFPulse,DOFContactors),Tqueens, 1
  PlaySoundAtVol SoundFXDOF("DTreset",114,DOFPulse,DOFContactors),DTJH, 1
  for i = 0 to 8
...
Sub kicker_timer
  select case kicker.uservalue
    case 4:
    playsoundAtVol SoundFXDOF("holekick",118,DOFPulse,DOFContactors), Kicker, VolKick
    DOF 139, DOFPulse
    Kicker.kick 215,6,0
    Pkickarm.rotz=15
    case 6:
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "CardWhiz" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / CardWhiz.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "CardWhiz" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / CardWhiz.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "CardWhiz" is the name of the table
    Dim tmp
    tmp = ball.x * 2 / CardWhiz.width-1
    If tmp > 0 Then
        Pan = Csng(tmp ^10)
    Else
        Pan = Csng(-((- tmp) ^10) )
...
sub savehs
    savevalue "CardWhiz", "credit", credit
    savevalue "CardWhiz", "hiscore", hisc
    savevalue "CardWhiz", "match", matchnumb
    savevalue "CardWhiz", "score1", score(1)
    savevalue "CardWhiz", "score2", score(2)
    savevalue "CardWhiz", "score3", score(3)
    savevalue "CardWhiz", "score4", score(4)
  savevalue "CardWhiz", "balls", balls
  savevalue "CardWhiz", "hsa1", HSA1
  savevalue "CardWhiz", "hsa2", HSA2
  savevalue "CardWhiz", "hsa3", HSA3
  savevalue "CardWhiz", "freeplay", freeplay

end sub

sub loadhs
    dim temp
  temp = LoadValue("CardWhiz", "credit")
    If (temp <> "") then credit = CDbl(temp)
    temp = LoadValue("CardWhiz", "hiscore")
    If (temp <> "") then hisc = CDbl(temp)
    temp = LoadValue("CardWhiz", "match")
    If (temp <> "") then matchnumb = CDbl(temp)
    temp = LoadValue("CardWhiz", "score1")
    If (temp <> "") then score(1) = CDbl(temp)
    temp = LoadValue("CardWhiz", "score2")
    If (temp <> "") then score(2) = CDbl(temp)
    temp = LoadValue("CardWhiz", "score3")
    If (temp <> "") then score(3) = CDbl(temp)
    temp = LoadValue("CardWhiz", "score4")
    If (temp <> "") then score(4) = CDbl(temp)
    temp = LoadValue("CardWhiz", "balls")
    If (temp <> "") then balls = CDbl(temp)
    temp = LoadValue("CardWhiz", "hsa1")
    If (temp <> "") then HSA1 = CDbl(temp)
    temp = LoadValue("CardWhiz", "hsa2")
    If (temp <> "") then HSA2 = CDbl(temp)
    temp = LoadValue("CardWhiz", "hsa3")
    If (temp <> "") then HSA3 = CDbl(temp)
    temp = LoadValue("CardWhiz", "freeplay")
