...
Sub TargetAlpha_KeyDown(ByVal keycode)
  ...
  If keycode = LeftFlipperKey and InProgress=true and TableTilted=false Then
    ...
    PlaySoundAtVol "Buzz", LeftFlipper2, VolFlip
    DOF 101,1
  End If
  ...
  If keycode = RightFlipperKey  and InProgress=true and TableTilted=false Then
    ...
    PlaySoundAtVol "Buzz1", RightFlipper2, VolFlip
    DOF 102,1
  End If
...
Sub TargetAlpha_KeyUp(ByVal keycode)
  ...
  If keycode = PlungerKey Then
    if ballinshooter=1 then
      playsoundAtVol "plungerreleaseball", Plunger, 1
      else
...
Sub TopTarget1_dropped()
  If TableTilted=false then
    playsoundAtVol SoundFXDOF("droptargetdropped",123,DOFPulse,DOFTargets), ActiveBall, 1
    ScoreUpperDrops
...
Sub TopTarget4_dropped()
  If TableTilted=false then
    playsoundAtVol SoundFXDOF("droptargetdropped",124,DOFPulse,DOFTargets), ActiveBall, 1
    ScoreUpperDrops
    bonuscounter=bonuscounter+1
...
Sub TopTarget8_dropped()
  If TableTilted=false then
    playsoundAtVol SoundFXDOF("droptargetdropped",125,DOFPulse,DOFTargets), ActiveBall, 1
    ScoreUpperDrops
    bonuscounter=bonuscounter+1
...
Sub LowerTarget1_dropped()
  If TableTilted=false then
    playsoundAtVol SoundFXDOF("droptargetdropped",126,DOFPulse,DOFTargets), ActiveBall, 1
    ScoreLowerDrops
    bonuscounter=bonuscounter+1
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "TargetAlpha" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / TargetAlpha.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "TargetAlpha" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / TargetAlpha.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "TargetAlpha" is the name of the table
    Dim tmp
    tmp = ball.x * 2 / TargetAlpha.width-1
    If tmp > 0 Then
        Pan = Csng(tmp ^10)
    Else
        Pan = Csng(-((- tmp) ^10) )
...
Function AudioFade(ball) ' Can this be together with the above function ?
  Dim tmp
  tmp = ball.y * 2 / TargetAlpha.height-1
  If tmp > 0 Then
    AudioFade = Csng(tmp ^10)
  Else
    AudioFade = Csng(-((- tmp) ^10) )
