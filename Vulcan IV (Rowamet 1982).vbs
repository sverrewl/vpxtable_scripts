...
Sub Vulcan_KeyDown(ByVal keycode)
...
Sub Vulcan_KeyUp(ByVal keycode)
...
Sub PairedlampTimer_timer
  ...
  ShootA3.state = Shoot3.state
  ShootA4.state = Shoot4.state
end sub
...
sub newgame
  ...
  PlaySoundAtVol SoundFXDOF("kickerkick",128,DOFPulse,DOFContactors), BallRelease, 1
end sub
...
Sub resetWT
  PlaySoundAtVol SoundFXDOF("BankReset",111,DOFPulse,DOFContactors), Drop3, 1
  for each objekt in droptargetsWhite: objekt.isdropped=false: Next
  for each light in WDTlights:light.state=0:next
end sub

Sub resetDT
  PlaySoundAtVol SoundFXDOF("BankReset",114,DOFPulse,DOFContactors), Drop3, 1
  DOF 114, DOFPulse
...
Sub Bumper1_Hit
   if tilt=false then
  PlaySoundAtVol SoundFXDOF("Bumpers_Top_1",107,DOFPulse,DOFContactors), ActiveBall, 1
  DOF 108,DOFPulse
...
Sub Bumper2_Hit
   if tilt=false then
  PlaySoundAtVol SoundFXDOF("Bumpers_Middle_1",109,DOFPulse,DOFContactors), ActiveBall, 1
  DOF 110,DOFPulse
...
sub SpotTarget_hit
  spot5.state=0
  PlaySoundAtVol SoundFXDOF("target",135,DOFPulse,DOFContactors), ActiveBall, 1
  if balls = 3 then
    addscore 3000
    else
    addscore 500
...
Sub TopHole_timer
  TopHole.Kick 205, 12
  PlaySoundAtVol SoundFXDOF("ballrelease",132,DOFPulse,DOFContactors), TopHole, 1
  DOF 131, DOFPulse
  me.timerenabled=0
End Sub
...
Sub LeftOutlane_Hit
  ...
  flashbumpers
    ...
    PlaySoundAtVol SoundFXDOF("knock",130,DOFPulse,DOFKnocker), ActiveBall, 1
    DOF 131,DOFPulse
    addcredit
    end if
  Drop1.State  = 1
...
Sub W1a_Hit
  PlaySoundAtVol SoundFXDOF("drop1",112,DOFPulse,DOFContactors), ActiveBall, 1
  addscore 500
  addbonus
...
Sub G1a_Hit
  PlaySoundAtVol SoundFXDOF("drop1",113,DOFPulse,DOFContactors), ActiveBall, 1
  addscore 500
  addbonus
  Hole1.state =1
  me.isdropped=true
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "Vulcan" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = tableobj.y * 2 / Vulcan.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "Vulcan" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = tableobj.x * 2 / Vulcan.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "Vulcan" is the name of the table
  Dim tmp
  On Error Resume Next
  tmp = ball.x * 2 / Vulcan.width-1
  If tmp > 0 Then
    Pan = Csng(tmp ^10)
  Else
    Pan = Csng(-((- tmp) ^10) )
...
sub savehs
    ...
    savevalue "Vulcan", "credit", credit
    savevalue "Vulcan", "hiscore", hisc
    savevalue "Vulcan", "match", matchnumb
    savevalue "Vulcan", "score1", score(1)
    savevalue "Vulcan", "score2", score(2)
    savevalue "Vulcan", "score3", score(3)
    savevalue "Vulcan", "score4", score(4)
  savevalue "Vulcan", "replays", replays
  savevalue "Vulcan", "balls", balls

end sub

sub loadhs
    dim temp
  temp = LoadValue("Vulcan", "credit")
    If (temp <> "") then credit = CDbl(temp)
    temp = LoadValue("Vulcan", "hiscore")
    If (temp <> "") then hisc = CDbl(temp)
    temp = LoadValue("Vulcan", "match")
    If (temp <> "") then matchnumb = CDbl(temp)
    temp = LoadValue("Vulcan", "score1")
    If (temp <> "") then score(1) = CDbl(temp)
    temp = LoadValue("Vulcan", "score2")
    If (temp <> "") then score(2) = CDbl(temp)
    temp = LoadValue("Vulcan", "score3")
    If (temp <> "") then score(3) = CDbl(temp)
    temp = LoadValue("Vulcan", "score4")
    If (temp <> "") then score(4) = CDbl(temp)
    temp = LoadValue("Vulcan", "replays")
    If (temp <> "") then replays = CDbl(temp)
    temp = LoadValue("Vulcan", "balls")
