...
'   103 - 104 Slings
'   105 -107 Bumpers
'   108 - 113 Upper X & O Rollovers
'   114 - 116 LCR Mid OX Rollovers
'   117 - 119 LCR Lower OX Rollovers
'   120 - 121 A & B Targets
'   122 - 123 Outlanes
'   131 Right Kicker, 130 Left Kicker
...
Const cGameName = "TripleX_1973"
Const hsFileName = "Triple X (Williams 1973)"
Const cOptions = "TripleX_1973_v1.08.txt"
...
' DoSTAnim            'handle stand up target animations
  Pgate.rotz = Gate0.CurrentAngle * 0.5
  tb.text = "Tic Tac Toe " & ticTacToe
  tb1.text = "tictac toe bonus" & ticTacToeBonus
End Sub
...
Sub Table1_init
  ...
  If freePlay = 1 Then creditLight.state = true
  If freePlay = 0 and Credit > 0 Then creditLight.state = true
...
Sub bootTable_Timer()
  ...
  End If
  If bootCount = 1 Then
    If vrOption > 0 Then vrBackGlass.image = "tripleXOn"
    If B2SOn Then
...
Sub releaseBall
  ...
  drainActive = 0
  If balls = 5 and ballinPlay = 5 Then bonusLight.state = 1: lastBall = True
  If balls = 3 and ballinPlay = 3 Then bonusLight.state = 1: lastBall = True
  clearAB
  rotateGateShut
  scoreMotorDone = 0
  If vrOption > 0 Then vrBip
...
Sub scoreBonus
  ...
  If square < 1 and shootAgainLight.state = 1 and scoreMotorLoop = 0 Then releaseBall
  If square < 1 and shootAgainLight.state = 0 and scoreMotorDone = 1 Then advancePlayers
End Sub
  ...
  If tilt = False Then
    Select Case (Index)
      ...
      case 2: playFieldSound "PopBump", 0, bumper3, 1
          ...
          OXOUpdate 'alternates the X and O lights
          If ticTacToe = True Then
            addScore 100
          Else
            addScore 10
...
'************** Saucers
Dim saucerSide, saucerFlag
Sub rightSaucer_Hit
  If saucerFlag > 0 Then Exit Sub
  set kickerBall = activeBall
  playFieldSound "SaucerIn", 0, rightSaucer, 1
  saucerFlag = 1
  saucerSide = 1
  ScoreKickers
End Sub
...
Sub leftSaucer_Hit
  If saucerFlag > 0 Then Exit Sub
  set kickerBall = activeBall
  playFieldSound "SaucerIn", 0, leftSaucer, 1
  saucerFlag = 1
  saucerSide = 0
  ScoreKickers
End Sub
...
Sub saucer_Timer
    ...
    Select Case kickStep
        Case 1: kickarmtop_prim.ObjRotX = 15: kickarmtop_prim1.ObjRotX = 15
        Case 2: kickarmtop_prim.ObjRotX = 15: kickarmtop_prim1.ObjRotX = 15
        Case 3: kickarmtop_prim.ObjRotX = 15: kickarmtop_prim1.ObjRotX = 15
        Case 4: kickarmtop_prim.ObjRotX = 8: kickarmtop_prim1.ObjRotX = 8
        Case 5: kickarmtop_prim.ObjRotX = 3: kickarmtop_prim1.ObjRotX = 3
        Case 6: kickarmtop_prim.ObjRotX = 0: kickarmtop_prim1.ObjRotX = 0
        saucer.Enabled = 0
    End Select
End Sub
...
Sub scoreKickers
  scoreMotorLoop = 0
  kickStep = 0
  If tilt = True Then scoreMotor.enabled = 1
  If ticTacToe = True and ticTacToeBonus = False and tilt = False Then
    ticTacToeBonus = True
    shootAgainLight.state = 1
    If vrOption > 0 Then vrShootAgain.visible = 1
    ebLight1.state = 0: ebLight2.state = 0
    If B2SOn Then controller.B2SSetShootAgain 36,1
    addScore 5000
  Else
    addScore 500
  End If
  If ticTacToe = False and tilt = False Then addScore 500
End Sub
...
Sub leftSlingShot_Timer
    Select Case lStep
        Case 3: lSling2.visible = 0: lSling1.visible = 1:leftSling.transZ = -10
...
Sub rightSlingShot_Timer
    Select Case rStep
        Case 3: rSling2.visible = 0: rSling1.visible = 1: rightSling.transZ = -10
...
'*************** Triggers
Dim targetNumber, targetA, targetB
Sub triggers_hit (Index)
  If tilt = False Then
    targetNumber = Index
    If Index < 6 Or Index > 8 and Index < 14 Then addScore 100
    If Index > 5 and Index < 9  Or Index > 13 Then addScore 1000
    Select Case Index
      Case 0: wireAnimation.enabled = 1: If grid(1) = 0 then grid(1) = 1
      Case 1: wireAnimation.enabled = 1: If grid(1) = 0 then grid(1) = 2
      Case 2: wireAnimation.enabled = 1: If grid(2) = 0 then grid(2) = 1
      Case 3: wireAnimation.enabled = 1: If grid(2) = 0 then grid(2) = 2:
      Case 4: wireAnimation.enabled = 1: If grid(3) = 0 then grid(3) = 1
      Case 5: wireAnimation.enabled = 1: If grid(3) = 0 then grid(3) = 2
      Case 6: wireAnimation.enabled = 1: If grid(4) = 0 Then grid(4) = OXO
          If specialLight1.state = 1 Then special
      Case 7: targetAnimation.enabled = 1: If grid(5) = 0 Then grid(5) = OXO
      Case 8: wireAnimation.enabled = 1: If grid(6) = 0 Then grid(6) = OXO
          If specialLight1.state = 1 Then special
      Case 9: If grid(7) = 0 Then grid(7) = OXO
      Case 10: buttonAnimation.enabled = 1: If grid(8) = 0 Then grid(8) = OXO
      Case 11: If grid(9) = 0 Then grid(9) = OXO
      Case 12: targetAnimation.enabled = 1: targetA = 1: ABLight1.state = 1: checkAB
      Case 13: targetAnimation.enabled = 1: targetB = 1: ABLight2.state = 1: checkAB
      Case 14: wireAnimation.enabled = 1
      Case 15: wireAnimation.enabled = 1
    End Select
    If B2SOn Then DOF 108 + Index, DOFPulse
    If SpecialGrid = False Then checkTicTacToe
  End If
End Sub
...
Sub buttonAnimation_Timer
  ...
  Select Case Button
    ...
    Case 4: RedRollOverButton.transz = 3: button = 0
...
Sub wireAnimation_Timer
  wire = wire + 1
  Select Case wire
    Case 1: EVAL ("wire" & targetNumber).transz = -10
    Case 2: EVAL ("wire" & targetNumber).transz = -4
    Case 3: EVAL ("wire" & targetNumber).transz = -1
    Case 4: EVAL ("wire" & targetNumber).transz = 0
        wire = 0
...
'***************Rotate AB Gate
Sub RotateGateOpen
  abDiverterGate.ObjRotZ = 71.5
  abGateWallOpen.isDropped = False
  abGateWallClosed.isdropped = True
End Sub

Sub RotateGateShut
  abDiverterGate.ObjRotZ = 97
  abGateWallOpen.isDropped = True
  abGateWallClosed.isdropped = False
End Sub

'***************Scoring Routines
Dim Flag10, Flag100, Flag1000, Point, Point10, BellRing
...
Sub totalUp(points)
  ...
  If score(player) > 99999 Then
    ...
    EVAL("Overflow" & player).text = "100000"
    If vrOption > 0 Then EVAL("vrRollover" & player).visible = 1
  End If
...
'***************Check Tic Tac Toe
Sub CheckTicTacToe
  If specialGrid = True Then Exit Sub
  For x = 1 to 7 step 3
    If grid(x) > 0 Then
  ...
  If gridFill = 9 Then
    specialGrid = True
    specialLight1.state = 1: specialLight2.state = 1
  End If
  If ticTacToe = True and ticTacToeBonus = False Then
...
Sub scoreMotor_timer
  ...
  If scoreMotorLoop < 6 Then playFieldSound "ScoreMotorSingleFire", 0, ebLight1, 0.2
  motorOn = 1

  If saucerFlag = 1 And scoreMotorLoop = 5 Then
    saucer.Enabled = 1
    saucerFlag = 0
    If saucerSide = 0 Then
      PlayFieldSound "SaucerKick",0, leftSaucer, 1
      If B2SOn Then DOF 130, DOFPulse
      kickBall kickerBall, 77, 15, 5, 30
    Else
      PlayFieldSound "SaucerKick",0, rightSaucer, 1
      If B2SOn Then DOF 131, DOFPulse
      kickBall kickerBall, -73, 15, 5, 30
    End If
  End If
...
' The bonus is paid out on the 2nd, 3rd and 4th positions of the score motor, the 6th position is the index reel notch
  If bonusFlag = 1 Then
    Select Case scoreMotorLoop
      Case 2: If lastBall = True Then
            If tilt = False Then totalUp 1000
          End If
      Case 3: If tilt = False Then totalUp 1000
      Case 4: If lastBall = True Then
            If tilt = False Then totalUp 1000
          End If
      Case 6: square = square - 1
          If square < 1 Then
            clearGrid
            clearAB
            scoreMotorLoop = 0
          ...
          End If
          If square > 0 Then scoreMotorLoop = 0
    End Select
  End If
End Sub
