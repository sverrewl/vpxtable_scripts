...
SolModCallBack(6)="Flasher6" 'Flasher 6
SolModCallBack(7)="Flasher7" 'Flasher 7
...
SolModCallBack(17)="SolGIOn"


Sub SolGIOn(level)
  If DebugGI Then debug.print "SolGIOn level = "&level
  SetGI level
End Sub

Sub Flasher6(level)
  If DebugFlashers Then debug.print "Flasher6 level = "&level
  F6a.state = level
  F6b.state = level
  F6c.state = level
  F6d.state = level
...
Sub Flasher7(level)
  If DebugFlashers Then debug.print "Flasher7 level = "&level
End Sub
...
' 1.0.2 - apophis - Fixed VR led backlass digits issue. Updated AudioFade and AudioPan.
' 1.0.3 - DGrimmReaper - Added VR backglass lamps
' 1.0.4 - apophis - Attempt to add PWM effects to lighting (assmumes latest VPinMame 3.7 build)
' 1.0.5 - DGrimmReaper - backglass digits fix. reduction in playfield reflections.  Debug Flashers and GI Off.
