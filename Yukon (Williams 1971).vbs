...
Const cGameName = "Yukon_1971"
Const B2STableName = "Yukon_1971"
...
sub loadhs
  ...
  Exit Sub
  End if
  If Not FileObj.FileExists(UserDirectory & "Yukon.txt") then
  Exit Sub
  End if
  Set ScoreFile=FileObj.GetFile(UserDirectory & "Yukon.txt")
...
sub gamblescore
  ...
  if light1.state=0 and light2.state=0 and light3.state=1 and light4.state=0 and light5.state=0 then addscore 4000: bip=bip+1: playsound "knocke":if bip>9 then bip=9: end if:If B2SOn then Controller.B2SSetBallInPlay bip: End If:end if
  if light1.state=0 and light2.state=0 and light3.state=0 and light4.state=1 and light5.state=0 then addscore 5000: bip=bip+2: playsound "knocke":if bip>9 then bip=9: end if:If B2SOn then Controller.B2SSetBallInPlay bip: End If: End If
  if light1.state=0 and light2.state=0 and light3.state=0 and light4.state=0 and light5.state=1 then addscore 5000: bip=bip+3: playsound "knocke":if bip>9 then bip=9: end if:If B2SOn then Controller.B2SSetBallInPlay bip: End If: End If
end sub
