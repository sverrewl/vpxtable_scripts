...
' Thalamus - increased the ballroll volume a bit, seems ok
...
Sub NextBall
  if BallinPlay < BallsperGame Then
    ...
    PlaySoundAtVol "Plunger_feed", Plunger, 1
...
Sub PostKicker_Hit
  PlaySoundAtVol "flip_hit_2", ActiveBall, 1
End Sub
...
Sub ShooterLaneLaunch_Hit
  if ActiveBall.vely < -8 then playsoundAtVol "Launch", Plunger, 1
  DOF 124, DOFPulse
End Sub
...
Sub BallShadowUpdate_timer()
    ...
    For b = 0 to UBound(BOT)
        If BOT(b).X < Cavalcade.Width/2 Then
            BallShadow(b).X = ((BOT(b).X) - (Ballsize/6) + ((BOT(b).X - (Cavalcade.Width/2))/21)) + 6
        Else
            BallShadow(b).X = ((BOT(b).X) + (Ballsize/6) + ((BOT(b).X - (Cavalcade.Width/2))/21)) - 6
        End If
