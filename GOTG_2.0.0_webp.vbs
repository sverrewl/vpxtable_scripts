...
PupPack_verified = "2.0.0" ' ONLY CHANGE WITH PUP PACK
...
Sub DMDSettingsInit()
  ...
  For i = 0 to MaxDMDSetting-1
    ...
    select case i
      ...
      case 11:DMDMenu(i).Setup "FREE PLAY",             kDMDStd_FreePlay,   -1,   "BOOL", 0
    ...
    if BallsOnPlayfield = 0 and bInService=False then         ' Dont allow this if balls are still on the PF
      if LFPress and RFPress then
        ...
        If keycode = LeftFlipperKey Then
          ...
          if pCurAttractPos=3 and LastScore(0)=0 then pCurAttractPos=2  ' No last high score - skip again
          pAttractNext
        End if
...
Sub EndOfBallComplete()
  ...
  dim i
    ...
    if Match = 0 then
      playUnderMedia "PupVideos", "Match-00.mp4", pUnderVid, -1
'     playmedia "Match-00.mp4", "PupVideos", pOverVid, "", -1, "", 1, 1
    Else
      playUnderMedia "PupVideos", "Match-"&Match&".mp4", pUnderVid, -1
...
Sub AwardMystery()
  ...
  if (bOrbMultiBall or bGrootMultiball or tmrImmolation.Enabled or tmrXandar.Enabled) and bAddedABall=False Then
...
'           End If
          ...
          elseif OrbMBJackpotHits = 8 then      ' ORB DoubleJackpot
            ...
            if tmrOrbHurryUp.UserValue>1000000 then
              thisScore=(tmrOrbHurryUp.UserValue \ 100000)/10 & "M"
            else
              thisScore=(tmrOrbHurryUp.UserValue \ 100)/10 & "K"
            End If
        ...
        Else
          thisScore=tmrOrbHurryUp.UserValue
          ...
          if thisScore>1000000 then
            thisScore=(thisScore \ 100000)/10 & "M"
          else
            thisScore=(thisScore \ 100)/10 & "K"
          End If
...
Debug.print "StartQQFinalHurryup:"
  HurryUpFinish
  tmrHurryUp.UserValue=INT(Score(CurrentPlayer) / 10)   ' 10% of score
...
Sub DoCBombComplete()
  ...
  ScenePlayMessage2 "Video-0x00B1.mp4", "SUPER", FormatScore(WizardModePoints) ,"JACKPOT", 4
...
Sub OrbPauseDone()  ' Drop ball after effects
  ...
  if (OrbMultiBallProgress = 3) Then        ' Third time dont release the ball (They have to hit it to start 3 multiball)
    ...
    tmrOrbHurryUp.Interval = 3500                   ' Initial delay (after the ball gets to the top)
...
debug.print "Game Over"
  ...
  if bUsePUPDMD then
    ...
    if (bFirstStart=False) then   ' Dont run this the 1st time through. It crashes Pup
      pAttractStartIdx 2    ' Take too long to start and kills Game Over \
      pAttractNext
    End if
    bFirstStart=False
  End If
...
Const pAttractInstantInfoIdx=17
Const pAttractEndIdx=22
...
debug.print "ATTRACT Next:" & pCurAttractPos & " " & bGameInPlayHidden & " " & bGameInPlay
  ...
  if bGameInPlay and pCurAttractPos=1 then pCurAttractPos=4   ' During InstantInfo skip Intro
