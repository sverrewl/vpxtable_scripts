'*
'*        Originally - Gottlieb's Tiger (1975)
'*        Table build/scripted by Loserman!
'*        Playfield/Plastics by GNance
'*      Complete redraw of all Art by Manbearpig
'*      nFozzy and Fleep added by Manbearpig
'*      Script help by Flux
'*      Ball shadow by Goldchicco
'*
'*
...
Const VolumeDial = 0.4        ' Recommended values should be no greater than 1.
...
Const HSFileName="Zissou.txt"
Const B2STableName="Zissou"
...
dim TigerComplete, WOWCounter, TiltEndsGame, NoveltyMode
...
Sub Table1_Init()
  ...
  next
  If Credits > 0 then
    CreditWhenLit.Image = "CreditWhenLit-Lit"
  Else
    CreditWhenLit.Image = "CreditWhenLit-Unlit"
  End If
  bump1=1
  bump2=1
  bump3=1
  InitPauser5.enabled=true
  If Credits > 0 Then DOF 130, DOFOn
End Sub
...
'END GNMOD
    Credits=Credits-1
    If Credits < 1 Then DOF 130, DOFOff
...
Sub TargetRight2_Hit()
  If TableTilted=false then
    DOF 114, DOFPulse
    SetMotor(1000)
    If RightWOWLight.state=1 then
      AddSpecial
      RightWOWLight.state=0
    end if
  end if
end Sub
...
Sub CheckAllTigerLightsOff
  dim x1
  If TigerComplete=true then
    exit sub
  end if
  x1=0
  for each obj in TigerLights
    if obj.state=0 then
      x1=x1+1
    end if
  next
...
Sub LeftVT_Hit(idx)
  ...
  If ActiveBall.VelY <0 Then
    ActiveBall.VelY = ActiveBall.VelY * 0.87
...
Sub vt18_Hit()
  ...
  IncreaseBonus
  IncreaseBonus
  if VariTargetSpecial.visible=True then
    AddSpecial
  end if
End Sub
...
'        LeftVari(LeftPosition_Vari).IsDropped = False
    ...
    If LeftPosition_Vari = 14 AND Lvariawarded = 0 Then
      SetMotor(3000): Lvariawarded = 1
      IncreaseBonus
      IncreaseBonus
      IncreaseBonus
    End If
    If LeftPosition_Vari = 11 AND Lvariawarded = 0 Then
      SetMotor(2000): Lvariawarded = 1
    ...
    If LeftPosition_Vari = 4 AND Lvariawarded = 0 Then
      SetMotor(1000): Lvariawarded = 1

      IncreaseBonus

    End If
    If LeftPosition_Vari = 1 AND Lvariawarded = 0 Then
      AddScore(1000): Lvariawarded = 1
      IncreaseBonus
    End If
        If LeftPosition_Vari = 0 Then
      Lvariawarded = 0
...
Sub AddSpecial()
  ...
  WOWLights(WOWCounter-1).state=1
  ...
  Select Case SpecialLightFlag
    case 1:
      if BallsPerGame=5 then
        LightingFlags="0100101100"
      else
        LightingFlags="0110111111"
      end if
    case 2:
      if BallsPerGame=5 then
        LightingFlags="0100100100"
      else
        LightingFlags="0110110110"
      end if
    case 3:
      if BallsPerGame=5 then
        LightingFlags="0000100100"
      else
        LightingFlags="0000110110"
      end if
  end select
  if Mid(LightingFlags,HorseshoeCounter+1,1)="1" then
    VariTargetSpecial.visible=1
  else
    VariTargetSpecial.visible=0
  end if
end sub
...
Sub LightPlayerUp
  for xx=1 to 4
    if B2SOn then
      Controller.B2SSetData 90+xx,0
    end if
  next
  Controller.B2SSetData 90+Player,1
end sub
...
Sub LightPlayerUp2
  ...
  next
  Controller.B2SSetData 90+TempPlayerUp,1
end sub
...
sub nextball
  ...
  If Player>Players Then
    If WOWCounter>0 then
      ...
      WOWLights(WOWCounter-1).state=0
      WOWCounter=WOWCounter-1
...
sub loadhs
  ...
  Set TextStr=ScoreFile.OpenAsTextStream(1,0)
    ...
    TiltEndsGame=cdbl(temp16)
    NoveltyMode=cdbl(temp17)
    if HighScore=1 then
      HSScore(1) = int(temp6)
      HSScore(2) = int(temp7)
      HSScore(3) = int(temp8)
...
Sub SetupReplayTables
  ...
  Replay1Table(2)=40000
  Replay1Table(3)=50000
  Replay1Table(4)=60000
  Replay1Table(5)=44000
  Replay1Table(6)=49000
  Replay1Table(7)=53000
  Replay1Table(8)=56000
  Replay1Table(9)=60000
  Replay1Table(10)=63000
  Replay1Table(11)=65000
  Replay1Table(12)=72000
  Replay1Table(13)=77000
  Replay1Table(14)=81000
  Replay1Table(15)=81000

  Replay2Table(1)=80000
  Replay2Table(2)=90000
  Replay2Table(3)=100000
  Replay2Table(4)=120000
  Replay2Table(5)=58000
  Replay2Table(6)=63000
  Replay2Table(7)=67000
  Replay2Table(8)=70000
  Replay2Table(9)=74000
  Replay2Table(10)=77000
  Replay2Table(11)=79000
  Replay2Table(12)=86000
  Replay2Table(13)=91000
  Replay2Table(14)=95000
  Replay2Table(15)=95000

  Replay3Table(1)=120000
  Replay3Table(2)=130000
  Replay3Table(3)=140000
  Replay3Table(4)=170000
  Replay3Table(5)=66000
  Replay3Table(6)=71000
  Replay3Table(7)=75000
  Replay3Table(8)=78000
  Replay3Table(9)=82000
  Replay3Table(10)=85000
  Replay3Table(11)=87000
  Replay3Table(12)=94000
  Replay3Table(13)=99000
  Replay3Table(14)=99000
...
Sub AddScore2(x)
  ...
  if Score(Player)=>100000 then
    RolloverReel.setvalue(Int (Score(Player)/100000))
    If B2SOn Then
      If Player=1 Then
...
'*************************************************
'      Ball Rolling and Drop Sounds (Updated)
'*************************************************
  ...
  ' play the rolling sound for each ball
    ...
    For b = 0 to UBound(BOT)
        If BallVel(BOT(b) ) > 1 AND BOT(b).z < 30 Then
            rolling(b) = True
            PlaySound ("BallRoll_" & b), -1, VolPlayfieldRoll(BOT(b)) * BallRollVolume, AudioPan(BOT(b)), 0, PitchPlayfieldRoll(BOT(b)), 1, 0, AudioFade(BOT(b))
...
Sub HighScoreTimer_Timer
  ...
  elseif InProgress then
    ...
    EMReel5.SetValue(HSScore(1))
    If B2SOn then
      Controller.B2SSetScorePlayer2 HSScore(1)
    end if
...
Const OptionLine4="Special light setting"
Const OptionLine5="Tilt penalty"
Const OptionLine6="Game Mode"
Const OptionLine7=""
    ...
    Select Case linecounter
      ...
      Case 4:
        ...
        Select Case SpecialLightFlag
          Case 1:
            if BallsPerGame=5 then
              tempstring = "Liberal/Balls 2,5,7,8"
            else
              tempstring = "Liberal/Balls 2,3,5,6,7,8,9"
            end if
          Case 2:
            if BallsPerGame=5 then
              tempstring = "Medium/Balls 2,5,8"
            else
              tempstring = "Medium/Balls 2,3,5,6,8,9"
            end if
          Case 3:
            if BallsPerGame=5 then
              tempstring = "Conservative/Balls 5,8"
            else
              tempstring = "Conservative/Balls 5,6,8,9"
            end if
        end Select
      ...
      Case 5:
        ...
        If TiltEndsGame=1 then
          tempstring = "Current ball plus 1 ball"
        else
          tempstring = "Current ball"
        end if
...
'****************************************
'*         Attract Mod: MauiPunter      *
'****************************************

Sub AttractMusic
  If MusicOption1 = 1 then
    PlayMusic "Zissou\01-Open Sea Theme.mp3"
  End If
End Sub

'**************************************
'*         Music Mod: MauiPunter      *
'**************************************

Dim musicNum
Sub NextTrack
  If MusicOption2 = 1 then
    musicNum = INT(20 * RND(1) )
    If musicNum = 0 Then PlayMusic "Zissou\01-Open Sea Theme.mp3" End If
    If musicNum = 1 Then PlayMusic "Zissou\02-Rebel Rebel.mp3" End If
    If musicNum = 2 Then PlayMusic "Zissou\03-Heres to you Nicola and Bart.mp3" End If
    If musicNum = 3 Then PlayMusic "Zissou\04-Search and Destroy.mp3" End If
    If musicNum = 4 Then PlayMusic "Zissou\05-Queen Bitch.mp3" End If
    If musicNum = 5 Then PlayMusic "Zissou\06-Let Me Tell You About My Boat.mp3" End If
    If musicNum = 6 Then PlayMusic "Zissou\07-Gut Feeling.mp3" End If
    If musicNum = 7 Then PlayMusic "Zissou\08-30 Century Man.mp3" End If
    If musicNum = 8 Then PlayMusic "Zissou\09-Life On Mars.mp3" End If
    If musicNum = 9 Then PlayMusic "Zissou\10-The Way I Feel Inside.mp3" End If
    If musicNum = 10 Then PlayMusic "Zissou\11-Zissou Society Blue Star Cadets.mp3" End If
    If musicNum = 11 Then PlayMusic "Zissou\12-Shark Attack Theme.mp3" End If
    If musicNum = 12 Then PlayMusic "Zissou\13-La Nina de Puerta Oscura.mp3" End If
    If musicNum = 13 Then PlayMusic "Zissou\14-Starman.mp3" End If
    If musicNum = 14 Then PlayMusic "Zissou\15-We Call Them Pirates Out Here.mp3" End If
    If musicNum = 15 Then PlayMusic "Zissou\16-Loquasto International Film Festival.mp3" End If
    If musicNum = 16 Then PlayMusic "Zissou\17-Five Years.mp3" End If
    If musicNum = 17 Then PlayMusic "Zissou\18-Ping Island-Lightning Strike Rescue Op.mp3" End If
    If musicNum = 18 Then PlayMusic "Zissou\19-Rock N Roll Suicide.mp3" End If
    If musicNum = 19 Then PlayMusic "Zissou\20-Life on Mars.mp3" End If
  End If
End Sub
