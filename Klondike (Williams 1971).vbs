...
Const cGameName = "Klondike_1971"
Const B2STableName = "Klondike_1971"
...
Sub Table1_Init
  ...
  Ramp3.image = "GoldRushReel_3_"&(rv(8)+1)
  checkreels
...
Sub Table1_KeyDown(ByVal keycode)
  ...
  If keycode = LeftFlipperKey Then
    ...
    PlaySound "FlipperUp", LeftFlipper, 1
  End If

  If keycode = RightFlipperKey Then
    RightFlipper.RotateToEnd
        PlaySound "FlipperUp", RightFlipper, 1
  End If
...
Sub Table1_KeyUp(ByVal keycode)
  ...
  If keycode = LeftFlipperKey Then
    ...
    stopsound "buzz"
    if state=true and tilt=false then PlaySound "FlipperDown", LeftFlipper, 1
  End If

  If keycode = RightFlipperKey Then
    RightFlipper.RotateToStart
    stopsound "buzz"
        if state=true and tilt=false then PlaySound "FlipperDown", RightFlipper, 1
  End If
...
sub addscore(points)
  ...
  If B2SOn then
    ...
    if truesc(currpl)=>replay3 and rep(currpl)=2 then
    credit=credit+1
    playsound "knocke"
    credittxt.text=credit
  If B2SOn then
...
sub ballrelease_hit
  if ballrelenabled=1 then PlaySoundAt "launchball", ActiveBall: ballrelenabled=0: end if
end sub

Sub Drain_Hit()
  playsound "drainshorter", Drain, 1
  Drain.DestroyBall
  nextball
End Sub
...
Sub RightSlingShot_Slingshot
  if tilt = false then
    PlaySoundAt "right_slingshot", ActiveBall, 1
    RSling.Visible = 0
    RSling1.Visible = 1
    sling1.TransZ = -20
    RStep = 0
...
Sub LeftSlingShot_Slingshot
  if tilt=false then
  PlaySoundAt "left_slingshot", ActiveBall, 1
  LSling.Visible = 0
  LSling1.Visible = 1
  sling2.TransZ = -20
  LStep = 0
...
sub loadhs
  ...
  Exit Sub
  End if
  If Not FileObj.FileExists(UserDirectory & "Klondike.txt") then
  Exit Sub
  End if
  Set ScoreFile=FileObj.GetFile(UserDirectory & "Klondike.txt")
...
sub ck_hit
  ...
  PlaySoundAt "kickerkick", ActiveBall
...
sub lk_hit
  ...
  if bumperlight1.state=1 and bumperlight2.state=1 then bumperlight5.state=1
    end if
    PlaySoundAt "kickerkick", ActiveBall
    kicktimer.enabled=true
 end sub
...
Sub Bumper5_Hit()
  If Bumperlight5.state=1 then addscore 1000 else addscore 100
  PlaySoundAt "jet1", ActiveBall, 1
End Sub
...
sub checkreels
  ...
  if ((rv(6)=1 or rv(6)=3 or rv(6)=6 or rv(6)=8) and (rv(7)=1 or (rv(7)=3 or rv(7)=6 or rv(7)=8)) and (rv(8)=0 or rv(8)=1 or rv(8)=3 or rv(8)=5 or rv(8)=7)) then
  light1.state=0
  light2.state=1
  light3.state=0
  light4.state=0
  ...
  if ((rv(6)=2 or rv(6)=7) and (rv(7)=2 or rv(7)=7) and (rv(8)=0 or rv(8)=2 or rv(8)=6 or rv(8)=9)) then
  light1.state=0
  light2.state=0
  light3.state=1
  light4.state=0
  ...
  if ((rv(6)=4 or rv(6)=9) and (rv(7)=4 or rv(7)=9) and (rv(8)=0 or rv(8)=4 or rv(8)=8)) then
  light1.state=0
  light2.state=0
  light3.state=0
  light4.state=1
  ...
  if ((rv(6)=0 or rv(6)=5) and (rv(7)=0 or rv(7)=5) and (rv(8)=0)) then
  light1.state=0
  light2.state=0
  light3.state=0
  light4.state=0
...
sub gamblescore
  if light1.state=1 and light2.state=0 and light3.state=0 and light4.state=0 and light5.state=0 then addscore 500: end if
  if light1.state=0 and light2.state=1 and light3.state=0 and light4.state=0 and light5.state=0 then addscore 3000: end if
  if light1.state=0 and light2.state=0 and light3.state=1 and light4.state=0 and light5.state=0 then addscore 4000: bip=bip+1: if bip>9 then bip=9: end if:If B2SOn then Controller.B2SSetBallInPlay bip: End If:end if
  if light1.state=0 and light2.state=0 and light3.state=0 and light4.state=1 and light5.state=0 then addscore 5000: bip=bip+2: if bip>9 then bip=9: end if:If B2SOn then Controller.B2SSetBallInPlay bip: End If: End If
  if light1.state=0 and light2.state=0 and light3.state=0 and light4.state=0 and light5.state=1 then addscore 5000: Credit=credit+1: credittxt.text=credit: If B2SOn then Controller.B2SSetCredits credit: End If:playsound "knocke": End If
end sub
