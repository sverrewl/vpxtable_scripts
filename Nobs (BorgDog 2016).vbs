...
Sub nobs_KeyDown(ByVal keycode)
  ...
  if tilt=false and state=true then
	If keycode = LeftFlipperKey Then
		...
		PlaySound "flipperup", 0, .67, -0.05, 0.05
...
Sub nobs_KeyUp(ByVal keycode)
   ...
   If tilt=false and state=true then
	If keycode = LeftFlipperKey Then
		...
		PlaySound "flipperdown", 0, 1, -0.05, 0.05
		StopSound "Buzz"
	End If
...
Sub RightSlingShot_Slingshot
    PlaySound "slingshot", 0, 1, 0.05, 0.05
	addscore 10
    RSling.Visible = 0
    RSling1.Visible = 1
    sling1.TransZ = -8
...
Sub OnBallBallCollision(ball1, ball2, velocity)
  If nobs.VersionMinor > 3 OR nobs.VersionMajor > 10 Then
