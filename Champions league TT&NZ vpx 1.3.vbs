...
Const cGameName = "uefa_cl"							'this is the table name needed by B2S and by VPreg.stg
Const cAssetsFolder="ChampionsLeague17"				'The folder that contains gifs, videos (the assets folder)
...
Sub OnScoreboardChanged
	...
	If gmover= 0 Then
		...
		Else
			UltraDMD.DisplayScoreboard NewPlayer, CurrentPlayer+1, p1score, p2score, p3score, p4score, creditString, "ball " & ballinplay(CurrentPlayer)
		End If
	Else
		If UltraDMDNew=1 Then
			UltraDMD.DisplayScoreboard00 NewPlayer, CurrentPlayer+1, p1score, p2score, p3score, p4score, creditString, " "
		Else
			UltraDMD.DisplayScoreboard NewPlayer, CurrentPlayer+1, p1score, p2score, p3score, p4score, creditString, " "
		End If
	End If
End Sub
...
'*GAME MUSIC*************************************************************
'************************************************************************
Dim rMusic,MusicList

Sub InitMusic
	Set MusicList = CreateObject("System.Collections.ArrayList")
	MusicList.Add "bgout_UEFACL - Ace of Spades Motorhead.mp3"						'index 0
	MusicList.Add "bgout_UEFACL - Angel Of Death Slayer.mp3"						'index 1
	MusicList.Add "bgout_UEFACL - Muse Assassin.mp3"								'index 2
	MusicList.Add "bgout_UEFACL - Dio Holy Diver.mp3"								'index 3
	MusicList.Add "bgout_UEFACL - Enter Sandman Metallica.mp3"						'index 4
	MusicList.Add "bgout_UEFACL - Hangar 18 Megadeth.mp3"							'index 5
	MusicList.Add "bgout_UEFACL - Iron Maiden Hallow Be Thy Name.mp3"				'index 6
	MusicList.Add "bgout_UEFACL - Judas Priest Painkiller.mp3"						'index 7
	MusicList.Add "bgout_UEFACL - master of puppets Metallica.mp3"					'index 8
	MusicList.Add "bgout_UEFACL - Paranoid Black Sabbath.mp3"						'index 9
	MusicList.Add "bgout_UEFACL - Rime Of The Ancient Mariner Iron Maiden.mp3"		'index 10
	MusicList.Add "bgout_UEFACL - Led Zeppelin Rock And Roll.mp3"					'index 11
	MusicList.Add "bgout_UEFACL - MultiMode.mp3"									'index 12
	MusicList.Add "bgout_UEFACL - MultiMode2.mp3"									'index 13
	MusicList.Add "bgout_UEFACL - GKMode.mp3"										'index 14
	MusicList.Add "bgout_UEFACL - HighScore.mp3"									'index 15
	MusicList.Add "bgout_UEFACL - IntroHS.mp3"										'index 16
	MusicList.Add "bgout_UEFACL - finalVideo.mp3"									'index 17
	MusicList.Add "bgout_UEFACL - win.mp3"											'index 18
	MusicList.Add "bgout_UEFACL - MVPMode.mp3"										'index 19
	MusicList.Add "bgout_UEFACL - Intro.mp3"										'index 20
	MusicList.Add "bgout_UEFACL - Intro2.mp3"										'index 21
End Sub

Sub Music
	If cAttractMode=0 AND gkmm=0 AND VMOn=0 AND MVPOfTheYear(CurrentPlayer)=0 AND NOT (lmultilock.state=1 OR Lmultiticket.state=1 OR Lmultiskill.state=1) Then rMusic=RndNum(0,11):PlayMusic MusicList(rMusic)
	If cAttractMode=0 AND gkmm=0 AND VMOn=0 AND MVPOfTheYear(CurrentPlayer)=0 AND (lmultilock.state=1 OR Lmultiticket.state=1 OR Lmultiskill.state=1) Then rMusic=RndNum(12,13):PlayMusic MusicList(rMusic)
	If GKMM=1 OR VMOn=1 Then PlayMusic MusicList(14)
	If HSPlace>=0 Then PlayMusic MusicList(15)
	If MVPOfTheYear(CurrentPlayer)=1 Then PlayMusic MusicList(19)
End Sub
...
Dim bip,creditgame,b1force,b2force,flforce,TeamSelection,SelectedTeams,CurrentPlayer,NewPlayer,ii,Lampstate(76)
Dim TeamList:TeamList=Array("BARCELONA","B.MUNCHEN","CHELSEA","MAN. CITY","JUVENTUS","PARIS S.G.","REAL MADRID")
Dim choiceteam(3),numBump(3), JetLevel(3), SkillShotMade(3),AllPlayersLampStates(3,76)
...
Sub Table1_Init()
	...
	RailL.visible=Table1.ShowDT
	RailR.visible=Table1.ShowDT
...
'*GAME INTRO*************************************************************
'************************************************************************
Dim rndIntro,introtime,displaystart:displaystart=0
Sub skipultradmd_timer
	If NOT UltraDMD.IsRendering Then
		me.enabled=0
		GIOn:StartAttractMode
		DisplayIntro.Interval=100
...
Sub DisplayIntro_timer()
	If NOT UltraDMD.IsRendering AND cAttractMode=1 Then
		...
		If displaystart=0 Then
			...
			If rndIntro=1 Then PlayMusic MusicList(20):introtime=41500
			If rndIntro=2 Then PlayMusic MusicList(21):introtime=38000
		End If
...
Sub GameInit
	...
	wkl1.isdropped=1:Wkl2.isdropped=1
	wkc1.isdropped=1:Wkc2.isdropped=1
	wkr1.isdropped=1:Wkr2.isdropped=1
	...
	DisplayInfoTimer.Enabled=0:icc=0
	...
	StopFlash TDFL1,0:StopFlash TDFL2,0:StopFlash TDFL3,0:StopFlash lClwin,0:StopFlash Fl4a,0
...
Sub ResetAllToys
	...
	If headLD.Z>0 Then bomberLdown.enabled=1:PlaySound SoundFXDOF("fx_postdown",134,DOFPulse,DOFDropTargets)
	If headRD.Z>0 Then bomberRdown.enabled=1:PlaySound SoundFXDOF("fx_postdown",135,DOFPulse,DOFDropTargets)
...
'****TEAM SELECTION************************************************************
	If keycode = LeftFlipperKey AND TeamSelection=1 Then
		If TeamLogos(0).state=2 OR TeamLogos(1).state=2 OR TeamLogos(2).state=2 OR TeamLogos(3).state=2 OR TeamLogos(4).state=2 OR TeamLogos(5).state=2 OR TeamLogos(6).state=2 Then
			For Each ii in TeamLogos:ii.state=0:Next
			MainB2SLampsOff
		End If
		choiceteam(CurrentPlayer)=choiceteam(CurrentPlayer)-1
		If choiceteam(CurrentPlayer)>7 Then choiceteam(CurrentPlayer)=1
		If choiceteam(CurrentPlayer)<1 Then choiceteam(CurrentPlayer)=7
		screenprim.image="logo" & choiceteam(CurrentPlayer):TeamLogos(choiceteam(CurrentPlayer)-1).state=2:FlashB2S choiceteam(CurrentPlayer),-1,250,0
		UltraDMD.CancelRendering
		UltraDMD.DisplayScene00Ex "logo" & choiceteam(CurrentPlayer) &".png","SELECT YOUR TEAM", 14, 3, TeamList(choiceteam(CurrentPlayer)-1), 1, 14, UltraDMD_Animation_None, 20000, UltraDMD_Animation_None
	End If
...
'****DISPLAY INFO ************************************************************
	If keycode = LeftFlipperKey AND DisplayInfo=0 AND cAttractMode=0 AND TeamSelection=0 AND FK=0 AND GKMM=0 AND VMOn=0 AND Tilted=0 Then DisplayInfoTimer.Enabled=1
	If keycode = RightFlipperKey AND DisplayInfo=0 AND cAttractMode=0 AND TeamSelection=0 AND FK=0 AND GKMM=0 AND VMOn=0 AND Tilted=0 Then DisplayInfoTimer.Enabled=1
...
'****GKMOVEMODE************************************************************
    If keycode = LeftFlipperKey AND GKMM=1 AND Tilted=0 Then
		PlaySound SoundFXDOF("fx_DropTargetUp",101,DOFOn,DOFFlippers)
	...
	If keycode = RightFlipperKey AND GKMM=1 AND Tilted=0 Then
		PlaySound SoundFXDOF("fx_DropTargetUp",102,DOFOn,DOFFlippers)
...
'*****PLUNGER**************************************************************
	If keycode = PlungerKey Then
		If TeamSelection=1 Then
			Opponents = RemoveArrayElement(TeamList, TeamList(choiceteam(CurrentPlayer)-1))
			StartSkillShot:Schedule.Enabled=1:exit sub
		End If
		If cAttractMode=0 AND TeamSelection=0 Then
			Plunger.Fire
			If ballInPlunger Then
				PlaySound SoundFXDOF("fx_AutoPlunger", 125, DOFPulse, DOFContactors),0,1,0,0.25
			End If
		End If
	End If
...
'*****TILT**************************************************************
	If KeyCode=LeftTiltKey Then
		...
		Nudge 90,RndNudge
		PlaySound SoundFX("fx_nudge",0),0,1,-0.05,0.25
	...
	If KeyCode=CenterTiltKey Then
		...
		RndNudge=RndNudge+2.25
		Nudge 0,RndNudge
		PlaySound SoundFX("fx_nudge",0),0,1,0,0.25
		If GKMM=0 AND VMOn=0 AND tilted=0 AND BIP>0 Then AddToTilt(RndNudge)
	End If
...
'****DISPLAY INFO ************************************************************
	If keycode = LeftFlipperKey AND DisplayInfoTimer.Enabled Then
		DisplayInfoTimer.Enabled=0:icc=0
	End If

	If keycode = LeftFlipperKey AND DisplayInfo=1 Then
		UltraDMD.CancelRendering:CheckGameInfo.Enabled=0:DisplayInfo=0:SCReset.Enabled=1:icc=0
	End If

	If keycode = RightFlipperKey AND DisplayInfoTimer.Enabled Then
		DisplayInfoTimer.Enabled=0:icc=0
	End If

	If keycode = RightFlipperKey AND DisplayInfo=1 Then
		UltraDMD.CancelRendering:CheckGameInfo.Enabled=0:DisplayInfo=0:SCReset.Enabled=1:icc=0
	End If
...
Sub ReleaseNewBall
	...
	BallRelease.CreateSizedBallWithMass Ballsize/2,BallMass:BallRelease.Kick 90, 7:PlaySound SoundFXDOF("fx_ballrel",121,DOFPulse,DOFContactors),0,1,0,0.25
...
Sub KGoalExit
	kickergoal.CreateSizedBallWithMass Ballsize/2,BallMass:PlaySound SoundFXDOF("fx_saucer_exit",122,DOFPulse,DOFContactors),0,1,0,0.25:kickergoal.Kick 0,10,1.56
	FlashForMs fl3,300,1000,0:DOF 124,DOFPulse
	FlashForMs fl3a,300,1000,0
End Sub
...
Sub Plunger_timer()
	...
	Plunger.Fire:PlaySound SoundFXDOF("fx_AutoPlunger", 125, DOFPulse, DOFContactors),0,1,0,0.25
End Sub
...
Sub DisplayInfoTimer_timer
	icc=icc+1
	'after 7 seconds
	If icc=69 AND UltraDMD.IsRendering Then UltraDMD.CancelRendering
	If icc=70 Then
		DisplayInfoTimer.Enabled=0
		DisplayInfo=1
		UltraDMD.DisplayScene00Ex "","   GAME INFO   ",14,-1,"",14,1,UltraDMD_Animation_None,2000,UltraDMD_Animation_None
		ShowGameInfo
	End If
End Sub

Sub CheckGameInfo_timer
	'restart the game info scene if key stay pressed
	If DisplayInfo=1 AND NOT UltraDMD.IsRendering Then
		Me.Enabled=0
		ShowGameInfo
	End If
End Sub
...
Sub ShowGameInfo
	...
	UltraDMD.DisplayScene00Ex "","  BALON DOR:   ",14,-1," " & (5-completed(CurrentPlayer)) & " MODES TO GO ",14,1,UltraDMD_Animation_None,2000,UltraDMD_Animation_None
...
Sub SelectTheTeam
	screenprim.image="logo" & choiceteam(CurrentPlayer):TeamLogos(choiceteam(CurrentPlayer)-1).state=2:FlashB2S choiceteam(CurrentPlayer),-1,250,0:StopSound "CLAPPING2":PlaySound "CLAPPING2"
	UltraDMD.DisplayScene00Ex "logo" & choiceteam(CurrentPlayer) &".png","SELECT YOUR TEAM", 14, 3, TeamList(choiceteam(CurrentPlayer)-1), 1, 14, UltraDMD_Animation_None, 20000, UltraDMD_Animation_None
End Sub
...
Sub Drain_Hit()
	...
	If GKMM=0 AND BIP=0 AND maxballs(CurrentPlayer)=(BallsPerGame + extraballmax(CurrentPlayer)) AND Lkag.state=0 Then
		...
		If Tilted=0 Then
			...
			PlaySound "DelusioneParata"
			Drain.TimerInterval=2000:Drain.TimerEnabled=1
			StopFlash lClWin,0:StopFlashB2S B2SLogoCL,0
			GiOff:StopMusic:cBonusM=1
		End If
	End If
...
If Tilted=0 Then
    ...
    If  L20.STATE=1 AND L21.STATE=1 AND gkmm=0 AND NOT moveGK.Enabled AND NOT (Lmultilock.state=1 OR Lmultiskill.state=1 OR Lmultiticket.state=1) Then
		...
		lpin.isdropped=0:rpin.isdropped=0:PlaySound SoundFXDOF("fx_postup",133,DOFPulse,DOFDropTargets)
...
Sub waitforkick_timer()
    ...
    If wfk=65 Then PlaySound "fischio":GiOn
    If wfk=70 Then lpin.isdropped=1:rpin.isdropped=1:PlaySound SoundFXDOF("fx_postdown",133,DOFPulse,DOFDropTargets):waitforkick.enabled=0:wfk=0:UltraDMD.CancelRendering:OnScoreboardChanged:exit sub
End Sub
...
Sub triggercorner_hit
	...
	If NOT moveGK.enabled AND Lgoal2.state=0 AND Lcorner.state=1 Then
		...
		FlashForMs fl3,2000,50,0
		FlashForMs fl3a,2000,50,0
		DOF 144, DOFPulse
	End If
End Sub
...
If Tilted=0 Then
  	PlaySound SoundFXDOF("fx_bumperL",107,DOFPulse,DOFContactors)
	FlashForMs fl2, 100, 50, 0:FlashForMs fl2a, 100, 50, 0:FlashForMs light30, 100, 50, 0:FlashB2S B2SLight2,50,25,0
...
Sub Bumper2_Hit
	...
	dmd_bumpersLvl6=UltraDMD.CreateAnimationFromImages (15, true, "bump6_1.png,bump6_2.png,bump6_3.png,bump6_4.png,bump6_5.png,bump6_6.png")
If Tilted=0 Then
  	PlaySound SoundFXDOF("fx_bumperR",109,DOFPulse,DOFContactors)
	FlashForMs fl1, 100, 50, 0:FlashForMs fl1a, 100, 50, 0:FlashForMs light32, 100, 50, 0:FlashB2S B2SLight1,50,25,0
...
If Tilted=0 Then
	...
	me.timerenabled=1
	PlaySound SoundFXDOF("fx_target", 111, DOFPulse, DOFTargets), 0, 1, pan(ActiveBall)
...
If Tilted=0 Then
	me.timerinterval=1000
	me.timerenabled=1
	PlaySound SoundFXDOF("fx_target", 104, DOFPulse, DOFTargets), 0, 1, pan(ActiveBall)
    If headLD.z<0 AND headRD.z<0 AND LMultiLock.state=0 AND lmultiskill.state=0 AND lmultiTicket.state=0 AND MVPOfTheYear(CurrentPlayer)=0 AND FKAward(CurrentPlayer)=0_
		AND BSMODE(CurrentPlayer)=0 AND SemiFinalAward(CurrentPlayer)=0 AND DefenceLocked(CurrentPlayer)=0 AND GKMM=0 AND TrapDoorP.Z=0 Then
...
If Tilted=0 Then
	me.timerinterval=1000
	me.timerenabled=1
	PlaySound SoundFXDOF("fx_target", 106, DOFPulse, DOFTargets), 0, 1, pan(ActiveBall)
    If headLD.z<0 AND headRD.z<0 AND LMultiLock.state=0 AND lmultiskill.state=0 AND lmultiTicket.state=0 AND MVPOfTheYear(CurrentPlayer)=0 AND FKAward(CurrentPlayer)=0_
		AND BSMODE(CurrentPlayer)=0 AND SemiFinalAward(CurrentPlayer)=0 AND DefenceLocked(CurrentPlayer)=0 AND GKMM=0 AND TrapDoorP.Z=0 Then
...
If Tilted=0 Then
	me.timerinterval=1000
	me.timerenabled=1
	PlaySound SoundFXDOF("fx_target", 108, DOFPulse, DOFTargets), 0, 1, pan(ActiveBall)
    If headLD.z<0 AND headRD.z<0 AND LMultiLock.state=0 AND lmultiskill.state=0 AND lmultiTicket.state=0 AND MVPOfTheYear(CurrentPlayer)=0 AND FKAward(CurrentPlayer)=0_
		AND BSMODE(CurrentPlayer)=0 AND SemiFinalAward(CurrentPlayer)=0 AND DefenceLocked(CurrentPlayer)=0 AND GKMM=0 AND TrapDoorP.Z=0 Then
...
Sub bomberT		'when you collect all boots
	...
	scores(CurrentPlayer)=scores(CurrentPlayer)+100000:PlaySound SoundFXDOF("fx_postup",134,DOFPulse,DOFDropTargets):DOF 135,DOFPulse
...
If Tilted=0 Then
    ...
    PlaySound SoundFXDOF("fx_hitplayer",119,DOFPulse,DOFShaker):bouncingR.Enabled = 1:brake=0:perc=perc+3:sbou=(Rndnum(-10,10))/10
	...
	If brhit(CurrentPlayer)=3 AND DefenceLocked(CurrentPlayer)=0 Then
		...
		If StrikerMode(CurrentPlayer)=2 Then
			...
			If gkmodel.state=1 Then
				...
				UDMD_Display "black.bmp", "Shoot the", 14,-1,"Snack Bar",14,-1, UltraDMD_Animation_None, 1500, UltraDMD_Animation_FadeOut,10
				tdfcolor=2:OpenTrapDoor True:StrikerModeStart(CurrentPlayer)=1
			Else
				UDMD_Display "black.bmp", "Still One", 14,-1,"Striker Up",14,-1, UltraDMD_Animation_None, 1500, UltraDMD_Animation_FadeOut,5
			End If
		End If
		...
		If StrikerMode(CurrentPlayer)=3 Then
			...
			If videomodel.state=1 Then
				...
				UDMD_Display "black.bmp", "Shoot the", 14,-1,"Snack Bar",14,-1, UltraDMD_Animation_None, 1500, UltraDMD_Animation_FadeOut,10
				tdfcolor=3:OpenTrapDoor True:StrikerModeStart(CurrentPlayer)=1
			Else
				UDMD_Display "black.bmp", "Still One", 14,-1,"Striker Up",14,-1, UltraDMD_Animation_None, 1500, UltraDMD_Animation_FadeOut,5
			End If
		End If
		PlaySound SoundFXDOF("fx_postdown",135,DOFPulse,DOFDropTargets)
...
Sub hitRD_Timer
	If NOT UltraDMD.IsRendering Then
		...
		If BLHit(CurrentPlayer)>0 Then bomberLup.enabled=1:brake2=0:perc2=perc2+3:sbou2=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",134,DOFPulse,DOFDropTargets)
		'move the right bomber up because it was up before the player locked the defence
		If BRHit(CurrentPlayer)>0 Then bomberRup.enabled=1:brake=0:perc=perc+3:sbou=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",135,DOFPulse,DOFDropTargets)
	End If
End Sub
...
If Tilted=0 Then
	...
	LightEffect 0:GIeffect 0:FlasherEffect 3
    PlaySound SoundFXDOF("fx_hitplayer",119,DOFPulse,DOFShaker):bouncingL.Enabled = 1:brake2=0:perc2=perc2+3:sbou2=(Rndnum(-10,10))/10
    ...
    If blhit(CurrentPlayer)=3 AND DefenceLocked(CurrentPlayer)=0 Then
       ...
       If StrikerMode(CurrentPlayer)=3 Then
			...
			End If
		End If
		PlaySound SoundFXDOF("fx_postdown",134,DOFPulse,DOFDropTargets)
...
Sub SJDelay_Timer
	Me.Enabled=0
	FlasherEffect 9:StopFlashB2s B2Sbkgnd,1:FlashB2S B2SLight1,2000,200,0:FlashB2S B2SLight2,2000,200,0:FlashB2S B2SLight3,2000,200,0
...
Sub EBAwardTimer_timer
	...
	If sfcc=1 Then
		...
		UDMD_Display "black.bmp", "Shoot the", 14,-1,"Snack Bar",14,-1, UltraDMD_Animation_None, 1500, UltraDMD_Animation_FadeOut,10
	End If
	If sfcc=2 Then Playsound "bell":Fl4.state=1:StopFlash Fl4a,1:StopFlash Fl4,1:Flash Fl4a,150:Flash Fl4,150:DOF 143,DOFPulse
...
'*VIDEO MODE*************************************************************
'************************************************************************
Dim RembMax,VMBall,ballside,g,ballrot,VMScore,AttemptPause,VMScoreBest,EBMade,VModeBS
Dim startvm, VMOn, Remb, Attempt:startvm=0:VMOn=0:Remb=-1:Attempt=1:VModeBS=0

Sub VMode
	Select Case VModeDifficulty
		Case 0	'easy
			screenprim2.image="VMode_15":RembMax=15
		Case 1	'medium
			screenprim2.image="VMode_20":RembMax=20
		Case 2	'hard
			screenprim2.image="VMode_30":RembMax=30
	End Select
	screenprim2.visible=1:screenprim1.image="VMode_sc2"
...
Sub VIDEOMODE_TIMER()
	startvm=startvm+1
	If startvm=400 Then screenprim2.visible=0:screenprim1.image="black"
	If startvm=600 Then PlaySound "bumpmult":headp.visible=1:ballprim.visible=1
	...
	If startvm=1000 Then UltraDMD.CancelRendering:SCreset.Enabled=1:PlaySound "fischio":VMOn=1:Music
	If startvm>1000 Then
		...
		If g>0.1 AND g<0.3 AND ballprim.Z> 185 Then
			...
			Else
				UltraDMD.DisplayScoreboard 1, 1, VMscore, VMscore, VMscore, VMscore, "attempt" & Attempt, "hits" & Remb
			End If
		...
		End If
		If ballprim.z<198 AND ballprim.z>196 Then ballside=((ballprim.x-headp.x)/10):ballrot=ballside*5
		If ballprim.z>196 Then ballprim.x=ballprim.x+ballside :ballprim.roty=ballprim.roty+ballrot
		...
		If ballprim.z<175 Then
			...
			If Attempt>2 Then
				...
				screenprim1.visible=0:headp.visible=0:ballprim.visible=0
				VMBall=0:VMOn=0:ballside=0:g=0:ballrot=0:Remb=-1
				Attempt=1:AttemptPause=0:startvm=0:EBMade=0:VMScoreBest=0
				ballprim.x=434:headp.x=434.1:ballprim.z=200
				PlaySound "DelusioneParata"
				ballrelease.CreateSizedBallWithMass Ballsize/2,BallMass:Ballrelease.kick 90,7:PlaySound SoundFXDOF("fx_ballrel",121,DOFPulse,DOFContactors),0,1,0,0.25:kicked=0:Music
				...
				If Lx1.state=1 AND Lx2.state=1 Then Lx1.state=0:Lx2.state=0
			End If
			If Attempt<3 AND ballprim.z<180 AND AttemptPause>100 Then VIDEOMODE.ENABLED=1:PlaySound "fischio":ballprim.x=434:headp.x=434.1:ballprim.z=200:AttemptPause=0:Attempt=Attempt+1
			VMScore=0
		End If
	End If
End Sub
...
Sub CPUTIMER_TIMER()
	vmcount=vmcount+1
	If vmcount=200 Then screenprim1.image="black"
	If vmcount>400 AND vmcount<450 Then HEADGK.X=387:bootsGK.X=387:bodyGK.X=387:kitGK.X=387:GKPOST.X=387	'left
	If vmcount=401 Then PlaySound SoundFXDOF("fx_DropTargetUp",101,DOFOn,DOFFlippers)
	If vmcount>450 AND vmcount<500 Then HEADGK.X=437:bootsGK.X=437:bodyGK.X=437:kitGK.X=437:GKPOST.X=437	'center
	If vmcount=451 Then PlaySound SoundFXDOF("fx_DropTargetDOWN",101,DOFOff,DOFFlippers)
	If vmcount>500 AND vmcount<550 Then HEADGK.X=487:bootsGK.X=487:bodyGK.X=487:kitGK.X=487:GKPOST.X=487	'right
	If vmcount=501 Then PlaySound SoundFXDOF("fx_DropTargetUp",102,DOFOn,DOFFlippers)
	If vmcount>550 AND vmcount<600 Then HEADGK.X=437:bootsGK.X=437:bodyGK.X=437:kitGK.X=437:GKPOST.X=437	'center
	If vmcount=551 Then PlaySound SoundFXDOF("fx_DropTargetDOWN",102,DOFOff,DOFFlippers)
	If vmcount=600 Then PlaySound "bumpmult":screenprim1.image="GKMode_bg"
	...
	If vmcount=1000 Then UltraDMD.CancelRendering:SCreset.Enabled=1:PlaySound "fischio":Music
	If vmcount>900  Then
		CPUCOUNT=CPUCOUNT+1
		If CPUCOUNT=20 Then tdfcolor=4:OpenTrapDoor True
		If CPUCOUNT>100 AND BALLOFF= 0 AND firstballout=1 Then
			trapkicker.CreateSizedBallWithMass 25,1.1:trapkicker.Kick 180,25,1.56:TrapKickBall
		...
		If  CPUCOUNT=368 AND firstballout=1 Then RightFlipper.RotateToEnd:PlaySound SoundFXDOF("fx_flipperup", 102, DOFOn, DOFFlippers), 0, 1, 0.05, 0.05
		...
		If  CPUCOUNT=401 AND firstballout=1 Then RightFlipper.RotateToStart:PlaySound SoundFXDOF("fx_flipperdown", 102, DOFOff, DOFFlippers), 0, 1, 0.05, 0.05
	End If
End Sub
...
Sub EndGKmm
	...
	End If
	RestoreMatchLampStates:UpdateKit:RightFlipper.Strength=flforce:LeftSlingshot.Disabled=0:RightSlingshot.Disabled=0
	screenprim1.visible=0:screenprim1.image="black":RInlane.collidable=1:RInlaneGK.collidable=0
	GKMode_1.visible=0:GKMode_2.visible=0:GKMode_3.visible=0:GKMode_4.visible=0:GKMode_5.visible=0
	ballrelease.CreateSizedBallWithMass Ballsize/2,BallMass:Ballrelease.kick 90,7:PlaySound SoundFXDOF("fx_ballrel",121,DOFPulse,DOFContactors),0,1,0,0.25:kicked=0
...
Sub GKHOLEin_TIMER()
    ...
    If GKHOLEup.rotx=-5 Then PlaySound SoundFXDOF("fx_postup",130,DOFPulse,DOFGear)
    If GKHOLEup.rotx<-90 Then
		gkholein.enabled=0
		If gkmm=1 Then
			kitgk.image="GK" & choiceteam(CurrentPlayer):headgk.image="k" & choiceteam(CurrentPlayer) & "0"
		End If
	End If
End Sub
...
Sub GKcount_timer
	...
	If moveback=2 AND GKMM=1 Then GKMM=0:Music:MoveGK.Enabled=1:moveback=0:Lshoot.state=2
...
'*SCHEDULE***************************************************************
'************************************************************************
Dim Opponents,skillcount
Sub Schedule_timer
	me.enabled=0
	ShuffleArray Opponents
	for ii=LBound(TeamList) To UBound(TeamList)
		If StrComp(Opponents(0), TeamList(ii), vbTextCompare) = 0 Then teamone(CurrentPlayer)=ii+1
		If StrComp(Opponents(1), TeamList(ii), vbTextCompare) = 0 Then teamtwo(CurrentPlayer)=ii+1
		If StrComp(Opponents(2), TeamList(ii), vbTextCompare) = 0 Then teamthree(CurrentPlayer)=ii+1
		If StrComp(Opponents(3), TeamList(ii), vbTextCompare) = 0 Then teamfour(CurrentPlayer)=ii+1
		If StrComp(Opponents(4), TeamList(ii), vbTextCompare) = 0 Then teamfive(CurrentPlayer)=ii+1
		If StrComp(Opponents(5), TeamList(ii), vbTextCompare) = 0 Then teamsix(CurrentPlayer)=ii+1
	next
	qualifyprim.image="logo" & teamone(CurrentPlayer):vs1(CurrentPlayer)=Opponents(0)
	groupprim.image="logo" & teamtwo(CurrentPlayer):vs2(CurrentPlayer)=Opponents(1)
	firstprim.image="logo" & teamthree(CurrentPlayer):vs3(CurrentPlayer)=Opponents(2)
	quarterprim.image="logo" & teamfour(CurrentPlayer):vs4(CurrentPlayer)=Opponents(3)
	semiprim.image="logo" & teamfive(CurrentPlayer):vs5(CurrentPlayer)=Opponents(4)
	finalprim.image="logo" & teamsix(CurrentPlayer):vs6(CurrentPlayer)=Opponents(5)
	UpdateKit:UpdateTeamLogos
	firstdefenceup.enabled=1:PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1
...
Sub ScheduleTS_Timer
	...
	If skillcount=1 Then
		...
		UDMD_Display "schedule.gif","qualify match", 14, 0, vs1(CurrentPlayer), 1, 14, UltraDMD_Animation_None, 1400, UltraDMD_Animation_FadeOut,7
	End If
	If skillcount=8 Then PlaySound "V.QUALIFY"
...
Select Case progress(CurrentPlayer)
	Case 0
		kitdef1.image="kit" & teamone(CurrentPlayer):kitDEF2.image="kit" & teamone(CurrentPlayer):kitDEF3.image="kit" & teamone(CurrentPlayer)
		If HeadLD.Z<0 Then kitLD.image="kit" & teamone(CurrentPlayer):kitRD.image="kit" & teamone(CurrentPlayer)
		headGK.image="K" & teamone(CurrentPlayer) &"0": headDEF1.image="K" & teamone(CurrentPlayer) &"1":headDEF2.image="K" & teamone(CurrentPlayer) &"2":headDEf3.image="K" & teamone(CurrentPlayer) &"3"
		If HeadLD.Z<0 Then headLD.image="K" & teamone(CurrentPlayer) &"4":headRD.image="K" & teamone(CurrentPlayer) &"5"
		BODYDEF1.IMAGE="BODY":BODYDEF2.IMAGE="BODY":bodyDEF3.IMAGE="BODY":kitGK.IMAGE="gk" & teamone(CurrentPlayer)
    Case 1
		kitdef1.image="kit" & teamtwo(CurrentPlayer):kitDEF2.image="kit" & teamtwo(CurrentPlayer):kitDEF3.image="kit" & teamtwo(CurrentPlayer)
		If HeadLD.Z<0 Then kitLD.image="kit" & teamtwo(CurrentPlayer):kitRD.image="kit" & teamtwo(CurrentPlayer)
		headGK.image="K" & teamtwo(CurrentPlayer) &"0": headDEF1.image="K" & teamtwo(CurrentPlayer) &"1":headDEF2.image="K" & teamtwo(CurrentPlayer) &"2":headDEf3.image="K" & teamtwo(CurrentPlayer) &"3"
		If HeadLD.Z<0 Then headLD.image="K" & teamtwo(CurrentPlayer) &"4":headRD.image="K" & teamtwo(CurrentPlayer) &"5"
		BODYDEF1.IMAGE="BODY":BODYDEF2.IMAGE="BODY":bodyDEF3.IMAGE="BODY":kitGK.IMAGE="gk" & teamtwo(CurrentPlayer)
	Case 2
		kitdef1.image="kit" & teamthree(CurrentPlayer):kitDEF2.image="kit" & teamthree(CurrentPlayer):kitDEF3.image="kit" & teamthree(CurrentPlayer)
		If HeadLD.Z<0 Then kitLD.image="kit" & teamthree(CurrentPlayer):kitRD.image="kit" & teamthree(CurrentPlayer)
		headGK.image="K" & teamthree(CurrentPlayer) &"0": headDEF1.image="K" & teamthree(CurrentPlayer) &"1":headDEF2.image="K" & teamthree(CurrentPlayer) &"2":headDEf3.image="K" & teamthree(CurrentPlayer) &"3"
		If HeadLD.Z<0 Then headLD.image="K" & teamthree(CurrentPlayer) &"4":headRD.image="K" & teamthree(CurrentPlayer) &"5"
		BODYDEF1.IMAGE="BODY":BODYDEF2.IMAGE="BODY":bodyDEF3.IMAGE="BODY":kitGK.IMAGE="gk" & teamthree(CurrentPlayer)
	Case 3
		kitdef1.image="kit" & teamfour(CurrentPlayer):kitDEF2.image="kit" & teamfour(CurrentPlayer):kitDEF3.image="kit" & teamfour(CurrentPlayer)
		If HeadLD.Z<0 Then kitLD.image="kit" & teamfour(CurrentPlayer):kitRD.image="kit" & teamfour(CurrentPlayer)
		headGK.image="K" & teamfour(CurrentPlayer) &"0": headDEF1.image="K" & teamfour(CurrentPlayer) &"1":headDEF2.image="K" & teamfour(CurrentPlayer) &"2":headDEf3.image="K" & teamfour(CurrentPlayer) &"3"
		If HeadLD.Z<0 Then headLD.image="K" & teamfour(CurrentPlayer) &"4":headRD.image="K" & teamfour(CurrentPlayer) &"5"
		BODYDEF1.IMAGE="BODY":BODYDEF2.IMAGE="BODY":bodyDEF3.IMAGE="BODY":kitGK.IMAGE="gk" & teamfour(CurrentPlayer)
	Case 4
		kitdef1.image="kit" & teamfive(CurrentPlayer):kitDEF2.image="kit" & teamfive(CurrentPlayer):kitDEF3.image="kit" & teamfive(CurrentPlayer)
		If HeadLD.Z<0 Then kitLD.image="kit" & teamfive(CurrentPlayer):kitRD.image="kit" & teamfive(CurrentPlayer)
		headGK.image="K" & teamfive(CurrentPlayer) &"0": headDEF1.image="K" & teamfive(CurrentPlayer) &"1":headDEF2.image="K" & teamfive(CurrentPlayer) &"2":headDEf3.image="K" & teamfive(CurrentPlayer) &"3"
		If HeadLD.Z<0 Then headLD.image="K" & teamfive(CurrentPlayer) &"4":headRD.image="K" & teamfive(CurrentPlayer) &"5"
		BODYDEF1.IMAGE="BODY":BODYDEF2.IMAGE="BODY":bodyDEF3.IMAGE="BODY":kitGK.IMAGE="gk" & teamfive(CurrentPlayer)
	Case 5
		kitdef1.image="kit" & teamsix(CurrentPlayer):kitDEF2.image="kit" & teamsix(CurrentPlayer):kitDEF3.image="kit" & teamsix(CurrentPlayer)
		If HeadLD.Z<0 Then kitLD.image="kit" & teamsix(CurrentPlayer):kitRD.image="kit" & teamsix(CurrentPlayer)
		headGK.image="K" & teamsix(CurrentPlayer) &"0": headDEF1.image="K" & teamsix(CurrentPlayer) &"1":headDEF2.image="K" & teamsix(CurrentPlayer) &"2":headDEf3.image="K" & teamsix(CurrentPlayer) &"3"
		If HeadLD.Z<0 Then headLD.image="K" & teamsix(CurrentPlayer) &"4":headRD.image="K" & teamsix(CurrentPlayer) &"5"
		BODYDEF1.IMAGE="BODY":BODYDEF2.IMAGE="BODY":bodyDEF3.IMAGE="BODY":kitGK.IMAGE="gk" & teamsix(CurrentPlayer)
End Select
End Sub
...
Sub moveGK_timer()
	If gkmm=0 Then
		...
		GKpost.transx=posGK
		posWALL=posGK/60
		If poswall<-0.8 AND poswall>-1 Then GKwall1.isdropped=0:StopSound "fx_motorGK":DOF 160,DOFOff:PlaySound SoundFXDOF("fx_motorGK",129,DOFOn,DOFGear),1,0.2,0,0.25
		...
		If poswall>0.8 AND poswall <1 Then GKwall10.isdropped=0:StopSound "fx_motorGK":DOF 160,DOFOff:PlaySound SoundFXDOF("fx_motorGK",129,DOFOn,DOFGear),1,0.2,0,0.25
	End If
End Sub

Sub GKWalls_Hit(idx)
If Tilted=0 Then
	PlaySound SoundFXDOF("fx_hitplayer",117,DOFPulse,DOFShaker)
...
Sub Wall32_timer:Me.TimerEnabled=0:UpdateKit:bomberLup.Enabled=1:BomberLState=0:End Sub
...
Sub Wall33_timer:Me.TimerEnabled=0:UpdateKit:bomberRup.Enabled=1:BomberRState=0:End Sub
...
If Tilted= 0 Then
	...
	StopSound "fx_hitplayer":PlaySound SoundFXDOF("fx_hitplayer",115,DOFPulse,DOFShaker)
	Select case idx
	Case 0	'left defender
		...
		FlashForMs fl3,100,50,0:FlashForMs fl3a,100,50,0:DOF 141, DOFPulse
	...
	Case 1	'center defender
		...
		FlashForMs fl2,100,50,0:FlashForMs fl2a,100,50,0:DOF 140, DOFPulse
	...
	Case 2	'right defender
		...
		FlashForMs fl1,100,50,0:FlashForMs fl1a,100,50,0:DOF 142, DOFPulse
	...
	If defC.state=1 AND defR.state=1 AND defL.state=1 AND FK=0 AND MVPOfTheYear(CurrentPlayer)=0 Then
		...
		If win(CurrentPlayer)=5 AND defencestate(CurrentPlayer)=6 AND DefenceLocked(CurrentPlayer)=0 Then
			...
			'move Strikers up
			If headLD.Z<0 Then bomberLup.enabled=1:brake2=0:perc2=perc2+3:sbou2=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",134,DOFPulse,DOFDropTargets)
			If headRD.Z<0 Then bomberRup.enabled=1:brake=0:perc=perc+3:sbou=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",135,DOFPulse,DOFDropTargets)
		End If
		If win(CurrentPlayer)=5 AND defencestate(CurrentPlayer)=6 AND DefenceLocked(CurrentPlayer)=2 Then
			win(CurrentPlayer)=6:UpdateMatchLevelLights 1:CheckMatchLevel:Flash lClWin,75:FlashB2S B2SLogoCL,-1,75,1:DefenceLocked(CurrentPlayer)=0
		End If
	End If
End If
End Sub
...
Sub CheckMatchLevel
	If win(CurrentPlayer)=defencestate(CurrentPlayer) Then
		...
		FlashForMs fl1,3000,100,0
		FlashForMs fl1a,3000,100,0
		FlashForMs fl2,3000,100,0
		FlashForMs fl2a,3000,100,0
		CornerModeOff:DOF 145, DOFPulse
		defencedown.enabled=1:MoveGK.enabled=1:PlaySound "bassdown":PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1
...
Else
	...
	If headDEF1.z>12 Then
		...
		uvel=0:ResetGKTimer.enabled=0:GKMM=0:Music:exit sub
	End If
End If
End Sub
...
If Tilted=0 Then
	...
	If Lmultilock.state=1 AND lockjackpot.state=2 Then
		...
		Lighteffect 4:GIEffect 0:FlasherEffect 6
	End If
...
'used when ball enters the right hole (multiball hole) and Multiball Lights are blinking: this will start Multiball Animation Sequence
    If (LockMultiMode=1 or TicketMultiMode=1 or SkillMultiMode=1) Then
		LightEffect 2:GiEffect 2:PlaySound "thunder"':StopMusic
...
'used when ball enters the right hole (multiball hole) and MVP Award is lit:this will award 50 millions!
	If finaljackpot.state=2 Then
		...
		FlashForMs fl1,2500,100,0
		FlashForMs fl1a,2500,100,0
		FlashForMs fl2,2500,100,0
		FlashForMs fl2a,2500,100,0
		FlashForMs fl3,2500,100,0
		FlashForMs fl3a,2500,100,0
		DOF 146, DOFPulse
		FlashB2S B2SLogoCL,-1,100,0:FlashB2S B2SLight1,2500,100,0:FlashB2S B2SLight2,2500,100,0:FlashB2S B2SLight3,2500,100,0
		StopMusic:PlayMusic MusicList(17)
		...
		LightEffect 1:FlasherEffect 12
...
Sub FinalJackpotCheck(n)
	If lockcomplete.state=1 AND skillcomplete.state=1 AND ticketcomplete.state=1 AND championcomplete.state=1 AND fkcomplete.state=1 Then
		MVPOfTheYear(CurrentPlayer)=1:FinalMode.timerInterval=100:FinalMode.timerenabled=1:FinalJackpotAward=n
	End If
End Sub
...
Sub CLwinWait_timer()
	Me.Enabled=0
	StopMusic:cupbase.image="cuplightoff":CLWin.Enabled=0:cuplights.visible=0:fcount=0
	...
	Lwinfinal.state=0:Lwinsemi.state=0:Lwinquarter.state=0:Lwinfirst.state=0:Lwingroup.state=0:Lwinqualify.state=0:StopFlash lClWin,0:StopFlashB2S B2SLogoCL,0
	For Each ii in TeamLogos:ii.state=0:Next:MainB2SLampsOff
	TeamLogos(choiceteam(CurrentPlayer)-1).state=1:StopFlashB2S choiceteam(CurrentPlayer),1
	...
	FinalMode.TimerInterval=100
	FinalMode.TimerEnabled=1
End Sub
...
Sub FinalMode_timer
	If NOT UltraDMD.IsRendering Then
		...
		If MVPOfTheYear(CurrentPlayer)=0 Then
			...
			Opponents = RemoveArrayElement(TeamList, TeamList(choiceteam(CurrentPlayer)-1)):Schedule.enabled=1					'reset the groups and create a new schedule
			BallRelease.CreateSizedBallWithMass Ballsize/2,BallMass:BallRelease.Kick 90, 7:PlaySound SoundFXDOF("fx_ballrel",121,DOFPulse,DOFContactors),0,1,0,0.25
		...
		Else
			...
			If finaljackpot.state=0 Then
				...
				UDMD_Display "black.bmp", "TO LIGHT THE", 14,-1,"BALON DOR!!!",14,-1,UltraDMD_Animation_FadeIn,1500,UltraDMD_Animation_FadeOut,10
				MVPTimer.Interval=9000:MVPTimer.Enabled=1:MVPEffectType=2:MVPEffect.Enabled=1
			...
			Else
				...
				UltraDMD.DisplayScene00Ex "black.bmp", "CONGRATULATIONS!", 14,-1,"",14,-1,UltraDMD_Animation_FadeIn,1000,UltraDMD_Animation_FadeOut
				UltraDMD.DisplayScene00Ex "black.bmp", "YOU WON THE", 14,-1,"BALON DOR!!!",14,-1,UltraDMD_Animation_FadeIn,1500,UltraDMD_Animation_FadeOut
				UDMD_Display "black.bmp", "AWARDED", 14,-1,"50 MILLIONS!",14,-1,UltraDMD_Animation_None,2500,UltraDMD_Animation_ScrollOffDown,10
				...
				LightSeqMVP.StopPlay:MVPwin.Enabled=0:StopFlashB2S B2SLogoCL,0:fcount=0:completed(currentplayer)=0
				TicketDiverter True:If TDState(CurrentPlayer)>0 Then TDState(CurrentPlayer)=0:StopMusic
				MVPWait.Interval=100:MVPWait.Enabled=1:MVPEffectType=1:MVPEffect.Enabled=1
...
Sub MVPTimer_timer
	...
	If BIP=0 Then
		...
		If win(CurrentPlayer)=defencestate(CurrentPlayer) Then						'move defenders up if they are down
			moveback=3:MOVEGK.enabled=0:ResetGK:firstdefenceup.enabled=1:PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1:Lshoot.state=0:ArrowEffect False
		End If
		BallRelease.CreateSizedBallWithMass Ballsize/2,BallMass:BallRelease.Kick 90, 7:PlaySound SoundFXDOF("fx_ballrel",121,DOFPulse,DOFContactors),0,1,0,0.25:kicked=0:BIP=BIP+1:Music:DOF 152, DOFPulse
		If Lx1.state=1 AND Lx2.state=1 Then Lx1.state=0:Lx2.state=0
	Else
...
If NOT UltraDMD.IsRendering Then
	...
	ballrelease.CreateSizedBallWithMass Ballsize/2,BallMass:ballrelease.Kick 90, 7:PlaySound SoundFXDOF("fx_ballrel",121,DOFPulse,DOFContactors),0,1,0,0.25:BIP=BIP+1
	...
	Select Case MVPEffectType
		...
		Case 3	'during mpv after you hit the snackbar
			...
			If MVPEffectcc=20 Then
				...
				UltraDMD.CancelRendering:UDMD_Display "MVP.gif", "BALON DOR",14,-1,"IS LIT!!!",14,-1,UltraDMD_Animation_None,2000,UltraDMD_Animation_FadeOut,10
			End If
...
Sub OpenTrapDoor(enabled)
	...
	Else
		...
		PlaySound SoundFXDOF("fx_td-down",131,DOFPulse,DOFContactors)
...
Sub OpenTD
	TrapDoorP.z=100:wall61.isdropped=0:PlaySound SoundFXDOF("fx_td-up",131,DOFPulse,DOFContactors)
	...
	If Tilted=0 Then
		If MVPOfTheYear(CurrentPlayer)=0 OR (MVPOfTheYear(CurrentPlayer)=1 AND finaljackpot.state=2) Then
			...
			If StrikerMode(CurrentPlayer)=3 AND FKAward(CurrentPlayer)=0 AND BSMODE(CurrentPlayer)=0 AND SemiFinalAward(CurrentPlayer)=0 Then
				...
				UltraDMD.CancelRendering:PlaySound "vmode"
				UltraDMD.DisplayScene00Ex "videomode.gif", "",14,-1,"",14,-1,UltraDMD_Animation_FadeIn,2900,UltraDMD_Animation_None
				screenprim1.image="VMode_sc1":screenprim1.visible=1
				For each ii in VMFlashers:FlashForMs ii,8000,50,0:next
				DOF 148, DOFPulse
				LeftFlipper.RotatetoStart:RightFlipper.RotatetoStart
				LeftFlipper1.RotatetoStart:RightFlipper1.RotatetoStart
...
Sub TrapKickBall:PlaySound SoundFXDOF("fx_td-exit",123,DOFPulse,DOFContactors):FlashForMs fl4,150,150,0:StopFlash fl4a,1:FlashForMs fl4a,150,150,0:DOF 143,DOFPulse:End Sub
...
If Tilted=0 Then
	...
	Else
		...
		UDMD_Display "MVP.gif", "NEXT TIME",15,2,"PLEASE",15,2,UltraDMD_Animation_None,1500,UltraDMD_Animation_FadeOut,6
	End If
	PlaySound SoundFXDOF("fx_board_hit",117,DOFPulse,DOFShaker):DOF 118,DOFPulse
	scores(CurrentPlayer)=scores(CurrentPlayer)+10000
	FlashForMs fl3,300,1000,0
	FlashForMs fl3a,300,1000,0
	DOF 147, DOFPulse
	FlashB2S B2SLight3,300,1000,0
End If
End Sub
...
Sub TabletUp
	...
	PlaySound SoundFXDOF("fx_motorT",127,DOFOn,DOFGear)
End Sub
...
Sub TicketDiverter(enabled)
	If Enabled Then
		PlaySound SoundFXDOF("fx_DiverterUp",136,DOFPulse,DOFContactors)
		wallticket.IsDropped=0
		DiverterP.Z=83.35
	Else
		PlaySound SoundFXDOF("fx_DiverterDown",136,DOFPulse,DOFContactors)
...
Sub triggergoal_hit()
	Me.DestroyBall:PlaySound "fx_Moat_enter",0,1,0,0.25
...
'first goal
	If Lgoal1.state=0 AND lgoal2.state=0 AND FK=0 AND GKMM=0 Then
		...
		FlashForMs fl2,3000,100,0
		FlashForMs fl2a,3000,100,0
		FlashForMs fl3,3000,100,0
		FlashForMs fl3a,3000,100,0
		DOF 145, DOFPulse
...
'third goal
	If Lgoal2.state=1 AND Lgoal3.state=0 AND FK=0 AND GKMM=0 Then
		...
		LightEffect 1:GiEffect 1
		If HeadLD.Z>0 Then BomberLState=1:bomberLdown.Enabled=1:hitLD.isdropped=1:WALL32.ISDROPPED=1:PlaySound SoundFXDOF("fx_postdown",134,DOFPulse,DOFDropTargets)
		If HeadRD.Z>0 Then BomberRState=1:bomberRdown.Enabled=1:hitRD.isdropped=1:WALL33.ISDROPPED=1:PlaySound SoundFXDOF("fx_postdown",135,DOFPulse,DOFDropTargets)
		GKHOLEin.enabled=1:moveGK.enabled=0:ResetGK
		If progress(CurrentPlayer)<6 Then
			If progress(CurrentPlayer)<=4 Then GKcount.Interval=2000:GKcount.enabled=1:defenceup.enabled=1:PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1
			progress(CurrentPlayer)=progress(CurrentPlayer)+1:UpdateMatchLevelLights 2:UpdateKit:UpdateTeamLogos
...
'************************************************************************
Sub targ1_hit
PlaySound SoundFXDOF("fx_target", 113, DOFPulse, DOFTargets), 0, 1, pan(ActiveBall)
If Tilted=0 Then
	scores(CurrentPlayer)=scores(CurrentPlayer)+10000
	If headLD.z<0 AND headRD.z<0 AND GKMM=0 AND TrapDoorP.Z=0 AND MVPOfTheYear(CurrentPlayer)=0 AND hotdog.state=0 Then
		...
		FlashForMs fl1,700,50,0:FlashForMs fl1a,700,50,0:B2SEffect 3:DOF 114,DOFPulse
		If hotdog.state=1 AND can.state=1 AND chips.state=1 Then tdfcolor=6:OpenTrapDoor True:BSMODE(CurrentPlayer)=1:lbreak1.state=2:lbreak2.state=2:hotdog.state=2:can.state=2:chips.state=2
	End If
End If
End Sub
...
'this is the Break Spin Mode Award
    hotdog.state=0:can.state=0:chips.state=0:lbreak1.state=0:lbreak2.state=0
'Award #1 = EXTRA BALL
...
'Award #2 = GOALKEEPER MODE
	If rndchoicereal=2 Then
		...
		trapkicker.TimerInterval=7000:trapkicker.TimerEnabled=1
		screenprim1.image="GKMode_sc1":screenprim1.visible=1:PlaySound "gkmode"
...
'Award #3 = VIDEO MODE
    If rndchoicereal=3 Then
		...
		screenprim1.image="VMode_sc1":screenprim1.visible=1
		For each ii in VMFlashers:FlashForMs ii,8000,50,0:next
		DOF 148, DOFPulse:PlaySound "vmode"
		UltraDMD.DisplayScene00Ex "videomode.gif", "",14,-1,"",14,-1,UltraDMD_Animation_FadeIn,2900,UltraDMD_Animation_None
	End If
...
Sub SavePlayerLampStates
	For ii = 0 To 76:AllPlayersLampStates(CurrentPlayer,ii)=AllInserts(ii).State:Next						'save the state to the lampstate array (for each Player)
End Sub

Sub RestorePlayerLampStates
	For ii = 0 To 76:AllInserts(ii).State=AllPlayersLampStates(CurrentPlayer,ii):Next						'restore the state from the lampstate array (for each Player)
End Sub
...
Sub UpdateTeamLogos
	Select Case progress(CurrentPlayer)
		Case 0
			TeamLogos(teamone(CurrentPlayer)-1).state=2
			If B2SOn Then FlashB2S teamone(CurrentPlayer),-1,250,1
		Case 1
			TeamLogos(teamone(CurrentPlayer)-1).state=1
			TeamLogos(teamtwo(CurrentPlayer)-1).state=2
			If B2SOn Then StopFlashB2S teamone(CurrentPlayer),1:FlashB2S teamtwo(CurrentPlayer),-1,250,1
		Case 2
			TeamLogos(teamtwo(CurrentPlayer)-1).state=1
			TeamLogos(teamthree(CurrentPlayer)-1).state=2
			If B2SOn Then StopFlashB2S teamtwo(CurrentPlayer),1:FlashB2S teamthree(CurrentPlayer),-1,250,1
		Case 3
			TeamLogos(teamthree(CurrentPlayer)-1).state=1
			TeamLogos(teamfour(CurrentPlayer)-1).state=2
			If B2SOn Then StopFlashB2S teamthree(CurrentPlayer),1:FlashB2S teamfour(CurrentPlayer),-1,250,1
		Case 4
			TeamLogos(teamfour(CurrentPlayer)-1).state=1
			TeamLogos(teamfive(CurrentPlayer)-1).state=2
			If B2SOn Then StopFlashB2S teamfour(CurrentPlayer),1:FlashB2S teamfive(CurrentPlayer),-1,250,1
		Case 5
			TeamLogos(teamfive(CurrentPlayer)-1).state=1
			TeamLogos(teamsix(CurrentPlayer)-1).state=2
			If B2SOn Then StopFlashB2S teamfive(CurrentPlayer),1:FlashB2S teamsix(CurrentPlayer),-1,250,1
		Case 6
			TeamLogos(teamsix(CurrentPlayer)-1).state=1
			If B2SOn Then StopFlashB2S teamsix(CurrentPlayer),1
	End Select
End Sub
...
'*****GI********************************************************
Sub GiOn
For each ii in GI:ii.state=1:next:StopFlashB2S B2Sbkgnd,1
...
Sub GiOff
For each ii in GI:ii.state=0:next:StopFlashB2S B2Sbkgnd,0
...
Dim cAttractMode
Sub StopAttractMode
	If B2SOn Then B2SAttract.Enabled=0:B2SLeftRight.Enabled=0:B2SID=0:tdir=1:tstate=1:B2SStep=0:ccc=0:InitB2SLamps
	For each ii in BumpLights:ii.state=1:next
...
Sub StartAttractMode
	If B2SOn AND NOT B2SAttract.Enabled Then B2SAttract.Enabled=1:FlashB2S B2SLight1,-1,100,0:FlashB2S B2SLight2,-1,150,0:FlashB2S B2SLight3,-1,200,0
...
'*RAMPS TRIGGERS*********************************************************
'************************************************************************
Sub LRailEnter_hit:PlaySound "fx_metalrolling":End Sub
Sub LRailExit_hit:stopsound "fx_metalrolling":PlaySound "fx_ball_drop":End Sub
Sub RRailEnter_hit:PlaySound "fx_metalrolling":End Sub
Sub RRailExit_hit:stopsound "fx_metalrolling":PlaySound "fx_ball_drop":End Sub
Sub RREnter_hit:If ActiveBall.VelY<0 Then PlaySound "fx_rlenter":End If:End Sub
Sub LREnter_hit:If ActiveBall.VelY<0 Then PlaySound "fx_rlenter":End If:End Sub
Sub LRHelp_hit:ActiveBall.VelX=ActiveBall.VelX*(0.5):End Sub
Sub LeftHole_hit:RandomSoundHole:End Sub
Sub RightHole_hit:PlaySound "fx_hole_enter":End Sub
...
Function ShuffleArray(MyArray())		'Shuffle an array, sorting the elements randomly
    Dim I, J, Temp
...
Function RemoveArrayElement(ByVal MyArray, ByVal Item)		'removes an item from an array
  Dim S, N
  S = ""
...
Sub RandomSoundFlipper()
	Select Case Int(Rnd*3)+1
		Case 1 : PlaySound "fx_flip_hit_1", 0, Vol(ActiveBall)*3, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0
		Case 2 : PlaySound "fx_flip_hit_2", 0, Vol(ActiveBall)*3, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0
		Case 3 : PlaySound "fx_flip_hit_3", 0, Vol(ActiveBall)*3, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0
	End Select
End Sub
...
Sub nameEntry
	...
	End If
	UltraDMD.DisplayScene00ExWithId "optnames00", false, "hiscore.png", "ENTER YOUR NAME", 15, 0, strinitial, 15, 0, UltraDMD_Animation_None, 900000, UltraDMD_Animation_None
End Sub
...
Sub DisplayMatch
	...
	Next
	rndMatch=0 & Right(0,1):PlaySound "DrumRoll"
	If UltraDMD.IsRendering Then UltraDMD.CancelRendering
...
Sub AddSpecial
	...
	Case 0		'awarded from match
		PlaySound SoundFXDOF("fx_knocker",137,DOFPulse,DOFKnocker):Playsound "0_highscore":DOF 150, DOFOn
		If FreePlay=0 Then
	...
	Case 1		'awarded when score>highscore 1
		PlaySound SoundFXDOF("fx_knocker",137,DOFPulse,DOFKnocker)
...
Sub KnockerSound_timer
	...
	If creditspecial>1 Then
		PlaySound SoundFXDOF("fx_knocker",137,DOFPulse,DOFKnocker):creditspecial=creditspecial-1:Me.Enabled=1
	Else
		creditspecial=0
	End If
End Sub
...
Const B2Sbkgnd = 99:Const B2SLogoCL = 8:Const B2SLight1 = 21:Const B2SLight2 = 22:Const B2SLight3 = 23
'ID=1	BAR			'ID=2	BAY			'ID=3	CHE
'ID=4	CIT			'ID=5	JUV			'ID=6	PSG
'ID=7	RMA
'ID=8	Logo CL		'ID=99	BackGround
'ID=21	Flasher1	'ID=22	Flasher2	'ID=23	Flasher3
Dim xx,bstart(200),B2SCase(200),B2Sdata(200),B2Sduration(200),B2SInterval(200),B2Sfstate(200),B2SLampState(200),tempState(200)

'************************INIT ALL LIGHTS ********************************************
Sub InitB2SLamps
	If B2SOn Then
		MainB2SLampsOff
		StopFlashB2S B2SLogoCL,0:StopFlashB2S B2SLight1,0:StopFlashB2S B2SLight2,0:StopFlashB2S B2SLight3,0
	End If
End Sub

'************************TURN OFF THE MAIN 7 LIGHTS ********************************************
Sub MainB2SLampsOff
	If B2SOn Then
		For ii=1 to 7:StopFlashB2S ii, 0:next
	End If
End Sub

'************************FLASH A LIGHT********************************************
'similar to FlashForMs routine,but here we need a timer for each b2s light
Sub FlashB2S(obj,duration,interval,finalstate)
If B2SOn Then
	If finalstate=2 Then	'remember previous state
		tempState(obj)=0
		CheckB2SState(obj)
	End If
	bstart(obj)=1:B2Sdata(obj)=obj:B2Sduration(obj)=duration
	B2SInterval(obj)=interval:B2Sfstate(obj)=finalstate
	ExecuteGlobal "b2sT" & B2Sdata(obj) & ".Interval=" & interval
	ExecuteGlobal "b2sT" & B2Sdata(obj) & ".Enabled=0"
	ExecuteGlobal "b2sT" & B2Sdata(obj) & ".Enabled=1"
	ExecuteGlobal "Sub b2sT"& obj &"_Timer:xx="& obj &":BlinkMe:End Sub"
End If
End Sub
...
Select Case n
Case 1
if b2ST1.Enabled Then tempState(1)=1:StopFlashB2S 1,0
Case 2
if b2ST2.Enabled Then tempState(2)=1:StopFlashB2S 2,0
Case 3
if b2ST3.Enabled Then tempState(3)=1:StopFlashB2S 3,0
Case 4
if b2ST4.Enabled Then tempState(4)=1:StopFlashB2S 4,0
Case 5
if b2ST5.Enabled Then tempState(5)=1:StopFlashB2S 5,0
Case 6
if b2ST6.Enabled Then tempState(6)=1:StopFlashB2S 6,0
Case 7
if b2ST7.Enabled Then tempState(7)=1:StopFlashB2S 7,0
Case 8
if b2ST8.Enabled Then tempState(8)=1:StopFlashB2S 8,0
Case 99
if b2ST99.Enabled Then tempState(99)=1:StopFlashB2S B2Sbkgnd,1
End Select
End Sub

Sub StopFlashB2S (obj,finalstate)
If B2SOn Then
	ExecuteGlobal "b2sT" & obj & ".Enabled=0"
	Controller.B2SSetData obj, finalstate
	B2SLampState(obj)=finalstate:B2SCase(obj)=0
End If
End Sub

Sub BlinkMe
	Dim dbsteps:dbsteps=bstart(xx)-1
	Controller.B2SSetData B2Sdata(xx), dbsteps MOD 2
	bstart(xx)=bstart(xx)+1
	If B2Sduration(xx)>0 Then
		B2SCase(xx)=B2SCase(xx)+1
		If B2SCase(xx)>=B2Sduration(xx)/B2SInterval(xx) Then
			If NOT B2Sfstate(xx)=2 Then
				StopFlashB2S B2Sdata(xx),B2Sfstate(xx)
				B2SLampState(xx)=B2Sfstate(xx)
			Else		'recover previous state
				If tempState(xx)<>1 Then
					StopFlashB2S B2Sdata(xx),B2SLampState(xx)
				Else
					StopFlashB2S B2Sdata(xx),B2SLampState(xx)
					FlashB2S B2Sdata(xx),-1,250,1
				End If
			End If
		End If
	Else
...
'************************ATTRACT MODE********************************************
dim B2SID,tdir,tstate,tempdir,randomID,B2SStep,ccc
B2SID=0:tdir=1:tstate=1:B2SStep=0:ccc=0
dim B2STeams:B2STeams=Array("",7,4,1,3,6,2,5,8)

Sub B2SAttract_timer
	ccc=ccc+1
	Select Case B2SStep
		Case 0	'start random sequence (0:00:00)
			Randomize:randomID=RndNum(1,8):Controller.B2SSetData randomID, 1
			B2SWait.Interval=80:B2SWait.Enabled=1
			if ccc=105 Then B2SStep=1:Me.Enabled=0:For ii=1 to 8:Controller.B2SSetData ii, 0:next:Me.Interval=600:Me.Enabled=1

		Case 1 	'at time (0:10:50)		blink the light 5 times
			For ii=1 to 8:Controller.B2SSetData ii, 1:next
			B2SWait.Interval=300:B2SWait.Enabled=1
			if ccc=110 Then B2SStep=2:Me.Enabled=0:For ii=1 to 8:Controller.B2SSetData ii, 0:next:Me.Interval=2000:Me.Enabled=1:For ii=1 to 8:Controller.B2SSetData ii, 1:next

 		Case 2	'at time (0:13:50)		slow blink
			For ii=1 to 8:Controller.B2SSetData ii, 1:next
			B2SWait.Interval=200:B2SWait.Enabled=1
			if ccc=116 Then B2SStep=3:Me.Enabled=0:For ii=1 to 8:Controller.B2SSetData ii, 0:next:Me.Interval=200:Me.Enabled=1

		Case 3	'at time (0:29:50)  fast blink
			For ii=1 to 8:Controller.B2SSetData ii, 1:next
			B2SWait.Interval=100:B2SWait.Enabled=1
			if ccc=166 Then B2SStep=4:Me.Enabled=0:For ii=1 to 8:Controller.B2SSetData ii, 0:next:Me.Interval=100:Me.Enabled=1

		Case 4	'at time (0:40:50)  faster blink
			For ii=1 to 8:Controller.B2SSetData ii, 1:next
			B2SWait.Interval=50:B2SWait.Enabled=1
			if ccc=236 Then B2SStep=5:Me.Enabled=0:For ii=1 to 8:Controller.B2SSetData ii, 0:next:Me.Interval=4500:Me.Enabled=1:tempdir=1:B2SLeftRight.Interval=50:B2SLeftRight.Enabled=1

		Case 5	'at time (0:45:50)		left right
			Me.Enabled=0
			B2SStep=6:B2SLeftRight.Enabled=0:For ii=1 to 8:Controller.B2SSetData ii, 0:next:Me.Interval=4500:Me.Enabled=1:tempdir=0:B2SLeftRight.Interval=25:B2SLeftRight.Enabled=1

		Case 6	'at time (0:54:50) 		right left
			Me.Enabled=0
			B2SStep=0:B2SLeftRight.Enabled=0:For ii=1 to 8:Controller.B2SSetData ii, 0:next:ccc=-1:Me.Interval=100:Me.Enabled=1
			'at time (1:03:50) restart the sequence
	End Select
End Sub

Sub B2SWait_timer
Me.Enabled=0
If B2SStep=0 Then Controller.B2SSetData randomID, 0
If B2SStep>0 Then For ii=1 to 8:Controller.B2SSetData ii, 0:next
End Sub

Sub B2SLeftRight_timer
Me.Enabled=0
Select Case tempdir
Case 0		'only left
Controller.B2SSetData B2STeams(B2SID+tdir), tstate:B2SID=B2SID+tdir
If B2SID>7 AND tdir=1 then tdir=-1:tstate=0
If B2SID=0 AND tdir=-1 then tdir=1:tstate=1

Case 1		'left then right
Controller.B2SSetData B2STeams(B2SID+tdir), tstate:B2SID=B2SID+tdir
If B2SID>7 AND tdir=1 then tdir=-1:tstate=0
If B2SID=0 AND tdir=-1 then B2SID=8:tstate=1:tempdir=-1

Case -1		'right then left
Controller.B2SSetData B2STeams(B2SID+tdir), tstate:B2SID=B2SID+tdir
If B2SID=0 AND tdir=-1 then B2SID=0:tdir=1:tstate=0
If B2SID>7 AND tdir=1 then B2SID=0:tstate=1:tempdir=1
End Select
Me.Enabled=1
End Sub
...
If B2SOn Then
	Select Case n
        Case 0 ' defenders/Strikers
			FlashB2S B2SLight1,300,100,0:FlashB2S B2SLight2,300,100,0:FlashB2S B2SLight3,300,100,0
        Case 1 'goal scoring
			For ii=1 to 8:FlashB2S ii,3500,100,2:Next
			FlashB2S B2Sbkgnd,3500,100,2
        Case 2 'multiball/extraball
			StopFlashB2S B2Sbkgnd,0
			FlashB2S B2SLight1,2100,100,0:FlashB2S B2SLight2,2100,150,0:FlashB2S B2SLight3,2100,200,0
        Case 3 'food targets
			FlashB2S B2SLight1,50,25,0
			FlashB2S B2SLight2,100,50,0
			FlashB2S B2SLight3,150,75,0
        Case 4 'jackpot
			FlashB2S B2SLight1,1000,50,0
			FlashB2S B2SLight2,1000,50,0
			FlashB2S B2SLight3,1000,50,0
		Case 5	'boots
			FlashB2S B2SLight1,500,100,0
			FlashB2S B2SLight2,500,150,0
			FlashB2S B2SLight3,500,200,0
	End Select
End If
End Sub
...
'* TABLE OPTIONS MENU *********************************************************************
'******************************************************************************************
Dim FreePlay, BallsPerGame, TiltSensitivity, TiltMaxWarnings, CornerTime, GKModeDifficulty,VModeDifficulty
Dim optFreeplay,optBallsPerGame,optTiltSensor,optTiltWarnings,optCornerMode,optGKMode,optVideoMode
Dim OptionValue,OptionPlace,StrCase,OptMenu,OptDir,StrOption(6),OptMenuEnabled:OptMenuEnabled=False
Dim StrFeature:StrFeature=Array("    FREE PLAY   "," BALLS PER GAME ","  TILT  SENSOR  "," TILT  WARNINGS ","   CORNER MODE  ","     GK MODE    ","   VIDEO MODE   "," FACTORY  RESET ")
Dim StrNames:StrNames = Array(Array("       NO       ","       YES      "),_
...
Sub ManageTableOptions (byVal keycode)
	...
	If keycode = 11 AND NOT OptMenuEnter.Enabled AND cAttractMode=1 Then							'key 0
		...
		If OptMenu=1 Then StartOptionMenu:Playsound "swoosh"
		If OptMenu=2 Then Playsound "fx_Enter":OptMainMenu.Enabled=0:DisplayTableOption:If OptionPlace<7 Then OptMenuChoice.Enabled=1
...
Sub OptMenuExit_timer
	If NOT UltraDMD.IsRendering Then
		...
		PlaySound "reset":DisplayIntro.Enabled=1
	End If
End Sub

Sub StartOptionMenu
	DisplayIntroHS.Enabled=0:DisplayIntro.Enabled=0:DisplayStartScreen.Enabled=0:StopMusic
	UltraDMD.CancelRendering:OptMenuEnabled=True
...
Sub OptMainMenu_timer
	If OptionPlace<0 Then OptionPlace=7
...
Sub DisplayTableOption
	StrOption(0)=StrNames(0)(optFreeplay):StrOption(1)=StrNames(1)(optBallsPerGame)
	StrOption(2)=StrNames(2)(optTiltSensor):StrOption(3)=StrNames(3)(optTiltWarnings)
	StrOption(4)=StrNames(4)(optCornerMode):StrOption(5)=StrNames(5)(optGKMode)
	StrOption(6)=StrNames(6)(optVideoMode):StrCase=1
	If OptionPlace=7 Then
		UltraDMD.DisplayScene00 "black.bmp", "HIT OK TO SET TO", 15, "DEFAULT  OPTIONS", 15, 14, 100000, 14
	Else
		UltraDMD.DisplayScene00ExWithId "optmenu00", false, "black.bmp", StrFeature(OptionPlace), 15, 0, StrOption(OptionPlace), 15, 0, 14, 100000, 14
	End If
End Sub
...
Sub OptMenuChoice_timer
	...
	Select Case OptionPlace
		Case 0
		If ndis=1 Then StrOption(0)=StrNames(0)(optFreeplay) Else StrOption(0)="" End If
		Case 1
		If ndis=1 Then StrOption(1)=StrNames(1)(optBallsPerGame) Else StrOption(1)="" End If
		Case 2
		If ndis=1 Then StrOption(2)=StrNames(2)(optTiltSensor) Else StrOption(2)="" End If
		Case 3
		If ndis=1 Then StrOption(3)=StrNames(3)(optTiltWarnings) Else StrOption(3)="" End If
		Case 4
		If ndis=1 Then StrOption(4)=StrNames(4)(optCornerMode) Else StrOption(4)="" End If
		Case 5
		If ndis=1 Then StrOption(5)=StrNames(5)(optGKMode) Else StrOption(5)="" End If
		Case 6
		If ndis=1 Then StrOption(6)=StrNames(6)(optVideoMode) Else StrOption(6)="" End If
	End Select
...
Sub LoadTableOptions
	...
	If OptionValue = "" then
		OptionValue = 0*2 + 0*4 + 1*16 + 2*64 + 1*256 + 1*1024 + 1*4096		'default value is 5520
...
Sub SaveTableOptions
	If OptionPlace<7 Then
		OptionValue = optFreePlay*2^1 + optBallsPerGame*2^2 + optTiltSensor*2^4 + optTiltWarnings*2^6 + optCornerMode*2^8 + optGKMode*2^10 + optVideoMode*2^12
	Else
		OptionValue = 0*2 + 0*4 + 1*16 + 2*64 + 1*256 + 1*1024 + 1*4096		'reset table options to factory
...
'save all lights , then all lights off and reset blink patterns
	...
	ArrowEffect False:StopFlash lClwin,0
...
'reset the Defenders
	...
	If headGK.Z>0 Then GKHOLEin.enabled=1
	defencedown.enabled=1:PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1
'reset the Strikers
	If headLD.Z>0 Then bomberLdown.enabled=1:PlaySound SoundFXDOF("fx_postdown",134,DOFPulse,DOFDropTargets):hitLD.isdropped=1:WALL32.ISDROPPED=1
	If headRD.Z>0 Then bomberRdown.enabled=1:PlaySound SoundFXDOF("fx_postdown",135,DOFPulse,DOFDropTargets):hitRD.isdropped=1:WALL33.ISDROPPED=1
'reset the Snack Bar
...
'update Defenders
	Select Case DefenceLocked(CurrentPlayer)
		Case 0		'defenders are not locked
			If win(CurrentPlayer)=defencestate(CurrentPlayer) Then
				If MVPOfTheYear(CurrentPlayer)=1 Then
					moveback=3:GKcount.Interval=2000:GKcount.enabled=1:firstdefenceup.enabled=1:PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1:CornerModeOn
				Else
					moveback=99:GKHOLEout.enabled=1:ArrowEffect True	'move only the gk
				End If
				If win(CurrentPlayer)=6 Then Flash lClWin,75:FlashB2S B2SLogoCL,-1,75,1
			Else
				GKcount.Interval=2000:GKcount.enabled=1:firstdefenceup.enabled=1:PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1:CornerModeOn
			End If
			If BLHit(CurrentPlayer)>0 Then bomberLup.enabled=1:brake2=0:perc2=perc2+3:sbou2=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",134,DOFPulse,DOFDropTargets)
			If BRHit(CurrentPlayer)>0 Then bomberRup.enabled=1:brake=0:perc=perc+3:sbou=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",135,DOFPulse,DOFDropTargets)
		Case 1		'defenders are locked (bombers up)
			GKcount.Interval=2000:GKcount.enabled=1:firstdefenceup.enabled=1:PlaySound SoundFXDOF("fx_motorD",128,DOFOn,DOFGear),-1:CornerModeOn
			bomberLup.enabled=1:brake2=0:perc2=perc2+3:sbou2=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",134,DOFPulse,DOFDropTargets)
			bomberRup.enabled=1:brake=0:perc=perc+3:sbou=(Rndnum(-10,10))/10:PlaySound SoundFXDOF("fx_postup",135,DOFPulse,DOFDropTargets)
