...
Sub BallShadowUpdate_timer()
    ...
    For b = 0 to UBound(BOT)
        BallShadow(b).X = BOT(b).X-maxXoffset*(1-(Bot(1).X)/(Jubilee.Width/2))
...
sub savehs
    ...
    savevalue "Jubilee", "match", matchnumb
    savevalue "Jubilee", "score1", score(1)
    savevalue "Jubilee", "score2", score(2)
    savevalue "Jubilee", "score3", score(3)
    savevalue "Jubilee", "score4", score(4)
    savevalue "Jubilee", "balls", balls
    savevalue "Jubilee", "leftballs", leftballs
    savevalue "Jubilee", "rightballs", rightballs
    savevalue "Jubilee", "hsa1", HSA1
    savevalue "Jubilee", "hsa2", HSA2
    savevalue "Jubilee", "hsa3", HSA3
...
sub loadhs
    ...
    temp = LoadValue("Jubilee", "hiscore")
    If (temp <> "") then hisc = CDbl(temp)
    temp = LoadValue("Jubilee", "match")
    If (temp <> "") then matchnumb = CDbl(temp)
    temp = LoadValue("Jubilee", "score1")
    If (temp <> "") then score(1) = CDbl(temp)
    temp = LoadValue("Jubilee", "score2")
    If (temp <> "") then score(2) = CDbl(temp)
    temp = LoadValue("Jubilee", "score3")
    If (temp <> "") then score(3) = CDbl(temp)
    temp = LoadValue("Jubilee", "score4")
    If (temp <> "") then score(4) = CDbl(temp)
    temp = LoadValue("Jubilee", "balls")
    If (temp <> "") then balls = CDbl(temp)
    temp = LoadValue("Jubilee", "leftballs")
    If (temp <> "") then leftballs = CDbl(temp)
    temp = LoadValue("Jubilee", "rightballs")
    If (temp <> "") then rightballs = CDbl(temp)
    temp = LoadValue("Jubilee", "hsa1")
    If (temp <> "") then HSA1 = CDbl(temp)
    temp = LoadValue("Jubilee", "hsa2")
    If (temp <> "") then HSA2 = CDbl(temp)
    temp = LoadValue("Jubilee", "hsa3")
    If (temp <> "") then HSA3 = CDbl(temp)
    temp = LoadValue("Jubilee", "freeplay")
