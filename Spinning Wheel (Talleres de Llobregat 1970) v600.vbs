...
'
'  Spinning Wheel / IPD No. 6402 / 1 Player /
'  Talleres del Llobregat S.A. [Automaticos], of Barcelona, 1966
'  VPX - version by JPSalas 2020, version 6.0.0
' ***********************************************************************
...
' Constants
Const TableName = "spinningwheel" ' file name to save highscores and other variables
Const cGameName = "spinningwheel" ' B2S name
Const MaxPlayers = 1              ' 1 to 4 can play
Const MaxMultiplier = 1           ' limit bonus multiplier
Const FreePlay = False            ' Free play or coins
Const Special1 = 3200             ' extra ball or credit when reaching this score
Const Special2 = 4000
Const Special3 = 5400
...
Sub LeftFlipper1_Animate: LeftFlipperP1.RotZ = LeftFlipper1.CurrentAngle: End Sub
Sub RightFlipper1_Animate: RightFlipperP1.RotZ = RightFlipper1.CurrentAngle: End Sub
...
Sub Verification_Match()
    PlaySound "fx_match"
    Match = INT(RND(1) * 10) ' random between 0 and 10
    Display_Match
...
Sub UpdateScore(points)
    Dim d1, d2, d3, d4
    Dim old1, old2, old3, old4
    old1 = Oldscore MOD 10:Oldscore = Oldscore \ 10
    old2 = Oldscore MOD 10:Oldscore = Oldscore \ 10
    old3 = Oldscore MOD 10:Oldscore = Oldscore \ 10
    old4 = Oldscore MOD 10:Oldscore = Oldscore \ 10
    d1 = points MOD 10:points = points \ 10
    d2 = points MOD 10:points = points \ 10
    d3 = points MOD 10:points = points \ 10
    d4 = points MOD 10:points = points \ 10
    If d1 + old1 > 9 Then d2 = d2 + 1
    If d2 + old2 > 9 Then d3 = d3 + 1
    If d3 + old3 > 9 Then d4 = d4 + 1
    ScoreDigit001.AddValue d1
    ScoreDigit002.AddValue d2
    ScoreDigit003.AddValue d3
    ScoreDigit004.AddValue d4
    If B2SOn then
        points = Score(CurrentPlayer)
        d1 = points MOD 10:points = points \ 10
        d2 = points MOD 10:points = points \ 10
        d3 = points MOD 10:points = points \ 10
        d4 = points MOD 10:points = points \ 10
        Controller.B2SSetScorePlayer 1, d4
        Controller.B2SSetScorePlayer 2, d3
        Controller.B2SSetScorePlayer 3, d2
        Controller.B2SSetScorePlayer 4, d1
    End If
End Sub

Sub ResetScores
    ScoreDigit001.ResetToZero
    ScoreDigit002.ResetToZero
    ScoreDigit003.ResetToZero
    ScoreDigit004.ResetToZero
    If B2SOn then
...
Sub RubberBand010_Hit
    If Tilted then Exit Sub
    AddScore 10
    Rub1 = 1:RubberBand010_Timer
End Sub

Sub RubberBand010_Timer
    Select Case Rub1
        Case 1:r1.Visible = 0:r2.Visible = 1:RubberBand010.TimerEnabled = 1
        Case 2:r2.Visible = 0:r3.Visible = 1
        Case 3:r3.Visible = 0:r1.Visible = 1:RubberBand010.TimerEnabled = 0
    End Select
    Rub1 = Rub1 + 1
End Sub
...
Sub RubberBand003_Hit
    If Tilted then Exit Sub
    AddScore 10
    Rub2 = 0:RubberBand003_Timer
End Sub

Sub RubberBand003_Timer
    Select Case Rub2
        Case 1:r4.Visible = 0:r5.Visible = 1:RubberBand003.TimerEnabled = 1
        Case 2:r5.Visible = 0:r6.Visible = 1
        Case 3:r6.Visible = 0:r4.Visible = 1:RubberBand003.TimerEnabled = 0
    End Select
    Rub2 = Rub2 + 1
End Sub
...
Sub Bumper001_Hit
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF ("fx_Bumper",405,DOFPulse,DOFContactors), bumper001
    DOF 207, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    AddScore 1 + 9 * LightBumper001.State
    TopLightsCounter = TopLightsCounter + 1:UpdateTopLights
End Sub

Sub Bumper002_Hit
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF ("fx_Bumper",406,DOFPulse,DOFContactors), bumper002
    DOF 208, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    AddScore 1 + 9 * LightBumper002.State
    TopLightsCounter = TopLightsCounter + 1:UpdateTopLights
End Sub

Sub Bumper003_Hit
    If Tilted Then Exit Sub
    PlaySoundAt SoundFXDOF ("fx_Bumper",407,DOFPulse,DOFContactors), bumper003
    DOF 209, DOFPulse
    If Epileptikdof = True Then DOF 202, DOFPulse End If
    AddScore 1 + 9 * LightBumper003.State
    TopLightsCounter = TopLightsCounter + 1:UpdateTopLights
End Sub
...
Sub Trigger002_Hit
    ...
    DOF 218, DOFPulse
    AddWheelValue
    vpmtimer.addtimer 500, "CloseGate '"
End Sub
...
Sub Trigger004_Hit
    ...
    If Tilted Then Exit Sub
    DOF 221, DOFPulse
    AddScore 10 + 90 * Light040.State
    light019.State = 1
    If light018.State Then 'enable 10x & bonus
        light017.State = 1
        light001.State = 1
    End If
End Sub
...
Sub Trigger005_Hit
    ...
    AddScore 10 + 90 * Light040.State
    light018.State = 1
    If light019.State Then 'enable 10x & bonus
        light017.State = 1
        light001.State = 1
    End If
End Sub
...
Sub UpdateTopLights
    Select Case TopLightsCounter
        Case 1:light043.State = 1:LightBumper001.State = 1
        Case 2:light042.State = 1:LightBumper002.State = 1
        Case 3:light041.State = 1:LightBumper003.State = 1
        Case 4:light040.State = 1
    End Select
End Sub
...
Sub kicker1_Timer
    PlaySoundAt SoundFXDOF ("fx_kicker", 408, DOFPulse, DOFContactors), kicker1
    DOF 210, DOFPulse
    DOF 230, DOFPulse
    kicker1.kick 220, 12
...
Sub kicker2_Hit
    ...
    If Tilted Then kicker2.TimerEnabled = 1:Exit Sub
    AddWheelValue
    kicker2.TimerEnabled = 1
End Sub

Sub kicker2_Timer
    PlaySoundAt SoundFXDOF ("fx_kicker", 409, DOFPulse, DOFContactors), kicker2
    DOF 211, DOFPulse
    DOF 230, DOFPulse
    kicker2.kick 150, 12
...
Sub kicker3_Hit
    ...
    If Tilted Then kicker3.TimerEnabled = 1:Exit Sub
    AddWheelValue
    kicker3.TimerEnabled = 1
End Sub

Sub kicker3_Timer
    PlaySoundAt SoundFXDOF ("fx_kicker", 410, DOFPulse, DOFContactors), kicker3
    DOF 212, DOFPulse
    DOF 230, DOFPulse
    kicker3.kick 210, 12
...
Sub AddWheelValue
    debug.print WheelValue
    WheelRotate.Enabled = 0
    AddScore WheelValue
    If WheelPos = 0 AND light017.State Then
        light017.State = 0
        light018.State = 0
        light019.State = 0
        AwardSpecial
    End If
    vpmtimer.addtimer 500, "WheelRotate.Enabled = 1 '"
End Sub

Sub WheelRotate_Timer
    'debug. print WheelPos
    WheelPos = (WheelPos + 1)MOD 10
    Select Case WheelPos
        Case 0:light006.State = 0:light007.State = 1:light025.State = 0:light026.State = 1:WheelValue = 20
        Case 1:light007.State = 0:light010.State = 1:light026.State = 0:light027.State = 1:WheelValue = 30
        Case 2:light010.State = 0:light009.State = 1:light027.State = 0:light028.State = 1:WheelValue = 20
        Case 3:light009.State = 0:light008.State = 1:light028.State = 0:light029.State = 1:WheelValue = 40
        Case 4:light008.State = 0:light011.State = 1:light029.State = 0:light020.State = 1:WheelValue = 50
        Case 5:light011.State = 0:light002.State = 1:light020.State = 0:light021.State = 1:WheelValue = 20
        Case 6:light002.State = 0:light003.State = 1:light021.State = 0:light022.State = 1:WheelValue = 30
        Case 7:light003.State = 0:light004.State = 1:light022.State = 0:light023.State = 1:WheelValue = 20
        Case 8:light004.State = 0:light005.State = 1:light023.State = 0:light024.State = 1:WheelValue = 40
        Case 9:light005.State = 0:light006.State = 1:light024.State = 0:light025.State = 1:WheelValue = 50
    End Select
    If B2SOn Then 'XSTAT
        Select Case WheelPos
            Case 0:Controller.B2SSetData 108, 0:Controller.B2SSetData 99, 1
            Case 1:Controller.B2SSetData 99, 0:Controller.B2SSetData 100, 1
            Case 2:Controller.B2SSetData 100, 0:Controller.B2SSetData 101, 1
            Case 3:Controller.B2SSetData 101, 0:Controller.B2SSetData 102, 1
            Case 4:Controller.B2SSetData 102, 0:Controller.B2SSetData 103, 1
            Case 5:Controller.B2SSetData 103, 0:Controller.B2SSetData 104, 1
            Case 6:Controller.B2SSetData 104, 0:Controller.B2SSetData 105, 1
            Case 7:Controller.B2SSetData 105, 0:Controller.B2SSetData 106, 1
            Case 8:Controller.B2SSetData 106, 0:Controller.B2SSetData 107, 1
            Case 9:Controller.B2SSetData 107, 0:Controller.B2SSetData 108, 1
        End Select
    End If
    If light001.State Then
        WheelValue = WheelValue * 10
        Select Case WheelPos
            Case 0, 2, 5, 7:light013.State = 0:light014.State = 0:light015.State = 0:light016.State = 1
            Case 1, 6:light013.State = 0:light014.State = 0:light015.State = 1:light016.State = 0
            Case 3, 8:light013.State = 0:light014.State = 1:light015.State = 0:light016.State = 0
            Case 4, 9:light013.State = 1:light014.State = 0:light015.State = 0:light016.State = 0
        End Select
    End If
End Sub

Sub OpenGate:GateF.RotatetoEnd:PlaySoundAt "fx_diverter", gatef:Light012.State = 1:End Sub
Sub CloseGate:GateF.RotatetoStart:PlaySoundAt "fx_diverter", gatef:Light012.State = 0:End Sub

Sub FlipsIn
    PlaySoundAt "fx_solenoidon2", RightFlipper1
...
Sub FlipsOut
    PlaySoundAt "fx_solenoidoff2", RightFlipper
...
' ******************************************************************
'      Multiple High Score Display and Collection by GNANCE
