'
'    ______  _______  ______   ______   ______   ______  _______  ______
'   / |        | |   | |  | | | |  | \ | | ____ | |  | |   | |   | |
'   '------.   | |   | |__| | | |__| | | |  | | | |__| |   | |   | |----
'    ____|_/   |_|   |_|  |_| |_|  \_\ |_|__|_| |_|  |_|   |_|   |_|____
'
'
...
'********************************************************************************************
'
' STARGATE v2.0 CREDITS
'
' VPW CREW
' --------
' mcarter78 - Initial project build, physics, sounds, scripts, blender model
' MechaEnron - 3D modeling and rendering, playfield image cleanup
' Sixtoe - Table build updates and fixes
' Rothbauerw - Kicker target and trough updates
' Apophis - Physics updates and support
' DaRdog - VR Mega room
' DGrimmReaper - VR updates and fixes
' Rawd - VR Stargate animation
' Tomate - 3D modeling support
' JLou - Table assets and real machine behaviour consulting
' Wylte - Real machine consulting
' Testers - PinStratsDan, Studlygoorite, bthlonewolf, Paladin, guus_8005, StangRetro, bord, superhac
'
'********************************************************************************************
'
'
...
'********************************************************************************************
'
' STARGATE v1.xx Original Credits
'
' from 32Assassin and JLouLoulou
' With help from:
...
Dim PlungerIM
Dim plungerpress
Dim LastFlasherColorIdx : LastFlasherColorIdx = -1
Dim FlasherBlinkCount, FlasherBlinkMaxSteps, newFlasherColorIdx
Dim LastFlasherBrightness : LastFlasherBrightness = -1
Dim newFlasherBrightness
...
Sub Table1_Init
  ...
  With Controller
        ...
        .SplashInfoLine = "Stargate (Gottlieb 1995)" & vbNewLine & "VPW"
  ...
  Set SBall4 = Drain.CreateSizedballWithMass(Ballsize/2,Ballmass)
  ...
  Dim BO
  'For Each BO in BL_BP: BO.opacity = 150 : Next
  'For Each BO in BL_BB: BO.opacity = 150 : Next
  'For Each BO in BL_BR: BO.opacity = 150 : Next
  LM_BC_L126_VRRR.opacity = 100
  LM_TL_VRRR.opacity=50
  'LM_TL_Metal.opacity=20
  For Each BO in BC: BO.opacity = 150 : Next
...
Sub Table1_OptionEvent(ByVal eventId)
  ...
  'Backglass Color
  v = Table1.Option("Backglass Lamps Color", 0, 7, 1, 0, 0, Array("Incandescent (warm white)", "LED (cool white)", "Purple", "Blue", "Red", "Green", "Yellow", "JLou Style"))
  SetBGColor v
  ...
  'Flasher Lamps Color
  newFlasherColorIdx = Table1.Option("Flasher Lamps Color", 0, 6, 1, 0, 0, Array("Incandescent (warm white)", "LED (cool white)", "Purple", "Blue", "Red", "Green", "Yellow"))
  SetFlashersColor newFlasherColorIdx
  If LastFlasherColorIdx = -1 Then
    LastFlasherColorIdx = newFlasherColorIdx
    ElseIf newFlasherColorIdx <> LastFlasherColorIdx Then
    StartFlashersBlink 3, 100   ' 3 flashes, 100 ms toggle
        LastFlasherColorIdx = newFlasherColorIdx
    End If

  'Flasher Brightness
  newFlasherBrightness = Table1.Option("Flasher Lamps Brightness", 0, 200, 25, 200, 0)
  SetFlashersBrightness newFlasherBrightness
  If LastFlasherBrightness = -1 Then
    LastFlasherBrightness = newFlasherBrightness
  ElseIf newFlasherBrightness <> LastFlasherBrightness Then
    StartFlashersBlink 3, 100        ' 3 flashes, 100 ms toggle
    LastFlasherBrightness = newFlasherBrightness
  End If
  ...
  'Set slope
  Slope = Table1.Option("Playfield Angle (degrees)", 5, 10, 0.5, 6, 0)
  Table1.SlopeMin = Slope
  Table1.SlopeMax = Slope

  'VR Room
  VRRoomChoice = Table1.Option("VR Room", 1, 4, 1, 2, 0, Array("Portal", "ControlRoom", "Minimal", "Cab Only"))
  SetupRoom
...
Dim cArray
cArray = Array(c2700k,c5000k,cPurple,cBlue,cRed,cGreen,cYellow,cOrange)
Dim jArray
...
Sub SetBGColor(c)
  Dim xx, BL
  If c = 7 Then
    For each BL in BL_BB: BL.color = cArray(3): Next
    For each BL in BL_BP: BL.color = jArray(4): Next
    For each BL in BL_BR: BL.color = cArray(4): Next
    For each BL in BL_BF: BL.color = cArray(4): Next
    For each BL in BL_BO: BL.color = cArray(7): Next
    For each BL in BL_TL: BL.color = cArray(2): Next
    For each BL in BC: BL.color = cArray(3): Next
    LM_BC_L126_VRRR.color = cArray(4)
  Elseif c > 1 Then
    For each BL in BL_BB: BL.color = cArray(0): Next
    For each BL in BL_BP: BL.color = cArray(0): Next
    For each BL in BL_BR: BL.color = cArray(0): Next
    For each BL in BL_BO: BL.color = cArray(0): Next
    For each BL in BL_BF: BL.color = cArray(c): Next
    For each BL in BL_TL: BL.color = cArray(c): Next
    For each BL in BC: BL.color = cArray(c): Next
    LM_BC_L126_VRRR.color = cArray(4)
  Else
    For each BL in BL_BB: BL.color = cArray(c): Next
    For each BL in BL_BP: BL.color = cArray(c): Next
    For each BL in BL_BR: BL.color = cArray(c): Next
    For each BL in BL_BF: BL.color = cArray(c): Next
    For each BL in BL_BO: BL.color = cArray(c): Next
    For each BL in BL_TL: BL.color = cArray(c): Next
    For each BL in BC: BL.color = cArray(c): Next
    LM_BC_L126_VRRR.color = cArray(4)
  End If
...
' ========= Blink helper =========
Sub StartFlashersBlink(times, intervalMs)
    FlasherBlinkCount = 0
    FlasherBlinkMaxSteps = times * 2   ' ON+OFF per flash
    TestFlasherTimer.Interval = intervalMs
    TestFlasherTimer.Enabled = True
End Sub

' Timer object on table: Name=TestFlasherTimer, Interval=100, Enabled=False
Sub TestFlasherTimer_Timer()
    Dim xx
    If (FlasherBlinkCount Mod 2) = 0 Then
        For Each xx In Flashers: xx.State = 1: Next
    Else
        For Each xx In Flashers: xx.State = 0: Next
    End If

    FlasherBlinkCount = FlasherBlinkCount + 1
    If FlasherBlinkCount >= FlasherBlinkMaxSteps Then
        TestFlasherTimer.Enabled = False
        For Each xx In Flashers: xx.State = 0: Next
    End If
End Sub

sub SetFlashersBrightness(v)
  Dim BL
  For each BL in BL_F_f0: BL.opacity = v*2: Next
  For each BL in BL_F_f1: BL.opacity = v: Next
  For each BL in BL_F_f2: BL.opacity = v: Next
  For each BL in BL_F_f3: BL.opacity = v: Next
  For each BL in BL_F_f4: BL.opacity = v: Next
  For each BL in BL_F_f5: BL.opacity = v: Next
end Sub
...
Sub UpdatePlunger()
  If plungerpress = 1 then
    ...
    End If
  Else
    VR_Primary_plunger.Y = -10 + (5* Plunger.Position) -20
  End If
End Sub
...
Sub SolBGRopeLights(Enabled)
  If Enabled Then
'   If RenderingMode = 2 OR TestVRonDT = True Then BGS22.visible = 1
    If RenderingMode = 2 OR TestVRonDT = True Then BGTube.state = 1
  Else
    If RenderingMode = 2 OR TestVRonDT = True Then BGTube.state = 0
  End If
End Sub
...
Sub SolBBGI(Enabled)
If Not Enabled Then
  'If RenderingMode = 2 OR TestVRonDT = True Then BGLights.state = 1
Else
  'If RenderingMode = 2 OR TestVRonDT = True Then BGLights.state = 0
End If
End Sub
...
Sub swTrough3_UnHit : Controller.Switch(34) = 0 : UpdateTrough : End Sub
Sub swTrough3_Hit   : Controller.Switch(34) = 1 : UpdateTrough : End Sub
Sub Drain_UnHit  : Controller.Switch(24) = 0 : UpdateTrough : End Sub
...
Sub UpdateTroughTimer_Timer
  ...
  'debug.print "sw34" & controller.Switch(34) & "sw24" & controller.Switch(24)
End Sub
...
Sub SolTrough(enabled)
  If enabled Then
    'debug.print "drain" & drain.BallCntOver
    Drain.kick 57, 20
...
Sub SolRelease(enabled)
  If enabled Then
    'debug.print "slot1" & swTrough1.BallCntOver
...
Sub LeftPop(Enable)
    ...
    If Not IsEmpty(KickerBall80) Then
      KickBall KickerBall80, 0, 0, 40, 0
...
'StandUp Target
Sub sw22_Hit:vpmTimer.PulseSw 22: PlaySoundAtVol SoundFX("Target",DOFTargets) , ActiveBall, VolTarg: End Sub
...
Dim LeftGuardianDown : LeftGuardianDown = True
...
Sub SolPivL(Enabled)
  if enabled then
    ...
    LeftGuardianDown = True
  end if
End Sub

Sub TrapTriggerL_Hit
  If LeftGuardianDown Then
    'ball is stuck behind, trigger solcallback
    SolPivL True
    vpmTimer.AddTimer 1500, "SolPivL False'"
  End If
End Sub
...
Dim RightGuardianDown : RightGuardianDown = True
...
Sub SolPivR(Enabled)
  if enabled then
    ...
    RightGuardianDown = True
  end if
End Sub

Sub TrapTriggerR_Hit
  If RightGuardianDown Then
    'ball is stuck behind, trigger solcallback
    SolPivR True
    vpmTimer.AddTimer 1500, "SolPivR False'"
  End If
End Sub
...
Dim GlideMtlArray: GlideMtlArray = Array("VLM.Bake.Glide")
Dim GlideMtlArray1: GlideMtlArray1 = Array("VLM.Bake.Glide1")

Sub SetGlideBrightness

  ' Lighting level
  Dim v: v=GliderY/138 * (NightDay/100) + .2
  Dim h: h=GliderY/138 * (NightDay/100) + .25
  'debug.print v

  Dim i: For i = 0 to UBound(GlideMtlArray)
    ModulateMaterialBaseColorGlide GlideMtlArray(i), i, v
  Next
  Dim j: For j = 0 to UBound(GlideMtlArray1)
    ModulateMaterialBaseColorGlide GlideMtlArray1(j), j, h
  Next
End Sub

Dim SavedMtlColorArrayGlide
SaveMtlColorsGlide
Sub SaveMtlColorsGlide
  ReDim SavedMtlColorArrayGlide(UBound(GlideMtlArray))
  Dim i: For i = 0 to UBound(GlideMtlArray)
    SaveMaterialBaseColorGlide GlideMtlArray(i), i
  Next
  ReDim SavedMtlColorArrayGlide2(UBound(GlideMtlArray1))
  Dim j: For j = 0 to UBound(GlideMtlArray1)
    SaveMaterialBaseColorGlide GlideMtlArray1(j), j
  Next
End Sub

Sub SaveMaterialBaseColorGlide(name, idx)
  Dim wrapLighting, roughness, glossyImageLerp, thickness, edge, edgeAlpha, opacity, base, glossy, clearcoat, isMetal, opacityActive, elasticity, elasticityFalloff, friction, scatterAngle
  GetMaterial name, wrapLighting, roughness, glossyImageLerp, thickness, edge, edgeAlpha, opacity, base, glossy, clearcoat, isMetal, opacityActive, elasticity, elasticityFalloff, friction, scatterAngle
  SavedMtlColorArrayGlide(idx) = round(base,0)
End Sub


Sub ModulateMaterialBaseColorGlide(name, idx, val)
  Dim wrapLighting, roughness, glossyImageLerp, thickness, edge, edgeAlpha, opacity, base, glossy, clearcoat, isMetal, opacityActive, elasticity, elasticityFalloff, friction, scatterAngle
...
Sub RightSlingShot_Slingshot
  ...
  PlaysoundAtVol "S04_SlingshotRight", sw113, VolTarg
End Sub
...
Sub LeftSlingShot_Slingshot
  ...
  PlaysoundAtVol "S03_SlingshotLeft", sw112, VolTarg
End Sub
...
Function KTAnimate(arr) '(, prim, switch,  animate, aball)
  ...
  If btwidth < kWidth/2 + 25 Then
    arr.prim.transy = arr.prim.transy + btdist - tdist
  Else
    arr.prim.transy = arr.prim.transy + 5
  ...
  'debug.print "Sw14 = " & arr.prim.transy & " " & btdist
  ...
  If arr.animate = 1 Then
    ...
    If btdist < tdist and btwidth < kWidth/2 + 25 Then
      ...
      End If
    Elseif btdist > tdist and btwidth < kWidth/2 + 25 then
      arr.ball.velx = arr.ball.velx - (kSpring * dsin(angle) * abs(arr.prim.transy) * animtime/1000)
      arr.ball.vely = arr.ball.vely + (kSpring * dcos(angle) * abs(arr.prim.transy) * animtime/1000)
...
Sub SetupRoom
  ...
  Else
    VRRoom = 0
    Dim BV
    For each BV in BL_BB: BV.visible=0: Next
    For each BV in BL_BR: BV.visible=0: Next
    For each BV in BL_BP: BV.visible=0: Next
    For each BV in BL_BF: BV.visible=0: Next
    For each BV in BL_BO: BV.visible=0: Next
    For each BV in BC: BV.visible=0: Next
    LM_BC_L126_VRRR.visible=0
    For each BV in BL_TL: BV.visible=0: Next
  End If
  If VRRoom = 1 Then
    for each VRThings in VRCab:VRThings.visible = 1:Next
    for each VRThings in VRMin:VRThings.visible = 0:Next
    for each VRThings in VRMega:VRThings.visible = 1: VRThings.x = -10009.52 : VRThings.y = 2100.889 : VRThings.z = -10:Next
    VR_Mega046.image ="VR_Gateroom_floor.001_Bake1_CyclesBake_COMBINED_1"
    VR_Mega047.image ="VR_Gateroom_floor.002_Bake1_CyclesBake_COMBINED_1shadow"

    VRPortalTimer.enabled = true

    VR_PortalLight.visible = 1
    VR_Portal.visible = 1
    VR_Portal2.visible = 1

    VR_PortalLight.x = -15090.79
    VR_PortalLight.y = 1584.16
    VR_PortalLight.z = 4800

    VR_Portal.x = -14985.39
    VR_Portal.y = 1732.575
    VR_Portal.z = 4800

    VR_Portal2.x = -14950.5
    VR_Portal2.y = 1732.575
    VR_Portal2.z = 4800
    'difference X : -15,594.08  , y: +596.741  ,  z: -2610
  ...
  If VRRoom = 2 Then
    for each VRThings in VRCab:VRThings.visible = 1:Next
    for each VRThings in VRMin:VRThings.visible = 0:Next
    for each VRThings in VRMega:VRThings.visible = 1: VRThings.x = -25603.6 : VRThings.y = 2697.63 : VRThings.z = -2620:Next
    VR_Mega046.image ="VR_Gateroom_floor.001_Bake1_CyclesBake_COMBINED_1shadow"
    VR_Mega047.image ="VR_Gateroom_floor.002_Bake1_CyclesBake_COMBINED_1"
    ...
    VR_Portal2.visible = 1
      ...
      VR_PortalLight.x = -30684.87
    VR_PortalLight.y = 2180.901
    VR_PortalLight.z = 2190

    VR_Portal.x = -30579.47
    VR_Portal.y = 2329.316
    VR_Portal.z = 2190

    VR_Portal2.x = -30544.58
    VR_Portal2.y = 2329.316
    VR_Portal2.z = 2190
  End If
  ...
  If VRRoom = 3 Then
    ...
    for each VRThings in VRMega:VRThings.visible = 0:Next
    VRPortalTimer.enabled = false
    VR_PortalLight.visible = 0
    VR_Portal.visible = 0
    VR_Portal2.visible = 0
  End If
...
Dim RoomBrightnessMtlArray: RoomBrightnessMtlArray = Array("VLM.Bake.Active","VLM.Bake.Solid", "VLM.Bake.Pyramid","VLM.Bake.Glide","VLM.Bake.Glide1","_noXtraShadinglight")
...
Dim BP_DT_sw26: BP_DT_sw26=Array(BM_DT_sw26, LM_GIG_DT_sw26, LM_GIP_GI_007_DT_sw26, LM_GIB_DT_sw26, LM_L_L17_DT_sw26, LM_L_L33_DT_sw26, LM_L_L34_DT_sw26, LM_F_f4_DT_sw26, LM_F_f5_DT_sw26)
...
Dim BP_GuardianL: BP_GuardianL=Array(BM_GuardianL, LM_GIG_GuardianL, LM_GIB_GuardianL, LM_L_L44_GuardianL, LM_F_f1_GuardianL, LM_F_f4_GuardianL)
...
Dim BP_GuardianR_001: BP_GuardianR_001=Array(BM_GuardianR_001, LM_GIG_GuardianR_001, LM_GIR_GuardianR_001, LM_GIY_GuardianR_001, LM_L_L67_GuardianR_001, LM_F_f2_GuardianR_001, LM_F_f3_GuardianR_001, LM_F_f4_GuardianR_001)
...
Dim BP_LF: BP_LF=Array(BM_LF, LM_GIP_GI_001_LF, LM_GIP_GI_002_LF, LM_GIG_LF, LM_L_L0_LF, LM_L_L82_LF, LM_L_L83_LF)
...
Dim BP_LSling: BP_LSling=Array(BM_LSling, LM_GIP_GI_001_LSling, LM_GIP_GI_002_LSling, LM_GIG_LSling, LM_GIP_GI_007_LSling, LM_L_L11_LSling, LM_L_L12_LSling, LM_L_L82_LSling, LM_F_f5_LSling)
Dim BP_LSling1: BP_LSling1=Array(BM_LSling1, LM_GIP_GI_001_LSling1, LM_GIP_GI_002_LSling1, LM_GIG_LSling1, LM_GIP_GI_007_LSling1, LM_L_L11_LSling1, LM_L_L12_LSling1, LM_L_L81_LSling1, LM_L_L82_LSling1, LM_F_f4_LSling1, LM_F_f5_LSling1)
Dim BP_LSling2: BP_LSling2=Array(BM_LSling2, LM_GIP_GI_001_LSling2, LM_GIP_GI_002_LSling2, LM_GIG_LSling2, LM_GIP_GI_007_LSling2, LM_L_L11_LSling2, LM_L_L12_LSling2, LM_L_L82_LSling2, LM_F_f4_LSling2, LM_F_f5_LSling2)
Dim BP_Layer1: BP_Layer1=Array(BM_Layer1, LM_GIP_GI_002_Layer1, LM_GIG_Layer1, LM_GIP_GI_006_Layer1, LM_GIR_Layer1, LM_GIB_Layer1, LM_GIY_Layer1, LM_L_L25_Layer1, LM_L_L26_Layer1, LM_L_L27_Layer1, LM_F_f1_Layer1, LM_F_f2_Layer1, LM_F_f4_Layer1, LM_F_f5_Layer1)
Dim BP_Layer2: BP_Layer2=Array(BM_Layer2, LM_GIP_GI_002_Layer2, LM_GIG_Layer2, LM_GIP_GI_007_Layer2, LM_GIR_Layer2, LM_L_L11_Layer2, LM_L_L12_Layer2, LM_L_L43_Layer2, LM_F_f1_Layer2, LM_F_f2_Layer2, LM_F_f3_Layer2)
...
Dim BP_RSling2: BP_RSling2=Array(BM_RSling2, LM_GIP_GI_001_RSling2, LM_GIP_GI_002_RSling2, LM_GIG_RSling2, LM_GIP_GI_006_RSling2, LM_L_L13_RSling2, LM_L_L6_RSling2, LM_L_L7_RSling2, LM_L_L85_RSling2, LM_F_f5_RSling2)
...
Dim BP_sw113: BP_sw113=Array(BM_sw113, LM_GIP_GI_001_sw113, LM_GIG_sw113, LM_F_f5_sw113)
...
Dim BP_Glass: BP_Glass=Array(BM_Glass, LM_BC_L90_Glass, LM_BF_Glass, LM_BO_Glass, LM_BP_Glass, LM_BB_Glass, LM_BR_Glass)
Dim BP_Metal: BP_Metal=Array(BM_Metal, LM_BC_L92_Metal, LM_BC_L112_Metal, LM_BC_L90_Metal, LM_BF_Metal, LM_BO_Metal, LM_BP_Metal, LM_BB_Metal, LM_BR_Metal, LM_TL_Metal)
Dim BP_VRRR: BP_VRRR=Array(BM_VRRR, LM_BC_L92_VRRR, LM_BC_L112_VRRR, LM_BC_L90_VRRR, LM_BF_VRRR, LM_BO_VRRR, LM_BP_VRRR, LM_BB_VRRR, LM_BR_VRRR, LM_BC_L126_VRRR, LM_TL_VRRR)
...
Dim BL_F_f1: BL_F_f1=Array(LM_F_f1_DT_sw36, LM_F_f1_Glider_1, LM_F_f1_Glider_1_001, LM_F_f1_Glider_2, LM_F_f1_GuardianL, LM_F_f1_GuardianL_001, LM_F_f1_GuardianR, LM_F_f1_KT_sw15, LM_F_f1_Layer1, LM_F_f1_Layer2, LM_F_f1_Parts, LM_F_f1_Plastics, LM_F_f1_Playfield, LM_F_f1_Pyramid1, LM_F_f1_RF1U, LM_F_f1_ST_sw32, LM_F_f1_SideBlades_Art, LM_F_f1_UnderPF)
Dim BL_F_f2: BL_F_f2=Array(LM_F_f2_Glider_1, LM_F_f2_Glider_1_001, LM_F_f2_Glider_1_002, LM_F_f2_Glider_2, LM_F_f2_Glider_2_001, LM_F_f2_GuardianR, LM_F_f2_GuardianR_001, LM_F_f2_KT_sw15, LM_F_f2_Layer1, LM_F_f2_Layer2, LM_F_f2_Parts, LM_F_f2_Plastics, LM_F_f2_Playfield, LM_F_f2_Pyramid1, LM_F_f2_Pyramid1_001, LM_F_f2_ST_sw22, LM_F_f2_SideBlades_Art, LM_F_f2_UnderPF, LM_F_f2_Wood_guide___Painted_Bl)
...
Dim BL_GIR: BL_GIR=Array(LM_GIR_DT_sw35, LM_GIR_GuardianR, LM_GIR_GuardianR_001, LM_GIR_KT_sw15, LM_GIR_KT_sw16, LM_GIR_Layer1, LM_GIR_Layer2, LM_GIR_Parts, LM_GIR_Plastics, LM_GIR_Playfield, LM_GIR_RF1, LM_GIR_RF1U, LM_GIR_SideBlades_Art, LM_GIR_UnderPF)
Dim BL_GIY: BL_GIY=Array(LM_GIY_Glider_1, LM_GIY_Glider_1_001, LM_GIY_Glider_1_002, LM_GIY_Glider_2, LM_GIY_GuardianR, LM_GIY_GuardianR_001, LM_GIY_KT_sw15, LM_GIY_Layer1, LM_GIY_Parts, LM_GIY_Plastics, LM_GIY_Playfield, LM_GIY_ST_sw22, LM_GIY_ST_sw32, LM_GIY_SideBlades_Art, LM_GIY_UnderPF)
Dim BL_L_L0: BL_L_L0=Array(LM_L_L0_LF, LM_L_L0_Parts, LM_L_L0_Playfield, LM_L_L0_RF, LM_L_L0_UnderPF)
Dim BL_L_L11: BL_L_L11=Array(LM_L_L11_LSling, LM_L_L11_LSling1, LM_L_L11_LSling2, LM_L_L11_Layer2, LM_L_L11_Parts, LM_L_L11_Plastics, LM_L_L11_Playfield)
...
Dim BL_L_L45: BL_L_L45=Array(LM_L_L45_DT_sw36, LM_L_L45_Parts, LM_L_L45_Playfield, LM_L_L45_UnderPF)
...
Dim BL_L_L86: BL_L_L86=Array(LM_L_L86_Parts, LM_L_L86_Playfield, LM_L_L86_RSling1, LM_L_L86_UnderPF)
Dim BL_L_L87: BL_L_L87=Array(LM_L_L87_Parts, LM_L_L87_Playfield, LM_L_L87_UnderPF)
Dim BL_BB: BL_BB=Array(LM_BB_Glass, LM_BB_Metal, LM_BB_VRRR)
Dim BL_BC_L112: BL_BC_L112=Array(LM_BC_L112_Metal, LM_BC_L112_VRRR)
Dim BL_BC_L126: BL_BC_L126=Array(LM_BC_L126_VRRR)
Dim BL_BC_L90: BL_BC_L90=Array(LM_BC_L90_Glass, LM_BC_L90_Metal, LM_BC_L90_VRRR)
Dim BL_BC_L92: BL_BC_L92=Array(LM_BC_L92_Metal, LM_BC_L92_VRRR)
Dim BL_BF: BL_BF=Array(LM_BF_Glass, LM_BF_Metal, LM_BF_VRRR)
Dim BL_BO: BL_BO=Array(LM_BO_Glass, LM_BO_Metal, LM_BO_VRRR)
Dim BL_BP: BL_BP=Array(LM_BP_Glass, LM_BP_Metal, LM_BP_VRRR)
Dim BL_BR: BL_BR=Array(LM_BR_Glass, LM_BR_Metal, LM_BR_VRRR)
Dim BL_TL: BL_TL=Array(LM_TL_Metal, LM_TL_VRRR)
Dim BL_World: BL_World=Array(BM_Glass, BM_Metal, BM_VRRR,BM_Autoplunger, BM_Bumper1_Ring, BM_Bumper1_Skirt, BM_Bumper2_Ring, BM_Bumper2_Skirt, BM_DT_sw17, BM_DT_sw26, BM_DT_sw27, BM_DT_sw35, BM_DT_sw36, BM_DT_sw37, BM_Glider_1, BM_Glider_1_001, BM_Glider_1_002, BM_Glider_2, BM_Glider_2_001, BM_GuardianL, BM_GuardianL_001, BM_GuardianR, BM_GuardianR_001, BM_KT_sw14, BM_KT_sw15, BM_KT_sw16, BM_LF, BM_LFU, BM_LSling, BM_LSling1, BM_LSling2, BM_Layer1, BM_Layer2, BM_Parts, BM_PinCab_Rails_021, BM_PinCab_Rails_022, BM_PinCab_Rails_023, BM_Plastics, BM_Playfield, BM_Pyramid1, BM_Pyramid1_001, BM_RF, BM_RF1, BM_RF1U, BM_RFU, BM_RSling, BM_RSling1, BM_RSling2, BM_ST_sw22, BM_ST_sw32, BM_SarcArm, BM_SarcArmU, BM_SideBlades_Art, BM_SlingL, BM_SlingR, BM_UnderPF, BM_Wood_guide___Painted_Blaxk_1, BM_sw111, BM_sw112, BM_sw113, BM_sw114, BM_sw115, BM_sw31)
' Global arrays
Dim BG_Bakemap: BG_Bakemap=Array(BM_Glass, BM_Metal, BM_VRRR,BM_Autoplunger, BM_Bumper1_Ring, BM_Bumper1_Skirt, BM_Bumper2_Ring, BM_Bumper2_Skirt, BM_DT_sw17, BM_DT_sw26, BM_DT_sw27, BM_DT_sw35, BM_DT_sw36, BM_DT_sw37, BM_Glider_1, BM_Glider_1_001, BM_Glider_1_002, BM_Glider_2, BM_Glider_2_001, BM_GuardianL, BM_GuardianL_001, BM_GuardianR, BM_GuardianR_001, BM_KT_sw14, BM_KT_sw15, BM_KT_sw16, BM_LF, BM_LFU, BM_LSling, BM_LSling1, BM_LSling2, BM_Layer1, BM_Layer2, BM_Parts, BM_PinCab_Rails_021, BM_PinCab_Rails_022, BM_PinCab_Rails_023, BM_Plastics, BM_Playfield, BM_Pyramid1, BM_Pyramid1_001, BM_RF, BM_RF1, BM_RF1U, BM_RFU, BM_RSling, BM_RSling1, BM_RSling2, BM_ST_sw22, BM_ST_sw32, BM_SarcArm, BM_SarcArmU, BM_SideBlades_Art, BM_SlingL, BM_SlingR, BM_UnderPF, BM_Wood_guide___Painted_Blaxk_1, BM_sw111, BM_sw112, BM_sw113, BM_sw114, BM_sw115, BM_sw31)
Dim BG_Lightmap: BG_Lightmap=Array(LM_BB_Glass, LM_BB_Metal, LM_BB_VRRR, LM_BC_L112_Metal, LM_BC_L112_VRRR, LM_BC_L126_VRRR, LM_BC_L90_Glass, LM_BC_L90_Metal, LM_BC_L90_VRRR, LM_BC_L92_Metal, LM_BC_L92_VRRR, LM_BF_Glass, LM_BF_Metal, LM_BF_VRRR, LM_BO_Glass, LM_BO_Metal, LM_BO_VRRR, LM_BP_Glass, LM_BP_Metal, LM_BP_VRRR, LM_BR_Glass, LM_BR_Metal, LM_BR_VRRR, LM_TL_Metal, LM_TL_VRRR,LM_F_f0_Bumper1_Ring, LM_F_f0_Bumper1_Skirt, LM_F_f0_Bumper2_Ring, LM_F_f0_Bumper2_Skirt, LM_F_f0_KT_sw14, LM_F_f0_Parts, LM_F_f0_Plastics, LM_F_f0_Playfield, LM_F_f0_SideBlades_Art, LM_F_f0_UnderPF, LM_F_f1_DT_sw36, LM_F_f1_Glider_1, LM_F_f1_Glider_1_001, LM_F_f1_Glider_2, LM_F_f1_GuardianL, LM_F_f1_GuardianL_001, LM_F_f1_GuardianR, LM_F_f1_KT_sw15, LM_F_f1_Layer1, LM_F_f1_Layer2, LM_F_f1_Parts, LM_F_f1_Plastics, LM_F_f1_Playfield, LM_F_f1_Pyramid1, LM_F_f1_RF1U, LM_F_f1_ST_sw32, LM_F_f1_SideBlades_Art, LM_F_f1_UnderPF, LM_F_f2_Glider_1, LM_F_f2_Glider_1_001, LM_F_f2_Glider_1_002, LM_F_f2_Glider_2, LM_F_f2_Glider_2_001, LM_F_f2_GuardianR, LM_F_f2_GuardianR_001, LM_F_f2_KT_sw15, LM_F_f2_Layer1, LM_F_f2_Layer2, LM_F_f2_Parts, LM_F_f2_Plastics, LM_F_f2_Playfield, LM_F_f2_Pyramid1, LM_F_f2_Pyramid1_001, LM_F_f2_ST_sw22, LM_F_f2_SideBlades_Art, LM_F_f2_UnderPF, LM_F_f2_Wood_guide___Painted_Bl, LM_F_f3_Glider_1, LM_F_f3_Glider_1_001, LM_F_f3_Glider_1_002, LM_F_f3_Glider_2, _
  LM_F_f3_Glider_2_001, LM_F_f3_GuardianR, LM_F_f3_GuardianR_001, LM_F_f3_Layer2, LM_F_f3_Parts, LM_F_f3_Plastics, LM_F_f3_Playfield, LM_F_f3_Pyramid1, LM_F_f3_Pyramid1_001, LM_F_f3_SideBlades_Art, LM_F_f3_UnderPF, LM_F_f3_Wood_guide___Painted_Bl, LM_F_f4_DT_sw26, LM_F_f4_DT_sw36, LM_F_f4_Glider_1, LM_F_f4_Glider_1_001, LM_F_f4_Glider_1_002, LM_F_f4_Glider_2, LM_F_f4_Glider_2_001, LM_F_f4_GuardianL, LM_F_f4_GuardianL_001, LM_F_f4_GuardianR, LM_F_f4_GuardianR_001, LM_F_f4_KT_sw15, LM_F_f4_LSling1, LM_F_f4_LSling2, LM_F_f4_Layer1, LM_F_f4_Parts, LM_F_f4_Plastics, LM_F_f4_Playfield, LM_F_f4_Pyramid1, LM_F_f4_RF1, LM_F_f4_RF1U, LM_F_f4_ST_sw22, LM_F_f4_ST_sw32, LM_F_f4_UnderPF, LM_F_f4_sw112, LM_F_f5_Bumper2_Ring, LM_F_f5_DT_sw17, LM_F_f5_DT_sw26, LM_F_f5_DT_sw27, LM_F_f5_DT_sw35, LM_F_f5_DT_sw36, LM_F_f5_DT_sw37, LM_F_f5_KT_sw16, LM_F_f5_LFU, LM_F_f5_LSling, LM_F_f5_LSling1, LM_F_f5_LSling2, LM_F_f5_Layer1, LM_F_f5_Parts, LM_F_f5_Plastics, LM_F_f5_Playfield, LM_F_f5_RF, LM_F_f5_RF1, LM_F_f5_RF1U, LM_F_f5_RFU, _
  LM_F_f5_RSling, LM_F_f5_RSling1, LM_F_f5_RSling2, LM_F_f5_SarcArm, LM_F_f5_SideBlades_Art, LM_F_f5_SlingL, LM_F_f5_UnderPF, LM_F_f5_sw113, LM_GIB_Bumper1_Skirt, LM_GIB_Bumper2_Ring, LM_GIB_Bumper2_Skirt, LM_GIB_DT_sw26, LM_GIB_DT_sw36, LM_GIB_DT_sw37, LM_GIB_GuardianL, LM_GIB_GuardianL_001, LM_GIB_KT_sw14, LM_GIB_KT_sw15, LM_GIB_Layer1, LM_GIB_Parts, LM_GIB_Plastics, LM_GIB_Playfield, LM_GIB_Pyramid1, LM_GIB_SideBlades_Art, LM_GIB_UnderPF, LM_GIG_Bumper1_Ring, LM_GIG_DT_sw17, LM_GIG_DT_sw26, LM_GIG_DT_sw27, LM_GIG_DT_sw36, LM_GIG_DT_sw37, LM_GIG_Glider_1, LM_GIG_Glider_1_002, LM_GIG_GuardianL, LM_GIG_GuardianL_001, LM_GIG_GuardianR, LM_GIG_GuardianR_001, LM_GIG_KT_sw15, LM_GIG_LF, LM_GIG_LFU, LM_GIG_LSling, LM_GIG_LSling1, LM_GIG_LSling2, LM_GIG_Layer1, LM_GIG_Layer2, LM_GIG_Parts, LM_GIG_Plastics, LM_GIG_Playfield, LM_GIG_Pyramid1, LM_GIG_RF, LM_GIG_RF1U, LM_GIG_RFU, LM_GIG_RSling, LM_GIG_RSling1, LM_GIG_RSling2, LM_GIG_ST_sw22, LM_GIG_ST_sw32, LM_GIG_SarcArmU, LM_GIG_SideBlades_Art, LM_GIG_SlingL, _
  LM_GIG_SlingR, LM_GIG_UnderPF, LM_GIG_sw111, LM_GIG_sw112, LM_GIG_sw113, LM_GIG_sw114, LM_GIP_GI_001_DT_sw17, LM_GIP_GI_001_DT_sw27, LM_GIP_GI_001_LF, LM_GIP_GI_001_LFU, LM_GIP_GI_001_LSling, LM_GIP_GI_001_LSling1, LM_GIP_GI_001_LSling2, LM_GIP_GI_001_Parts, LM_GIP_GI_001_Plastics, LM_GIP_GI_001_Playfield, LM_GIP_GI_001_RF, LM_GIP_GI_001_RFU, LM_GIP_GI_001_RSling, LM_GIP_GI_001_RSling1, LM_GIP_GI_001_RSling2, LM_GIP_GI_001_SideBlades_Art, LM_GIP_GI_001_SlingR, LM_GIP_GI_001_UnderPF, LM_GIP_GI_001_sw113, LM_GIP_GI_002_Bumper1_Ring, LM_GIP_GI_002_DT_sw17, LM_GIP_GI_002_DT_sw37, LM_GIP_GI_002_LF, LM_GIP_GI_002_LFU, LM_GIP_GI_002_LSling, LM_GIP_GI_002_LSling1, LM_GIP_GI_002_LSling2, LM_GIP_GI_002_Layer1, LM_GIP_GI_002_Layer2, LM_GIP_GI_002_Parts, LM_GIP_GI_002_Plastics, LM_GIP_GI_002_Playfield, LM_GIP_GI_002_RF, LM_GIP_GI_002_RFU, LM_GIP_GI_002_RSling, LM_GIP_GI_002_RSling1, LM_GIP_GI_002_RSling2, LM_GIP_GI_002_SarcArm, LM_GIP_GI_002_SarcArmU, LM_GIP_GI_002_SideBlades_Art, LM_GIP_GI_002_SlingL, _
  LM_GIP_GI_002_UnderPF, LM_GIP_GI_002_sw111, LM_GIP_GI_002_sw112, LM_GIP_GI_006_DT_sw17, LM_GIP_GI_006_DT_sw27, LM_GIP_GI_006_DT_sw37, LM_GIP_GI_006_KT_sw16, LM_GIP_GI_006_Layer1, LM_GIP_GI_006_Parts, LM_GIP_GI_006_Plastics, LM_GIP_GI_006_Playfield, LM_GIP_GI_006_RF1, LM_GIP_GI_006_RF1U, LM_GIP_GI_006_RSling, LM_GIP_GI_006_RSling1, LM_GIP_GI_006_RSling2, LM_GIP_GI_006_SideBlades_Art, LM_GIP_GI_006_UnderPF, LM_GIP_GI_007_Bumper1_Ring, LM_GIP_GI_007_Bumper1_Skirt, LM_GIP_GI_007_Bumper2_Ring, LM_GIP_GI_007_Bumper2_Skirt, LM_GIP_GI_007_DT_sw17, LM_GIP_GI_007_DT_sw26, LM_GIP_GI_007_DT_sw27, LM_GIP_GI_007_DT_sw36, LM_GIP_GI_007_DT_sw37, LM_GIP_GI_007_LFU, LM_GIP_GI_007_LSling, LM_GIP_GI_007_LSling1, LM_GIP_GI_007_LSling2, LM_GIP_GI_007_Layer2, LM_GIP_GI_007_Parts, LM_GIP_GI_007_Plastics, LM_GIP_GI_007_Playfield, LM_GIP_GI_007_SideBlades_Art, LM_GIP_GI_007_UnderPF, LM_GIP_GI_007_sw115, LM_GIR_DT_sw35, LM_GIR_GuardianR, LM_GIR_GuardianR_001, LM_GIR_KT_sw15, LM_GIR_KT_sw16, LM_GIR_Layer1, LM_GIR_Layer2, LM_GIR_Parts, _
  LM_GIR_Plastics, LM_GIR_Playfield, LM_GIR_RF1, LM_GIR_RF1U, LM_GIR_SideBlades_Art, LM_GIR_UnderPF, LM_GIY_Glider_1, LM_GIY_Glider_1_001, LM_GIY_Glider_1_002, LM_GIY_Glider_2, LM_GIY_GuardianR, LM_GIY_GuardianR_001, LM_GIY_KT_sw15, LM_GIY_Layer1, LM_GIY_Parts, LM_GIY_Plastics, LM_GIY_Playfield, LM_GIY_ST_sw22, LM_GIY_ST_sw32, LM_GIY_SideBlades_Art, LM_GIY_UnderPF, LM_L_L0_LF, LM_L_L0_Parts, LM_L_L0_Playfield, LM_L_L0_RF, LM_L_L0_UnderPF, LM_L_L11_LSling, LM_L_L11_LSling1, LM_L_L11_LSling2, LM_L_L11_Layer2, LM_L_L11_Parts, LM_L_L11_Plastics, LM_L_L11_Playfield, LM_L_L12_LSling, LM_L_L12_LSling1, LM_L_L12_LSling2, LM_L_L12_Layer2, LM_L_L12_Parts, LM_L_L12_Playfield, LM_L_L12_UnderPF, LM_L_L13_Parts, LM_L_L13_Playfield, LM_L_L13_RSling, LM_L_L13_RSling1, LM_L_L13_RSling2, LM_L_L13_UnderPF, LM_L_L14_Parts, LM_L_L14_Playfield, LM_L_L14_UnderPF, LM_L_L15_Bumper1_Skirt, LM_L_L15_DT_sw37, LM_L_L15_Parts, LM_L_L15_Playfield, LM_L_L15_UnderPF, LM_L_L16_Parts, LM_L_L16_Playfield, LM_L_L16_UnderPF, LM_L_L17_Bumper1_Skirt, _
  LM_L_L17_DT_sw26, LM_L_L17_Parts, LM_L_L17_Playfield, LM_L_L17_UnderPF, LM_L_L21_Parts, LM_L_L21_Playfield, LM_L_L21_UnderPF, LM_L_L22_Parts, LM_L_L22_Playfield, LM_L_L22_UnderPF, LM_L_L23_Parts, LM_L_L23_Playfield, LM_L_L23_UnderPF, LM_L_L24_Parts, LM_L_L24_Playfield, LM_L_L24_UnderPF, LM_L_L25_Layer1, LM_L_L25_Parts, LM_L_L25_Playfield, LM_L_L25_RF1, LM_L_L25_RF1U, LM_L_L25_UnderPF, LM_L_L26_Layer1, LM_L_L26_Parts, LM_L_L26_Playfield, LM_L_L26_RF1U, LM_L_L26_UnderPF, LM_L_L27_DT_sw35, LM_L_L27_Layer1, LM_L_L27_Parts, LM_L_L27_Playfield, LM_L_L27_UnderPF, LM_L_L32_Parts, LM_L_L32_Playfield, LM_L_L32_UnderPF, LM_L_L33_DT_sw26, LM_L_L33_Parts, LM_L_L33_Playfield, LM_L_L33_UnderPF, LM_L_L34_DT_sw26, LM_L_L34_DT_sw36, LM_L_L34_Parts, LM_L_L34_Playfield, LM_L_L34_UnderPF, LM_L_L35_Parts, LM_L_L35_Playfield, LM_L_L35_RF1U, LM_L_L36_Parts, LM_L_L36_Playfield, LM_L_L36_UnderPF, LM_L_L37_Parts, LM_L_L37_Playfield, LM_L_L37_UnderPF, LM_L_L41_Bumper1_Ring, LM_L_L41_Bumper1_Skirt, LM_L_L41_Parts, LM_L_L42_Bumper2_Ring, _
  LM_L_L42_Bumper2_Skirt, LM_L_L42_Parts, LM_L_L42_Plastics, LM_L_L42_UnderPF, LM_L_L43_Bumper1_Ring, LM_L_L43_Bumper1_Skirt, LM_L_L43_Layer2, LM_L_L43_Parts, LM_L_L43_Playfield, LM_L_L43_SideBlades_Art, LM_L_L43_UnderPF, LM_L_L44_Bumper2_Ring, LM_L_L44_Bumper2_Skirt, LM_L_L44_GuardianL, LM_L_L44_GuardianL_001, LM_L_L44_KT_sw14, LM_L_L44_Parts, LM_L_L44_Playfield, LM_L_L44_UnderPF, LM_L_L45_DT_sw36, LM_L_L45_Parts, LM_L_L45_Playfield, LM_L_L45_UnderPF, LM_L_L46_DT_sw36, LM_L_L46_Parts, LM_L_L46_Playfield, LM_L_L46_ST_sw32, LM_L_L46_UnderPF, LM_L_L47_Parts, LM_L_L47_Playfield, LM_L_L47_ST_sw32, LM_L_L47_UnderPF, LM_L_L5_Parts, LM_L_L5_Playfield, LM_L_L5_RSling1, LM_L_L55_Parts, LM_L_L55_Playfield, LM_L_L55_ST_sw32, LM_L_L55_UnderPF, LM_L_L56_Parts, LM_L_L56_Playfield, LM_L_L56_UnderPF, LM_L_L57_KT_sw15, LM_L_L57_Parts, LM_L_L57_Playfield, LM_L_L57_UnderPF, LM_L_L6_Parts, LM_L_L6_Playfield, LM_L_L6_RSling, LM_L_L6_RSling2, LM_L_L6_UnderPF, LM_L_L65_GuardianR, LM_L_L65_Parts, LM_L_L65_Playfield, LM_L_L65_UnderPF, _
  LM_L_L66_GuardianR, LM_L_L66_Parts, LM_L_L66_Playfield, LM_L_L66_UnderPF, LM_L_L67_GuardianR, LM_L_L67_GuardianR_001, LM_L_L67_Parts, LM_L_L67_Playfield, LM_L_L67_UnderPF, LM_L_L7_Parts, LM_L_L7_Playfield, LM_L_L7_RSling, LM_L_L7_RSling1, LM_L_L7_RSling2, LM_L_L7_UnderPF, LM_L_L71_Parts, LM_L_L71_Playfield, LM_L_L71_UnderPF, LM_L_L72_Parts, LM_L_L72_Playfield, LM_L_L72_UnderPF, LM_L_L73_Parts, LM_L_L73_Playfield, LM_L_L73_UnderPF, LM_L_L74_Parts, LM_L_L74_Playfield, LM_L_L74_UnderPF, LM_L_L75_Parts, LM_L_L75_Playfield, LM_L_L75_UnderPF, LM_L_L76_KT_sw16, LM_L_L76_Parts, LM_L_L76_Playfield, LM_L_L76_UnderPF, LM_L_L77_Parts, LM_L_L77_Playfield, LM_L_L77_UnderPF, LM_L_L81_LSling1, LM_L_L81_Parts, LM_L_L81_Playfield, LM_L_L81_UnderPF, LM_L_L82_LF, LM_L_L82_LFU, LM_L_L82_LSling, LM_L_L82_LSling1, LM_L_L82_LSling2, LM_L_L82_Parts, LM_L_L82_Playfield, LM_L_L82_UnderPF, LM_L_L83_LF, LM_L_L83_LFU, LM_L_L83_Parts, LM_L_L83_Playfield, LM_L_L83_UnderPF, LM_L_L84_Parts, LM_L_L84_Playfield, LM_L_L84_RF, LM_L_L84_RFU, _
  LM_L_L84_UnderPF, LM_L_L85_Parts, LM_L_L85_Playfield, LM_L_L85_RF, LM_L_L85_RFU, LM_L_L85_RSling, LM_L_L85_RSling1, LM_L_L85_RSling2, LM_L_L85_UnderPF, LM_L_L86_Parts, LM_L_L86_Playfield, LM_L_L86_RSling1, LM_L_L86_UnderPF, LM_L_L87_Parts, LM_L_L87_Playfield, LM_L_L87_UnderPF)
Dim BG_All: BG_All=Array(BM_Glass, BM_Metal, BM_VRRR, LM_BB_Glass, LM_BB_Metal, LM_BB_VRRR, LM_BC_L112_Metal, LM_BC_L112_VRRR, LM_BC_L126_VRRR, LM_BC_L90_Glass, LM_BC_L90_Metal, LM_BC_L90_VRRR, LM_BC_L92_Metal, LM_BC_L92_VRRR, LM_BF_Glass, LM_BF_Metal, LM_BF_VRRR, LM_BO_Glass, LM_BO_Metal, LM_BO_VRRR, LM_BP_Glass, LM_BP_Metal, LM_BP_VRRR, LM_BR_Glass, LM_BR_Metal, LM_BR_VRRR, LM_TL_Metal, LM_TL_VRRR,BM_Autoplunger, BM_Bumper1_Ring, BM_Bumper1_Skirt, BM_Bumper2_Ring, BM_Bumper2_Skirt, BM_DT_sw17, BM_DT_sw26, BM_DT_sw27, BM_DT_sw35, BM_DT_sw36, BM_DT_sw37, BM_Glider_1, BM_Glider_1_001, BM_Glider_1_002, BM_Glider_2, BM_Glider_2_001, BM_GuardianL, BM_GuardianL_001, BM_GuardianR, BM_GuardianR_001, BM_KT_sw14, BM_KT_sw15, BM_KT_sw16, BM_LF, BM_LFU, BM_LSling, BM_LSling1, BM_LSling2, BM_Layer1, BM_Layer2, BM_Parts, BM_PinCab_Rails_021, BM_PinCab_Rails_022, BM_PinCab_Rails_023, BM_Plastics, BM_Playfield, BM_Pyramid1, BM_Pyramid1_001, BM_RF, BM_RF1, BM_RF1U, BM_RFU, BM_RSling, BM_RSling1, BM_RSling2, BM_ST_sw22, BM_ST_sw32, BM_SarcArm, BM_SarcArmU, BM_SideBlades_Art, BM_SlingL, BM_SlingR, BM_UnderPF, BM_Wood_guide___Painted_Blaxk_1, BM_sw111, BM_sw112, BM_sw113, BM_sw114, BM_sw115, BM_sw31, LM_F_f0_Bumper1_Ring, LM_F_f0_Bumper1_Skirt, LM_F_f0_Bumper2_Ring, LM_F_f0_Bumper2_Skirt, LM_F_f0_KT_sw14, LM_F_f0_Parts, LM_F_f0_Plastics, LM_F_f0_Playfield, LM_F_f0_SideBlades_Art, LM_F_f0_UnderPF, _
  LM_F_f1_DT_sw36, LM_F_f1_Glider_1, LM_F_f1_Glider_1_001, LM_F_f1_Glider_2, LM_F_f1_GuardianL, LM_F_f1_GuardianL_001, LM_F_f1_GuardianR, LM_F_f1_KT_sw15, LM_F_f1_Layer1, LM_F_f1_Layer2, LM_F_f1_Parts, LM_F_f1_Plastics, LM_F_f1_Playfield, LM_F_f1_Pyramid1, LM_F_f1_RF1U, LM_F_f1_ST_sw32, LM_F_f1_SideBlades_Art, LM_F_f1_UnderPF, LM_F_f2_Glider_1, LM_F_f2_Glider_1_001, LM_F_f2_Glider_1_002, LM_F_f2_Glider_2, LM_F_f2_Glider_2_001, LM_F_f2_GuardianR, LM_F_f2_GuardianR_001, LM_F_f2_KT_sw15, LM_F_f2_Layer1, LM_F_f2_Layer2, LM_F_f2_Parts, LM_F_f2_Plastics, LM_F_f2_Playfield, LM_F_f2_Pyramid1, LM_F_f2_Pyramid1_001, LM_F_f2_ST_sw22, LM_F_f2_SideBlades_Art, LM_F_f2_UnderPF, LM_F_f2_Wood_guide___Painted_Bl, LM_F_f3_Glider_1, LM_F_f3_Glider_1_001, LM_F_f3_Glider_1_002, LM_F_f3_Glider_2, LM_F_f3_Glider_2_001, LM_F_f3_GuardianR, LM_F_f3_GuardianR_001, LM_F_f3_Layer2, LM_F_f3_Parts, LM_F_f3_Plastics, LM_F_f3_Playfield, LM_F_f3_Pyramid1, LM_F_f3_Pyramid1_001, LM_F_f3_SideBlades_Art, LM_F_f3_UnderPF, _
  LM_F_f3_Wood_guide___Painted_Bl, LM_F_f4_DT_sw26, LM_F_f4_DT_sw36, LM_F_f4_Glider_1, LM_F_f4_Glider_1_001, LM_F_f4_Glider_1_002, LM_F_f4_Glider_2, LM_F_f4_Glider_2_001, LM_F_f4_GuardianL, LM_F_f4_GuardianL_001, LM_F_f4_GuardianR, LM_F_f4_GuardianR_001, LM_F_f4_KT_sw15, LM_F_f4_LSling1, LM_F_f4_LSling2, LM_F_f4_Layer1, LM_F_f4_Parts, LM_F_f4_Plastics, LM_F_f4_Playfield, LM_F_f4_Pyramid1, LM_F_f4_RF1, LM_F_f4_RF1U, LM_F_f4_ST_sw22, LM_F_f4_ST_sw32, LM_F_f4_UnderPF, LM_F_f4_sw112, LM_F_f5_Bumper2_Ring, LM_F_f5_DT_sw17, LM_F_f5_DT_sw26, LM_F_f5_DT_sw27, LM_F_f5_DT_sw35, LM_F_f5_DT_sw36, LM_F_f5_DT_sw37, LM_F_f5_KT_sw16, LM_F_f5_LFU, LM_F_f5_LSling, LM_F_f5_LSling1, LM_F_f5_LSling2, LM_F_f5_Layer1, LM_F_f5_Parts, LM_F_f5_Plastics, LM_F_f5_Playfield, LM_F_f5_RF, LM_F_f5_RF1, LM_F_f5_RF1U, LM_F_f5_RFU, LM_F_f5_RSling, LM_F_f5_RSling1, LM_F_f5_RSling2, LM_F_f5_SarcArm, LM_F_f5_SideBlades_Art, LM_F_f5_SlingL, LM_F_f5_UnderPF, LM_F_f5_sw113, LM_GIB_Bumper1_Skirt, LM_GIB_Bumper2_Ring, LM_GIB_Bumper2_Skirt, _
  LM_GIB_DT_sw26, LM_GIB_DT_sw36, LM_GIB_DT_sw37, LM_GIB_GuardianL, LM_GIB_GuardianL_001, LM_GIB_KT_sw14, LM_GIB_KT_sw15, LM_GIB_Layer1, LM_GIB_Parts, LM_GIB_Plastics, LM_GIB_Playfield, LM_GIB_Pyramid1, LM_GIB_SideBlades_Art, LM_GIB_UnderPF, LM_GIG_Bumper1_Ring, LM_GIG_DT_sw17, LM_GIG_DT_sw26, LM_GIG_DT_sw27, LM_GIG_DT_sw36, LM_GIG_DT_sw37, LM_GIG_Glider_1, LM_GIG_Glider_1_002, LM_GIG_GuardianL, LM_GIG_GuardianL_001, LM_GIG_GuardianR, LM_GIG_GuardianR_001, LM_GIG_KT_sw15, LM_GIG_LF, LM_GIG_LFU, LM_GIG_LSling, LM_GIG_LSling1, LM_GIG_LSling2, LM_GIG_Layer1, LM_GIG_Layer2, LM_GIG_Parts, LM_GIG_Plastics, LM_GIG_Playfield, LM_GIG_Pyramid1, LM_GIG_RF, LM_GIG_RF1U, LM_GIG_RFU, LM_GIG_RSling, LM_GIG_RSling1, LM_GIG_RSling2, LM_GIG_ST_sw22, LM_GIG_ST_sw32, LM_GIG_SarcArmU, LM_GIG_SideBlades_Art, LM_GIG_SlingL, LM_GIG_SlingR, LM_GIG_UnderPF, LM_GIG_sw111, LM_GIG_sw112, LM_GIG_sw113, LM_GIG_sw114, LM_GIP_GI_001_DT_sw17, LM_GIP_GI_001_DT_sw27, LM_GIP_GI_001_LF, LM_GIP_GI_001_LFU, LM_GIP_GI_001_LSling, _
  LM_GIP_GI_001_LSling1, LM_GIP_GI_001_LSling2, LM_GIP_GI_001_Parts, LM_GIP_GI_001_Plastics, LM_GIP_GI_001_Playfield, LM_GIP_GI_001_RF, LM_GIP_GI_001_RFU, LM_GIP_GI_001_RSling, LM_GIP_GI_001_RSling1, LM_GIP_GI_001_RSling2, LM_GIP_GI_001_SideBlades_Art, LM_GIP_GI_001_SlingR, LM_GIP_GI_001_UnderPF, LM_GIP_GI_001_sw113, LM_GIP_GI_002_Bumper1_Ring, LM_GIP_GI_002_DT_sw17, LM_GIP_GI_002_DT_sw37, LM_GIP_GI_002_LF, LM_GIP_GI_002_LFU, LM_GIP_GI_002_LSling, LM_GIP_GI_002_LSling1, LM_GIP_GI_002_LSling2, LM_GIP_GI_002_Layer1, LM_GIP_GI_002_Layer2, LM_GIP_GI_002_Parts, LM_GIP_GI_002_Plastics, LM_GIP_GI_002_Playfield, LM_GIP_GI_002_RF, LM_GIP_GI_002_RFU, LM_GIP_GI_002_RSling, LM_GIP_GI_002_RSling1, LM_GIP_GI_002_RSling2, LM_GIP_GI_002_SarcArm, LM_GIP_GI_002_SarcArmU, LM_GIP_GI_002_SideBlades_Art, LM_GIP_GI_002_SlingL, LM_GIP_GI_002_UnderPF, LM_GIP_GI_002_sw111, LM_GIP_GI_002_sw112, LM_GIP_GI_006_DT_sw17, LM_GIP_GI_006_DT_sw27, LM_GIP_GI_006_DT_sw37, LM_GIP_GI_006_KT_sw16, LM_GIP_GI_006_Layer1, LM_GIP_GI_006_Parts, _
  LM_GIP_GI_006_Plastics, LM_GIP_GI_006_Playfield, LM_GIP_GI_006_RF1, LM_GIP_GI_006_RF1U, LM_GIP_GI_006_RSling, LM_GIP_GI_006_RSling1, LM_GIP_GI_006_RSling2, LM_GIP_GI_006_SideBlades_Art, LM_GIP_GI_006_UnderPF, LM_GIP_GI_007_Bumper1_Ring, LM_GIP_GI_007_Bumper1_Skirt, LM_GIP_GI_007_Bumper2_Ring, LM_GIP_GI_007_Bumper2_Skirt, LM_GIP_GI_007_DT_sw17, LM_GIP_GI_007_DT_sw26, LM_GIP_GI_007_DT_sw27, LM_GIP_GI_007_DT_sw36, LM_GIP_GI_007_DT_sw37, LM_GIP_GI_007_LFU, LM_GIP_GI_007_LSling, LM_GIP_GI_007_LSling1, LM_GIP_GI_007_LSling2, LM_GIP_GI_007_Layer2, LM_GIP_GI_007_Parts, LM_GIP_GI_007_Plastics, LM_GIP_GI_007_Playfield, LM_GIP_GI_007_SideBlades_Art, LM_GIP_GI_007_UnderPF, LM_GIP_GI_007_sw115, LM_GIR_DT_sw35, LM_GIR_GuardianR, LM_GIR_GuardianR_001, LM_GIR_KT_sw15, LM_GIR_KT_sw16, LM_GIR_Layer1, LM_GIR_Layer2, LM_GIR_Parts, LM_GIR_Plastics, LM_GIR_Playfield, LM_GIR_RF1, LM_GIR_RF1U, LM_GIR_SideBlades_Art, LM_GIR_UnderPF, LM_GIY_Glider_1, LM_GIY_Glider_1_001, LM_GIY_Glider_1_002, LM_GIY_Glider_2, LM_GIY_GuardianR, _
  LM_GIY_GuardianR_001, LM_GIY_KT_sw15, LM_GIY_Layer1, LM_GIY_Parts, LM_GIY_Plastics, LM_GIY_Playfield, LM_GIY_ST_sw22, LM_GIY_ST_sw32, LM_GIY_SideBlades_Art, LM_GIY_UnderPF, LM_L_L0_LF, LM_L_L0_Parts, LM_L_L0_Playfield, LM_L_L0_RF, LM_L_L0_UnderPF, LM_L_L11_LSling, LM_L_L11_LSling1, LM_L_L11_LSling2, LM_L_L11_Layer2, LM_L_L11_Parts, LM_L_L11_Plastics, LM_L_L11_Playfield, LM_L_L12_LSling, LM_L_L12_LSling1, LM_L_L12_LSling2, LM_L_L12_Layer2, LM_L_L12_Parts, LM_L_L12_Playfield, LM_L_L12_UnderPF, LM_L_L13_Parts, LM_L_L13_Playfield, LM_L_L13_RSling, LM_L_L13_RSling1, LM_L_L13_RSling2, LM_L_L13_UnderPF, LM_L_L14_Parts, LM_L_L14_Playfield, LM_L_L14_UnderPF, LM_L_L15_Bumper1_Skirt, LM_L_L15_DT_sw37, LM_L_L15_Parts, LM_L_L15_Playfield, LM_L_L15_UnderPF, LM_L_L16_Parts, LM_L_L16_Playfield, LM_L_L16_UnderPF, LM_L_L17_Bumper1_Skirt, LM_L_L17_DT_sw26, LM_L_L17_Parts, LM_L_L17_Playfield, LM_L_L17_UnderPF, LM_L_L21_Parts, LM_L_L21_Playfield, LM_L_L21_UnderPF, LM_L_L22_Parts, LM_L_L22_Playfield, LM_L_L22_UnderPF, _
  ...
  LM_L_L43_SideBlades_Art, LM_L_L43_UnderPF, LM_L_L44_Bumper2_Ring, LM_L_L44_Bumper2_Skirt, LM_L_L44_GuardianL, LM_L_L44_GuardianL_001, LM_L_L44_KT_sw14, LM_L_L44_Parts, LM_L_L44_Playfield, LM_L_L44_UnderPF, LM_L_L45_DT_sw36, LM_L_L45_Parts, LM_L_L45_Playfield, LM_L_L45_UnderPF, LM_L_L46_DT_sw36, LM_L_L46_Parts, LM_L_L46_Playfield, LM_L_L46_ST_sw32, LM_L_L46_UnderPF, LM_L_L47_Parts, LM_L_L47_Playfield, LM_L_L47_ST_sw32, LM_L_L47_UnderPF, LM_L_L5_Parts, LM_L_L5_Playfield, LM_L_L5_RSling1, LM_L_L55_Parts, LM_L_L55_Playfield, LM_L_L55_ST_sw32, LM_L_L55_UnderPF, LM_L_L56_Parts, LM_L_L56_Playfield, LM_L_L56_UnderPF, LM_L_L57_KT_sw15, LM_L_L57_Parts, LM_L_L57_Playfield, LM_L_L57_UnderPF, LM_L_L6_Parts, LM_L_L6_Playfield, LM_L_L6_RSling, LM_L_L6_RSling2, LM_L_L6_UnderPF, LM_L_L65_GuardianR, LM_L_L65_Parts, LM_L_L65_Playfield, LM_L_L65_UnderPF, LM_L_L66_GuardianR, LM_L_L66_Parts, LM_L_L66_Playfield, LM_L_L66_UnderPF, LM_L_L67_GuardianR, LM_L_L67_GuardianR_001, LM_L_L67_Parts, LM_L_L67_Playfield, LM_L_L67_UnderPF, _
  LM_L_L7_Parts, LM_L_L7_Playfield, LM_L_L7_RSling, LM_L_L7_RSling1, LM_L_L7_RSling2, LM_L_L7_UnderPF, LM_L_L71_Parts, LM_L_L71_Playfield, LM_L_L71_UnderPF, LM_L_L72_Parts, LM_L_L72_Playfield, LM_L_L72_UnderPF, LM_L_L73_Parts, LM_L_L73_Playfield, LM_L_L73_UnderPF, LM_L_L74_Parts, LM_L_L74_Playfield, LM_L_L74_UnderPF, LM_L_L75_Parts, LM_L_L75_Playfield, LM_L_L75_UnderPF, LM_L_L76_KT_sw16, LM_L_L76_Parts, LM_L_L76_Playfield, LM_L_L76_UnderPF, LM_L_L77_Parts, LM_L_L77_Playfield, LM_L_L77_UnderPF, LM_L_L81_LSling1, LM_L_L81_Parts, LM_L_L81_Playfield, LM_L_L81_UnderPF, LM_L_L82_LF, LM_L_L82_LFU, LM_L_L82_LSling, LM_L_L82_LSling1, LM_L_L82_LSling2, LM_L_L82_Parts, LM_L_L82_Playfield, LM_L_L82_UnderPF, LM_L_L83_LF, LM_L_L83_LFU, LM_L_L83_Parts, LM_L_L83_Playfield, LM_L_L83_UnderPF, LM_L_L84_Parts, LM_L_L84_Playfield, LM_L_L84_RF, LM_L_L84_RFU, LM_L_L84_UnderPF, LM_L_L85_Parts, LM_L_L85_Playfield, LM_L_L85_RF, LM_L_L85_RFU, LM_L_L85_RSling, LM_L_L85_RSling1, LM_L_L85_RSling2, LM_L_L85_UnderPF, LM_L_L86_Parts, _
  LM_L_L86_Playfield, LM_L_L86_RSling1, LM_L_L86_UnderPF, LM_L_L87_Parts, LM_L_L87_Playfield, LM_L_L87_UnderPF)
' VLM  Arrays - End


'VR Portal animation........
dim PortalBrightness
Dim PortalBrightnessUp
PortalBrightnessUp = true
PortalBrightness = 1

sub VRPortalTimer_Timer()
  VR_Portal.rotx = VR_Portal.rotx + .1
  VR_Portal2.rotx = VR_Portal2.rotx - .1
  VR_PortalLight.blenddisableLighting = PortalBrightness

  If PortalBrightness < 200 and PortalBrightnessUp = true then PortalBrightness = PortalBrightness + 1
  if PortalBrightness => 200 then PortalBrightnessUp = false
  If PortalBrightness > 0 and PortalBrightnessUp = false then PortalBrightness = PortalBrightness - 1
  if PortalBrightness =< 0 then PortalBrightnessUp = true
End Sub
...
' CHANGE LOG
' 1.3.0 - Table created by 32Assassin and JLouLoulou
' 1.3.1 - mcarter78 - Initial physics test (allF visual objects hidden, no targets) -- Full physics overhaul, new physical trough & kickers, new playfield mesh, VUK ramps, new posts & rubbers, VPM mapped lights, roth targets added but not hooked up yet, new SSF (will add back in custom Stargate sounds where applicable)
...
' 1.3.30 - MechaEnron - Redrew Art Blades, fixed transparancy of plastics, turned screws/bolts/etc, added foam on top of guardian bracket, fixed UV unwrapping of sidewall
' 1.3.31 - MechaEnron - Fix Glider animation/materials
' 1.3.32 - MechaEnron - Fixed VR code to hide VLM rails in VR, visual updates
' 1.3.33 - tomate - phisical plunger ramp hole enlarged to match visual ramp, depth mask checked on plunger ramp
' 1.3.34 - mcarter78 - Add triggers to release balls stuck behind guardians
' 1.3.35 - DGrimmReaper - VR flipper buttons and plunger animations.  VR Backglass lights (used hauntfreaks b2s assets).  Removed old VRRoom ramps and replaced with primitives.
' 1.3.36 - DGrimmReaper - DaRdog's VR Mega room added with 2 table location options in F12 menu.
' 1.3.37 - MechaEnron - Added new VR backbox with lighting options
' 1.3.38 - MechaEnron - Updates to VR backbox
' 1.3.39 - DGrimmReaper - VR Mega room set to day/night.  Backbox side art added to nestmap
' 1.3.40 - DGrimmReaper - VR Image cleanup and VR Cab linked to day/night
' 1.3.41 - DGrimmReaper - VR unlinked the ramp lights and control room board from day/night
' 1.3.42 - mcarter78 - Add slope adjustment to F12 menu (adjustable between 5 - 10 degrees)
' 1.3.43 - apophis - Tweaks to subway ramp and associated triggers.
' 1.3.44 - MechaEnron - Added Flasher Brightness option
' 1.3.45 - apophis - Fixed trough. Made default flasher brightness 200.
' 1.3.46 - Sixtoe - Tweaked pop bumpers, slings and kicker targets as per JLous values, fixed kickout lane guide direction, redid physical plunger lane end drop hole thing, tweaked left bank target position, added missing metal guide,
' 1.3.47 - Sixtoe - Tweaked trough entrance, messed around with top left vuk and primitives, messed around with left outlane exit from bumpers, update troughtimer to 300ms.
' 1.3.48 - DGrimmReaper - VR adjustments: new cabinet art, moved dmd, cabinet shadows for each mega room location (DaRdog), backbox blocker.
' 1.3.49 - Rawd - Added Stargate animation for VRrooms and positioned.
' 1.3.50 - Sixtoe - Redesigned the top left VUK *again*.
' 1.3.51 - apophis - Adjusted postion of swtich 34. Refactored flasher brightness option. Made right kicker targets a little easier to trigger.
' 1.3.52 - rothbauerw - Adjusted postion of swtich 34 to swtrough3, Removed swtrough4. Tuned kicking targets and fixed boundary condition where ball skims corner of target.
' 1.3.53 - rothbauerw - Debug code for trough testing?
' 1.3.54 - MechaEnron - Removed Rebellion Flasher from flasher color code, added flashes everytime a flasher option is changed
' RC1 - apophis - Timers cleanup. Comments cleanup. Added desktop DMD and visibility option. Added table info and instruction card.
' RC2 - Sixtoe - Reinforced layout, numerous small physics layout tweaks, removed redundant sw22 things, turned off collidable in vr objects, cleaned up file and removed redundant assets.
' RC3 - apophis - Enabled reflections for BM_Parts. VR control room now default. Converted slingshot sfx from stereo to mono and changed position.
