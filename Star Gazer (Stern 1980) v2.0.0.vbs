...
'| (    \/   ) (   | (   ) || (    )|    | (    \/| (   ) |\/   )  || (    \/| (    )|
'| (_____    | |   | (___) || (____)|    | |      | (___) |    /   )| (__    | (____)|
'(_____  )   | |   |  ___  ||     __)    | | ____ |  ___  |   /   / |  __)   |     __)
'      ) |   | |   | (   ) || (\ (       | | \_  )| (   ) |  /   /  | (      | (\ (
'/\____) |   | |   | )   ( || ) \ \__    | (___) || )   ( | /   (_/\| (____/\| ) \ \__
'\_______)   )_(   |/     \||/   \__/    (_______)|/     \|(_______/(_______/|/   \__/
'
'
'               Stern Electronics, Incorporated, 1980; Stern M-200 MPU
'       Star Gazer / IPD No. 2346 / August, 1980 / 4 Players, 869 units produced
'               https://www.ipdb.org/machine.cgi?id=2346
'
'
...
'     Added latest VPW standards, most recent VPW physics as of 2025, GI VLM, 3D inserts, AllLamps routines, shadows,
'   updated backglass for desktop, animated bumpers, baked walls, roth targets, standalone compatibility, sling corrections,
'   updated flipper physics, updated spinner logic and functionality, VR and fully hybrid,
'   Playfield scan provided by apophis, backglass images provided by Hauntfreaks. Thanks also to the VPW team for testing!
'   Playfield, plastics, targets, apron, and other images created and/or updated by Redbone.
...
' ALL OPTIONS ARE IN THE F12 MENU, EXCEPT FOR SetDIPSwitches and ForceVR

' Insert brightness scale
const insertfactor  = 1.0 ' adjusts the level of the brightness of the inserts when on
...
' NOTE:  IF want to change the ROM adjustments regardless of dip switch settings, the instructions are here:
'       https://www.vpforums.org/index.php?app=tutorials&article=109

' I've set the ROM dip switch settings to match the instruction card (left: 12C-2-127-24A;   Right: 12D-SC-2-79A)
' I've created a .nvram file to set the high score replay levels of 720,000 and 1,440,000.

'Here is the procedure to set replay levels on Stern MPUs M-100 and M-200. These tables were manufactured between 1977 and 1984.

' 1. Press 7 to enter test mode.
' 2. Continue to press 7 until you see 01 in the match/ball in play display.
' 3. Press and hold the 1 key to set the replay level. The score will increment by 10,000 points as you hold in the 1 key.
'    (NOTE: If you pass the score you want to set, release the 1 key then press the 9 key to set the score to zero, then press and hold the 1 key)
'    Once you get to the score level you want, release the 1 key.  (For Star Gazer it's recommended to be 720,000 for the first replay)
' 4. Press the 7 key to see 02 in the match/ball in play display.
' 5. Repeat step 3 to set the second replay level.  (For Star Gazer it's recommended to be 1,440,000,000 for the second replay)
' 6. Press the 7 key to see 03 in the match/ball in play display.
' 7. Repeat step 3 to set the third replay level.   (For Star Gazer it's recommened that there is no 3rd level for replay)
' 8. Press the 7 key to see 04 in the match/ball in play display.
' 9. Repeat step 3 to set the high score to date.   (For Star Gazer initial, it's typically set to 2,360,000 for initial high score to beat: but I set at zero.)
' 10. Either press 7 to scroll through the other options or press F3 to save your changes and reset the table. (NOTE: If you do not want a replay level set, set that level to zero. This will disable that high score level. However, setting the high score to date to zero will not disable it)
...
Dim DT22up,DT23up,DT24up,DT25up,DT26up,DT27up,DT28up,DT29up,DT30up
...
Dim gate1soundrdy: gate1soundrdy = 0  'used for gate1 metal hit sounds after ball goes through gate.

'rubber color variables
Const rcolor = 211
Const gcolor = 200
Const bcolor = 169

'standup target material to go past the thickness

Const stthick = 10

UpdateMaterial "Plastic with an imageST1", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST2", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST3", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST4", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST5", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST6", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST7", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST8", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST9", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST10", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST11", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0
UpdateMaterial "Plastic with an imageST12", 0, 0.75, 0.431, stthick, 1, 1, 1, RGB(255,255,255), RGB(100,100,100), RGB(0,0,0), False, True, 0, 0, 0, 0

'  Standard definitions

cGameName = "stargzr"     'PinMAME ROM name
...
Dim BL_All_Lights_gi_008: BL_All_Lights_gi_008=Array(LM_All_Lights_gi_008_Playfield,LM_All_Lights_gi_008_Walls)
Dim BL_All_Lights_gi_009: BL_All_Lights_gi_009=Array(LM_All_Lights_gi_009_Playfield,LM_All_Lights_gi_009_Walls)
Dim BL_All_Lights_gi_010: BL_All_Lights_gi_010=Array(LM_All_Lights_gi_010_Playfield,LM_All_Lights_gi_010_Walls)
Dim BL_All_Lights_gi_011: BL_All_Lights_gi_011=Array(LM_All_Lights_gi_011_Playfield,LM_All_Lights_gi_011_Walls)
Dim BL_All_Lights_gi_012: BL_All_Lights_gi_012=Array(LM_All_Lights_gi_012_Playfield,LM_All_Lights_gi_012_Walls)
Dim BL_All_Lights_gi_013: BL_All_Lights_gi_013=Array(LM_All_Lights_gi_013_Playfield,LM_All_Lights_gi_013_Walls)
...
Dim BL_All_Lights_gi_025: BL_All_Lights_gi_025=Array(LM_All_Lights_gi_025_Playfield,LM_All_Lights_gi_025_Walls)
Dim BL_All_Lights_gi_027: BL_All_Lights_gi_027=Array(LM_All_Lights_gi_027_Playfield,LM_All_Lights_gi_027_Walls)
Dim BL_All_Lights_gi_028: BL_All_Lights_gi_028=Array(LM_All_Lights_gi_028_Playfield,LM_All_Lights_gi_028_Walls)
Dim BL_All_Lights_l46: BL_All_Lights_l46=Array(LM_All_Lights_l46_Walls)
Dim BL_All_Lights_l59: BL_All_Lights_l59=Array(LM_All_Lights_l59_Walls)
Dim BL_All_Lights_l62: BL_All_Lights_l62=Array(LM_All_Lights_l62_Walls)
Dim BL_World: BL_World=Array(BM_Walls)
' Global arrays
Dim BG_Bakemap: BG_Bakemap=Array(BM_Walls)
Dim BG_Lightmap: BG_Lightmap=Array(LM_All_Lights_gi_001_Playfield, LM_All_Lights_gi_002_Playfield, LM_All_Lights_gi_003_Playfield, LM_All_Lights_gi_004_Playfield, LM_All_Lights_gi_005_Playfield, LM_All_Lights_gi_006_Playfield, LM_All_Lights_gi_007_Playfield, LM_All_Lights_gi_008_Playfield, LM_All_Lights_gi_009_Playfield, LM_All_Lights_gi_010_Playfield, LM_All_Lights_gi_011_Playfield, LM_All_Lights_gi_012_Playfield, LM_All_Lights_gi_013_Playfield, LM_All_Lights_gi_014_Playfield, LM_All_Lights_gi_015_Playfield, LM_All_Lights_gi_016_Playfield, LM_All_Lights_gi_017_Playfield, LM_All_Lights_gi_018_Playfield, LM_All_Lights_gi_019_Playfield, LM_All_Lights_gi_020_Playfield, LM_All_Lights_gi_021_Playfield, LM_All_Lights_gi_022_Playfield, LM_All_Lights_gi_023_Playfield, LM_All_Lights_gi_025_Playfield, LM_All_Lights_gi_027_Playfield, LM_All_Lights_gi_028_Playfield,_
                  LM_All_Lights_gi_001_Walls, LM_All_Lights_gi_002_Walls, LM_All_Lights_gi_003_Walls, LM_All_Lights_gi_004_Walls, LM_All_Lights_gi_005_Walls, LM_All_Lights_gi_007_Walls, LM_All_Lights_gi_008_Walls, LM_All_Lights_gi_009_Walls, LM_All_Lights_gi_010_Walls, LM_All_Lights_gi_011_Walls, LM_All_Lights_gi_012_Walls, LM_All_Lights_gi_013_Walls, LM_All_Lights_gi_014_Walls, LM_All_Lights_gi_015_Walls, LM_All_Lights_gi_016_Walls, LM_All_Lights_gi_017_Walls, LM_All_Lights_gi_018_Walls, LM_All_Lights_gi_019_Walls, LM_All_Lights_gi_020_Walls, LM_All_Lights_gi_021_Walls, LM_All_Lights_gi_022_Walls, LM_All_Lights_gi_023_Walls, LM_All_Lights_gi_025_Walls, LM_All_Lights_gi_027_Walls, LM_All_Lights_gi_028_Walls, LM_All_Lights_l46_Walls, LM_All_Lights_l59_Walls, LM_All_Lights_l62_Walls)
Dim BG_All: BG_All=Array(LM_All_Lights_gi_001_Playfield, LM_All_Lights_gi_002_Playfield, LM_All_Lights_gi_003_Playfield, LM_All_Lights_gi_004_Playfield, LM_All_Lights_gi_005_Playfield, LM_All_Lights_gi_006_Playfield, LM_All_Lights_gi_007_Playfield, LM_All_Lights_gi_008_Playfield, LM_All_Lights_gi_009_Playfield, LM_All_Lights_gi_010_Playfield, LM_All_Lights_gi_011_Playfield, LM_All_Lights_gi_012_Playfield, LM_All_Lights_gi_013_Playfield, LM_All_Lights_gi_014_Playfield, LM_All_Lights_gi_015_Playfield, LM_All_Lights_gi_016_Playfield, LM_All_Lights_gi_017_Playfield, LM_All_Lights_gi_018_Playfield, LM_All_Lights_gi_019_Playfield, LM_All_Lights_gi_020_Playfield, LM_All_Lights_gi_021_Playfield, LM_All_Lights_gi_022_Playfield, LM_All_Lights_gi_023_Playfield, LM_All_Lights_gi_025_Playfield, LM_All_Lights_gi_027_Playfield, LM_All_Lights_gi_028_Playfield,_
            BM_Walls, LM_All_Lights_gi_001_Walls, LM_All_Lights_gi_002_Walls, LM_All_Lights_gi_003_Walls, LM_All_Lights_gi_004_Walls, LM_All_Lights_gi_005_Walls, LM_All_Lights_gi_007_Walls, LM_All_Lights_gi_008_Walls, LM_All_Lights_gi_009_Walls, LM_All_Lights_gi_010_Walls, LM_All_Lights_gi_011_Walls, LM_All_Lights_gi_012_Walls, LM_All_Lights_gi_013_Walls, LM_All_Lights_gi_014_Walls, LM_All_Lights_gi_015_Walls, LM_All_Lights_gi_016_Walls, LM_All_Lights_gi_017_Walls, LM_All_Lights_gi_018_Walls, LM_All_Lights_gi_019_Walls, LM_All_Lights_gi_020_Walls, LM_All_Lights_gi_021_Walls, LM_All_Lights_gi_022_Walls, LM_All_Lights_gi_023_Walls, LM_All_Lights_gi_025_Walls, LM_All_Lights_gi_027_Walls, LM_All_Lights_gi_028_Walls, LM_All_Lights_l46_Walls, LM_All_Lights_l59_Walls, LM_All_Lights_l62_Walls)
' VLM  Arrays - End
...
Sub Table1_Init
  ...
  With Controller
    ...
    .SplashInfoLine = "Star Gazer (Stern 1980)"&chr(13)&"by UnclePaulie"
    .HandleKeyboard = 0
    .ShowTitle = 0
    .ShowDMDOnly = 1
    .ShowFrame = 0
...
' If user wants to set dip switches themselves it will force them to set it via F6.
  ...
  Set SGBall1 = BallRelease.CreateSizedballWithMass(Ballsize/2,Ballmass)
...
Dim FlipperColor, ScrewCapColor, cabsideblades, SideWallColor, PegColor, PlasticColor, TargetColor, TargetMarks, SpinnerColor, SUTargetColor, undercab, SpinnerFlasher, SlingFlasher, Bumperflasher
...
Sub Table1_OptionEvent(ByVal eventId)
  ...
  ROMVolume = Table1.Option("ROM Volume (-32 is off; 0 is max db) - Requires Table Restart", -32, 0, 1, -12, 0)
    ...
    ' Ball Choice
    ChooseBall = Table1.Option("Ball Choice", 0, 14, 1, 2, 0, Array("0: Hauntfreaks dark", "1: Hauntfreaks not as dark", _
      "2: Hauntfreaks bright (default)", "3: Hauntfreaks brightest", "4: Hauntfreaks warm", "5: MrBallDark", "6: DarthVito Ball1", _
  ...
  ' Table Options
  FlipperColor = Table1.Option("Flipper Color", 0, 5, 1, 5, 0, Array("Red", "Blue", "Green", "Orange/Black", "Orange", "Yellow (default)"))
  PegColor = Table1.Option("Peg Color", 0, 2, 1, 0, 0, Array("Blue (default)", "Red", "Yellow"))
  SUTargetColor = Table1.Option("Stand Up Target Color", 0, 1, 1, 1, 0, Array("Yellow", "White (default)"))
  TargetColor = Table1.Option("Drop Target Color", 0, 2, 1, 2, 0, Array("White", "S Sticker", "White Star (default)"))
  TargetMarks = Table1.Option("Ball Marks across Targets", 0, 1, 1, 0, 0, Array("No (default)", "Yes"))
  SpinnerColor = Table1.Option("Spinner Color", 0, 5, 1, 1, 0, Array("Yellow", "Yellow with scratches (default)", "White", "White with scratches", "Glitter", "Glitter with scratches"))
  SpinnerFlasher = Table1.Option("Spinner Flasher Option", 0, 1, 1, 0, 0, Array("Original No Flash (default)", "Flasher On When Spinning"))
  ...
  SideWallColor = Table1.Option("Side Wall Color", 0, 1, 1, 0, 0, Array("Blue (default)", "Black"))
  ...
  ' VR Room Environment
  WallClock = Table1.Option("VR Wall Clock On/Off", 0, 1, 1, 1, 0, Array("Off", "On (default)"))
    CustomWalls = Table1.Option("VR Room Environment", 0, 4, 1, 2, 0, _
    Array("UP Original", "Sixtoe Arcade", "DarthVito Home (default)", "DarthVito Plaster", "DarthVito Blue"))
  Poster = Table1.Option("VR Poster 1 On/Off", 0, 1, 1, 1, 0, Array("Off", "On (default)"))
  Poster2 = Table1.Option("VR Poster 2 On/Off", 0, 1, 1, 1, 0, Array("Off", "On (default)"))
  undercab = Table1.Option("VR Undercab lighting On/Off", 0, 1, 1, 1, 0, Array("Off", "On (default)"))
...
Sub Gate_001_Animate
  dim a: a = (Gate_001.CurrentAngle)
  Gate_001p.Rotx = max(a,30)
End Sub


'*******************************************
' Gate Metal Hit sounds when gates are closed
' Note: Gate_001_Trig_sound_rdy turns off when drain is hit or at initial Start
'*******************************************

Sub Gate_001_Trig_sound_rdy_hit()
  gate1soundrdy = 1
End Sub

Sub Gate_001_Trig_sound_hit()
  If Gate_001.open = false then
    if gate1soundrdy =1 Then
      RandomSoundMetal
    End If
  End If
End Sub
...
Sub AnimateBumperSkirts
  ...
  For r = 0 To 2
    ...
    tz = 0
    If g < 80 * 80 Then
      tz = -4.5 '(UP changed the distance to be 8 VPU... so bumper skirts are now z=-1.5 and than transz is total of -3.5 + -4.5 = -8)
    End If
...
Const PLTop = 800       'Y position of punger lane top
...
Dim girubbercolor_r, girubbercolor_g, girubbercolor_b, x
  ...
  For each x in VRCabLL: x.blenddisablelighting = 0.05 * (LightLevel/lightlevelinitial): Next
...
' GI and GI Bulb Prims
  ...
  For each x in GIBulbPrims: x.blenddisablelighting = (6 * (gilvl)): Next

' rubbers
  girubbercolor_r = (gilvl * 44) + rcolor
  girubbercolor_g = (gilvl * 40) + gcolor
  girubbercolor_b = (gilvl * 57) + bcolor

  MaterialColor "Rubber White",RGB(girubbercolor_r,girubbercolor_g,girubbercolor_b)

' flippers
  For Each x in GIFlip: x.blenddisablelighting = (0.0375 * gilvl) + (0.0025 * LightLevel/lightlevelinitial): Next
...
' targets
  ...
  For each x in GITargetsBack: x.blenddisablelighting = (0.1 * gilvl) + (0.025 * LightLevel/lightlevelinitial): Next

' prims (Pegs)
  For each x in GIPegs: x.blenddisablelighting = (0.1 * gilvl) + (0.005 * LightLevel/lightlevelinitial) : Next
...
' prims (Cutouts)
  ...
  For each x in GIBumperHoles: x.blenddisablelighting = (0.025 * (gilvl)) + (0.1 * LightLevel/lightlevelinitial): Next

' prims (apron, brackets)
  ApronStern.blenddisablelighting = 0.05 * gilvl * LightLevel/lightlevelinitial + (0.1 * LightLevel/lightlevelinitial)
  PlungerCover.blenddisablelighting = 0.15 * gilvl * LightLevel/lightlevelinitial + (0.1 * LightLevel/lightlevelinitial)

' prims (spinner)
  SpinnerPrim.blenddisablelighting =  (0.05 * gilvl) + (0.0375 * LightLevel/lightlevelinitial)
  SpinnerPrim2.blenddisablelighting = (0.05 * gilvl) + (0.0375 * LightLevel/lightlevelinitial)
  SpinnerPrim3.blenddisablelighting = (0.05 * gilvl) + (0.0375 * LightLevel/lightlevelinitial)
...
' prims
  If SideWallColor = 0 Then
    OuterPrimBlack_DT.blenddisablelighting = (0.0875 * gilvl) + (0.0625 * LightLevel/lightlevelinitial)
  Else
    OuterPrimBlack_DT.blenddisablelighting = (0.0125 * gilvl) + (0.0125 * LightLevel/lightlevelinitial)
  End If

  cutout_prim_large.blenddisablelighting = (0.5 * (gilvl)) + (0.1 * LightLevel/lightlevelinitial)
...
' Baked Walls
  BM_Walls.blenddisablelighting = LightLevel/lightlevelinitial
...
Sub Table1_KeyUp(ByVal keycode)
  If keycode = PlungerKey Then
    Plunger.firespeed = RndInt(110,116)     ' Generate random value variance of +/- 3 from 113
      'debug.print plunger.firespeed
    Plunger.Fire
    If BIPL = 1 Then
...
SolCallback(3)    = "DTBankCenterReset" ' Drop Target Center Bank Reset
SolCallback(4)    = "DTBankRightReset"  ' Drop Target Right Bank Reset
SolCallback(6)    = "SolKnocker"      ' Knocker solenoid
SolCallback(7)    = "DTBankLeftReset"   ' Drop Target Left Bank Reset
SolCallback(12)   = "SolBallRelease"    ' Trough to Plunger Lane

SolCallback(19)   = "SolGameOn"     ' Normal Playfield Game On
...
Sub BallRelease_Hit   : Controller.Switch(33) = 1 : End Sub
Sub BallRelease_UnHit : RandomSoundBallRelease BallRelease : Controller.Switch(33) = 0 : End Sub
...
Sub Drain_Hit()
  ...
  gate1soundrdy = 0 ' used for gate1 metal sound when hit after first hit.
  UpdateTrough
End Sub
...
Sub RightSlingShot_Timer
    Select Case RStep
        Case 3: RSLing1.Visible = 0: RSLing2.Visible = 1: sling1.rotx = 10: if SlingFlasher = 1 Then slingrightflash.state = 1
        Case 4: RSLing2.Visible = 0: RSLing.Visible = 1: sling1.rotx = 0: RightSlingShot.TimerEnabled = 0: vpmTimer.Addtimer 100, "slingrightflash.state = 0'"
...
Sub LeftSlingShot_Timer
    Select Case LStep
        Case 3:LSLing1.Visible = 0:LSLing2.Visible = 1:sling2.rotx = 10: if SlingFlasher = 1 Then slingleftflash.state = 1
        Case 4:LSLing2.Visible = 0:LSLing.Visible = 1:sling2.rotx = 0:LeftSlingShot.TimerEnabled = 0: vpmTimer.Addtimer 100, "slingleftflash.state = 0'"
    End Select
    LStep = LStep + 1
End Sub
...
dim spindamping3: spindamping3 = 0.9925
dim ntimelastspin3, ntimecurrentspin3, ntimedifspin3

' adjust these constants to control how long the spinner spins
Const dampingtime = 0.000375
Const dampingtimebase = 0.996
...
Sub sw9trig_hit()
    'debug.print abs(gBOT(0).vely)
  ntimelastspin3 = gametime

  If abs(gBOT(0).vely) < 10 Then
    spindamping3 = 0.9925
  Else
    spindamping3 = .999
    spindamping3timer.enabled = 1
  End If
End Sub

Sub spindampingtimer_timer()
  If spindamping > dampingtimebase Then
    spindamping = spindamping - dampingtime
  Else
    spindamping = .96
...
Sub spindamping2timer_timer()
  If spindamping2 > dampingtimebase Then
    spindamping2 = spindamping2 - dampingtime
  Else
    spindamping2 = .96
...
Sub spindamping3timer_timer()
  If spindamping3 > dampingtimebase Then
    spindamping3 = spindamping3 - dampingtime
  Else
    spindamping3 = .96
    spindamping3timer.enabled = 0
  End If
End Sub
...
'   debug.print (ntimedifspin2)
  sw4.damping = spindamping2
...
Sub sw9_Spin()
  ntimecurrentspin3 = gametime
  ntimedifspin3 = ntimecurrentspin3 - ntimelastspin3
'   debug.print (ntimedifspin3)
  sw9.damping = spindamping3

  If ntimedifspin3 < 750 Then
    vpmtimer.PulseSw 9
    SoundSpinner sw9
  End If

  ntimelastspin3 = ntimecurrentspin3
End Sub
...
Sub sw4_Animate
  ...
  a = sw4.CurrentAngle
  SpinnerPrim2.Rotx = a
  SpinnerRod2.TransZ = (cos((sw4.CurrentAngle + 180) * (PI/180))+1) * SpinnerRadius
  SpinnerRod2.TransY = sin((sw4.CurrentAngle) * (PI/180)-0.25) * -SpinnerRadius
...
Sub sw9_Animate
  ...
  a = sw9.CurrentAngle
  SpinnerPrim3.Rotx = a
  SpinnerRod3.TransY = sin((a) * (PI/180)-0.25) * -SpinnerRadius
  SpinnerRod3.TransZ = (cos((a + 180) * (PI/180))+1) * SpinnerRadius
...
Sub swBIPL_Hit(): BIPL = 1: MatchEn = 1: End Sub  'Plunger Lane Rollover Switch (MatchEnable will now work)
...
Sub sw35_Animate: psw35.transz = sw35.CurrentAnimOffset: End Sub
...
Sub sw36_Hit:vpmTimer.PulseSw 36:me.timerenabled = 0:AnimateStar star36, sw36, 1:End Sub
Sub sw36_UnHit:me.timerinterval = 7:me.timerenabled = 1:End Sub
Sub sw36_timer:AnimateStar star36, sw36, 0:End Sub

Sub sw37_Hit:vpmTimer.PulseSw 37:me.timerenabled = 0:AnimateStar star37, sw37, 1:End Sub
Sub sw37_UnHit:me.timerinterval = 7:me.timerenabled = 1:End Sub
Sub sw37_timer:AnimateStar star37, sw37, 0:End Sub
...
'********************** Drop Targets ************************
Sub sw22_Hit: DTHit 22: TargetBouncer Activeball, 0.5: DT22up = 0: End Sub
Sub sw23_Hit: DTHit 23: TargetBouncer Activeball, 0.5: DT23up = 0: End Sub
Sub sw24_Hit: DTHit 24: TargetBouncer Activeball, 0.5: DT24up = 0: End Sub
Sub sw25_Hit: DTHit 25: TargetBouncer Activeball, 0.5: DT25up = 0: End Sub
Sub sw26_Hit: DTHit 26: TargetBouncer Activeball, 0.5: DT26up = 0: End Sub
Sub sw27_Hit: DTHit 27: TargetBouncer Activeball, 0.5: DT27up = 0: End Sub
Sub sw28_Hit: DTHit 28: TargetBouncer Activeball, 0.5: DT28up = 0: End Sub
Sub sw29_Hit: DTHit 29: TargetBouncer Activeball, 0.5: DT29up = 0: End Sub
Sub sw30_Hit: DTHit 30: TargetBouncer Activeball, 0.5: DT30up = 0: End Sub


'********************** Stand Up Targets ************************

Sub sw10_hit: STHit 10: End Sub
Sub sw10o_Hit: TargetBouncer ActiveBall, 1: End Sub
...
Sub sw32o_Hit: Targetbouncer ActiveBall, 1: End Sub
Sub sw38_hit: STHit 38: End Sub
Sub sw38o_Hit: TargetBouncer ActiveBall, 1: End Sub
Sub sw39_hit: STHit 39: End Sub
Sub sw39o_Hit: TargetBouncer ActiveBall, 1: End Sub
...
const dtdelaytime = 300

Sub DTBankLeftReset (enabled)
  If enabled then
...
Sub DTBankRightReset (enabled)
  If enabled then
  vpmTimer.AddTimer dtdelaytime, "RandomSoundDropTargetReset sw29p'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 28'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 29'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 30'"
    vpmTimer.AddTimer dtdelaytime, "DT28up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT29up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT30up=1'"
    vpmTimer.AddTimer dtdelaytime, "dtsh28.visible=True'"
    vpmTimer.AddTimer dtdelaytime, "dtsh29.visible=True'"
    vpmTimer.AddTimer dtdelaytime, "dtsh30.visible=True'"
  End if
End Sub

Sub DTBankCenterReset (enabled)
  If enabled then
  vpmTimer.AddTimer dtdelaytime, "RandomSoundDropTargetReset sw26p'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 25'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 26'"
    vpmTimer.AddTimer dtdelaytime, "DTRaise 27'"
    vpmTimer.AddTimer dtdelaytime, "DT25up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT26up=1'"
    vpmTimer.AddTimer dtdelaytime, "DT27up=1'"
    vpmTimer.AddTimer dtdelaytime, "dtsh25.visible=True'"
    vpmTimer.AddTimer dtdelaytime, "dtsh26.visible=True'"
    vpmTimer.AddTimer dtdelaytime, "dtsh27.visible=True'"
  End if
End Sub
...
Dim DT22, DT23, DT24, DT25, DT26, DT27, DT28, DT29, DT30
...
Dim DTArray
DTArray = Array(DT22, DT23, DT24, DT25, DT26, DT27, DT28, DT29, DT30)
...
Dim ST10,ST11,ST17,ST18,ST19,ST20,ST21,ST31,ST32,ST38,ST39,ST40
...
Set ST10 = (new StandupTarget)(sw10, sw10p, 10, 0)
Set ST11 = (new StandupTarget)(sw11, sw11p, 11, 0)
...
Dim STArray
STArray = Array(ST10,ST11,ST17,ST18,ST19,ST20,ST21,ST31,ST32,ST38,ST39,ST40)

'Configure the behavior of Stand-up Targets
Const STAnimStep =  1.5     'vpunits per animation step (control return to Start)
Const STMaxOffset = 7       'max vp units target moves when hit. vpw sets to 9.  I changed to accomodate the separate prims
...
const a_insert_blue_off = 0.5
const a_insert_green_on = 1
const a_insert_green_off = 1.25
const a_insert_purple_on = 1.5
const a_insert_purple_off = 2
...
Sub l22_animate
  ...
  bulb22.BlendDisableLighting = bulb_off + bulb_on * (l20.GetInPlayIntensity / l22.Intensity)
End Sub
...
Sub l250_animate
  p250.BlendDisableLighting = insert_white_off + insert_white_on * (l250.GetInPlayIntensity / l250.Intensity)
  p250off.BlendDisableLighting = a_insert_white_off - a_insert_white_on * (l250.GetInPlayIntensity / l250.Intensity)
  bulb250.BlendDisableLighting = bulb_off + bulb_on * (l250.GetInPlayIntensity / l250.Intensity)
End Sub
...
Sub l570_animate
  p570.BlendDisableLighting = insert_white_off + insert_white_on * (l570.GetInPlayIntensity / l570.Intensity)
  p570off.BlendDisableLighting = a_insert_white_off - a_insert_white_on * (l570.GetInPlayIntensity / l570.Intensity)
  bulb570.BlendDisableLighting = bulb_off + bulb_on * (l570.GetInPlayIntensity / l570.Intensity)
End Sub
...
Sub l14_animate
  p14.BlendDisableLighting = insert_red_off + insert_red_on * (l14.GetInPlayIntensity / l14.Intensity)
  p14off.BlendDisableLighting = a_insert_red_off - a_insert_red_on * (l14.GetInPlayIntensity / l14.Intensity)
...
Sub l43_animate
  p43.BlendDisableLighting = insert_purple_off + insert_purple_on * (l43.GetInPlayIntensity / l43.Intensity)
  p43off.BlendDisableLighting = a_insert_purple_off - a_insert_purple_on * (l43.GetInPlayIntensity / l43.Intensity)
  bulb43.BlendDisableLighting = bulb_off + bulb_on * (l14.GetInPlayIntensity / l43.Intensity)
End Sub
...
Sub l4_animate
  p4.BlendDisableLighting = insert_blue_off + insert_blue_on * (l4.GetInPlayIntensity / l4.Intensity)
  p4off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l4.GetInPlayIntensity / l4.Intensity)
  bulb4.BlendDisableLighting = bulb_off + bulb_on * (l4.GetInPlayIntensity / l4.Intensity)
End Sub

Sub l5_animate
  p5.BlendDisableLighting = insert_blue_off + insert_blue_on * (l5.GetInPlayIntensity / l5.Intensity)
  p5off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l5.GetInPlayIntensity / l5.Intensity)
  bulb5.BlendDisableLighting = bulb_off + bulb_on * (l5.GetInPlayIntensity / l5.Intensity)
End Sub

Sub l6_animate
  p6.BlendDisableLighting = insert_blue_off + insert_blue_on * (l6.GetInPlayIntensity / l6.Intensity)
  p6off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l6.GetInPlayIntensity / l6.Intensity)
  bulb6.BlendDisableLighting = bulb_off + bulb_on * (l6.GetInPlayIntensity / l6.Intensity)
End Sub

Sub l36_animate
  p36.BlendDisableLighting = insert_blue_off + insert_blue_on * (l36.GetInPlayIntensity / l36.Intensity)
  p36off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l36.GetInPlayIntensity / l36.Intensity)
  bulb36.BlendDisableLighting = bulb_off + bulb_on * (l36.GetInPlayIntensity / l36.Intensity)
End Sub

Sub l37_animate
  p37.BlendDisableLighting = insert_blue_off + insert_blue_on * (l37.GetInPlayIntensity / l37.Intensity)
  p37off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l37.GetInPlayIntensity / l37.Intensity)
  bulb37.BlendDisableLighting = bulb_off + bulb_on * (l37.GetInPlayIntensity / l37.Intensity)
End Sub

Sub l38_animate
  p38.BlendDisableLighting = insert_blue_off + insert_blue_on * (l38.GetInPlayIntensity / l38.Intensity)
  p38off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l38.GetInPlayIntensity / l38.Intensity)
  bulb38.BlendDisableLighting = bulb_off + bulb_on * (l38.GetInPlayIntensity / l38.Intensity)
End Sub

Sub l60_animate
  p60.BlendDisableLighting = insert_blue_off + insert_blue_on * (l60.GetInPlayIntensity / l60.Intensity)
  p60off.BlendDisableLighting = a_insert_blue_off - a_insert_blue_on * (l60.GetInPlayIntensity / l60.Intensity)
...
Sub l7_animate
  p7.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l7.GetInPlayIntensity / l7.Intensity)
  p7off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l7.GetInPlayIntensity / l7.Intensity)
  bulb7.BlendDisableLighting = bulb_off + bulb_on * (l7.GetInPlayIntensity / l7.Intensity)
End Sub

Sub l8_animate
  p8.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l8.GetInPlayIntensity / l8.Intensity)
  p8off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l8.GetInPlayIntensity / l8.Intensity)
...
Sub l90_animate
  p90.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l90.GetInPlayIntensity / l90.Intensity)
  p90off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l90.GetInPlayIntensity / l90.Intensity)
  bulb90.BlendDisableLighting = bulb_off + bulb_on * (l90.GetInPlayIntensity / l90.Intensity)
End Sub
...
Sub l24_animate
  p24.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l24.GetInPlayIntensity / l24.Intensity)
  p24off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l24.GetInPlayIntensity / l24.Intensity)
  bulb24.BlendDisableLighting = bulb_off + bulb_on * (l24.GetInPlayIntensity / l24.Intensity)
End Sub

Sub l27_animate
  p27.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l27.GetInPlayIntensity / l27.Intensity)
  p27off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l27.GetInPlayIntensity / l27.Intensity)
...
Sub l29_animate
  p29.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l29.GetInPlayIntensity / l29.Intensity)
  p29off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l29.GetInPlayIntensity / l29.Intensity)
  bulb29.BlendDisableLighting = bulb_off + bulb_on * (l29.GetInPlayIntensity / l29.Intensity)
End Sub

Sub l39_animate
  p39.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l39.GetInPlayIntensity / l39.Intensity)
  p39off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l39.GetInPlayIntensity / l39.Intensity)
  bulb39.BlendDisableLighting = bulb_off + bulb_on * (l39.GetInPlayIntensity / l39.Intensity)
End Sub

Sub l40_animate
  p40.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l40.GetInPlayIntensity / l40.Intensity)
  p40off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l40.GetInPlayIntensity / l40.Intensity)
...
Sub l410_animate
  p410.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l410.GetInPlayIntensity / l410.Intensity)
  p410off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l410.GetInPlayIntensity / l410.Intensity)
  bulb410.BlendDisableLighting = bulb_off + bulb_on * (l410.GetInPlayIntensity / l410.Intensity)
End Sub

Sub l55_animate
  p55.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l55.GetInPlayIntensity / l55.Intensity)
  p55off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l55.GetInPlayIntensity / l55.Intensity)
  bulb55.BlendDisableLighting = bulb_off + bulb_on * (l55.GetInPlayIntensity / l55.Intensity)
End Sub

Sub l56_animate
  p56.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l56.GetInPlayIntensity / l56.Intensity)
  p56off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l56.GetInPlayIntensity / l56.Intensity)
  bulb56.BlendDisableLighting = bulb_off + bulb_on * (l56.GetInPlayIntensity / l56.Intensity)
End Sub

Sub l59_animate
  p59.BlendDisableLighting = insert_yellow_off + insert_yellow_on * (l59.GetInPlayIntensity / l59.Intensity)
  p59off.BlendDisableLighting = a_insert_yellow_off - a_insert_yellow_on * (l59.GetInPlayIntensity / l59.Intensity)
...
Sub l26_animate
  p26.BlendDisableLighting = insert_green_off + insert_green_on * (l26.GetInPlayIntensity / l26.Intensity)
  p26off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l26.GetInPlayIntensity / l26.Intensity)
  bulb26.BlendDisableLighting = bulb_off + bulb_on * (l26.GetInPlayIntensity / l26.Intensity)
End Sub

Sub l42_animate
  p42.BlendDisableLighting = insert_green_off + insert_green_on * (l42.GetInPlayIntensity / l42.Intensity)
  p42off.BlendDisableLighting = a_insert_green_off - a_insert_green_on * (l42.GetInPlayIntensity / l42.Intensity)
...
Dim FlBumperDisk(6), FlBumperBase(6), FlBumperBulb(6), FlBumperscrews(6), FlBumperActive(6), FlBumperHighlight(6), FlBumperSmallLighta(6)
Dim cnt
...
FlInitBumper 1, "StarG"
FlInitBumper 2, "StarG"
FlInitBumper 3, "StarG"
...
Sub FlInitBumper(nr, col)
  ...
  Set FlBumperSmallLighta(nr) = Eval("bumpersmalllighta" & nr)
  ...
  FlBumperBigLight(nr).color = RGB(255,255,0) : FlBumperBigLight(nr).colorfull = RGB(255,230,190)

  Select Case col

    Case "StarG"
      FlBumperSmallLight(nr).color = RGB(BumpR,BumpG,BumpB)
      FlBumperSmallLight(nr).colorfull = RGB(BumpR,BumpG,BumpB)
      FlBumperSmallLighta(nr).color = RGB(BumpR,BumpG,BumpB)
      FlBumperSmallLighta(nr).colorfull = RGB(BumpR,BumpG,BumpB)
      FlBumperBigLight(nr).color = RGB(BumpR,BumpG,BumpB)
      FlBumperBigLight(nr).colorfull = RGB(BumpR,BumpG,BumpB)
      FlBumperHighlight(nr).color = RGB(BumpR,BumpG,BumpB)
      FlBumperSmallLight(nr).BulbModulateVsAdd = 1
      FlBumperSmallLight(nr).TransmissionScale = 0.005
      FlBumperSmallLighta(nr).BulbModulateVsAdd = 1
      FlBumperSmallLighta(nr).TransmissionScale = 0.1
...
Sub FlFadeBumper(nr, Z)
  ...
  FlBumperBase(nr).BlendDisableLighting = ((0.0025 * LightLevel/lightlevelinitial) + Z * 0.0025 )
  FlBumperDisk(nr).BlendDisableLighting = ((0.5 * LightLevel/lightlevelinitial) + Z * 0.2 )
  ...
  Select Case FlBumperColor(nr)
    ...
    Case "StarG"
      UpdateMaterial "bumperbulbmat" & nr, 0, 0.75 , 0, 1 - Z, 1 - Z, 1 - Z, 0.9999, RGB(255, 190 + 40 * z, 190 * Z), RGB(255,255,255), RGB(32,32,32), False, True, 0, 0, 0, 0
      FlBumperSmallLight(nr).intensity = bumperscale * (8000 * Z)
      FlBumperSmallLighta(nr).intensity = bumperscale * (3000 * Z)
      FlBumperTop(nr).BlendDisableLighting = bumperscale * ((0.1 * LightLevel/lightlevelinitial) + 1.65 * Z)
      FlBumperBulb(nr).BlendDisableLighting = bumperscale * ((10 * LightLevel/lightlevelinitial) + 1000 * Z)
      Flbumperbiglight(nr).intensity = bumperscale * (80 * Z )
      FlBumperHighlight(nr).opacity = bumperscale * (4500 * (Z^3))
      MaterialColor "bumpertopmat" & nr, RGB(BumpR,BumpG, BumpB - 60 * z)
...
' Flipper Color
  If FlipperColor = 1 Then
    For Each C in GIFlip
      C.image = "FlipperUV-Blue-White"
      FlipperLRubber.material = "Rubber_Blue_Left"
      FlipperRRubber.material = "Rubber_Blue_Right"
    Next
  Elseif FlipperColor = 2 Then
    For Each C in GIFlip
      C.image = "FlipperUV-Green-White"
      FlipperLRubber.material = "Rubber_Flip_Left"
      FlipperRRubber.material = "Rubber_Flip_Right"
    Next
  Elseif FlipperColor = 3 Then
    For Each C in GIFlip
      C.image = "FlipperUV-Orange-Black"
      FlipperLRubber.material = "Rubber_Flip_Left"
      FlipperRRubber.material = "Rubber_Flip_Right"
    Next
  Elseif FlipperColor = 4 Then
    For Each C in GIFlip
      C.image = "FlipperUV-Orange-White"
      FlipperLRubber.material = "Rubber_Flip_Left"
      FlipperRRubber.material = "Rubber_Flip_Right"
    Next
  Elseif FlipperColor = 5 Then
    For Each C in GIFlip
      C.image = "FlipperUV-Yellow-White"
      FlipperLRubber.material = "Rubber_Flip_Left"
      FlipperRRubber.material = "Rubber_Flip_Right"
    Next
  Else
    For Each C in GIFlip
      C.image = "FlipperUV-Red-White"
      FlipperLRubber.material = "Rubber_Flip_Left"
      FlipperRRubber.material = "Rubber_Flip_Right"
    Next
  End If
...
' Target Color and Marks

  If TargetMarks = 1 Then

    If SUTargetColor = 0 Then
      sw10p.image  = "SternT35StandUp-Yellow-A"
      sw11p.image  = "SternT35StandUp-Yellow-B"
      sw17p.image  = "SternT35StandUp-Yellow-C"
      sw18p.image  = "SternT35StandUp-Yellow-A"
      sw19p.image  = "SternT35StandUp-Yellow-A"
      sw20p.image  = "SternT35StandUp-Yellow-C"
      sw21p.image  = "SternT35StandUp-Yellow-C"
      sw31p.image  = "SternT35StandUp-Yellow-C"
      sw32p.image  = "SternT35StandUp-Yellow-A"
      sw38p.image  = "SternT35StandUp-Yellow-C"
      sw39p.image  = "SternT35StandUp-Yellow-B"
      sw40p.image  = "SternT35StandUp-Yellow-C"
    Else
      sw10p.image  = "SternT35StandUp-A"
      sw11p.image  = "SternT35StandUp-B"
      sw17p.image  = "SternT35StandUp-C"
      sw18p.image  = "SternT35StandUp-A"
      sw19p.image  = "SternT35StandUp-A"
      sw20p.image  = "SternT35StandUp-C"
      sw21p.image  = "SternT35StandUp-C"
      sw31p.image  = "SternT35StandUp-C"
      sw32p.image  = "SternT35StandUp-A"
      sw38p.image  = "SternT35StandUp-C"
      sw39p.image  = "SternT35StandUp-B"
      sw40p.image  = "SternT35StandUp-C"
    End If


    If TargetColor = 0 Then
      sw22p.image = "DT_Stern_White-A"
      sw23p.image = "DT_Stern_White-B"
      sw24p.image = "DT_Stern_White-C"
      sw25p.image = "DT_Stern_White-A"
      sw26p.image = "DT_Stern_White-B"
      sw27p.image = "DT_Stern_White-C"
      sw28p.image = "DT_Stern_White-A"
      sw29p.image = "DT_Stern_White-B"
      sw30p.image = "DT_Stern_White-C"

    Elseif TargetColor = 1 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-1-Scuffed"
      Next

    Else
      sw22p.image = "DT_Stern_WhiteStar-A"
      sw23p.image = "DT_Stern_WhiteStar-B"
      sw24p.image = "DT_Stern_WhiteStar-C"
      sw25p.image = "DT_Stern_WhiteStar-A"
      sw26p.image = "DT_Stern_WhiteStar-B"
      sw27p.image = "DT_Stern_WhiteStar-C"
      sw28p.image = "DT_Stern_WhiteStar-A"
      sw29p.image = "DT_Stern_WhiteStar-B"
      sw30p.image = "DT_Stern_WhiteStar-C"
    End If
  ...
  Else
    ...
    If SUTargetColor = 0 Then
      For Each C in GITargets
        C.image = "SternT35StandUp-Yellow"
      Next

    Else
      For Each C in GITargets
        C.image = "SternT35StandUp"
      Next
    End If

    If TargetColor = 0 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White"
      Next

    Elseif TargetColor = 1 Then
      For Each C in GIdtTargets
        C.image = "DT_Stern_White-Sticker-1"
      Next

    Else
      For Each C in GIdtTargets
        C.image = "DT_Stern_WhiteStar"
      Next
    End If
...
' Outer Wall Color
  ...
  If SideWallColor = 1 Then
    OuterPrimBlack_DT.image = "OuterPrim_DT_UV-Black"
  Else
    OuterPrimBlack_DT.image = "OuterPrim_DT_UV-Blue"
  End If
...
' Spinner Colors
  ...
  If SpinnerColor = 0 Then
    SpinnerPrim.image = "Spinner-Yellow"
    SpinnerPrim2.image = "Spinner-Reversed-Yellow"
    SpinnerPrim3.image = "Spinner-Yellow"
  Elseif SpinnerColor = 1 Then
    SpinnerPrim.image = "Spinner-Yellow-with-scratches"
    SpinnerPrim2.image = "Spinner-Reversed-Yellow-with-scratches"
    SpinnerPrim3.image = "Spinner-Yellow-with-scratches"
  Elseif SpinnerColor = 2 Then
    SpinnerPrim.image = "Spinner-White"
    SpinnerPrim2.image = "Spinner-Reversed-White"
    SpinnerPrim3.image = "Spinner-White"
  Elseif SpinnerColor = 3 Then
    SpinnerPrim.image = "Spinner-White-with-scratches"
    SpinnerPrim2.image = "Spinner-Reversed-White-with-scratches"
    SpinnerPrim3.image = "Spinner-White-with-scratches"
  Elseif SpinnerColor = 4 Then
    SpinnerPrim.image = "Spinner-Glitter"
    SpinnerPrim2.image = "Spinner-Reversed-Glitter"
    SpinnerPrim3.image = "Spinner-Glitter"
  Else
    SpinnerPrim.image = "Spinner-Glitter-scratched"
    SpinnerPrim2.image = "Spinner-Reversed-Glitter-with-scratches"
    SpinnerPrim3.image = "Spinner-Glitter-scratched"
  End If
...
'Stern Star Gazer
' method originally by Inkochnito, updated by UnclePaulie
...
' Note: these are set to factory defaults, or conservative play.

' 1-4   0000  1 credit per coin in coin chute 1
' 5     1   Flashing value lite speed (fast = 0, slow = 1)
' 6     1   High Score Feature (awards free game at each high score level)
' 7     0   3 balls per game
' 8     1   Background Sound Off
' 9-12    0000  1 credit per coin in coin chute 2
' 13    0   Add A Ball feature is maxed at 1
' 14    0   Maximum Add a balls = 3 - zero if sw13 is off... which it is.
' 15-16   11    High Game to Date earns 3 free games
' 17    0   Extra Ball Lite alternates (conservative play)
' 18-19   11    40 max credits accepted
' 20    1   Credit is displayed
' 21    1   Match Feature is On
' 22    1   Along with switch 24:  Zodiac Special Awarded on 4th time hit (Conservative - both on)
' 23    1   All three Special lites positions (drop target and inlanes) alternate (Conservative)
' 24    1   Along with switch 22:  Zodiac Special Awarded on 4th time hit (Conservative - both on)
' 25-28   0000  1 credit per coin in coin chute 3
' 29    1   Bottom Banks spot next zodiac (liberal)
' 30    1   Extra Ball lite control - allow extra balls.
' 31-32   11    Special Award: Replay


Sub SetDefaultDips
  If SetDIPSwitches = 0 Then
    Controller.Dip(0) = 176   '8-1 = 10110000
    Controller.Dip(1) = 192   '16-9 = 11000000
    Controller.Dip(2) = 254   '24-17 = 11111110
    Controller.Dip(3) = 240   '32-25 = 11110000
    Controller.Dip(4) = 0
    Controller.Dip(5) = 0
  End If
End Sub
...
if VR_Room = 0 and cab_mode = 0 Then
  ...
  for each VRThings in VRStuffWalls:VRThings.sidevisible = 0:Next
  for each VRThings in VRStuffWalls:VRThings.visible = 0:Next
...
Elseif VR_Room = 0 and cab_mode = 1 Then
  ...
  for each VRThings in TopHardware:VRThings.visible = 0:Next
    if cabsideblades = 0 Then
      OuterPrimBlack_DT.z = -200
      OuterPrimBlack_DT.size_y = 217
    Else
      OuterPrimBlack_DT.z = -329
      OuterPrimBlack_DT.size_y = 165
    End If
...
Else
  ...
  for each VRThings in VRStuffWalls:VRThings.sidevisible = 1:Next
  for each VRThings in VRStuffWalls:VRThings.visible = 1:Next
...
Sub UpdateDTLamps()
  ...
  If Controller.Lamp(13) = 0 Then: HighScoreReel.setValue(0):   Else: HighScoreReel.setValue(1)
  If Controller.Lamp(45) = 0 Then: GameOverReel.setValue(0):    Else: GameOverReel.setValue(1)
  If Controller.Lamp(61) = 0 Then: TiltReel.setValue(0):      Else: TiltReel.setValue(1)

  If Controller.Lamp(11) = 0 Then 'Shoot Again
    ShootAgainReel.setValue(0)
  Else
    IF Controller.Lamp(45) = 0 AND GOn = 1 Then
  ...
  Else
    ...
    If Controller.Lamp(63) = 0 Then BIPReel.setValue(1): MatchReel.setValue(0): Else: BIPReel.setValue(0): MatchReel.setValue(0)
...
Sub UpdateVRLamps()
  ...
  If Controller.Lamp(63) = 0 Then: fMatch.visible=0: else: If MatchEn = 1 Then: fMatch.visible=1 ' match
  If Controller.Lamp(63) = 0 AND GOn = 1 Then: fBIP.visible=1: else: fBIP.visible=0 ' ball in play
...
'0.01 Playfield scan obtained from apophis, and then major cleanup, scaling, etc. done by Redbone.
'   Started with UP's Dragonfist as baseline for script, physics, etc. to utilize latest VPW standards
'   Removed glass top physics
'   Created a DIPs menu and default DIPs at game launch.
'   Set the ROM dip switch settings to match the instruction card (left: 12C-2-127-24A;   Right: 12D-SC-2-79A)
'   Created a .nvram file to set the high score replay levels of 720,000 and 1,440,000; and initial high score of zero.
'   Removed high spins to date.
'   Added little things in apron, and pegs in corners
...
'   Added plunger lane gate and gate trigger sounds and gate trigger ready logic..  Set at 30 degree closed.
'   Adjusted plunger groove lane
'   Adjusted slope of table to 5.65. Closer to slope of that era, and better ball play. Difficulty to 50.
'   Set ball reflectivity to 0.4
'   Updated ball choices and ball brightness code.
'   Added blue, red, and yellow peg material and added option to change.
'   Adjusted the playfield material to be white base, and 0.5 glossy layer.
'   Adjusted ball brightness in plunger lane.
'   Added flippers, flipper physics, accurate flipper prims and images. Adjusted the end angles of flippers, and the flipper physics.
'   Updated the LiveCatch for flippers.
...
'   Updated the bumperbiglight color regardless of bumpertops
'   Updated bumper subroutines, colors, brightness, and added options for on and off bumper brightness
'   Added another bumper light for higher intensity light coming off bulb and to see through cap
'   Softened the color on the bumper big lights (on the playfield) so not as deep color.
'   Adjusted the inner bumper light intensity
'   Tied bumper lights to GI and added logic to control the primitives
'   Set table bloom strength to zero, as it was causing signicant bumper blooms on the ball.
'     Enabled the backglass reels, SA, Tilt, GameOver, Match, BIP, HSTD for Desktop Mode. Also did logic for VR Backglass.
'   Put code in to control match and BIP in desktop game on and off modes.
'   Placed all peg posts, metal pegs, white posts, and white screw caps.  Also put longer peg inside posts where they go through plastics.
'   Added guide rails, rubbers, and all associated physics
'   Corrected the audiopan and audiofade functions to raise to power of 5 instead of 10.  It wasn't panning correctly.
'   Updated the slings audio pan and fade in the sound manager.
'   Increased bdl on the GI bulb prims from 1.25 to 4.
'   Added white screw caps, and all peg posts to GI.
'   Added rollovers, and animated them.
'   Adjusted the sling peg posts to align with each other closer.
'   Did not use NoTargetBouncer
'   Placed rubbers around posts and pegs, as well as the physics elements
'   Created a new stern peg in blender and imported.
'   Adjusted the rubbers to a height of 27 to align with new pegs.  Also adjusted the sling prims
...
'   All walls and guide rails placed
'   Had to bend the sling rubbers and the upper orbit wall ruber around the metal wall brackets.  Tried to emulate the real table as close as possible.
...
'   Meticulously placed the center targets AND the left orbit wall to match exact shots on videos.
'    Example right flipper to first left spinner
'     31:05: normal or rolling ball on flipper shot up orbit does NOT hit the drop targets.
'     https://www.youtube.com/watch?v=1whS3x1EAag
'     34:09 but... sometimes it DOES hit the first and only the first target
'   Set plunger.fire strength with a bit of randomness.
'   Adjusted the plunger gate to match shots
'    Example right orbit to flipper shots
'     1:51:40.  two shots for above gate to flippers... sort of... plunger doesn't make it, and shortly afterwards, it falls back.  weird bouncing off that gate.
'     https://www.youtube.com/watch?v=1whS3x1EAag
'     7:40 a full top around orbit on right, will drop it perfectly to the left flipper area.
'     https://www.youtube.com/watch?v=bj9f8vDpop4
'   Added all three spinners and animated them
'   Added logic to control spinner damping, fine tuned it for low speed and high speed hits to match videos, (5 seconds of spin on a full cradle on flipper to spinner shot)
'   Slings are pretty active, but not touchy.  Adjusted physics to match video shots.
'   Lowered the friction on the curved metal walls by flippers slightly.
'   Added a little more scatter angle on the bumpers.
...
'   Added an option for a flashers on slings, bumpers, and spinners.  Each selectable separately in the F12 options.
'   Added 100ms to the sling flasher option
'   Added VR cab and backbox images, VR posters.  All darthvito environments, and mine, and sixtoes.  Used Rajo Joey's VR images.
...
'   Animated the backglass text. (In GIMP I used a supernova lighting filter 35 radius, color 255, overlay for text on VR BG)
'   Moved the VR room back a little, as it appeared the cab was a little too close to the walls.
'   Redbone updated the playfield for lots of issues... wood, borders, and so many other cleanups.
'   Redbone cut holes in playfield for inserts.
'   Added a light under the star targets for reflections, and tied to GI. Adjusted bdl and other materials as well.  Added spearate insertoff1 and 2 for animation.
...
'   Changed the glow rings to have intensities ranging from 4 to 12.
...
'   Warmed the white inserts up some, but not as much as eight ball.  Between EB and DF.
...
'   Added options for colors on flippers, screw caps, drop targets, spinner, and marks on targets from ball wear with images from redbone
'   UP created a bumper image based on old redbone image, and added star and rings.  Adjusted inner bulb intensity and bdl's.
'   UP created plastic image based off of internet images.  Tried to upscale, but will need redbone to touch up.
'   Created plastic walls, acrylics, and placed and rotated screws
'   Added option for LED plastics edges
'   Redbone cut holes in the playfield for the bumpers and star targets, inserts, gi bulbs, targets, and moved the rollover holes, and plunger groove.
'   Redbone moved and shrunk the 2000 scores by the flippers as well.  And did the 5000 to match.
'   Added GI tops and main GI (hidden for baking)
'   Redbone created new plastics with better upscale and saturation.  Also updated the playfield blue hue some.
'   Had to hide both side and top of VR walls when not in VR mode.
'   Enlarged the inner bumper light falloff power some.  Went from 2 down to 1.25.
...
'   Increased the bdl on the GI bulb prims a little
'   Added code to control bdl on the stand up targets when the associated inserts were on in front.
...
'   Fixed the undercab LED lighting in VR when day/night mode adjusts (with option on or offset) as well as the new floor.
'   Increased the bdl of the cutout prims and holes.
'   Redbone added several images for the targets, spinners, apron, and plunger cover.
'   Adjusted the material of the apron a tad darker, and the instructions cards a tad "yellower/aged".
'0.02 Baked the metal walls and have GI and insert reflections on them.
'   Baked the inner walls and have GI and insert reflections on them.
'   Removed the bdl on targets, as caused performance issue on some systems
...
'   Drastically reduced the size of dtshadows.  Image, scale, and placed in GIMP and common flasher in vpx.  Adjusted opacity as well.
'   Updated the drop target physics.
'   Updated the bluegreen screw caps material slightly
'   Changed the rollover insertoff material to be 0.7 shiny.
'   Redbone updated scratched spinners
'0.03 Added a glass top physics to ensure that if the ball gets hit by a resetting target, it'll stay on the table.
'   Adjusted the material on the targets and spinners to match recent tables I've done, as well as adjusted the code for bdl.
'   Ensured screws were not collidable and only toys.  No need for that.
'   Redbone created new standup target images with 99% transparency to them to let some light through.
'   Had to adjust depth bias and material thickness to set just right.  targets went to -10.  Thickness had to be in script, and have a constant of 20.
'   Broke the stand up targets to two separate prims so you could get a better transparent look with the leaf behind it.
'   Updated the target images to remove a minor line that could be seen with them transparent.
'   Updated the max vp units for the standup target moves when hit. vpw defalut has it set to 9.  I changed to 7 accomodate the separate prims
'   Changed the default ball to Hauntfreaks Bright
'   Updated table info.
'2.0.0  Adjusted the backbox to be a little shorter.  Seemed too tall in VR.
'   Released

' Thanks to Redbone for updating the playfield, plastics, apron, targets, and other images.
' Thanks to apophis for providing the playfield scan, helping with metal wall bakes in blender, as well as pinpointing a couple performance improvements.
' Thanks to Hauntfreaks for providing the updated backglass images for VR.
