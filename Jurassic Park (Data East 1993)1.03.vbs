...
Sub LeftScoopEject(enabled)
  If enabled Then
    If sw35.ballCntOver then
      PlaysoundAtVol SoundFX(sfxkick,DOFContactors), sw35, VolKick
    Else
      PlaysoundAtVol SoundFX(SSolenoidOn,DOFContactors), sw35, VolKick
      End If
    sw35.kick 0,30,1.5
...
sub sw35_hit
  ...
  PlaySoundAtVol sfxHole, sw35, VolKick
  Controller.Switch(35) = True
End sub

sub Trigger2_hit
  sw35wall.collidable = False
  PlaysoundAtVol "Hit3", sw35, VolKick
End sub
'******************************************************
'         Middle Scoop Trough
'******************************************************
...
sub sw37_hit
  activeball.velx = 0
  vpmTimer.PulseSw 37
    PlaysoundAtVol "Hit1", ActiveBall, 1
End sub
...
sub Trigger1_hit
  PlaysoundAtVol "Hit3", ActiveBall, 1
End sub
...
Sub VukKick(enabled)
  If enabled Then
    If vuk.ballCntOver then
      PlaysoundAtVol SoundFX(sfxkick,DOFContactors), ActiveBall, VolKick
      PlaySound "wireramp_right", 0, 0.3*volumedial, Csng((900 * 2 / table1.width-1) ^10), 0, 100, 1, 0, AudioFade(ActiveBall)
    Else
...
sub vuk_hit
  ...
  PlaySoundAtVol sfxHole, Vuk, VolKick
...
Sub BoatDockSaucer(enabled)
  If enabled Then
    If Controller.Switch(56) = True then
      PlaysoundAtVol SoundFX(SfxSaucer,DOFContactors), sw56, VolKick
    Else
      PlaysoundAtVol SoundFX(SSolenoidOn,DOFContactors), sw56, VolKick
    End If
...
sub sw56_hit()
  PlaySoundAtVol "Kicker_enter_center", sw56, VolKick
...
Sub RaptorKick(enabled)
  If enabled Then
    PlaySoundAtVol SfxPlunger, sw29, VolKick
    sw29.Kick 180, 41
...
'Stand up Targets
  ...
  Sub sw45_Hit() :PlaySoundAtVol SoundFX("fx_bumper1",DOFContactors), ActiveBall, VolBump: vpmtimer.pulsesw 45: End Sub ' Jet Bumper (Top)
  Sub sw46_Hit() :PlaySoundAtVol SoundFX("fx_bumper2",DOFContactors), ActiveBall, VolBump: vpmtimer.pulsesw 46: End Sub ' Jet Bumper (Left)
  Sub sw47_Hit() :PlaySoundAtVol SoundFX("fx_bumper3",DOFContactors), ActiveBall, VolBump: vpmtimer.pulsesw 47: End Sub ' Jet Bumper (Right)
...
Sub RRail_Hit()
  PlaySound "wireramp_right", 0, 0.3*volumedial, Pan(ActiveBall), 0, Pitch(ActiveBall), 1, 0, AudioFade(ActiveBall)
End Sub
...
Sub RexSaucer(enabled)
  If enabled Then
    If Controller.Switch(55) = True then
      PlaysoundAtVol SoundFX(SfxSaucer,DOFContactors), sw55, VolKick
    Else
      PlaysoundAtVol SoundFX(SSolenoidOn,DOFContactors), sw55, VolKick
    End If
...
sub sw55_hit()
  PlaySoundAtVol "Kicker_enter_center", sw55, VolKick
...
If RexBendRot < trforwardangle + 1 Then RexBendRot = RexBendRot + RexBendDir/10 Else RexBendRot = RexBendRot + RexBendDir End If
    ...
    If RexBendRot <= trforwardangle then
      ...
      if sw55.ballCntOver then
        PlaysoundAtVol "1-Pickup", sw55, 1
        Controller.Switch(55) = False
        ballinmouth = 1
    ...
    If RexBendRot >= 0-1.5 Then
      ...
      if ballinmouth = 1 then
        PlaySoundAtVol "2-Swallow", sw55, 1
        sw55.Kick -13,2
        ballinmouth = 0
      end if
    Else
...
Sub Gate2_Hit():Gate2Open=1:Gate2Angle=0:PlaySoundAtVol "Gate2", ActiveBall, 1:End Sub
Sub Gate3_Hit():Gate3Open=1:Gate3Angle=0:PlaySoundAtVol "Gate3", ActiveBall, 1:End Sub
Sub Gate1_Hit():PlaySoundAtVol "BallRelease", ActiveBall, 1:End Sub
  ...
  ' play the rolling sound for each ball
    For b = 0 to UBound(BOT)
        If BallVel(BOT(b) ) > 1 AND BOT(b).z < 130 Then
            rolling(b) = True
            PlaySound("fx_ballrolling" & b), -1, Vol(BOT(b))*4*VolumeDial, Pan(BOT(b)), 0, Pitch(BOT(b)), 1, 0, AudioFade(BOT(b) )
    Elseif BallVel(BOT(b) ) > 1 AND BOT(b).z > 130 Then
            rolling(b) = True
            PlaySound("plasticroll" & b), -1, Vol(BOT(b))*10*2*VolumeDial, Pan(BOT(b)), 0, Pitch(BOT(b)), 1, 0, AudioFade(BOT(b) )
        Else
