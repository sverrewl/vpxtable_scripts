...
Sub a_Spinner_Spin(idx)
    PlaySound SoundFXDOF("fx_spinner",129,DOFPulse,DOFKnocker),0,VolSpin,0,0.25, AudioFade(SpinnerRod)
End Sub
...
Function AudioFade(tableobj) ' Fades between front and back of the table (for surround systems or 2x2 speakers, etc), depending on the Y position on the table. "Buccaneer" is the name of the table
  Dim tmp
  tmp = tableobj.y * 2 / Buccaneer.height-1
  If tmp > 0 Then
...
Function AudioPan(tableobj) ' Calculates the pan for a tableobj based on the X position on the table. "Buccaneer" is the name of the table
  Dim tmp
  tmp = tableobj.x * 2 / Buccaneer.width-1
  If tmp > 0 Then
...
Function Pan(ball) ' Calculates the pan for a ball based on the X position on the table. "Buccaneer" is the name of the table
    Dim tmp
    tmp = ball.x * 2 / Buccaneer.width-1
    If tmp > 0 Then
        Pan = Csng(tmp ^10)
    Else
        Pan = Csng(-((- tmp) ^10) )
