' Galaxy / IPD No. 979 / 1973 / 1 Player
' Sega Enterprises, Ltd., of Tokyo, Japan (1971-1979)
' VPX8 table by jpsalas & Leonard on the graphics v6.0.0
' and STAT on the directb2s
...
' Valores Constants
Const TableName = "SegaGalaxy" ' file name to save highscores and other variables
Const cGameName = "SegaGalaxy" ' B2S name
Const MaxPlayers = 1           ' this tabke is a 1 player only
Const MaxMultiplier = 1        ' limit bonus multiplier
Const FreePlay = False         ' Free play or coins
Const Special1 = 38000         ' extra ball or credit
Const Special2 = 50000
Const Special3 = 61000
Const Special4 = 72000
...
Sub AddScore(Points)
    ...
    UpdateScore
    ' sounds
    If Points <> 0 Then PlaySound "bell"
...
Sub Flipper001_Animate: plungergate.RotZ = Flipper001.CurrentAngle: End Sub
...
Sub Bumper1_Hit 'left bumper
    If Tilted Then Exit Sub
Dim tmpx, tmpy
tmpx = ActiveBall.VelX/2
tmpy = ActiveBall.VelY/2
    PlaySoundAt "fx_bumper", bumper1
    AddScore 10 + 90 * Lbumper1b.State
    Bumper1p.X = 340 - tmpx
    Bumper1p.Y = 585 - tmpy
    BumperCap1.X = 340 - tmpx
    BumperCap1.Y = 585 - tmpy
    Me.TimerEnabled = 1
End Sub

Sub Bumper1_Timer:Bumper1p.X = 340:Bumper1p.Y = 585:BumperCap1.X = 340:BumperCap1.Y = 585:Me.TimerEnabled = 0:End Sub
...
tmpy = ActiveBall.VelY/2
    ...
    AddScore 10 + 90 * Lbumper2b.State
    Bumper2p.X = 565 - tmpx
    Bumper2p.Y = 535 - tmpy
    BumperCap2.X = 565 - tmpx
    BumperCap2.Y = 535 - tmpy
    Me.TimerEnabled = 1
End Sub

Sub Bumper2_Timer:Bumper2p.X = 565:Bumper2p.Y = 535:BumperCap2.X = 565:BumperCap2.Y = 535:Me.TimerEnabled = 0:End Sub

Sub Bumper3_Hit 'center bumper
    If Tilted Then Exit Sub
Dim tmpx, tmpy
tmpx = ActiveBall.VelX/2
tmpy = ActiveBall.VelY/2
    PlaySoundAt "fx_bumper", Bumper3
    AddScore 10 + 90 * Lbumper3b.State
    Bumper3p.X = 469 - tmpx
    Bumper3p.Y = 744 - tmpy
    BumperCap3.X = 469 - tmpx
    BumperCap3.Y = 744 - tmpy
    Me.TimerEnabled = 1
End Sub

Sub Bumper3_Timer:Bumper3p.X = 469:Bumper3p.Y = 744:BumperCap3.X = 469:BumperCap3.Y = 744:Me.TimerEnabled = 0:End Sub
...
Sub LeftSlingShot_Slingshot
    If Tilted Then Exit Sub
    PlaySoundAt "fx_slingshot", Leftsling
    LeftSling.TransY = -18
    LStep = 0
...
Sub LeftSlingShot_Timer
    Select Case LStep
        Case 1:LeftSLing.TransY = -12
        Case 2:LeftSLing.TransY = -6
        Case 3:LeftSLing.TransY = 0:LeftSlingShot.TimerEnabled = 0
    End Select
    LStep = LStep + 1
End Sub
...
Sub RightSlingShot_Slingshot
    If Tilted Then Exit Sub
    PlaySoundAt "fx_slingshot", RightSling
    RightSling.TransY = -18
    RStep = 0
...
Sub RightSlingShot_Timer
    Select Case RStep
        Case 1:RightSLing.TransY = -12
        Case 2:RightSLing.TransY = -6
        Case 3:RightSLing.TransY = 0:RightSlingShot.TimerEnabled = 0
    End Select
    RStep = RStep + 1
End Sub
...
Sub sw1_Hit
    ...
    If NOT Tilted Then
        AddScore 300 + 2700 * Light002.State
    End If
End Sub
...
Sub sw2_Hit
    ...
    If NOT Tilted Then
        AddScore 300 + 2700 * Light003.State
    End If
End Sub

Sub sw2_UnHit:CloseGate:End Sub
...
' special and extra ball triggers

Sub sw3_Hit 'extra ball & open gate
    PlaySoundAt "fx_sensor", sw3
    If NOT Tilted Then
        AddScore 100
        OpenGate
        If Light010.State Then AwardExtraBall:ShootAgain.State = 1
    End If
End Sub

Sub OpenGate
    PlaySoundAt "fx_diverter", Flipper001
...
Sub Flipper001_Collide(parm)
    PlaySoundAt "fx_metalwire", Flipper001
End Sub
...
Sub sw4_Hit 'special
    ...
    If NOT Tilted Then
        AddScore 100
        UpPost
        If Light011.State Then AwardSpecial:Light011.State = 0
    End If
End Sub

Sub UpPost
    PlaySoundAt "fx_diverter", popup
    popup.Z = 0
    popupw.IsDropped = 0
    Light001.State = 1
End Sub
...
' Top gate/trigger 3000 Points

Sub sw9_Hit
    PlaySoundAt "fx_sensor", sw9
    If NOT Tilted Then
        Addscore 3000
        FlashForMs topLight, 1000, 50, 0
    End If
End Sub
...
Sub kicker1_Hit
    ...
    If NOT Tilted Then
        AddScore 100
    End If
    If kicker1L.State Then
        EnableSpaceWalk
        vpmTimer.AddTimer 1500, "PlaySoundAt ""fx_kicker"", kicker1:kicker1.Kick 45,8 '"
        kicker1L.State = 0
        kicker1L2.State = 0
    Else
        DisableSpaceWalk
        ...
        kicker2L.State = 0
        kicker2L2.State = 0
        vpmTimer.AddTimer 1500, "PlaySoundAt ""fx_kicker"", kicker1:kicker1.Kick 40, 20 '"
    End If
End Sub
...
Sub kicker2_Hit
    ...
    If NOT Tilted Then
        AddScore 100
    End If
    If kicker2L.State Then
        EnableSpaceWalk
        vpmTimer.AddTimer 1500, "PlaySoundAt ""fx_kicker"", kicker2:kicker2.Kick -45,8 '"
        kicker2L.State = 0
        kicker2L2.State = 0
    Else
        DisableSpaceWalk
        ...
        kicker1L.State = 0
        kicker1L2.State = 0
        vpmTimer.AddTimer 1500, "PlaySoundAt ""fx_kicker"", kicker2:kicker2.Kick 10, 40 '"
    End If
End Sub
...
Sub rsband011_Hit:AddScore 10:End Sub
...
Sub EnableSpaceWalk
    skypath.Collidable = 1
    sw10.Enabled = 1
    sw11.Enabled = 1
    sw12.Enabled = 1
    sw13.Enabled = 1
    sw14.Enabled = 1
    FlashForMs spacewalkLight, 1500, 50, 0
End Sub

Sub DisableSpaceWalk
    skypath.Collidable = 0
    sw10.Enabled = 0
    sw11.Enabled = 0
    sw12.Enabled = 0
    sw13.Enabled = 0
...
' Turn on some special Lights

Sub CheckSpecials
    If Light005.State + Light007.State + Light009.State = 3 Then 'the 3 top lanes has been hit, enable the extra score on the outlanes
        Light002.State = 1
        Light003.State = 1
    End If
    If Light005.State + Light006.State + Light007.State + Light008.State + Light009.State = 5 Then 'all 5 red lights are on
        Light010.State = 1
        Light011.State = 1
    End If
End Sub
