...
Const HSFileName="GrandPrix_76VPX.txt"
Const B2STableName="GrandPrix_1976"
...
Dim KickerCounter, DoodleTargetCount, YellowBumperFlag, GreenBumperFlag, DoodleBGLight, SpecialFlag, SpecialSetting, ScoreMotorStepper

Dim RightBonusCounter, BonusSide, LeftSpinnerCounter, RightSpinnerCounter, TopTargetCounter, BottomTargetCounter
...
Sub Table1_Init()
  ...
  KickerCounter=0
  LeftBonus000.state=0
  LeftBonus001.state=0
  RightBonus000.state=1
  RightBonus011.state=1
  DoodleTargetCount=1
  DoodleBugFlag=false
  ...
  RightSpinnerCounter=0
  LeftSpinnerLights(LeftSpinnerCounter).state=1
  RightSpinnerLights(RightSpinnerCounter).state=1



  InstructCard.image="IC"+FormatNumber(SpecialSetting,0)
...
' PlaySound "Plunger"
  ...
  If Credits > 0 Then DOF 127, 1:CreditLight.state=1
  for i=1 to 2
    player=i
    If B2SOn Then
...
Sub Table1_KeyDown(ByVal keycode)
  ...
  If keycode = AddCreditKey or keycode = 4 then
   ...
   if keycode = StartGameKey and Credits>0 and InProgress=true and Players>0 and Players<4 and BallInPlay<2 then
    Credits=Credits-1
    If Credits <1 Then DOF 127, DOFOff:CreditLight.state=0
    CreditsReel.SetValue(Credits)
...
'END GNMODthen
    Credits=Credits-1
    If Credits < 1 Then DOF 127, 0:CreditLight.state=0
    CreditsReel.SetValue(Credits)
    Players=1
...
Sub RightSlingShot_Timer
    Select Case RStep
        Case 3:RSLing1.Visible = 0:RSLing2.Visible = 1:sling1.TransZ = -4
...
Sub RubberWallSwitches_Hit(idx)
  if TableTilted=false then
    If idx<2 then
      ...
      SetMotor(500)
      If idx=2 then IncreaseBonus
      If idx=3 then IncreaseRightBonus
    end if
  end if
end Sub
...
Sub RightSpinner_Spin
  if TableTilted=false then
    ...
    IncreaseRightSpinner
  end if
end sub

Sub IncreaseLeftSpinner
  LeftSpinnerLights(LeftSpinnerCounter).state=0
  LeftSpinnerCounter=LeftSpinnerCounter+1
  if LeftSpinnerCounter>9 then LeftSpinnerCounter=0
  LeftSpinnerLights(LeftSpinnerCounter).state=1
  If LeftSpinnerCounter=5 or LeftSpinnerCounter=9 then
    IncreaseBonus
  end if
  If LeftSpinnerCounter=2 AND BallsPerGame=3 then
    IncreaseBonus
  end if
end sub

sub IncreaseRightSpinner
  RightSpinnerLights(RightSpinnerCounter).state=0
  RightSpinnerCounter=RightSpinnerCounter+1
  if RightSpinnerCounter>9 then RightSpinnerCounter=0
  RightSpinnerLights(RightSpinnerCounter).state=1
  If RightSpinnerCounter=5 or RightSpinnerCounter=9 then
    IncreaseRightBonus
  end if
  If RightSpinnerCounter=2 AND BallsPerGame=3 then
    IncreaseRightBonus
  end if
end sub
...
Sub AllTargets_Hit(idx)
  if TableTilted=false then
    DOF 107+idx,DOFPulse
    AddScore(1000)
    AllTargets(idx).IsDropped=true
    If idx <2 then
      TopTargetCounter=TopTargetCounter+1
    else
      BottomTargetCounter=BottomTargetCounter+1
    end if
    CheckTableLights
  end if
end sub

dim t1step,t2step,t3step,t4step
...
Sub target001_Timer
    Select Case t1Step
        Case 3:target3.Transz = 30
        Case 4:target3.transz=44:target001.TimerEnabled = 0
    End Select
    t1Step = t1Step + 1
End Sub
...
Sub Target002_Hit
  target4.transz=15
  t2step = 0
  target002.timerenabled =1
End Sub

Sub target002_Timer
    Select Case t2Step
        Case 3:target4.Transz = 30
        Case 4:target4.transz=44:target002.TimerEnabled = 0
    End Select
    t2Step = t2Step + 1
End Sub
...
Sub Target003_Hit
  target1.transz=15
  t3step = 0
  target003.timerenabled =1
End Sub

Sub target003_Timer
    Select Case t3Step
        Case 3:target1.Transz = 30
        Case 4:target1.transz=44:target003.TimerEnabled = 0
    End Select
    t3Step = t3Step + 1
End Sub
...
Sub Target004_Hit
  target2.transz=15
  t4step = 0
  target004.timerenabled =1
End Sub

Sub target004_Timer
    Select Case t4Step
        Case 3:target2.Transz = 30
        Case 4:target2.transz=44:target004.TimerEnabled = 0
    End Select
    t4Step = t4Step + 1
End Sub

Sub CheckTableLights
  if TopTargetCounter>1 OR BottomTargetCounter>1 then
    ResetDrops
    If StarLights(0).state=0 then
      StarLights(0).state=1
    elseif StarLights(1).state=0 then
      StarLights(1).state=1
    elseif StarLights(2).state=0 then
      StarLights(2).state=1
    elseif StarLights(3).state=0 then
      StarLights(3).state=1
    end if
    DisplayAltRelay
  end if
end sub
...
Sub Bumper1_Hit
  If TableTilted=false then
    PlaySoundAtVol SoundFXDOF("bump",105,DOFPulse,DOFContactors), ActiveBall, 1
...
Sub Bumper2_Hit
  If TableTilted=false then
    PlaySoundAtVol SoundFXDOF("bump",106,DOFPulse,DOFContactors), ActiveBall, 1
  ...
  If TableTilted=false then
    ...
    Select case idx
      ...
      case 2:
        ...
        if RightExtraBall.state=1 then
          ShootAgainLight.state=1
          ShootAgainReel.Setvalue(1)
          If B2SOn then
            Controller.B2SSetShootAgain 1
...
Sub Kicker1_Timer()
  ...
  Kicker1.timerenabled=0
    PlaySoundAtVol SoundFXDOF("scoopexit",113,DOFPulse,DOFContactors), Pkickarm1, 1
  DOF 126, DOFPulse
  X= INT(RND()*5+1)
  Kicker1.kick 170+X,18
  Pkickarm1.rotz=15
...
Sub Kicker2_Timer()
  If ScoreBonusInGame.enabled=true then exit sub
  Dim X

  if MotorRunning<>0 then exit sub
  Kicker2.timerenabled=0
    PlaySoundAtVol SoundFXDOF("scoopexit",111,DOFPulse,DOFContactors), Pkickarm2, 1
  DOF 126, DOFPulse
  X= INT(RND()*5+1)
  Kicker2.kick 150+X,15
  BonusCounter=1
  LeftBonus(1).state=1
  Pkickarm2.rotz=15
...
Sub Kicker3_Timer()
  ...
  Kicker3.timerenabled=0
    PlaySoundAtVol SoundFXDOF("scoopexit",112,DOFPulse,DOFContactors), Pkickarm3, 1
  DOF 126, DOFPulse
  X= INT(RND()*5+1)
  Kicker3.kick 205+X,15
  RightBonusCounter=1
  RightBonus(1).state=1
  Pkickarm3.rotz=15
  KickerArm2.enabled=true
end sub
...
Sub AddSpecial()
  ...
  PlaySound SoundFXDOF("knocker",128,DOFPulse,DOFContactors)
  DOF 126,2
  Credits=Credits+1
  DOF 127,1
...
Sub DisplayAltRelay
  If AltRelayValue=0 then
    ...
    LeftExtraBall.state=0
    RightExtraBall.state=0
    If SpecialSetting=1 then
      If StarLights(3).state=1 then
        LeftOutlaneLight.state=1
      end if
    else
      if StarLights(2).state=1 then
        LeftOutlaneLight.state=1
      end if
    end if
    If StarLights(1).state=1 then
      RightExtraBall.state=1
    end if
  else
...
sub ResetDropsTimer_timer
  ...
  target4.transz=0
  TopTargetCounter=0
  PlaysoundAtVol "droptargetreset", target3, 1
  PlaysoundAtVol "droptargetreset", target4, 1
end sub
...
sub ResetDropsTimer2_timer
  ...
  Target2.transz=0
  BottomTargetCounter=0
  PlaysoundAtVol "droptargetreset", target1, 1
  PlaysoundAtVol "droptargetreset", target2, 1
end sub
...
Sub ToggleArrowRelay
  Select Case BallInPlay
    case 1,3,5:
      LeftBonus000.state=1
      LeftBonus001.state=1
      RightBonus000.state=0
      RightBonus011.state=0
    case 2,4:
      LeftBonus000.state=0
      LeftBonus001.state=0
      RightBonus000.state=1
...
'   Controller.B2SSetScorePlayer4 0
  ...
  LeftSpinnerCounter=0
  RightSpinnerCounter=0
  LeftSpinnerLights(0).state=1
  RightSpinnerLights(0).state=1
  ToggleArrowRelay
  ResetBalls
...
sub savehs
  ...
  Set ScoreFile=FileObj.CreateTextFile(UserDirectory & HSFileName,True)
    ...
    ScoreFile.WriteLine SpecialSetting
...
Sub PlasticsOff
  ...
  For each xx in layer4col: xx.image = "layer4_off": Next
  plastics.image="plastics_off"
end sub

Sub SetupReplayTables

  Replay1Table(1)=450000
  Replay1Table(2)=470000
  Replay1Table(3)=510000
  ...
  Replay2Table(1)=660000
  Replay2Table(2)=680000
  Replay2Table(3)=720000
  ...
  Replay3Table(1)=810000
  Replay3Table(2)=820000
  Replay3Table(3)=860000
...
Const OptionLine4=""
Const OptionLine5="Gameplay Setting"
Const OptionLine6=""
Const OptionLine7=""
