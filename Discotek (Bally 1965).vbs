' Discotek / IPD No. 687 / September, 1965 / 2 Players
' Bally Manufacturing Corporation (1931-1983) [Trade Name: Bally]
' Table and graphics by jpsalas 2026
...
'DOF config - (server must be updated)
'101 Left Flipper
'102 Right Flipper
'103 left slingshot
'104 kicker used to be Right Slingshot
'105 left mushrrom
'106 center mushroom
'107 right mushroom
'108 Bumper lower
'109 Bumper Left
'110 Bumper Right
'111 Top mushroom (Left)
'112 Top mushroom
'113 Top mushroom
'114 Top mushroom (right)
'115
'116
'117
'118
...
' Valores Constants
Const TableName = "Discotek" ' file name to save highscores and other variables
Const cGameName = "Discotek" ' B2S name & DOF config
Const MaxPlayers = 2         ' 1 to 4 can play
Const MaxMultiplier = 1      ' limit bonus multiplier
Const MaxBonus = 100          ' highest bonus count
...
Sub LeftFlipper2_Animate:LeftFlipperP2.Rotz = LeftFlipper2.CurrentAngle:LeftFlipperRubber2.Rotz = LeftFlipper2.CurrentAngle:End Sub
...
Sub UpdateBonusLights
    Select Case Bonus
        Case 0:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 0
        Case 10:bl1.State = 1:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 0
        Case 20:bl1.State = 0:bl2.State = 1:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 0
        Case 30:bl1.State = 0:bl2.State = 0:bl3.State = 1:bl4.State = 0:bl5.State = 0:bl6.State = 0
        Case 40:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 1:bl5.State = 0:bl6.State = 0
        Case 50:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 1:bl6.State = 0
        Case 6:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 1
        Case 7:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 1
        Case 8:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 1
        Case 9:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 1
        Case 100:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 0:bl6.State = 1
    End Select
End Sub
...
Sub AddScore(Points)
    ...
    Select Case Points
        Case 1
            ...
            UpdateScore points
            ' sounds
            PlaySound SoundFXDOF("Bell1", 131, DOFPulse, DOFChimes)
        Case 10
            Score(CurrentPlayer) = Score(CurrentPlayer) + points
            UpdateScore points
            ' sounds
...
Sub UpdateBallInPlay 'update backdrop lights
    ...
    End If
    ' Update player light & reels
    Select Case CurrentPlayer
        Case 0:pl1.State = 0:pl2.State = 0: ScoreReel1.Image = "Bally_desktop_reel_yellow": ScoreReel2.Image = "Bally_desktop_reel_yellow"
        Case 1:pl1.State = 1:pl2.State = 0: ScoreReel1.Image = "Bally_desktop_reel_yellow_ON": ScoreReel2.Image = "Bally_desktop_reel_yellow"
        Case 2:pl1.State = 0:pl2.State = 1: ScoreReel1.Image = "Bally_desktop_reel_yellow": ScoreReel2.Image = "Bally_desktop_reel_yellow_ON"
    End Select
    If B2SOn then
...
Sub Bumper003_Hit 'lower bumper
    If Tilted Then Exit Sub
...
Sub rlband008_Hit:AddScore 1:TurnON aBumper1Light: End Sub
Sub rlband003_Hit:AddScore 1:TurnON aBumper2Light: End Sub
Sub rsband006_Hit:AddScore 1:TurnON aBumper3Light: End Sub
Sub rlband007_Hit:AddScore 1:TurnOFF aBumper2Light: End Sub
Sub rlband004_Hit:AddScore 1:TurnOFF aBumper1Light: End Sub
Sub rlband005_Hit:AddScore 1:TurnOFF aBumper3Light: End Sub

Sub rsband004_Hit 'rotate bumper Lights
Dim tmp
tmp = BumperL001.State
If BumperL003.State Then
 TurnON aBumper1Light
Else
 TurnOFF aBumper1Light
End If
If BumperL002.State Then
 TurnON aBumper3Light
Else
 TurnOFF aBumper3Light
End If
...
Sub CloseGate
    PlaySoundAt "Fx_solenoidOff", gateF
    Gatef.RotateToStart
    Light002.State = 0
End Sub
...
Sub closedoor_Hit:CloseGate:End Sub

' Mushrooms

Sub mushroom001_Hit '(Right)
    PlaySoundAtBall SoundFXDOF("fx_passive_bumper", 107, DOFPulse, DOFTargets)
    If Tilted Then Exit Sub
...
Sub mushroom007_Hit '(center)
    PlaySoundAtBall SoundFXDOF("fx_passive_bumper", 106, DOFPulse, DOFTargets)
    If Tilted Then Exit Sub
...
Sub mushroom007_Animate:mushrromP007.TransZ = - mushroom007.CurrentRingOffset / 4:End Sub

Sub mushroom002_Hit '(left - bonus)
    PlaySoundAtBall SoundFXDOF("fx_passive_bumper", 105, DOFPulse, DOFTargets)
    If Tilted Then Exit Sub
    'Count the Bonus
    AddScore Bonus
    Bonus = 0
...
Sub mushroom003_Hit ' collect bonus
    PlaySoundAtBall SoundFXDOF("fx_passive_bumper", 114, DOFPulse, DOFTargets)
    If Tilted Then Exit Sub
...
Sub mushroom004_Hit
    PlaySoundAtBall SoundFXDOF("fx_passive_bumper", 113, DOFPulse, DOFTargets)
    If Tilted Then Exit Sub
...
Sub mushroom006_Animate:mushrromP006.TransZ = - mushroom006.CurrentRingOffset / 4:End Sub
...
Sub Kicker001_Hit
    ...
    If Tilted Then
       vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 104, DOFPulse, DOFContactors), kicker001:kicker001.kick 326,7: RemF.RotateToEnd: Remf.TimerEnabled = 1 '"
       Exit Sub
    End If
    Addscore 1
    vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 104, DOFPulse, DOFContactors), kicker001:kicker001.kick 326,24: RemF.RotateToEnd: Remf.TimerEnabled = 1 '"
End Sub

Sub RemF_Animate:Remk.RotX = RemF.CurrentAngle:End Sub
Sub RemF_Timer:RemF.TimerEnabled = 0:RemF.RotateToStart:End Sub

Sub CheckBonusAwards
    Select Case Bonus
        Case 20:Light001.State = 1 'left slingshot
        Case 40:OpenTopGate
        Case 50:OpenGate
        Case 100:Light003.State = 1 'special
    End Select
End Sub

Sub OpenTopGate
    PlaySoundAt "fx_solenoidOn", Gate001
    Gate001.Open = True
    Light016.State = 1
End Sub

Sub CloseTopGate
    PlaySoundAt "fx_solenoidOff", Gate001
    Gate001.Open = False
    Light016.State = 0
End Sub
...
Dim LUTImage, BallsPerGame, bumpercolor
...
Sub Table1_OptionEvent(ByVal eventId)
    ...
    ' bumper caps
    x = Table1.Option("Bumper cap color", 0, 1, 1, 1, 0, Array("White", "Red") )
    If X Then
      cap1.image = "bumper_red"
      cap2.image = "bumper_red"
      cap3.image = "bumper_red"
    Else
      cap1.image = "bumper"
      cap2.image = "bumper"
      cap3.image = "bumper"
