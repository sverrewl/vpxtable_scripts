...
Const cGameName = "SparkPlugs_1951"
...
Const HSFileName="SparkPlugs_51VPX.txt"
Const B2STableName="SparkPlugs_1951"
...
Sub Table1_KeyDown(ByVal keycode)
  ...
  if (keycode=30 OR keycode=RightMagnasave) and InProgress=true and BallLiftOption=1 and BallsPlayed<BallsPerGame and LaneClear=0 then
    PlaySoundAtVol "BallLifter", Ballrelease, 1
...
Dim HossSelector, HossPointer, HossSelectorWin, HossSelectorPlace, HossSelectorShow
Dim HossWin, HossPlace, HossShow
HossSelector =      Array(3,5,1,2,4,3,1,6,2,1,4,5,3,1,6,2,4,1,3,5,1,4,2,6,1)
HossSelectorWin =   Array(3,4,5,6,1,4,5,2,1,4,3,6,5,1,4,3,6,5,2,1,4,5,6,1,2)
HossSelectorPlace = Array(6,1,2,3,4,5,6,1,3,5,2,1,4,4,3,6,5,4,1,6,5,2,1,4,5)
HossSelectorShow =  Array(5,2,1,4,5,3,1,4,6,2,5,4,1,5,2,1,4,3,6,5,6,1,4,3,6)
    ...
    case 2:  'move forward 1 step
      ...
      If tempxx<HOSSSpots6(HorseLoc6) then
        If HorseLoc6=11 then
          If HossWin=false then
            WinnerHorse(6)=true
          else
            if HossPlace=false then
              PlaceHorse(6)=true
            else
              ShowHorse(6)=true
            end if
...
Sub MoveHoss(HossNumber)
  ...
  If WinnerHorse(HossNumber)=true then exit sub
  If PlaceHorse(HossNumber)=true then exit sub
  If ShowHorse(HossNumber)=true then exit sub
...
sub ResetHosses
  ...
  HossPlace001.state=0
  HossPlace002.state=0
  HossPlace003.state=0
  HossBGPos1=0
  HossBGPos2=0
  HossBGPos3=0
  HossBGPos4=0
...
Sub ScoreWinner
  Dim WinCredits
  HorsePFLightCS.state=0
  If HossWin=false then
    HossWin=true
    if WinnerHorse(HossSelectorWin(PlayerHorse))=false then
      EndOfGameTimer.enabled=true
    else
      AddMultiSpecials=2
      AddSpecialTimer.enabled=true
      If B2SOn then Controller.B2SSetData 7,1
      HossPlace001.state=1
    end if
  else
    if HossPlace=false then
      HossPlace=true
      If PlaceHorse(HossSelectorPlace(PlayerHorse))=false then
        EndOfGameTimer.enabled=true
      else
        AddMultiSpecials=3
        AddSpecialTimer.enabled=true
        If B2SOn then Controller.B2SSetData 17,1
        HossPlace002.state=1
      end if
    else
      HossShow=true
      If ShowHorse(HossSelectorShow(PlayerHorse))=true then
        AddMultiSpecials=20
        AddSpecialTimer.enabled=true
        If B2SOn then Controller.B2SSetData 27,1
        HossPlace003.state=1
      end if
      EndOfGameTimer.enabled=true
    end if
  end if
...
Sub HorseSelection
  ...
  for i = 1 to 6
    ...
    EVAL("PlaceHoss00"&i).state=0
    EVAL("ShowHoss00"&i).state=0
    ...
    if B2SOn then
      ...
      For ix = 1 to 6
        ...
        Controller.B2SSetData ix+20,0

      next
    end if
  next
  EVAL("YourHoss00"&HossSelectorWin(HossPointer)).state=1
  EVAL("PlaceHoss00"&HossSelectorPlace(HossPointer)).state=1
  EVAL("ShowHoss00"&HossSelectorShow(HossPointer)).state=1
  'EVAL("HorsePFLight00"&HossSelector(HossPointer)).state=1
  If B2SOn then
    Controller.B2SSetData (HossSelectorWin(HossPointer)),1
    Controller.B2SSetData (10+HossSelectorPlace(HossPointer)),1
    Controller.B2SSetData (20+HossSelectorShow(HossPointer)),1
  end if
  ...
  If TableTilted=false then
    ...
    Select Case (idx)
      case 0:
        ...
        SetMotor(5)
      case 1:
        If HorsePFLightCS.state=1 then
          HorseSelection
        end if
        AddScore(1)
      case 2:
...
Sub AddSpecial()
  ...
  if Credits>199 then Credits=199
  If B2SOn Then
    Controller.B2SSetCredits Credits
  End If
  CreditsReel.SetValue(Credits)
...
Dim CreditsOnes, CreditsTens
CreditsOnes = Array (74,75,76,77,78,79,105,106,107,108)
CreditsTens = Array (134,135,136,137,138,139,165,166,167,168)
Sub DisplayCreditReel
  ...
  ones=Credits mod 10
  tens=(Credits \ 10) mod 10
  hund =(Credits \100) mod 10

  If ones=0 then
    Credit1s.SetValue(0)
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData CreditsOnes(tempscore),0
    next
  end if
  If ones>0 then
'   EVAL("Score00"&ones).state=1
    ...
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData CreditsOnes(tempscore),0
    next
    If B2SOn then Controller.B2SSetData CreditsOnes(ones),1
    Credit1s.SetValue(ones)
  end if
  If tens=0 then
    Credit10s.setvalue(0)
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData CreditsTens(tempscore),0
    next
  end if
  If tens>0 then
'   EVAL("Score0"&tens&"0").state=1
    ...
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData CreditsTens(tempscore),0
    next
    If B2SOn then Controller.B2SSetData CreditsTens(tens),1
    Credit10s.setvalue(tens)
  end if
  If hund=0 then
    Credit100s.setvalue(0)
    If B2SOn then Controller.B2SSetCanPlay 0
  else
    Credit100s.Setvalue(1)
    If B2SOn then Controller.B2SSetCanPlay 1
  end if
...
'   Controller.B2SSetScorePlayer4 0
  ...
  Bumper3LightEven.state=1
  Bumper4LightOdd.state=1
...
sub BumpersOff
  ...
  Bumper3LightEven.visible=0
  Bumper4LightOdd.visible=0
...
sub BumpersOn
  ...
  Bumper3LightEven.visible=1
  Bumper4LightOdd.visible=1
...
Sub LightScore
  ...
  If ones=0 then
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData 230+tempscore,0
    next
  end if
  If ones>0 then
    EVAL("Score00"&ones).state=1
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData 230+tempscore,0
    next
    If B2SOn then Controller.B2SSetData 230+ones,1
  end if
  If tens=0 then
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData 240+tempscore,0
    next
  end if
  If tens>0 then
    EVAL("Score0"&tens&"0").state=1
    for tempscore = 1 to 9
      If B2SOn then Controller.B2SSetData 240+tempscore,0
    next
    If B2SOn then Controller.B2SSetData (240+tens),1
  end if
