' Moulin Rouge / IPD No. 1634 / May 26, 1965 / 1 Player
' Williams Electronic Manufacturing Corporation (1958-1967) [Trade Name: Williams]
' VPX8 table by jpsalas with graphics by Halen, version 5.5.0
...
' Valores Constants
Const TableName = "moulinrouge" ' file name to save highscores and other variables
Const cGameName = "moulinrouge" ' B2S name
Const MaxPlayers = 1            ' 1 to 4 can play
Const MaxMultiplier = 1         ' limit bonus multiplier
Const FreePlay = False          ' Free play or coins
Const Special1 = 3700           ' extra ball or credit
Const Special2 = 4800
Const Special3 = 5600
Const Special4 = 6400
...
Sub UpdateBonusLights
    ...
    Select Case Bonus
        Case 1:bl1.State = 1
        Case 2:bl2.State = 1
        Case 3:bl3.State = 1:Light010.State = 1
        Case 4:bl4.State = 1:Light005.State = 1
        Case 5:bl5.State = 1
        Case 6:bl6.State = 1:Light011.State = 1
        Case 7:bl7.State = 1:Light001.State = 1
        Case 8:bl8.State = 1
        Case 9:bl9.State = 1:Light012.State = 1
        Case 10:bl10.State = 1:Light002.State = 1
        Case 11:bl11.State = 1
        Case 12:bl12.State = 1:Light013.State = 1
        Case 13:bl13.State = 1:Light004.State = 1
        Case 14:bl14.State = 1
        Case 15:bl15.State = 1:Light014.State = 1
    End Select
End Sub
...
Sub Game_Init 'called at the start of a new game
    ...
    Light004.State = 1
    Light005.State = 1
    BumperL003.State = 0 '100 center bumper
    BumperL007.State = 0 'red bumpers
    BumperL006.State = 0
    TopBumpersON 0
    'picture Lights
    pic001.State = 0
...
Sub ToggleBumpers
    ToggleB = ABS(ToggleB - 1)
    TopBumpersON ToggleB
End Sub

Sub TopBumpersON(Enabled)
    BumperL001.State = Enabled
    BumperL001a.State = Enabled
    BumperL002.State = Enabled
    BumperL002a.State = Enabled
    BumperL004.State = Enabled
    BumperL004a.State = Enabled
    BumperL005.State = Enabled
    BumperL005a.State = Enabled
    If CenterBumperON Then BumperL003.State = Enabled
End Sub

' yellow 10 points  bumpers
...
Sub Bumper007_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_bumper", Bumper007
    If BumperL007.State Then
        AddScore 10
    Else
        Addscore 1
        ToggleBumpers
...
Sub Trigger008_Hit 'red bumpers
...
Sub Trigger009_Hit '10p
    PlaySoundAt "fx_sensor", Trigger009
    If Tilted Then Exit Sub
    AddScore 10
End Sub

Sub Trigger010_Hit '10p
...
Sub rlband002_Hit:AddScore 1:ToggleBumpers:End Sub
Sub rlband001_Hit:AddScore 1:ToggleBumpers:End Sub
...
Sub CheckTargets
    If Light011.State + Light012.State + Light013.State + Light014.State = 0 Then
        CenterBumperON = True
    End If
End Sub

Sub CheckSideSpecials
    If Light015.State + Light003.State + Light002.State + Light004.State + Light005.State + Light020.State + Light011.State + Light012.State + Light013.State + Light014.State = 0 Then
        Light001.State = 1
        Light006.State = 1
        AwardSpecial
    End If
...
Sub Kicker004_Hit 'left A
    PlaySoundAt "fx_kicker_enter", Kicker004
    If Tilted Then vpmTimer.AddTimer 200, "PLaySoundAt""fx_kicker"", kicker004:kicker004.kick 30,10 '"
    'AddScore
    AddScore 50
    vpmTimer.AddTimer 1500, "PLaySoundAt""fx_kicker"", kicker004:kicker004.kick 30,20 '"
    'check A
...
Sub Kicker005_Hit 'right A
    PlaySoundAt "fx_kicker_enter", Kicker005
    If Tilted Then vpmTimer.AddTimer 200, "PLaySoundAt""fx_kicker"", kicker005:kicker005.kick -30,10 '"
    'AddScore
    AddScore 50
    vpmTimer.AddTimer 1500, "PLaySoundAt""fx_kicker"", kicker005:kicker005.kick -30,20 '"
    'check A
    If Light008.State Then
        Light008.State = 0
        Light007.State = 1
...
Sub Kicker001_Hit 'B
    ...
    If Tilted Then vpmTimer.AddTimer 200, "PLaySoundAt""fx_kicker"", kicker001:kicker001.kick 185,10 '": Exit Sub
    'AddScore
    AddScore 50
    vpmTimer.AddTimer 1500, "PLaySoundAt""fx_kicker"", kicker001:kicker001.kick 185,15 '"
    'check B
...
Sub Kicker003_Hit 'C
    ...
    If Tilted Then vpmTimer.AddTimer 200, "PLaySoundAt""fx_kicker"", kicker003:kicker003.kick 198,10 '": Exit Sub
    'AddScore
    AddScore 50
    vpmTimer.AddTimer 1500, "PLaySoundAt""fx_kicker"", kicker003:kicker003.kick 198,15 '"
    'check C
    If Light009.State Then
        Light009.State = 0
        Light010.State = 1 'center special
    End If
End Sub

Sub Kicker002_Hit 'Center hole
    PlaySoundAt "fx_kicker_enter", Kicker002
    If Tilted Then vpmTimer.AddTimer 200, "PLaySoundAt""fx_kicker"", kicker002:kicker002.kick 188,10 '": Exit Sub
    'AddScore
    If Light008.State Then Addscore 200
    If Light007.State Then Addscore 300
    If Light009.State Then Addscore 400
    vpmTimer.AddTimer 1500, "PLaySoundAt""fx_kicker"", kicker002:kicker002.kick 188,15 '"
    'check Special
    If Light010.State Then
        AwardSpecial
        Light010.State = 0
        Light008.State = 1 '200 points
        a.State = 1
        b.State = 0
        c.State = 0
        If B2SOn then
