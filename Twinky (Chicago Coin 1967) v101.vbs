' Twinky / IPD No. 2692 / July 18, 1967 / 2 Players
' Chicago Coin Machine Manufacturing Company (1932-1977)
' VPX8 table by jpsalas, September 2025
...
' Valores Constants
Const TableName = "twinky" ' file name to save highscores and other variables
Const cGameName = "twinky" ' B2S name
Const MaxPlayers = 2       ' 1 to 4 can play
Const MaxMultiplier = 1    ' limit bonus multiplier
Const FreePlay = True      ' Free play mostly because this game has no credits reel
Const Special1 = 1400      ' extra ball or credit
Const Special2 = 2300
Const Special3 = 3200
Const Special4 = 4100
Const Special5 = 5000
...
Sub Table1_KeyDown(ByVal Keycode)
    ...
    If PeacockEnabled then
        If keycode = LeftMagnaSave OR keycode = RightMagnaSave OR keycode = PlungerKey Then KickPeacockBall
    End If
...
Sub AddScore(Points)
    ...
    If Points = 10 then RotateLights
    Select Case Points
        Case 1, 10, 100
...
Sub AddBonus(bonuspoints)
    If(Tilted = False) Then
        ...
        End If
        UpdateBonusLights
        If Bonus = 5 AND bOutlaneBonus = False Then
            Opengate
            bOutlaneBonus = True
            Light004.State = 1
            Light005.State = 1
...
Sub UpdateBonusLights
    Select Case Bonus
        ...
        Case 2:bl1.State = 0:bl2.State = 1:bl3.State = 0:bl4.State = 0:bl5.State = 0
        Case 3:bl1.State = 0:bl2.State = 0:bl3.State = 1:bl4.State = 0:bl5.State = 0
        Case 4:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 1:bl5.State = 0
        Case 5:bl1.State = 0:bl2.State = 0:bl3.State = 0:bl4.State = 0:bl5.State = 1
    End Select
End Sub
...
Sub AddCredits(value) 'limit to 2 credits
    If Credits <2 Then
...
Sub Loadhs
    ...
    If(x <> "") Then HSScore(1) = CDbl(x) Else HSScore(1) = 1300 End If
    x = LoadValue(TableName, "HighScore1Name")
    If(x <> "") Then HSName(1) = x Else HSName(1) = "AAA" End If
    x = LoadValue(TableName, "HighScore2")
    If(x <> "") then HSScore(2) = CDbl(x) Else HSScore(2) = 1200 End If
    x = LoadValue(TableName, "HighScore2Name")
    If(x <> "") then HSName(2) = x Else HSName(2) = "BBB" End If
    x = LoadValue(TableName, "HighScore3")
    If(x <> "") then HSScore(3) = CDbl(x) Else HSScore(3) = 1100 End If
    x = LoadValue(TableName, "HighScore3Name")
    If(x <> "") then HSName(3) = x Else HSName(3) = "CCC" End If
    x = LoadValue(TableName, "HighScore4")
    If(x <> "") then HSScore(4) = CDbl(x) Else HSScore(4) = 1000 End If
    x = LoadValue(TableName, "HighScore4Name")
    If(x <> "") then HSName(4) = x Else HSName(4) = "DDD" End If
    x = LoadValue(TableName, "HighScore5")
    If(x <> "") then HSScore(5) = CDbl(x) Else HSScore(5) = 900 End If
...
Sub Closegate:Gate002.Open = False:End SUb
Sub Opengate:Gate002.Open = True:End SUb
...
Sub rsband010_Hit:AddScore 1:End Sub
Sub rsband011_Hit:AddScore 1:End Sub
Sub rlband006_Hit:AddScore 1:End Sub
Sub rlband011_Hit:AddScore 1:End Sub
...
Sub Target001_Hit 'Left
    ...
    If Light006.State Then
        ...
        If TargetHits> 5 Then Light003.State = 1:Light007.State = 1 'enabled Special lights
        Else
            AddScore 50
    End If
End Sub
...
Sub Trigger003_Hit 'left outlane
    ...
    Else
        AddScore 100
    End If
    If Light007.State Then AwardExtraBall
End Sub
...
Sub kicker002_Hit 'Top hole
    Dim tmp
    PlaySoundAt SoundFXDOF("fx_kicker_enter", 109, DOFPulse, DOFTargets), kicker002
    If Tilted Then vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker002:kicker002.kick 170,10 '"
    DOF 211, DOFPulse
    DOF 231, DOFPulse
    PeacockR.SetValue 1
    If B2SOn Then
...
Dim PeaStep:PeaStep = 0

Sub StartPeacock: Peacocktimer.Enabled = 1:PeacockEnabled = True: End Sub

Sub Peacocktimer_Timer
    PeaStep = (PeaStep + 1) MOD 5
    Select Case PeaStep
        Case 0:l50.State = 1:l100.State = 0:l200.State = 0:l300.State = 0:l500.State = 0:PlaySound "fx_peacock"
        Case 1:l50.State = 0:l100.State = 1:l200.State = 0:l300.State = 0:l500.State = 0
        Case 2:l50.State = 0:l100.State = 0:l200.State = 1:l300.State = 0:l500.State = 0
        Case 3:l50.State = 0:l100.State = 0:l200.State = 0:l300.State = 1:l500.State = 0
        Case 4:l50.State = 0:l100.State = 0:l200.State = 0:l300.State = 0:l500.State = 1
    End Select
    If B2SOn Then
        Select Case PeaStep
            Case 0:Controller.B2SSetData 70, 1:Controller.B2SSetData 71, 0:Controller.B2SSetData 72, 0:Controller.B2SSetData 73, 0:Controller.B2SSetData 74, 0
            Case 1:Controller.B2SSetData 70, 0:Controller.B2SSetData 71, 1:Controller.B2SSetData 72, 0:Controller.B2SSetData 73, 0:Controller.B2SSetData 74, 0
            Case 2:Controller.B2SSetData 70, 0:Controller.B2SSetData 71, 0:Controller.B2SSetData 72, 1:Controller.B2SSetData 73, 0:Controller.B2SSetData 74, 0
            Case 3:Controller.B2SSetData 70, 0:Controller.B2SSetData 71, 0:Controller.B2SSetData 72, 0:Controller.B2SSetData 73, 1:Controller.B2SSetData 74, 0
            Case 4:Controller.B2SSetData 70, 0:Controller.B2SSetData 71, 0:Controller.B2SSetData 72, 0:Controller.B2SSetData 73, 0:Controller.B2SSetData 74, 1
        End Select
    End If
End Sub
...
Sub Trigger001_Hit 'Peacock award trigger
    Peacocktimer.Enabled = 0
    Addscore PeacockValue(PeaStep)
    vpmTimer.addtimer 1200, "Resetpeacock '"
End Sub

Sub ResetPeacock 'turn off lights and kick the ball
    PeacockEnabled = False
    PeacockR.SetValue 0
    l50.State = 0:l100.State = 0:l200.State = 0:l300.State = 0:l500.State = 0
    If B2sOn Then
        Controller.B2SSetData 70, 0:Controller.B2SSetData 71, 0:Controller.B2SSetData 72, 0
        Controller.B2SSetData 73, 0:Controller.B2SSetData 74, 0:Controller.B2SSetData 75, 0
    End If
    If kicker002.BallCntOver Then PlaySoundAt SoundFXDOF("fx_kicker", 110, DOFPulse, DOFContactors), kicker002:kicker002.kick 170, 10
End Sub

' Center Hole

Sub kicker003_Hit 'Center hole
    Dim tmp
    PlaySoundAt SoundFXDOF("fx_kicker_enter", 109, DOFPulse, DOFTargets), kicker003
    If Tilted Then vpmTimer.AddTimer 200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker003:kicker003.kick 170,10 '"
    DOF 211, DOFPulse
    DOF 231, DOFPulse
    Addscore BonusValue(Bonus)
    vpmTimer.AddTimer 1200, "PlaySoundAt SoundFXDOF (""fx_kicker"", 110, DOFPulse, DOFContactors), kicker003:kicker003.kick 170,10 '"
End Sub
