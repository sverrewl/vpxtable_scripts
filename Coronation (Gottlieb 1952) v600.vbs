' Coronation / IPD No. 568 / November, 1952 / 1 Player
' D. Gottlieb & Company (1931-1977) [Trade Name: Gottlieb]
' VPX8 table by jpsalas 2024. graphics by Halen
...
' Valores Constants
Const TableName = "Coronation" ' file name to save highscores and other variables
Const cGameName = "Coronation" ' B2S name
Const MaxPlayers = 1           ' 1 to 4 can play
Const MaxMultiplier = 2        ' limit bonus multiplier
Const FreePlay = False         ' Free play or coins
Const Special1 = 5200000        ' extra ball or credit
Const Special2 = 5500000
Const Special3 = 5800000
Const Special4 = 5900000
Const Special5 = 6000000
Const Special6 = 6100000
Const Special7 = 6200000
Const Special8 = 6300000
...
Dim Special8Awarded(4)
Dim SSpecial1Awarded(4)
Dim SSpecial2Awarded(4)
Dim SSpecial3Awarded(4)
Dim SSpecial4Awarded(4)
Dim SSpecial5Awarded(4)
Dim SSpecial6Awarded(4)
Dim SSpecial7Awarded(4)
Dim Score(4)
Dim HighScore
'Dim Match
Dim Tilt
...
Sub Table1_Init()
    ...
    'turn on GI lights
    vpmtimer.addtimer 1000, "TurnON aBumpersL:GiOn '"
...
Sub ResetForNewGame()
    ...
    For i = 1 To MaxPlayers
        ...
        Special8Awarded(i) = False
        SSpecial1Awarded(i) = False
        SSpecial2Awarded(i) = False
        SSpecial3Awarded(i) = False
        SSpecial4Awarded(i) = False
        SSpecial5Awarded(i) = False
        SSpecial6Awarded(i) = False
        SSpecial7Awarded(i) = False
...
Sub EndOfBall2()
    ...
    Else ' no extra ball
        ...
        If(BallsRemaining(CurrentPlayer) <= 0) Then
            vpmTimer.AddTimer 1500, "CheckHighScore() '" 'wait for the scores finish updating
        End If
...
Sub CheckScoreAwards
    ...
    End If
    If Score(CurrentPlayer) >= Special8 AND Special8Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special8Awarded(CurrentPlayer) = True
    End If
End Sub
...
Sub UpdateScore
    ...
    tmp = tmp \ 10
    Select Case tmp MOD 10
        Case 1:mi001.State = 1
        Case 2:mi002.State = 1
        Case 3:mi003.State = 1
        Case 4:mi004.State = 1
        Case 5:mi005.State = 1
        Case 6:mi006.State = 1
        Case 7:mi007.State = 1
    End Select
    If B2SOn Then
        B2sid = 79 + tmp MOD 10
        Controller.B2SSetData B2sid, 1
...
Sub ResetScores
    ...
    If B2SOn Then
        For i = 60 to 68:Controller.B2SSetData i, 0:Next
        For i = 70 to 78:Controller.B2SSetData i, 0:Next
        For i = 80 to 86:Controller.B2SSetData i, 0:Next
    End If
End Sub
...
Sub Game_Init() 'called at the start of a new game
    ...
    'Start music?
    'Init variables?
    For i = 0 To 7:holes(i) = 0:Next
    'Start or init timers
    PlaySoundAt "fx_releaseballs", drain_dummy
    balllock.IsDropped = 1
    balllock2.IsDropped = 1
    For each i in aMetalHoles:i.Isdropped = 1:next 'hole metal locks
    balllock.TimerEnabled = 1
    'Init lights?
    TurnOffPlayfieldLights()
    'turn on bumpers
    TurnOn aBumpersL
    ' turn off bumper lights on backdrop/B2S
    TurnOFF aBumpersDL
    If B2SOn Then
        For i = 1 To 7:Controller.B2SSetData 49 + i, 0:Next
    End If
    SingleScore = 0
    UpdateSingleScore
End Sub
...
Sub balllock_Timer
    ...
    balllock.IsDropped = 0
    balllock2.IsDropped = 0
    For each i in aMetalHoles:i.isdropped = 0:Next
End Sub
...
Sub Bumper8_Hit '8 bumper, center
    If Tilted Then Exit Sub
    PlaySoundAtBall "fx_bumper"
    'Addscore 10000
    If Bumper8L.State Then AddSingleScore 1
End Sub
...
Sub Trigger009_Hit 'Special button
    If Tilted Then Exit Sub
...
Sub AddSingleScore(Points)
    If SingleScore < 39 Then
        SingleScore = SingleScore + Points
        UpdateSingleScore
    End If
    'check for awards
    If SingleScore >= SSpecial1 AND Special1Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special1Awarded(CurrentPlayer) = True
    End If
    If SingleScore >= SSpecial2 AND Special2Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special2Awarded(CurrentPlayer) = True
    End If
    If SingleScore >= SSpecial3 AND Special3Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special3Awarded(CurrentPlayer) = True
    End If
    If SingleScore >= SSpecial4 AND Special4Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special4Awarded(CurrentPlayer) = True
    End If
    If SingleScore >= SSpecial5 AND Special5Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special5Awarded(CurrentPlayer) = True
    End If
    If SingleScore >= SSpecial6 AND Special6Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special6Awarded(CurrentPlayer) = True
    End If
    If SingleScore >= SSpecial7 AND Special7Awarded(CurrentPlayer) = False Then
        AwardSpecial
        Special7Awarded(CurrentPlayer) = True
    End If
End Sub

Sub UpdateSingleScore
    Select Case SingleScore MOD 10
        Case 0:p001.State = 0:p002.State = 0:p003.State = 0:p004.State = 0:p005.State = 0:p006.State = 0:p007.State = 0:p008.State = 0:p009.State = 0
        Case 1:p001.State = 1:p002.State = 0:p003.State = 0:p004.State = 0:p005.State = 0:p006.State = 0:p007.State = 0:p008.State = 0:p009.State = 0
        Case 2:p001.State = 0:p002.State = 1:p003.State = 0:p004.State = 0:p005.State = 0:p006.State = 0:p007.State = 0:p008.State = 0:p009.State = 0
        Case 3:p001.State = 0:p002.State = 0:p003.State = 1:p004.State = 0:p005.State = 0:p006.State = 0:p007.State = 0:p008.State = 0:p009.State = 0
        Case 4:p001.State = 0:p002.State = 0:p003.State = 0:p004.State = 1:p005.State = 0:p006.State = 0:p007.State = 0:p008.State = 0:p009.State = 0
        Case 5:p001.State = 0:p002.State = 0:p003.State = 0:p004.State = 0:p005.State = 1:p006.State = 0:p007.State = 0:p008.State = 0:p009.State = 0
        Case 6:p001.State = 0:p002.State = 0:p003.State = 0:p004.State = 0:p005.State = 0:p006.State = 1:p007.State = 0:p008.State = 0:p009.State = 0
        Case 7:p001.State = 0:p002.State = 0:p003.State = 0:p004.State = 0:p005.State = 0:p006.State = 0:p007.State = 0:p001.State = 0:p009.State = 0
        Case 8:p001.State = 0:p002.State = 0:p003.State = 0:p004.State = 0:p005.State = 0:p006.State = 0:p007.State = 0:p008.State = 1:p009.State = 0
        Case 9:p001.State = 0:p002.State = 0:p003.State = 0:p004.State = 0:p005.State = 0:p006.State = 0:p007.State = 0:p008.State = 0:p009.State = 1
    End Select
    Select Case SingleScore
        Case 0:p010.State = 0:p010a.State = 0:p020.State = 0:p020a.State = 0:p030.State = 0:p030a.State = 0
        Case 10:p010.State = 1:p010a.State = 1
        Case 20:p020.State = 1:p020a.State = 1:p010.State = 0:p010a.State = 0
        Case 30:p030.State = 1:p030a.State = 1:p020.State = 0:p020a.State = 0
    End Select
    If B2SOn Then
        Select Case SingleScore MOD 10
            Case 0:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 1:Controller.B2SSetData 87, 1:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 2:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 1:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 3:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 1:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 4:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 1:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 5:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 1:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 6:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 1:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 7:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 1:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 0
            Case 8:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 1:Controller.B2SSetData 95, 0
            Case 9:Controller.B2SSetData 87, 0:Controller.B2SSetData 88, 0:Controller.B2SSetData 89, 0:Controller.B2SSetData 90, 0:Controller.B2SSetData 91, 0:Controller.B2SSetData 92, 0:Controller.B2SSetData 93, 0:Controller.B2SSetData 94, 0:Controller.B2SSetData 95, 1
        End Select
        Select Case SingleScore
            Case 0:Controller.B2SSetData 96, 0:Controller.B2SSetData 96, 0:Controller.B2SSetData 98, 0
            Case 10:Controller.B2SSetData 96, 1
            Case 20:Controller.B2SSetData 97, 1:Controller.B2SSetData 96, 0
            Case 30:Controller.B2SSetData 98, 1:Controller.B2SSetData 97, 0
        End Select
    End If
End Sub
...
Sub Trigger002_Hit
    ...
    If l003.State Then AddSingleScore 1
End Sub
...
Sub TrHole008_Hit 'top hole
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole008
    Addscore 500000
    vpmTimer.AddTimer 1000, "AddScore 500000 '"
    Bumper8L.State = 1
    Bumper1L.State = 0
    Bumper1La.State = 0
    l003.State = 1
...
Sub Check4inLine
    If holes(1) + holes(3) + holes(5) + holes(7) = 4 Then
        AwardSpecial
        If holes(0) Then vpmTimer.AddTimer 1000, "AwardSpecial '"
    End If
    If holes(1) + holes(2) + holes(4) + holes(6) = 4 Then
        AwardSpecial
        If holes(0) Then vpmTimer.AddTimer 1000, "AwardSpecial '"
    End If
End Sub
...
Sub TrHole001_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole001
    Addscore 500000
    holes(1) = 1
    TurnON aPlasticLights
    Check4inLine
    BallsOnPlayfield = BallsOnPlayfield - 1
    EndOfBall
End Sub
...
Sub TrHole002_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole002
    Addscore 500000
    holes(2) = 1
    TurnON aPlasticLights
    Check4inLine
...
Sub TrHole003_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole003
    Addscore 500000
    holes(3) = 1
    TurnON aPlasticLights
    Check4inLine
...
Sub TrHole004_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole004
    Addscore 500000
    holes(4) = 1
    TurnON aPlasticLights
    Check4inLine
...
Sub TrHole005_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole005
    Addscore 500000
    holes(5) = 1
    TurnON aPlasticLights
    Check4inLine
...
Sub TrHole006_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole006
    Addscore 500000
    holes(6) = 1
    TurnON aPlasticLights
    Check4inLine
...
Sub TrHole007_Hit
    If Tilted Then Exit Sub
    PlaySoundAt "fx_kicker", TrHole007
    Addscore 500000
    holes(7) = 1
    TurnON aPlasticLights
    Check4inLine
