...
'   130 - 136 Wire Rollovers
'   141 Chime1-10s, 142 Chime2-100s, 143 Chime3-1000s
'   151 - 152 Butterfly Rollovers
'   160 - 161 Rollover Buttons
'   170 - 173 Mushrooms
'   199 credit light
'   128 Knocker & Strobe
'   165 Ball In Shooter Lane
'
'   Code Flow
'                  EndGame
'                   ^
...
'   Press "c" during play to activate, the arrow keys control the ball, "." will levitate the ball, press "c" to drop it.
'**********************************************************************************************************************

'Physical elements are on layer 2 w/ leaf switch rubbers separated for individual hit events.
...
Const cOptions = "GrandTour_1964.txt"
...
'************************************
' If you are having probelms with running ther table in desktop mode
' please remove the ' in the following line of code

' SlideReel.image = "DTSlidesReel65"
'************************************
  ...
  If balls = 5 Then
    instructCard.image = "5 Ball GT"
    coincard.image = "GT 5b coin"
  Else
    instructCard.image = "3 Ball GT"
...
Sub Table1_KeyDown(ByVal keycode)
  ...
  End If
    ...
    If keycode = rightFlipperKey and state = False and operatorMenu = 1 Then
        playFieldSound "metalhit2", 0, SoundPointScoreMotor, .2
      Select Case (options)
    Case 0:
            If freePlay = 0 Then
                freePlay = 1
...
Sub bootTable_Timer()
  ...
  If bootCount = 1 Then
    ...
    For each xx in layer1acol: xx.image = "layer1a": Next
    ...
    bumper1Off: bumper2Off: bumper3Off
    tenPointLeftLight.state = 0: tenPointRightLight.state = 0
    plastic_blue.image="blueplastic"
    gates.image="gates"
    inserts001.image="inserts"
...
Sub resetTable
  For x = 1 to 4
    EVAL("Mushroom" & x & "Light").state = 1
  Next
  OpenTourGateLight.state = 1
  TourGateLight.state = 0
  FreeBallGateLight.state = 0
  bottomgate.rotateToStart
  gate1.collidable = 1: gates004.visible = 1: gates003.visible = 0
  fiftyPointStepper
  mushroomsHit = 0
  'playFieldSound "ScoreMotor", 0, SoundPointScoreMotor, 1
  playFieldSound "GTReset", 0, ButterflyLight2, 1
End Sub

'************** Bumpers and Skirt Animation
dim bump1step, bump2step, bump3step
Sub bumpers_Hit(Index)
  EVAL ("bumper" & (index + 1)).timerenabled = 1
  PointStepper
  If tilt = False Then
    Select Case (Index)
      Case 0: If bumpLight1.state = 0 Then
            addScore 1
          Else
            addScore 10
          End If
      ...
      Case 1: If bumpLight2.state = 0 Then
            addScore 1
          Else
            addScore 10
          End If
      ...
      Case 2: If bumpLight3.state = 0 Then
            addScore 1
          Else
            addScore 10
          End If
...
Sub bumper1_timer
  Select Case bump1Step
    ...
    Case 7: bumpskirt1.RotY=0:bumper1.timerenabled = 0
  End Select
...
Sub bumper2_timer
  Select Case bump2Step
    ...
    Case 7: bumpskirt2.RotY=0:bumper2.timerenabled = 0
  End Select
...
Sub bumper3_timer
  Select Case bump3Step
    ...
    Case 7: bumpskirt3.RotY=0:bumper3.timerenabled = 0
  End Select
...
'***************Butterfly Rollovers
Sub butterfly1_Hit
  layer2butterflyl.roty = 0
  If B2SOn Then DOF 151, DOFPulse
  If ButterflyLight1.state = 1 Then
    addScore(50): fiftyPointStepper
  Else
    addScore(10)
  End If
End Sub

Sub butterfly1_unHit:layer2butterflyl.roty=-10:End Sub

Sub butterfly2_Hit
  layer2butterflyr.roty = 0
  If B2SOn Then DOF 152, DOFPulse
  If ButterflyLight2.state = 1 Then
    addScore(50): fiftyPointStepper
  Else
    addScore(10)
  End If
...
Sub butterfly2_unHit:layer2butterflyr.roty=10:End Sub


'************** Wire RollOvers
Dim wireNumber
Sub Trigger_Hit (Index)
  wireNumber = Index
  wireAnimation.enabled = 1
  If B2SOn Then DOF (130 + Index), DOFPulse
  Select Case (Index)
    Case 0: addScore(10): PointStepper: OpenTourGateLight.state = 0: TourGateLight.state = 1:gate1.collidable = 0:Gates004.visible = 0:gates003.visible = 1
    Case 1: addScore(10): PointStepper
    Case 2: addScore(10): PointStepper
    Case 3: addScore(100): advanceSlides
    Case 4: addScore(50): fiftyPointStepper
    Case 5: addScore(50): fiftyPointStepper
    Case 6: addScore(50): fiftyPointStepper
  End Select
End Sub
...
Sub rollOverAnimation_Timer
  button = button + 1
  Select Case Button
    Case 1: EVAL ("Button" & ButtonNumber + 1).transz = -2
    Case 2: EVAL ("Button" & ButtonNumber + 1).transz = 0
    Case 3: EVAL ("Button" & ButtonNumber + 1).transz = 1
    Case 4: EVAL ("Button" & ButtonNumber + 1).transz = 1
        button = 0
...
'************** Mushrooms
Dim mushroomNumber, mushroomsHit
Sub mushroom_Hit (Index)
  If B2SOn Then DOF (170 + Index), DOFPulse
  mushroomNumber = (Index)
  PointStepper
  mushroomAnimation.enabled = 1
  Select Case (Index)
    Case 0: If Mushroom1Light.state = 1 Then addScore(10): mushroomsHit = mushroomsHit + 1: Else addScore(1)
    Case 1: If Mushroom2Light.state = 1 Then addScore(10): mushroomsHit = mushroomsHit + 1: Else addScore(1)
    Case 2: If Mushroom3Light.state = 1 Then addScore(10): mushroomsHit = mushroomsHit + 1: Else addScore(1)
    Case 3: If Mushroom4Light.state = 1 Then addScore(10): mushroomsHit = mushroomsHit + 1: Else addScore(1)
  End Select
  EVAL("Mushroom" & Index + 1 & "Light").state = 0
  If mushroomsHit > 2 Then TourGateLight.state = 1: OpenTourGateLight.state = 0:gate1.collidable = 0:gates004.visible = 0:gates003.visible = 1
  If mushroomsHit > 3 Then bottomgate.rotateToEnd: FreeBallGateLight.state = 1
End Sub
...
Sub rightSlingShot_Slingshot
  ...
  If tenPointRightLight.state = 1 Then
    addScore (10)
  Else
    addScore (1)
  End If
...
Sub leftSlingShot_Slingshot
  ...
  If tenPointLeftLight.state = 1 Then
    addScore (10)
  Else
    addScore (1)
  End If
...
'************** One Point Stepper Unit
Dim upPoint: upPoint = 1:

Sub pointStepper
  upPoint = upPoint + 1
  If upPoint > 10 Then upPoint = 1
  Select Case upPoint
    Case 1: bumper1Off: bumper2Off: bumper3Off: tenPointLeftLight.state = 0: tenPointRightLight.state = 0
    Case 2: bumper1Off: bumper2Off: bumper3On: tenPointLeftLight.state = 1: tenPointRightLight.state = 0
    Case 3: bumper1Off: bumper2On: bumper3Off: tenPointLeftLight.state = 0: tenPointRightLight.state = 1
    Case 4: bumper1Off: bumper2Off: bumper3Off: tenPointLeftLight.state = 0: tenPointRightLight.state = 0
    Case 5: bumper1Off: bumper2Off: bumper3Off: tenPointLeftLight.state = 0: tenPointRightLight.state = 0
    Case 6: bumper1On: bumper2On: bumper3Off: tenPointLeftLight.state = 0: tenPointRightLight.state = 1
    Case 7: bumper1Off: bumper2Off: bumper3Off: tenPointLeftLight.state = 0: tenPointRightLight.state = 0
    Case 8: bumper1Off: bumper2Off: bumper3Off: tenPointLeftLight.state = 0: tenPointRightLight.state = 0
    Case 9: bumper1On: bumper2Off: bumper3On: tenPointLeftLight.state = 1: tenPointRightLight.state = 0
    Case 10: bumper1Off: bumper2Off: bumper3Off : tenPointLeftLight.state = 0: tenPointRightLight.state = 0
  End Select
End Sub

Dim fiftyPoint: fiftyPoint = 1
Sub fiftyPointStepper
  fiftyPoint = fiftyPoint + 1
  If fiftyPoint > 3 Then fiftyPoint = 1
  If fiftyPoint = 3 Then
    ButterflyLight1.state = 1: ButterflyLight2.state = 1
  Else
    ButterflyLight1.state = 0: ButterflyLight2.state = 0
  End If
End Sub

'***************Advance Backglass Slides
Dim slide
Sub advanceSlides
  slide = slide + 1
  chooseCity
  resetTable
  If slide = 80 Then triplePayout.enabled = 1
End Sub

Dim currentCity
Sub chooseCity
  If B2SOn then controller.B2SSetData slide, 1
  Select Case slide
    Case 70: currentCity = ""
    Case 71: currentCity = "1) Stockholm"
    Case 72: currentCity = "2) Helsinki"
    Case 73: currentCity = "3) Copenhagen"
    Case 74: currentCity = "4) Amsterdam"
    Case 75: currentCity = "5) Brussels"
    Case 76: currentCity = "6) London"
    Case 77: currentCity = "7) Bern"
    Case 78: currentCity = "8) Rome"
    Case 79: currentCity = "9) Vienna"
    Case 80: currentCity = "10) Athens"
  End Select
  SlideReel.setvalue(slide - 70)
End Sub

Dim triple
Sub triplePayout_Timer
  triple = triple + 1
  playFieldSound "ScoreMotorSingleFire", 0, SoundPointScoreMotor, .3
  Select Case triple
    Case 1: playsound SoundFXDOF("Knocker",128,DOFPulse,DOFKnocker): credit = credit + 1
    Case 3: playsound SoundFXDOF("Knocker",128,DOFPulse,DOFKnocker): credit = credit + 1
    Case 5: playsound SoundFXDOF("Knocker",128,DOFPulse,DOFKnocker): credit = credit + 1:triple = 0: triplePayout.enabled = 0
  End Select
...
Sub scoremotor5_Timer
  ...
  If scoremotorCount = 5 Then
    If relBall = 1 Then
      relBall = 0
      playFieldSound "ScoreMotorSingleFire", 0, SoundPointScoreMotor, .3 'need to set location of score motor under the PF
      releaseBall
    End If
    If addCredit = 1 Then
      credit = credit + 1
...
Sub updateReels
  For playerTest = 1 to maxPlayers
    If B2SOn and showDT = False Then controller.B2SSetScorePlayer playerTest, score(playerTest)
    If showDT = True Then sReel.setvalue (score(playerTest))
  Next
...
Sub resetReel_Timer
  ...
  If resetLoop = 1 and score(1) = 0 Then
    resetLoop = 0
    If testFlag = 0 Then resetTable: releaseBall
    testFlag = 0
    resetReel.enabled = 0
    Exit Sub
  End If
  Select Case resetLoop
    ...
    Case 12: countUp: updateReels:
      ...
      reelFlag = 0
      reelStop = 0
      If testFlag = 0 Then releaseBall: resetTable
      testFlag = 0
      resetReel.enabled = 0
      Exit Sub
  End Select
