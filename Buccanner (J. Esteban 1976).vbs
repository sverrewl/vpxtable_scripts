...
sub JunglePrincess_init
  LoadEM
  maxplayers=4
  Replay1Table(1)=130000
  Replay2Table(2)=130000
  Replay1Table(3)=130000
  ...
  set cplay(3) = CanPlay3
  set cplay(4) = CanPlay4
  set p100k(1) = P1100k
  set p100k(2) = P2100k
  set p100k(3) = P3100k
  set p100k(4) = P4100k
...
' Remove desktop items in FS mode
    If JunglePrincess.ShowDT then
        For each i in backdropstuff
            i.Visible = 1
        Next
    Else
        For each i in backdropstuff
...
Sub PairedlampTimer_timer
  ...
  bumperLighta1.state = bumperlightb1.state
  bumperlighta2.state = bumperlight2.state
  bumperLighta2.state = bumperlightb2.state
  bumperlighta3.state = bumperlight3.state
  bumperLighta3.state = bumperlightb3.state
...
sub TGtopB_hit
  ...
  BumperLightA2.state=1
  bumperlightb2.state=1
    LtopB.state=0
  addscore 500
  addbonus
  checkaward
...
sub DTMonkey6_dropped
  PlaySoundAtVol SoundFXDOF("drop1",115,DOFPulse,DOFContactors), ActiveBall, 1
  Ldt6.state=1
  checkaward
end Sub
...
sub KtopL_timer
  Select Case lkickstep
    Case 2:
    PlaySoundAtVol SoundFXDOF("holekick",132,DOFPulse,DOFContactors), PkickarmL, 1
    DOF 139, DOFPulse
    PkickarmL.rotz=10
    KtopL.kick 150,15
    Case 3:
...
sub resetDT_timer
  select case dtreset
    Case 1:
    PlaySoundAtVol SoundFXDOF("BankReset",114,DOFPulse,DOFContactors), DTMonkey7, 1
    for i = 1 to 5: target(i).isdropped=False: DTLights(i-1).state=0: Next
      Case 2:
    PlaySoundAtVol SoundFXDOF("BankReset",116,DOFPulse,DOFContactors), DTMonkey7, 1
